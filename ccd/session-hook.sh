...
_hook_memory_converge() {   # -> converge this (home, project) pair; prints nothing; always 0
  ...
  #
  # FOUR PREFIXES, ONE RULE — "the OS scratch root", which is not spelled the
  # same on both platforms this tool ships to. `-tmp*` alone was the LINUX
...
# AFTER the shape guard, deliberately: the guard normalises what the FILE said,
# and this overrides that normalised value. Before it, a corrupt `.subagents`
# on a SessionStart would be cleared to `[]` and then re-normalised to `[]` —
# identical today, but it would make the clear depend on the guard's fallback
# rather than on the boundary, which is the coupling D-306's arm exists to keep
# out of this file.
(( ${clear_subs:-0} )) && subs="[]"
[[ "$gq" =~ ^[0-9]+$ ]] || gq=0
[[ "$gd" =~ ^[0-9]+$ ]] || gd=0
