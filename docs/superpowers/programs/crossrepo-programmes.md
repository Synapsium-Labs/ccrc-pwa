...
The read **was** an operator act at the time it was taken: `run_events` was exposed by no HTTP
route. **No longer true as of 2026-09-16** (routing spec 2026-09-14 §5.3, slice 5, Task 2):
`POST /api/runs/:id/route`, the coordinator's escalation/demotion door, now calls
`coord.runEvents(id)` (`server/src/coord/routes.ts`) to derive its own `priorSameKind`/
`lastDemotion` bookkeeping from the run's event trail — a route DOES serve it now, for a
narrow, run-scoped purpose (routing ladder state), not a general trail-reader. The 2026-09-14
measurement below is unaffected — it predates that route and was genuinely taken by operator act
against `~/.ccrc/coord.db` directly, through `node:sqlite`'s `DatabaseSync(...,
{ readOnly: true })` (`sqlite3` is not installed on the server box) — from nowhere else. It
printed two integers and no rows.
