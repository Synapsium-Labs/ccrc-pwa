...
(`ccd/session-hook.sh:862`), step 6's **conditional ambiguous-card `rm -f "$cardf"` (`:1241`)** — `rm` is an
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  absence of one:** the shipped arm (`ccd/ccd:12170-12175`) never reads `_reg_purge`'s status, so left as it
...
- **Card text reaches the model through one envelope, and never through a shell.** The card and standing
  ...
  shell word" spelling forbade the shipped tree's own `mv -f "$f" "$claim"` at `ccd/session-hook.sh:986` and
