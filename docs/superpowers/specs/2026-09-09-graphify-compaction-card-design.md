...
`ccd/compact-card.d.mts` and `server/test/*` alike — assert the token quoted beside it occurs within exactly
those lines of the shipped file. The `session-hook.sh`-only scoping was measurably insufficient: the merge that
brought Task 8's `hasFullPathOccurrence`/`hasUniqueSuffixOccurrence` split shifted `ccd/compact-card.mjs` by +8
below `~:681`, and eight anchors in the two documents (`REQUIRED_CARD`, the `k in o` loop, the `Number(...)`
idiom and the nonempty-nonce check) stayed at their pre-merge values through a round that re-measured the test
file's anchors and not the module's. Reverting this anchor, or any one corrected anchor by ±1 line, reds; a
scan re-scoped to `session-hook.sh` alone makes all eight mutants green, which is the pin the widening buys.
...
emitter** — `_lc_refused <act> <id> <tx> <token> <detail> [k v]…`, a sibling of `_lc_fail` that emits the
...
new `_lc_refused <act>`.** `_lc_tx` (`ccd/ccd:2144-2156`) mints `<ns>.<BASHPID>.<seq>` with
...
fences, exact-one helper output, raw JSONL, sixteen keys, no persisted `n`, and no hookstate cache. **Its
per-task home is named, not left as "the table" (round 13, A-M6):** the per-task **Mutation-checks steps** of
`docs/superpowers/plans/2026-09-10-graphify-compaction-card-plan-a.md` (Tasks 1–8 — numbered lists, not
tables; measured, they are the plan's eight `Step N: Mutation checks` headings and the plan contains no
`expected red` column at all). **That
...
suite. **Row NAMES are quoted, behaviours are not, and the two are no longer mixed under one set of quotes
(round 13, A-M1):** the quoted strings below occur verbatim in the Task 1–8 table; the unquoted ones are
behaviours pinned at the anchor beside them and are named by no row. "The heading regex loses a spelling" is
`server/test/compact-card.test.ts:833-840`, which exercises
`3. Files and Code Sections:` and its `**`-wrapped form; the fence rule and its unmatched opener are pinned at
`:842-871`, with the complete fence grammar at `:873-895`; "`cited` folded to 0, or `scope` to `main`, without
a set" is `:949-951` plus `:959-968`; `files: []` measured as a KNOWN empty set rather than a missing one —
`setSize: 0`/`cited: 0`, never null — is pinned at `:969` and is a behaviour, not a row (measured:
`grep -c 'known empty'` returns 0 in the Task 1–8 table, and the nearest real row is "helper exit 3 writes no
set"); "PostCompact measures
raw text" is `:827-831`, where an `<analysis>` block is excluded from `chars`; and the citation boundary and
within-set suffix-uniqueness rules are pinned at `:897-931`. **The `cited` row's suffix rule — a suffix's left boundary
...
Round 1 deleted the PROSE those rows pin and left every one
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  exactly one `_lc_refused destroy <id> "$lctx"` — the **non-fatal, tx-bearing refusal emitter Task 9 adds**
...
- **A compaction that outlives `COMPACT_CARD_MAX_AGE` can have its journal line attributed to a SUCCESSOR's
  ...
  **The SERVE half of the same premise, restated (round 13, B-I6 — the pre-D-2605 §10 carried a wrong-context
  ...
  resolution having happened. **Which closure would shut it:** of the two listed above, **only the nonce
