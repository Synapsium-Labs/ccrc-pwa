...
therefore no excuse for leaving it out — the exact-family sweep `find` (`:869`), the
`at=$(_hook_epoch_ms)` **command substitution (`:871`)**, which forks a subshell even though
...
(`ccd/session-hook.sh:781`) runs `rm -f "$tmp"; return 1` and NEVER reaches the `mv -f` at `:782`, so on the
...
own established idiom.** `ccd/compact-card.mjs:803` already reads
...
`REQUIRED_CARD`'s membership test is `k in o` (`ccd/compact-card.mjs:801`), which an empty-string value
...
| `ccd/ccd:10202-10204` | `cmd_ws_reap` | fail **CLOSED** — `command -v flock >/dev/null 2>&1 \` at `:10202` guarding `_lc_refuse reap "$id" flock-unavailable` at `:10203`, pinned by `server/test/ccd-ws-reap.test.ts:342`, `it('refuses to run the destructive verb unserialised when flock is missing')` |
...
- **`forget` recovers by re-running ONLY while a tmux server is answering.** Its liveness precondition is
  `case "$(_session_verdict "$id")" in` (`ccd/ccd:15543`), whose `unknown` arm is
  `_lc_refuse forget "$id" session-verdict-unknown` (`:15545`) — and that EMITS, THEN DIES. Measured on tmux
  ...
  `ccd/ccd:1506-1507` names cmd_forget in by name — "Every other caller — `_alive`, `_ws_status`,
...
- **The remedy, traced rather than assumed: bring a tmux server back up, then re-run.** Any server on the
  ...
  measured: its dead-reg arm recomposes an id from `$project-$slug` (`ccd/ccd:11668`) and its scan walks only
  `for f in "$REG"/*.workspace` (`:11464`), while `forget` refuses any row with a `workspace` field at all
  (`:15526-15528`) — `ccd/ccd:15479-15482` states the same disjointness in shipped prose ("archive/reap are
...
measured, none of the four reads `_reg_purge`'s status today (`cmd_ws_rm` at `ccd/ccd:4896`, `_ws_reap_tail`
at `:11167`, `_ws_gc_prune_row` at `:11668`, `cmd_forget` at `:15555`, each falling straight through to an
...
`ccd/ccd:2757-2758`), and the withheld `purge` fact is itself what lets a caller or a reader distinguish a
lock-miss
...
`_reg_purge` runs at `:11668`, and `_gc_declined` (`ccd/ccd:11308`) is
...
next row. `_lc_refuse` cannot serve here: it "EMITS, THEN DIES. Never returns." (`ccd/ccd:2766`, `die` at
`:2784`), and `ws-gc --prune` must keep sweeping. No rollback fiction.
...
positionals against `_lc_refuse`'s four — `act id token detail [k v]...` (`ccd/ccd:2766`), bound by
...
new `_lc_refuse_return <act>`.** `_lc_tx` (`ccd/ccd:2144-2156`) mints `<ns>.<BASHPID>.<seq>` with
`printf '%s.%s.%s'` (`:2155`) and can never return empty, so the non-empty key is exactly the set of minted
...
tx positional at all (`ccd/ccd:2766`) and emitting a literal `""` tx (`:2783`) — and separately it never
returns, so it can never be followed by a terminal fact; (b) `_lc_done purge`, which occurs once in the whole
file (`ccd/ccd:1828`, measured — `grep -c '_lc_done purge'` is 1) and always passes a LITERAL empty tx, its
own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not half of a pair", `:1822-1823`); and
...
- null rather than 0 without a set — plan `:2707`, "In `measureCommand`, make `cited: paths ? … : 0` → the
  null-vs-0 case goes red"; pinned at `server/test/compact-card.test.ts:949-951` plus `:959-968`;
...
- the empty working set — plan `:1914`, "In `cardCommand`, move `writeAtomic(o.set, …)` below
  ...
  red"; pinned at `server/test/compact-card.test.ts:969`, where `files: []` is measured as a KNOWN empty set
...
complete fence grammar at `:873-895`, and the citation boundary and within-set suffix-uniqueness rules at
`:897-931`. **No string in this paragraph is presented as a row name of a table in another file**: every
...
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse_return <act>` on the same (act, id, MINTED tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:11813` / `:11816`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:4858`, `die` at `:1022`) so its own `_lc_done destroy` (`:4913`) is reached too; the guard reds, and removing the guard makes both mutants green. **The tx key is `_lc_tx`-MINTED and NON-EMPTY** (`ccd/ccd:2144-2156`, `printf '%s.%s.%s'` at `:2155`, never empty), which is what keeps the three literal-empty-tx emitters outside it — `_lc_refuse` (`:2766`, emitting `""` at `:2783`, and never returning), `_lc_done purge` (one call site, `:1828`, empty by design, `:1822-1823`) and `cmd_ws_restore`'s `:6780`/`:6838`/`:6844` triple, which a `""`-admitting guard would red on the CORRECT shipped tree. **GREEN control:** drive `ws-restore` to a landed undo plus a failed spawn and assert the guard passes; widening the guard to admit empty-tx facts must red that same control |
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:11665-11667` with zero terminal facts and reds; restoring the shipped status-blind arm (`:11665-11670`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
...
- **Waits and lock division.** Ordinary hook paths are lock-free. PreCompact, compact SessionStart, and
  ...
  itself dies at `ccd/ccd:4341`. Gating them would refuse spawn on every row that has ever compacted,
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  beside `_lc_fail`/`_lc_refuse` (`ccd/ccd:2756-2785`), which emits the `refused` fact and RETURNS so the
...
- **Card text reaches the model through one envelope, and never through a shell.** The card and standing
  ...
  shell word" spelling forbade the shipped tree's own `mv -f "$f" "$claim"` at `ccd/session-hook.sh:954` and
...
- **A compaction that outlives `COMPACT_CARD_MAX_AGE` can have its journal line attributed to a SUCCESSOR's
  set.** The overlap check is young-only (`find "$set" -mmin "-$mins"`, `ccd/session-hook.sh:862`; §3.1 item 5
  ...
  **The SERVE half of the same premise, restated (round 13, B-I6 — the pre-D-2605 §10 carried a wrong-context
  ...
  (`ccd/session-hook.sh:865`), so B's overwrite would take A's card with it and leave A's SessionStart nothing
