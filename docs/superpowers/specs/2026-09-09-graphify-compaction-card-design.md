...
refusals (`ccd/ccd:6110`, `:8185`, `:11742`); round 9 asserted a blanket over "ccd's own registry operations"
...
`_hook_lock_acquire` opens a private alias named `lock-open` (`ccd/session-hook.sh:1251`) and never the
...
(`ccd/ccrc:9741`), `_upd_backup_copy` in `ccrc update` (`ccd/ccrc:11068`), and the matching removal line
`"$reg/compact-card.mjs"` in `ccrc uninstall`'s own list (`ccd/ccrc:11713`). **Task 11** is this documentation
...
PreCompact ──► scope + graph-measure (no lock) ──► lock, generation validate, overlap/young-claim check
   ...
   │              set-stage into canonical (card first; the stage-2-only `steered` stamp and the STEER_TEXT
   │              print sit between the two renames and do NOT run in Plan A, so the helper's staged
   │              `steered:false` is published unchanged); rc 3 renames only
...
**Constants, each defined once, in the hook — except `COMPACT_LOCK_WAIT`, whose `where` column names its
second literal home and the pin that keeps the two byte-equal:**
...
| `COMPACT_CARD_MAX_AGE` | 1200 s | hook | the in-flight window, **split by artifact (round 13, A-M8): an aged CARD is removed unread** (compact SessionStart, `ccd/session-hook.sh:2129-2131` — the `COMPACT_CARD_MAX_AGE` `find` at `:2129`, the `_hook_lock_still_canonical` recheck at `:2130`, the `rm -f "$f"` at `:2131`), while **an aged canonical SET is still claimed and measured at settlement** — age decides provenance eligibility only, never the claim. Measured, no arm removes an aged canonical set unread: PreCompact publishes over it, SessionStart removes only a card, PostCompact claims and measures it.  An artifact older than this belongs to no compaction that can still arrive; an unconsumed canonical set *younger* than this at PreCompact means overlap (§3.0). At settlement, PostCompact saves the canonical set's age for provenance eligibility, then touches the hard-linked private claim so claim mtime instead signals active liveness until it is consumed or later stale-swept. An originally aged claim permits nonce-only marker settlement, never measurement provenance. Argued from the longest measured compaction, 826 s on the gpt lane, ×1.45 |
...
| `COMPACT_LOCK_WAIT` | 5 s | **TWO literal homes, byte-equal, pinned:** `ccd/session-hook.sh` for the hook's four acquisitions, and `ccd/ccd` for its own three (row creation, `_spawn_start`, `_reg_purge`) | `flock -w "$1"` — the wait is the acquire helper's **first positional parameter**, never a constant spelled inside the helper, which is what lets `COMPACT_LOCK_WAIT_SERVE` and this constant coexist as two independently callable bounds. **Why two homes and not one (round 14, B-I6):** `ccd/ccd` does not source `ccd/session-hook.sh` and the hook sources nothing at all, so a value "defined once, in the hook" is unreadable from `_reg_purge`, row creation and `_spawn_start`. Measured, `ccd/ccd:266` already states the shipped convention for exactly this case — "`ccd/session-hook.sh`'s `_hook_epoch_ms` is a deliberate local copy of this body (the hook sources nothing); a test pins the two identical" — with the hook's reciprocal note at `ccd/session-hook.sh:38`, "This is a LOCAL copy of ccd's `_plat_epoch_ms`". `COMPACT_LOCK_WAIT` follows that precedent: each file carries its own literal, each comment names the other file as its twin, and Task 9 adds the value-equality pin below, so a re-derivation applied to one copy and not the other is a red suite rather than a silent drift. `COMPACT_LOCK_WAIT_SERVE` stays hook-only — compact SessionStart is its only caller and `ccd` never acquires with it. **The pin:** a `single-definition`-style rule over the bash corpus asserting that `COMPACT_LOCK_WAIT=` appears in exactly `ccd/ccd` and `ccd/session-hook.sh` with byte-equal values and `COMPACT_LOCK_WAIT_SERVE=` in exactly `ccd/session-hook.sh` — buildable there, measured: `server/test/single-definition.test.ts:1274` is `const bashRoots = [path.join(ccrcRoot, 'ccd'), path.join(ccrcRoot, 'deploy')];` and `:1319-1320` already lists `ccd/ccd` and `ccd/session-hook.sh` by name among the files that scan must have found. Used for every acquisition except compact SessionStart's, all of them off the hot path, none of them costing a human a wait — but **what a miss costs is NOT one blanket (round 13, B-I5)**: PreCompact's two acquisitions (initial, and the reacquire after the helper returns), PostCompact's settlement acquisition and its final-transaction reacquisition, and `_reg_purge` each lose a durable artifact on a miss — a set, a card, a journal line, a purge. The other two lose no artifact and are stated on their own in §3.4's row-creation/spawn section and as their own §4 rows: `_spawn_start` fails **OPEN** on a contended miss (it spawns with no `CCRC_SESSION_GENERATION`, warns on stderr naming the lock, and that session's compaction lifecycle is inert until its next respawn), because a swap must never wedge on a compaction lock; row creation fails **CLOSED** (it acquires before writing any row field, so a miss dies with a retryable message and leaves nothing). Round 6's separate `COMPACT_JOURNAL_FINAL_LOCK_WAIT` is retired: under round 7's option-A protocol PostCompact's settlement and final-transaction acquisitions are symmetric with PreCompact's two (both off the hot path, both losing a durable artifact on a miss), so they share this one bound rather than each inventing its own. Task 9 measures each held section's p95 and max on the fleet box the way Task 6 measured the helper (with `_hook_graph_measure` moved out of the lock, §3.1) and sets this bound at ≥ 8× measured p95 rounded up — 5 s stands only if that measurement supports it; fix the method, the number follows |
...
scope/provenance tuple runs without `--set`, with `scope` null and those same fields null. **A fourth trigger,
stated here because §4's aged-set row and §3.4's "an originally aged set remains nonce-only/no-provenance"
depend on it and nothing else spelled the mechanism (round 13, A-M7):** an ORIGINALLY AGED claim — one whose
canonical set was older than `COMPACT_CARD_MAX_AGE` when PostCompact saved its age — is provenance-INELIGIBLE,
so it too runs `measure` WITHOUT `--set`, leaving `scope`, `cited`, `setSize` and all six provenance fields
null. Age never gates the claim itself; it gates only what may be attributed. Without this trigger an
...
(`null` versus `ambiguous`). **Age-ineligibility is evaluated LAST and WINS: an aged `overlap:true` claim
commits `scope: null`.** One value, named once, so a behaviour fixture on that shape is writable. Note what
precedence does and does not decide here: both triggers already normalise to the SAME no-`--set` record —
`measure` runs without `--set` either way, so `cited`, `setSize` and all six provenance fields are null under
both — so the ORDER can only change the single `scope` member and nothing else about the committed line.
`JOURNAL_RECORD_PRED` accepts both spellings (measured under jq 1.7: `scope: null` with `NULL_PROVENANCE`
true, and `scope: "ambiguous"` with `NULL_PROVENANCE` true), which is exactly why the value has to be fixed
in prose — the predicate is not the mechanism that decides it.
...
forms no marker path and yields `served:false`. Only a fully valid, provenance-ELIGIBLE claim whose `overlap` is absent or
false is passed to `measure` with `--set` — eligibility excluding the originally aged claim above. Summary-only fields remain measurable in every normalized case. The
...
5. **Acquire the stable lock** (protocol step 4, `COMPACT_LOCK_WAIT`). It creates an absent stable lock only
   ...
   **the redundant-canonical-alias unlink this item used to require is WITHDRAWN (D-2756, fix
   round 1).** Measured against the shipped arm it changes nothing it could change. Its ONLY reachable
   premise is a PostCompact SIGKILLed between its claim `link` and its canonical `rm`; in that state the
   overlap `find` below already matches `.<id>.compactpost.*.claim` inside the window, so the verdict is
   `ambiguous` with or without the unlink, and `_hook_write_atomic "$set" …` runs on EVERY non-inert path
   INCLUDING the ambiguous one (the ambiguous return sits after the publication), where its `mv -f`
   replaces the directory entry — which IS the "unlink the redundant canonical alias" effect, leaving the
   predecessor's inode reachable only through its claim. What this item still requires is unchanged, and
   is the half that was never redundant: do not read,
...
7. Still under that lock (protocol step 8, continued): the graph gate — **a gate failure withholds the
   COMPACT card only; the standing graph card still reports the freshness that failed the gate, so a
   compaction with no compact card is explained to the session rather than silent (round 14, B-M3, restoring
   a clause the deleted §4 row carried and nothing replaced).** The gate: `_hook_graph_measure` (step 3,
   above)
   ...
   **WHAT A USERLAND WITH NEITHER `timeout` NOR `gtimeout` GETS WAS RESTATED BY THE MERGE WITH `origin/main`
   ...
   FD/journal failure after settlement") is a different, post-settlement condition. The claim stands on the
   protocol alone: settlement precedes `measure`, so the set is consumed either way and no record commits.
   Note the vocabulary too — under §3.4's option A PostCompact CLAIMS and unlinks canonical; "consumed" was
   pre-option-A wording.
...
reports exactly **one** `clone`, against **zero** for an otherwise identical harness that calls the
function without the substitution; `BASHPID` differs inside) — it stays inside the section because
protocol step 7 mints the nonce from `at`, and `at` is the publication timestamp the nonce embeds, so it
must be taken inside the section that publishes — the `jq -cn` that builds the
initial set
...
beside the fork-then-release legs, so adding or deleting any child reds. Neither §2's `COMPACT_LOCK_WAIT`
row nor §5 carries that assertion: §2's row directs Task 9 only to measure each held section's p95 and
max on the fleet box, and §5 has no held-section multiset row. **The scope `find`s are NOT in it, and round 9's own enumeration wrongly put
...
tests and `[ -f "$COMPACT_HELPER" ]` is a builtin. **The second section's list is split the same way, and
for the same reason.** PLAN A — the second section forks the card-stage rename, the set-stage rename, and
the stage `rm`s on every non-publishing outcome. STAGE 2 / PLAN C ONLY — the round-8 `jq` that stamps
`steered` into the still-private set-stage, between the print and the set-stage rename; in Plan A it does
not run (protocol step 13, §3.1 item 10, §3.5), so a Plan A multiset that contains it is wrong in the
other direction. Its ownership re-read is the bounded `read -N` plus bash regex and
...
list as built, so adding or deleting ANY child there reds, not only a third `find`. **The multiset is stated
...
`at=$(_hook_epoch_ms)` substitution (`:1262`), the `jq -cn` (`:1275`), and `_hook_write_atomic`'s publishing
`mv -f` (`:799`) — plus, once Task 9 builds them, the generation-read `link` and its alias-unlink.
**THE TWO `find` NUMBERS ALONE in that enumeration are on the PRE-Task-9 tree `8e457995` this section was
written against and are superseded there (round 14, A-M3 narrowed this disclosure from three families to
two):** Task 9 rewrote both `find` invocations, so no tip line is byte-identical to either and re-pointing
them would falsify their quotations. Every OTHER line in the enumeration is cited AT THE TIP and byte-proven
there against `8e457995`, each a UNIQUE byte-identical block: `at=$(_hook_epoch_ms)` `:871` → `:1262`, the
`jq -cn` `:875` → `:1275`, the ambiguous-card `rm -f "$cardf"` `:865` → `:1241`, and `_hook_write_atomic`'s
publishing `mv -f` `:782` → `:799`. Its printf-failure arm is the one line here that moved by NAME rather
than by bytes: base `:781` has no byte-identical tip line, because Task 9's nonce parameter shifted the
...
it: the ambiguous-card `rm -f "$cardf"` (`:1241`) fires only when `CS_SCOPE` is `ambiguous`, and that arm
...
own established idiom.** `ccd/compact-card.mjs:705` already reads
...
`REQUIRED_CARD`'s membership test is `k in o` (`ccd/compact-card.mjs:703`), which an empty-string value
...
operator-off guard. **Immediately after that guard, before any CARD, SET, CLAIM or MARKER pathname is inspected,
aged, read, matched, claimed, deleted, emitted, or published — the lock family being the acquire's own and
therefore outside this rule, since the acquire itself inspects, `link`-creates and unlinks two §3.4 family
members (round 13, B-M6) — it acquires the §3.4 stable lock (wait = `COMPACT_LOCK_WAIT_SERVE`, 2 s — the one
acquisition a human is waiting on; every other arm's acquisition in this design uses `COMPACT_LOCK_WAIT`,
§2) and validates the environment generation ONCE, under that lock, before any card/set/claim/marker
inspection — and again at step 5 before the marker publication, which is a different pairing.**
**The single under-lock validation is the runtime's choice and the correct one (whole-branch review,
A-M4-NEW).** An earlier spelling of this sentence prescribed validate → acquire → validate-again and no arm
ever implemented it: `_hook_compact_card` (`ccd/session-hook.sh:2082-2086`) acquires, then calls
`_hook_generation_ok` once, and step 5's second validation lives in `_hook_compact_mark_served`
(`ccd/session-hook.sh:2098`). The safety property §3.3 exists for holds either way — no lifecycle
observation before the lock, the generation proved under the lock before any inspection — and a PRE-lock
validation would BREAK the rule this same sentence opens with: `_hook_generation_ok` publishes and unlinks
a `.<id>.generation-read.*` alias (`ccd/session-hook.sh:1310`, `:1315`, `:1318`), a §3.4 family member, so
running it before the acquire creates private lifecycle residue outside the lock — which the parenthetical
above exempts for the LOCK family alone. **It retains that one
...
forks repeatedly: `$(find "$f" -mmin …)` and the `rm -f "$f"` beside it at `ccd/session-hook.sh:2129-2131`, the
...
no-clobber `link`. `_plat_uuid` itself (`ccd/ccd:395-397`) emits a trailing LF on both its branches
...
own `case`/`${#x}` idiom (`ccd/session-hook.sh:975-976`) is the equivalent already shipped here. §6 states the
...
(`ccd/ccd:6791` in `cmd_ws_rm`, `:13020` in `_ws_reap_tail` under `_ws_reap_locked`, `:13602` in
`_ws_gc_prune_row` under `ws-gc --prune`, `:19488` in `cmd_forget`) — three of them only AFTER irreversible
...
five, read at this tip (D-2605's own acquire gate, `ccd/ccd:2653`, is the sixth `command -v flock` in the
...
| `ccd/ccd:5810-5811` | `cmd_ws_add` | fail **CLOSED** — `die "flock (util-linux) is unavailable — refusing to create a workspace unserialised"` |
...
Concretely, this ruling governs `_reg_purge` reached through `cmd_ws_rm` (`:5667`), `cmd_forget` (`:16655`)
...
contradicting it — `ccd/ccd:11032-11033`: "NOT a degraded mode. Without `flock` the serialisation is absent,
not weaker, so the destructive verb does not run" (the shipped comment wraps mid-sentence between "absent,"
and "not", so the two lines are joined here with the comment marker elided and NO word omitted — an earlier
spelling put an ellipsis there and claimed it covered only the marker, while it also dropped "not weaker,",
the clause the doctrine turns on). Round 9's "row creation and `_spawn_start` …
...
reachable on the second declared userland: `ccd/ccd:684-705` records that "a LaunchAgent gets launchd's own
minimal PATH" and that this "already bit this port once", and `_svc_job_path` (`:711-717`) appends
...
Together those mean a PRESENT `.generation` proves a flock-capable row creation happened HERE, and that this
row's hooks MAY hold the lock right now. It does not prove they were handed a generation, and the gate does
not need it to (round 14, A-8 = B-M2): the gate errs CLOSED on the possibility, and a spawn that exported
nothing cannot hold the lock at all. **Two named paths leave the generation present with no lock-capable
hook**, and the gate over-approximates across both, always in the safe direction: `_spawn_start`'s single
acquisition can MISS on contention and fail OPEN, spawning a session that carries no
`CCRC_SESSION_GENERATION` (§4's row for it), and a REUSED generation leaves the earlier session permanently
inert (§4's generation-reuse row). In both the row has `.generation`, the session has none, and every hook arm
on that row refuses for the session's whole life — so the purge refuses a row that provably has no publisher
to race. That is over-refusal, never under-refusal: the load-bearing half is the other direction, and it is
exact — ABSENT ⇒ no hook on this row ever received a generation ⇒ nothing to race. So: **generation present
AND `flock` unresolvable ⇒ `_reg_purge` fails CLOSED**, returning the
...
`cmd_ws_rm` (`ccd/ccd:6778`) and `cmd_forget` (`:19488`) report `_lc_fail` naming the cause (mechanism absent
with a live generation); `_ws_gc_prune_row`'s dead-reg arm — the third GOVERNED caller — reaches `_reg_purge`
at `:12595` as its FIRST destructive act (measured: `_lc_tx`, `_lc_intent destroy`, then `_reg_purge`, then a
terminal fact), so it is the pre-action caller and DECLINES — which, because the sequence on the tree this
sentence was written against ignored `_reg_purge`'s status, Task 9 had to BUILD: branch on the status and
emit `_gc_declined` in place of an unconditional `_lc_done destroy` and `_gc_reclaimed`, or the arm answers
a refusal with a false success. It now does (`:12596`, `:12602`, `:12612`), and the pre-Task-9 arm it
replaced — `8e457995:ccd/ccd:11665-11670`, with `:11669` and `:11670` — is superseded. And `_ws_reap_locked` cannot be the FLOCK-absence caller, since `cmd_ws_reap` dies at its own
...
- **`ws-rm` recovers by re-running from a flock-capable PATH**, full stop. Measured: its re-run skips the
  dirty/foreign gates when `[[ -d "$workdir" ]]` is false (`ccd/ccd:6591`, the gate inside `cmd_ws_rm` — its base
...
- **`forget` recovers by re-running ONLY while a tmux server is answering.** Its liveness precondition is
  `case "$(_session_verdict "$id")" in` (`ccd/ccd:20945`), whose `unknown` arm is
  `_lc_refuse forget "$id" session-verdict-unknown` (`:19455`) — and that EMITS, THEN DIES. Measured on tmux
  3.4 against an isolated socket: with a server up and the session gone, `tmux has-session` prints
  `can't find session: nosuch`; after the last session is killed it prints `no server running on <socket>`;
  with no socket at all, `error connecting to <socket> (No such file or directory)`. `_session_probe`'s
  verdict `case` maps only `*"can't find session"*` to `gone` (`ccd/ccd:2158`); both absence spellings fall
  through to that `case`'s `*)` arm and become `unknown`, which the shipped comment at
  `ccd/ccd:2139-2140` names cmd_forget in by name — "Every other caller — `_alive`, `_ws_status`,
  ...
  (`tmux kill-session`, `:16644`) eleven lines before `_reg_purge` (`:16655`), a forget of the box's LAST session
...
- **The remedy, traced rather than assumed: bring a tmux server back up, then re-run.** Any server on the
  ...
  from a flock-capable PATH reaches `_reg_purge`. **`ccd ws-gc --prune` is NOT an alternative remedy here**,
  measured: its dead-reg arm recomposes an id from `$project-$slug` (`ccd/ccd:14328`) and its scan walks only
  `for f in "$REG"/*.workspace` (`:14446`), while `forget` refuses any row with a `workspace` field at all
  (`:19440-19442`) — `ccd/ccd:21237-21240` states the same disjointness in shipped prose ("archive/reap are
  workspace-only by contract, and ws-gc's dead-reg arm walks only `.workspace` entries — so a dead wrapper
  session on a project's main checkout was an immortal fleet row, removable by nothing"). No arm of the prune
  can reach a forget-eligible row.

So the row is not wedged forever, but it is wedged on TWO conditions for `forget`, not one: a PATH that
resolves `flock`, and a tmux server that answers.
...
is indeed TypeScript-only, but the file carries a SECOND corpus: `bashRoots` (`:1274`) is
`[<repo>/ccd, <repo>/deploy]`, `BASH` (`:1298`) is every bash file under those plus `install.sh`, and
`holdersOf` (`:1303-1304`) filters exactly that corpus on non-comment lines — the mechanism seven existing rules
already use, and `:1319-1323` pins `ccd/ccd` and `ccd/session-hook.sh` as members by name. So a single-spelling
...
are the TypeScript packages, as this branch's own README") to "addition says." on `:1177`. The REMAINDER of
`:1177` and `:1178` carry a SEPARATE and true sentence ("Keeping the three in step is a reading discipline,
not a mechanism; if you widen one, widen the other two by hand."), which is why the range stops at `:1177`
and the restatement must not swallow the rest of that line. Lines `:1167-1168` carry a different, CORRECT sentence
about `CCRC_WD_CLASS` deriving from the class constant — an implementer told to "restate as measured" at that
anchor would rewrite a true statement and leave the false one standing. **Task 9's citation audit covers EVERY
tracked-file reference in either document, not `ccd/session-hook.sh` alone (round 13, A-I1/B-I4):** for every
`<tracked file>:<n>[-<m>]` reference — `ccd/session-hook.sh`, `ccd/ccd`, `ccd/compact-card.mjs`,
`ccd/compact-card.d.mts` and `server/test/*` alike. **Stated so it is SATISFIABLE on a correct tree (round
14, A-4)** — round 13's form, "the token quoted beside it occurs within exactly those lines", is red on a
correct tree and its only green path was to weaken it, which is the failure mode round 11's B-I2 named for
the allow-list equality:

1. **Premise.** The rule relates a QUOTATION to a RANGE. Where a reference's clause quotes nothing, there is
   nothing to check and the rule does not apply — a correct citation may point at a line with no token to
   quote. Measured, `ccd/session-hook.sh:843` is a BLANK line and is cited as one; a universal form refuses
   it by construction.
2. **The rule.** At least ONE quoted token or quoted excerpt in the SAME CLAUSE occurs within the cited
   lines. One clause may carry several quotations and one quotation may anchor several references; a single
   hit anywhere in the clause satisfies it, and an excerpt with an ellipsis is satisfied when each part
   occurs.
3. **Sub-rule A.** A citation that names a function while pointing at a STATEMENT must have its range inside
   that function's body. `citedCount` is declared at `ccd/compact-card.mjs:601` and its body reaches
   `ccd/compact-card.mjs:610`, which is the line that calls `hasUniqueSuffixOccurrence` — so "called from
   `citedCount` at `ccd/compact-card.mjs:610`" stands under this sub-rule while a citation naming a function
   and landing outside its body does not. The example is MEASURED against this tree and not an illustration:
   a rule whose worked example does not hold where the rule runs teaches the next reader the wrong test,
   which is what the `:708`/`:717` spelling this replaces did for ten rounds (whole-branch review, B-M4).
4. **Sub-rule B.** A `pinned by <test>:N` citation must land on that test's `it(` / `describe(` / `expect(`
   line: what it asserts is that a named test pins the claim.
5. **Allow-list.** Quoted HISTORY is exempt — a reference inside the `## Deviations found` ledger entry, or
   inside a quotation the same sentence marks as superseded. A scan that reds on the record of a correction
   teaches the next round to delete that record.

The `session-hook.sh`-only scoping was measurably insufficient: the merge that brought Task 8's
`hasFullPathOccurrence`/`hasUniqueSuffixOccurrence` split moved four `ccd/compact-card.mjs` anchors, each
written here as **pre-merge → this tree** with only the right-hand value live — `REQUIRED_CARD`
(`:779`→`:787`), the
`k in o` loop (`:793`→`:801`), the `Number(...)` idiom (`:795`→`:803`) and the nonempty-nonce check
(`:799`→`:807`) — all four by +8, each verified byte-identical at both ends — through a round that
re-measured the test file's anchors and not the module's. Reverting this anchor, or any one corrected anchor
by ±1 line, reds; a scan re-scoped to `session-hook.sh` alone makes all eight mutants green, which is the pin
the widening buys. **The +8 does NOT generalise (round 14, A-10):** measured against the pre-merge helper
(820 lines / 39571 bytes versus this tree's 828 / 39768, a net of +8 across three interleaved hunks),
`citedCount` moved `:697`→`:708` (+11) while the renamed `hasAlignedOccurrence` → `hasFullPathOccurrence`
moved `:684`→`:682` (−2). Three displacements in one region, so no other anchor there may be repaired by
arithmetic — the reason the audit re-measures every reference rather than applying an offset.
...
A-I2).**
`_reg_purge` reached through `ws-rm` (`ccd/ccd:5680`), `forget` (`:16655`) and `ws-gc --prune` (`:12608`) keeps
...
| `at` | epoch ms | integer |
| `agent` | the subagent id, copied from the set; null on every other scope and without a set — **on no wire or chip**, Plan A having neither | string or null |
| `transcript` | the transcript §3.0 resolved, copied from the set; null for `ambiguous` and without a set — **on no wire or chip**, Plan A having neither | string or null |
...
every return before arbitrary setup work and before `_spawn_start`. **Its acquisition happens BEFORE any row
field is written, and a miss FAILS CLOSED:** it dies with a retryable message naming the lock and leaves
nothing behind — no `.uuid`, no `.generation`, no partial row — so nothing irreversible has happened and
re-running the verb is the whole remedy. `_spawn_start` acquires exactly once
...
**A CONTENDED miss there — the acquire times out at `COMPACT_LOCK_WAIT` — FAILS OPEN (round 13, B-I5):** the
spawn proceeds WITHOUT `CCRC_SESSION_GENERATION` in the child's environment, and the arm prints a visible
stderr warning naming the lock. The cost is stated plainly rather than hidden: that session's compaction
lifecycle is inert for its whole life, until its next respawn, because a hook with no generation "fails
closed for compaction lifecycle work only". Two reasons, both load-bearing. A swap must never wedge on a
compaction lock — a rescue swap contending with the outgoing pane's own PreCompact, which can hold the lock
for seconds, would otherwise not come back up — and a 5 s contended hold at spawn time is itself an anomaly,
which is why the warning is printed rather than swallowed. It is also self-consistent with the generation
gate rather than an exception to it: a session that carries no generation cannot hold this lock, so there is
no publisher for the missed acquisition to have raced. This is the same outcome §4's generation-REUSE row
records, reached by a different cause, and it likewise cannot self-heal by any later event — hence the
warning, which that row has no way to emit.
...
walks `$REG/$id.*` (`for f in "$REG/$id".*; do`, `ccd/ccd:3050`) and skipped only
`suffix == archived || suffix == reaping` would otherwise delete it in ordinary glob order like any other
field. Task 9 extended that same skip condition to its shipped three-term form —
`[[ "$suffix" == archived || "$suffix" == reaping || "$suffix" == generation ]] && continue`
(`ccd/ccd:3260`) — so the loop leaves it standing, and added one further explicit
`rm -f "$REG/$id.generation"` (`ccd/ccd:3307`) after the existing archived/reaping tail (which itself must run first, per the
...
`_reg_purge`'s body, the same treatment round 6 correctly gave `session-hook.test.ts:2484`. **Its four
...
`_rm_prc` in `cmd_ws_rm` (`ccd/ccd:6791`), `_rt_prc` in `_ws_reap_tail` (`:13746`), `_pr_prc` in
`_ws_gc_prune_row` (`:13602`) and `_fg_prc` in `cmd_forget` (`:19488`) — and each renders the value through
`_compact_lock_why_remedy` (`ccd/ccd:2754`) rather than falling straight through to an unconditional
...
`ccd/ccd:4220-4221`), and the withheld `purge` fact is itself what lets a caller or a reader distinguish a
lock-miss
failure from a completed purge. The fourth caller, `_ws_gc_prune_row`'s dead-reg arm, calls `_reg_purge`
BEFORE any irreversible action on that row, so it alone may decline on a lock miss — but a decline is not the
absence of a lifecycle fact. Measured on the arm BEFORE Task 9 (`8e457995:ccd/ccd:11665-11667`,
superseded at this tip by `ccd/ccd:12579-12581`), `$lctx` had already been minted by
`local lctx` plus `lctx=$(_lc_tx)` and an `_lc_intent destroy "$project-$slug" "$lctx" verb ws-gc …`
emitted before `_reg_purge` ran at `8e457995:ccd/ccd:11668`, superseded at this tip by `ccd/ccd:12608`, and
`_gc_declined` (`ccd/ccd:12212`) is
`GC_DECLINED=$(( … )); _gc_row declined "$1"` — a printf report row, not a lifecycle emit. Declining with
`_gc_declined` alone would therefore leave a minted `intent destroy` for (destroy, id, `$lctx`) with ZERO
terminal facts, permanently, on every sweep of that row. **So Task 9 ADDS a non-fatal, tx-bearing refusal
emitter** — `_lc_refuse_return <act> <id> <tx> <token> <detail> [k v]…`, a sibling of `_lc_fail` that emits the
`refused` fact and RETURNS rather than dying — and the dead-reg arm emits it beside `_gc_declined`. Every
minted tx then closes with exactly one terminal fact, no intent is orphaned, and the sweep continues to the
next row. `_lc_refuse` cannot serve here: it "EMITS, THEN DIES. Never returns." (`ccd/ccd:4229`, `die` at
`:4341`), and `ws-gc --prune` must keep sweeping. No rollback fiction.
**The NAME carries the distinguishing property, deliberately (round 14, B-M5).** The earlier spelling,
`_lc_refused`, differed from `_lc_refuse` by one trailing character while differing in arity — FIVE
positionals against `_lc_refuse`'s four — `act id token detail [k v]...` (`ccd/ccd:4229`), bound by
`local a="${1-}" i="${2-}" tok="${3-}" msg="${4-}"; shift 4` (`:2782`) — and in fatality.
A call site that typed `_lc_refuse` where it meant the sibling would silently shift every argument by one
(`token` taking the tx, `detail` taking the token) **and** kill the `ws-gc --prune` sweep at the first
declining row, which is precisely the outcome this emitter exists to avoid. `_lc_refuse_return` puts the
property that distinguishes them — it returns — in the name, so the mistyping is a name that does not
exist rather than a name that does something else. Task 9 adds the source pin alongside: every `_lc_refuse`
call site passes exactly four positionals and every `_lc_refuse_return` call site exactly five, the two name
sets disjoint. Measured, the new name is also safe against the guard its neighbourhood must evade —
`server/test/ccd-wsaudit-nonpoison.test.ts:32-35` harvests `_reap_refuse\s+`, `"refused":"`, `'!` and
`"verdict":"`, none of which `_lc_refuse_return` matches, so the pin `expect.soft(scan(src)).toHaveLength(55)`
(`:78`) is unaffected.

**The one-terminal-fact guard is keyed on the CALLER's own act and on a `_lc_tx`-MINTED, NON-EMPTY tx:
exactly one terminal fact per (act, id, tx) among a caller's own `_lc_done <act>`, `_lc_fail <act>` and the
new `_lc_refuse_return <act>`.** `_lc_tx` (`ccd/ccd:3590-3602`) mints `<ns>.<BASHPID>.<seq>` with
`printf '%s.%s.%s'` (`:3693`) and can never return empty, so the non-empty key is exactly the set of minted
transactions and nothing else. **Three emitters fall OUTSIDE the guard, all for the same literal-empty-tx
reason, stated here once and nowhere else:** (a) `_lc_refuse`, declared `act id token detail [k v]…` with no
tx positional at all (`ccd/ccd:4229`) and emitting a literal `""` tx (`:4325`) — and separately it never
returns, so it can never be followed by a terminal fact; (b) `_lc_done purge`, which occurs once in the whole
file (`ccd/ccd:3238`, measured — `grep -c '_lc_done purge'` is 1) and always passes a LITERAL empty tx, its
own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not half of a pair", `:3264-3265`); and
(c) `cmd_ws_restore`'s `_lc_done restore "$id" ""` (`:6780`) beside its two `_lc_fail restore "$id" ""`
failures (`:6838`, `:6844`) — the same act, the same id and the same literal empty tx, which a tx-keyed
guard admitting `""` would red on the CORRECT shipped tree. Measured, **twenty-one** lifecycle emits in
`ccd/ccd` pass a literal empty tx — `_lc_done` at `:1735`, `:1828`, `:1907`, `:1951`, `:4491`, `:5324`,
`:5647`, `:5738`, `:6293`, `:6780`, `:6885`, `:12657`, `:13492`, `:13948`, `:14058`, `:15315`, `:15382`,
`:15415`, `:15471` and `_lc_fail` at `:6838`, `:6844` (the four `_lc_refuse … ""` sites at `:6630`, `:6638`,
`:6646`, `:6706` pass an empty ID, not an empty tx) — so admitting `""` buckets unrelated acts across the
whole file and its only producible fixtures are nonsense pairs, `ccd ws-add`'s `_lc_done create "$id" ""`
(`:4491`) against a later `ccd ws-rm`'s `_lc_done purge "$id" ""` (`:1828`) on one id. That, not an absence
of fixtures, is why the key is the caller's own act over a minted tx. The keyed tuple has real subjects, and
both carry a minted `$lctx`, so the narrowing costs the pin nothing: `_ws_gc_prune_row`'s orphan arm emits
`_lc_done destroy … "$lctx"` (`:11813`) and `_lc_fail destroy … "$lctx"` (`:11816`) for the SAME
(act, id, tx), and `cmd_ws_rm` likewise at `:4913` and `:4858`. Measured, neither pair can fire twice today
— the first is an `if`/`else`, and the second is kept apart only by `die`'s `exit 1` (`:1022`) — so the
guard is a MUTATION pin, not a live-bug pin: it reds when a fixture flattens that `if`/`else` or drops the
`die`, which is precisely the regression it exists to catch.
...
(`ccd/ccd:5585-5596` and `:6020-6029`), and `cmd_ws_add`'s refusal interpolates the second into the message
...
`die "slug in use: $slug — in $REG: $(_ws_slug_residue "$project" "$slug")"` (`ccd/ccd:5091-5092`) — a
...
shipped contract stated six lines above it (`ccd/ccd:5081-5085`: "The refusal NAMES WHAT IT FOUND … those
...
(`ccd/ccd:4568-4572`), and it silently strands the slug in `_ws_slug_new` (`:4695-4705`) forever. So
...
unexpanded. Three shipped comments assert the old
...
dot-skip alone); `ccd/session-hook.sh:1098` ("The same dot-free shape is what `_ws_slug_free` scans"),
which is no longer the whole of what it scans; and `server/test/ccd-authdead.test.ts:68-72` (round 14, A-6),
whose describe-block comment carries the same falsified reason — "All three glob `\"$REG/$id\".*`, which
requires a literal dot AFTER the id" — and takes the same restatement: invisible because the marker is
DOTLESS **and** matches no `.<id>.`-prefixed private family, not because of the dot-skip alone. That third
copy's own assertions stay GREEN through Task 9, so nothing reds if it is left alone; the comment simply
becomes a lie, which is why it is named here and in Task 9's comment scope rather than left to a suite to
catch. The count and the enumeration are one fact: three named, three listed.
...
| stable canonical lock absent | initializer may publish it only from the private init source using `link`; a later canonical disappearance/replacement refuses, never recreates it — "later" MEASURED and not assumed: an exact-family `lock-open` alias on disk, or a descriptor on one in `/proc`, under the §3.4 gates and with §3.4's residuals (D-2793) **ON LINUX. On a box with no `/proc` the descriptor half cannot be asked at all, so a later disappearance under a live holder reads as a first-ever mint and the acquire mints — recorded Darwin platform residue (round M4), named in place at both `[ -d /proc ]` gates, with the cases that assert this row Linux-only for that reason.** |
...
| helper exited 0/3 but PreCompact's reconfirm of nonce ownership failed (round 7, newly reachable under option A) | publish nothing; no `STEER_TEXT` and no `steered` stamp — both stage-2-only acts that do not run in Plan A at all (§3.1 item 10, protocol step 13, §3.5), so in Plan A this row's content is the absence of the two renames; remove only this process's own stages (age-eligible residue on a failed reacquire); the compaction is measured against whatever a sibling published |
| (round 8, M1; **STAGE 2 / PLAN C ONLY** — its premise is a `STEER_TEXT` print, which Plan A never performs, so no Plan A fixture can reach this row) `STEER_TEXT` printed and the card-stage already renamed, but the private `steered` stage-rewrite or the following set-stage rename fails | canonical card exists and was already read by the model; canonical set remains the hook's earlier `files:null` placeholder (or an un-renamed, updated stage only); PostCompact records that placeholder — `cited:null`, no working set — for a compaction that WAS carded |
| `_spawn_start`'s single acquisition MISSES on contention (times out at `COMPACT_LOCK_WAIT`) | fail OPEN: the session spawns, carries no `CCRC_SESSION_GENERATION`, and a stderr warning names the lock. Its compaction lifecycle is inert until the next respawn — every arm fails closed on an absent generation — and nothing else about the session is affected. Failing closed here is rejected outright: it would let an in-flight PreCompact wedge a rescue swap (§3.4, row creation/spawn) |
| row creation's acquisition MISSES (contention, or an unsafe lock) | fail CLOSED, before any row field is written: die with a retryable message naming the lock, leaving NOTHING on disk — no `.uuid`, no `.generation`, no partial row. Nothing irreversible has happened, so re-running the verb is the whole remedy |
...
| ccd purge LOCK miss (or unsafe lock) | no purge-done fact, NOTHING deleted, generation and registry remain; the three post-action callers report `_lc_fail`; the dead-reg pre-action caller alone may DECLINE — but declining is a code change, not the shipped arm: it must branch on `_reg_purge`'s status and emit `_gc_declined` **beside one `_lc_refuse_return destroy <id> "$lctx"`** (the non-fatal, tx-bearing refusal emitter Task 9 adds, §3.4), with no `_lc_done destroy` and no `_gc_reclaimed`. The refusal is not optional garnish: `$lctx` was minted and an `intent destroy` already emitted at `ccd/ccd:14299-14301` before `_reg_purge` ran, so a decline with no terminal fact orphans that intent on every sweep |
| ccd purge MUTATION failure (an `rm -f` inside the unlink loop) | **the purge-done fact is ALREADY journaled** — the emit (`_lc_done purge`, `ccd/ccd:3238`) precedes the loop unconditionally (before `:3329-3381`) — so registry and/or generation may remain WITH the fact on disk; callers report `_lc_fail`, never success. (Round 9's single combined row said "no purge-done fact" for this case too, which is false; corrected round 10, B-4) |
| (round 9, SCOPED round 10) lock MECHANISM absent — no `flock(1)` resolvable by the calling process, established by `command -v flock` before any acquire is attempted, never by a failed acquire | the three hook arms fail CLOSED: no card, set, stage, claim, marker or journal is ever published. `ccd ws-add` (`ccd/ccd:6110`), `ccd ws-restore` (`:8185`) and `ccd ws-reap` (`:11742`) keep their SHIPPED fail-CLOSED refusals and are outside this ruling. Of the `_reg_purge` callers, only `ws-rm` (`:5667`), `forget` (`:19488`) and `ws-gc --prune` (`:12595`) are governed, and they are gated on `$REG/<id>.generation`: **absent** ⇒ fail OPEN (`_reg_purge` runs its unlink loop and emits its purge-done fact, exactly as before D-2605 — no hook on that row ever held a generation, so nothing can race); **present** ⇒ fail CLOSED with the distinct mechanism-absent refusal, the **two** post-action callers that can be reached on such a box — `cmd_ws_rm` (`:5667`) and `cmd_forget` (`:19488`) — reporting `_lc_fail`, while `ws-gc --prune`'s dead-reg arm (`:12595`), which reaches `_reg_purge` before any irreversible act, DECLINES — and `_ws_reap_locked` is unreachable. **Declining is not "unchanged".** The arm BEFORE Task 9 (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:13588-13653`) never read `_reg_purge`'s status — `ccd/ccd:9` is `set -uo pipefail` with no `-e`, then and now — and followed it UNCONDITIONALLY with `_lc_done destroy … "$lctx"` (`:11669`) and `_gc_reclaimed "dead registry entry …"` (`:11670`), so left unchanged it would have journaled a completed `destroy` and printed `reclaimed` for a row whose registry entry and `.generation` still stand, and repeated that on every sweep — the success claim §3.4's "never claims success" forbids. Task 9 must branch on `_reg_purge`'s status and, on a non-zero result, emit `_gc_declined` (this function's own decline reporter, the shape at `ccd/ccd:14287`, `:14641`, `:14747`) **plus exactly one `_lc_refuse_return destroy <id> "$lctx"`**, with **no `_lc_done destroy` and no `_gc_reclaimed`** — the refusal closing the `intent destroy` already emitted at `:14634-14636`. Recovery is per-verb, not one clause (round 14, B-I5): `ws-rm` recovers by re-running from a flock-capable PATH; `forget` recovers by re-running from a flock-capable PATH **only while a tmux server is answering**, because its own liveness precondition — `case "$(_session_verdict "$id")" in` (`ccd/ccd:20945`), whose `unknown` arm emits `_lc_refuse forget "$id" session-verdict-unknown` (`:19455`) — refuses on `unknown`, which is what both no-server spellings yield, and since `forget` runs `tmux kill-session` (`:19120`) one line before `_reg_purge` (`:19488`), forgetting the box's LAST session takes the server down with it. The remedy is to bring any tmux server up and re-run; `ws-gc --prune` cannot substitute, its scan walking `for f in "$REG"/*.workspace` (`:12365`) while `forget` refuses any row carrying a `workspace` field (`:16603-16605`). Row creation and `_spawn_start` skip generation initialization/export and continue **for row writes that do not pass through `cmd_ws_add`** — `ws-add` itself dies first. `_ws_slug_free` answers consistently, since on a box where no arm ever published, no compaction private residue exists to see (§3.4 Platform outcome, the five-site table, the generation gate, and facts i–iv) |
...
| **(restored round 11, Package C — every measurement row below was deleted from this table by D-2605 round 1 and none had an equivalent at the chain tip)** served session: the payload's `transcript_path` is empty | no set, no card, no steering; PostCompact still measures, committing one record with `chars`/`filesChars`/`fences` and `scope`, `cited`, `setSize` **and all six provenance fields** null — the normalized no-set form, which is what `JOURNAL_RECORD_PRED` accepts (a `scope:null` record carrying any non-null provenance field is rejected and no line commits at all) |
...
| PostCompact settles a canonical set older than `COMPACT_CARD_MAX_AGE` | the set is still CLAIMED under the lock and canonical unlinked, exactly as §3.4's Settlement "Canonical set present" branch specifies — age is saved for provenance eligibility, it never gates the claim. Its age makes it provenance-INELIGIBLE, so the one committed record carries `scope`, `cited`, `setSize` **and all six provenance fields** null, while `served` is still derived from the exact nonce marker (nonce-only settlement, §3.4 and §2's `COMPACT_CARD_MAX_AGE` row). Measured with this section's own `JOURNAL_RECORD_PRED` fence under jq 1.7: that record is accepted with `served:true` and with `served:false`, while carrying any single provenance field through — `transcript`, `cwd` or `liveAgents:0` — is REJECTED, so a partial null list loses the line entirely |
| the claimed set will not parse, or any `files[]` entry lacks a non-empty string `path` | measured as NO set — `measure` runs without `--set`, so `scope`/`cited`/`setSize` **and all six provenance fields** are null, the same normalized no-set form the predicate requires. **Never a smaller valid working set, and never 0**: dropping a corrupt entry would report a citation ratio against a set size the session never had |
...
| card text reaches the model | repo-controlled labels and paths only (graph labels, repo-relative paths, symbol names). **Three distinct acts on three distinct subjects, not two alternatives (round 13, B-M5):** the STANDING text is clipped by `CARD_MAX_CHARS` (`ccd/session-hook.sh:90`), the COMPACT card appended after it is clipped by `COMPACT_CARD_MAX_CHARS` (`:97`), and the CONCATENATION is pinned by the derived `CARD_TOTAL_MAX_CHARS` (`ccd/session-hook.sh:98`, derived at `ccd/session-hook.sh:2472`) — so the compact card passes under two ceilings, not one of two. It reaches stdout only through `_hook_emit_context`'s `jq -cn --arg c` envelope (`ccd/session-hook.sh:103`); never interpolated into a `jq` program's text, and never left as an unquoted expansion in a command word, `eval` or `bash -c` — a quoted expansion into an argument of `mv`/`rm`/`link` being the compliant form — with the `steered` stage rewrite (§3.1 item 10, §3.5) bound the same way when stage 2 ships |
...
The earlier tasks' mutation steps remain binding for scope, card, helper, normalization, citations, fences
and exact-one helper output. **Their home is a MEASURED set of plan items, quoted from the plan's own
...
what makes the clause checkable at all.

**Four subjects §8 records as D-2605's headline rejections were listed here as "covered" while no mutation
named them (round 14, A-2):** raw physical-line JSONL, the sixteen-key record, the absence of a persisted
`n`, and the absence of a hookstate cache. Measured over plan Tasks 1–8, `sixteen`, `raw JSONL`,
`` persisted `n` `` and `hookstate cache` each return zero hits. They are pinned by §3.4's predicate fence
and by nothing executable — so they are now Task 9 rows in the table below rather than a deferral to a step
that never mentioned them.

**The `cited` row's suffix rule — a suffix's left boundary
may be `/`, while a full path's may not — is implemented on this tree by `hasUniqueSuffixOccurrence`
(`ccd/compact-card.mjs:585`, called from `citedCount` at `ccd/compact-card.mjs:610`, distinct from
`hasFullPathOccurrence` at `ccd/compact-card.mjs:575`, called at `ccd/compact-card.mjs:604`), so the prose
and the code agree here rather than the prose describing an unported mechanism.**
Round 1 deleted the PROSE those plan items pin and left every one
...
| restore `exec {fd}<>"$lock"` or create-capable canonical open, **or delete the `_hook_lock_vanished` / `_compact_lock_vanished` gate from its acquire** | canonical-disappearance/old-inode holder race refuses promptly, recreates no canonical, and never admits split critical sections. The last mutant is the one this row went unbuilt on: with the gate deleted, a real holder plus a stranger's unlink lets the next acquirer mint a SECOND inode at the same pathname and both be told by `flock` that they hold "the" lock — measured before the gate, inode 167576 held, canonical unlinked, second acquirer `RC=0` at inode 167577. **THE MUTANT AND THE DARWIN BOX ARE THE SAME TREE (round M4):** where `/proc` is absent the gate is present but answers nothing, so this row's property is NOT held there and its cases skip — recorded platform residue, never a weakened assertion |
...
| **(round 9, REWRITTEN round 10 — A-I2/B-2) lock-mechanism absence behaves as §4's row says** | Fixture HOME with `flock` made unresolvable by SHIMMING `command` — `command() { [[ "${1-}" == -v && "${2-}" == flock ]] && return 1; builtin command "$@"; }` — per `server/test/ccd-ws-reap.test.ts:357`'s own idiom, **never by emptying PATH**, because every stub in that suite spells its passthrough `command git`/`command find` and an emptied PATH would break them. Assert, leg by leg: (a) `ccd ws-add` REFUSES, exit non-zero, stderr containing its shipped `refusing to create a workspace unserialised`, and creates no registry field; (b) `ccd ws-reap` REFUSES with its shipped `_lc_refuse reap … flock-unavailable` / `refusing to run the destructive verb unserialised`, prints no document, and emits no `purge` fact; (c) `ccd ws-rm`, `ccd forget` and `ccd ws-gc --prune` on a **generation-ABSENT** row COMPLETE and `_reg_purge` emits exactly one `_lc_done purge`; **(d) SPLIT IN TWO (round 11, A-I5/B-M5), because the three governed callers do not answer alike and the round-10 wording let an implementer guess** — (d1) `ccd ws-rm` and `ccd forget` on a **generation-PRESENT** row REFUSE with the distinct mechanism-absent condition and each emits exactly ONE named `_lc_fail` and no `_lc_done purge`; (d2) `ccd ws-gc --prune` on the SAME row DECLINES, emitting neither `_lc_fail` nor `_lc_done purge`, because its dead-reg arm calls `_reg_purge` before any irreversible act — **and the leg must assert the decline POSITIVELY and the false success NEGATIVELY**, or the shipped arm passes it: assert a `declined` row naming the id, **exactly ONE `refused` fact for that minted `$lctx` (round 13, A-I4)**, NO `_lc_done destroy` and NO `_lc_fail` for that `tx`, NO `reclaimed` row, and `$REG/<id>.uuid` and `$REG/<id>.generation` still standing. The arm BEFORE Task 9 emitted `_lc_done destroy` (`8e457995:ccd/ccd:11669`) and `_gc_reclaimed` (`:11670`) unconditionally — superseded at this tip, where the status-0 `else` alone reaches that pair (`ccd/ccd:13998-14000`) — so leg (d2) as a bare "emits neither `_lc_fail` nor `_lc_done purge`" is passed by exactly the false-success implementation it exists to forbid. A leg asserting a THIRD `_lc_fail` is unsatisfiable on this fixture: `_ws_reap_locked` is never entered, `cmd_ws_reap` having died at `ccd/ccd:12758-12760`. Mutations: making `ws-gc --prune` emit `_lc_fail` reds (d2); restoring the status-ignoring arm Task 9 replaced so a `_lc_done destroy` and a `reclaimed` row appear reds (d2); suppressing either reporter reds (d1); (e) the three hook arms publish NOTHING — no card, set, stage, claim, marker or journal line; (f) `_ws_slug_free` answers consistently with that state. Mutations, each reddening exactly one leg: deleting `ccd/ccd:6065-6066` or `:12032-12034` reds (a)/(b); making `_reg_purge` refuse on mechanism-absence regardless of generation reds (c); making it proceed regardless of generation reds (d1) and (d2); making the hook arms fall back to an unlocked publish reds (e) — that is §10's forbidden second regime. And establishing mechanism-absence by ATTEMPTING AN ACQUIRE instead of `command -v flock` reds — **but not for the reason round 10 gave (round 11, M7).** This fixture shims the QUESTION and leaves the real `flock` on PATH, so inside it an acquire-probe does not fail: measured here, with `command` shimmed, `command -v flock` answers **1** while a direct `flock -w 1 <fd>` on the row's own lock answers **0**. The mutant therefore concludes "mechanism present", proceeds, and reds (d1)/(d2) — the right red for the opposite reason to the one stated, which would send a reader debugging it to the wrong place. Add the explicit control asserting that direct `flock -w 1` answers 0 inside this fixture, so the mutation's redness is attributable to the probe substitution and not to a missing binary. The GENERAL argument — that `command -v` absence and a contended `flock -w` both spell their failure `1`, so the distinction must be carried by WHICH probe answered — is fact (iv), stated in §3.4, and it is measured there (uncontended acquire 0, contended acquire 1, `command -v` on a PATH without the binary 1; a genuinely missing binary invoked as a command answers **127**, not 1, which is a third value neither fact names) |
| interpolate card, set, summary or graph-derived text into a `jq` program's text or a shell word instead of passing it as `--arg`/`--argjson` | source scan over `ccd/session-hook.sh`'s three compaction arms: every `jq` invocation that receives lifecycle or graph text must receive it through `--arg`/`--argjson`/`--rawfile`, and no `eval`, `bash -c`, or unquoted expansion of a card/set/summary/graph variable may appear in those arms. Non-vacuity: the scan must FIND `:94`'s `jq -cn --arg c "$text"` and `:875-885`'s `--arg`/`--argjson` set-document build. Mutations, **Plan A**: rewrite `_hook_emit_context` to interpolate `$text` into the program instead of `--arg c` ⇒ reds; and the `mktemp`/`link`/`rm`/`jq` statements Task 9 actually adds around nonces, claim names and marker names — build any of those command words by unquoted expansion, or any of those `jq` programs by interpolation, ⇒ reds. **STAGE 2 / PLAN C ONLY, exactly as §5's steering-order row below is:** build the `steered` stage stamp (§3.1 item 10) by string interpolation ⇒ reds. Plan A builds no such statement — §3.1 item 10, §3.5 and §6 all say the stamp is one of the two stage-2-only acts that do not run here — so on a correct Plan-A tree that leg has no target and must NOT be attempted; running it is how a stage-2 act gets built into Plan A to satisfy a matrix |
...
| ignore `_reg_purge` result in any of four callers, or report a post-action lock-miss as decline/unchanged instead of `_lc_fail` | held-lock real fixture emits a named `_lc_fail` (not a suppressed success or a bare decline) for `cmd_ws_rm`, `_ws_reap_locked`, and `cmd_forget`, naming completed action and retained registry/generation. The dead-reg pre-action caller may DECLINE, but it may not ignore the result either, and its decline must carry one `_lc_refuse_return destroy` for the minted `$lctx`: restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:12942-13007`) makes a `_lc_done destroy` and a `reclaimed` row appear for a row still standing, and reds |
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse_return <act>` on the same (act, id, MINTED tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:14535` / `:14886`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:6700`, `die` at `:1467`) so its own `_lc_done destroy` (`:6822`) is reached too; the guard reds, and removing the guard makes both mutants green. **The tx key is `_lc_tx`-MINTED and NON-EMPTY** (`ccd/ccd:3590-3602`, `printf '%s.%s.%s'` at `:3693`, never empty), which is what keeps the three literal-empty-tx emitters outside it — `_lc_refuse` (`:4229`, emitting `""` at `:4138`, and never returning), `_lc_done purge` (one call site, `:3238`, empty by design, `:3264-3265`: "UNCONDITIONAL, and with NO `tx`: this is not half of a pair") and `cmd_ws_restore`'s `:9665`/`:9710`/`:9729` triple, which a `""`-admitting guard would red on the CORRECT shipped tree. **GREEN control:** drive `ws-restore` to a landed undo plus a failed spawn and assert the guard passes; widening the guard to admit empty-tx facts must red that same control |
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:14299-14301` with zero terminal facts and reds; restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:13601-13666`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
...
| **(round 7, amended by the whole-branch re-review B-M2) stage completeness under a killed helper** | kill the helper mid-write (SIGKILL between its `.part` write and rename); assert that NEITHER a stage file NOR its `.part` survives — the arm's own residue sweep removes the `.part` too — and that the hook publishes nothing; pinned by `server/test/session-hook.test.ts:3776`, whose terminal assertion at `:3791` reads `expect(stages(), 'neither the stage nor its .part survives').toEqual([])`. `stages()` (`server/test/session-hook.test.ts:3718-3719`) filters on `n.includes('.stage')`, so it covers `.stage.part` and that assertion is over both. The narrower residual the EFFECT fixture this row first prescribed was written to observe cannot be observed at all: measured with `strace -f -e trace=write` over a `writeFileSync` of 4 KiB, 64 KiB, 256 KiB and 1 MiB, there is exactly ONE `write(2)` at every size, so no SIGKILL lands between two of them. What ships for the intra-`writeAtomic` window instead is the SOURCE pin D-2802 names, `server/test/compact-card.test.ts`'s `A STAGE EXISTS IFF IT IS COMPLETE` |
| **(round 7, corrected round 9, SPLIT round 10 — A-I3; the order leg re-mechanised round 11 — B-M9) publication order — the PLAN A half, owned by Task 9** | **Leg 1, a SOURCE-ORDER pin, not a behaviour fixture:** inside `_hook_compact_pre`'s reacquired held section, assert the card-stage `mv`'s source offset is LESS than EVERY set-stage `mv`'s, and assert the COUNTS beside the order — exactly one card-stage rename statement, and exactly N set-stage rename statements with N pinned. **The counts are load-bearing because protocol step 13 has two publishing arms** (rc 0 renames card-stage then set-stage; rc 3 renames set-stage alone), so a natural `case "$helper_rc" in 0) mv card; mv set ;; 3) mv set ;; esac` has TWO set-stage renames and "the set-stage `mv`'s offset" is then undefined — the definite article assumed a factoring this document never required. Either implementation is permitted; the pin records which shipped, and splitting or merging the set-stage rename changes N and reds rather than being silently absorbed. Swapping the two statements reds, and the CONTROL is that with them swapped every behaviour test stays green — which is what shows a behaviour fixture could never have carried this leg. Round 10 kept "assert the card-stage rename precedes the set-stage rename (swapping the two reds)" with no mechanism named, and in Plan A there is none to find: the two renames are adjacent inside ONE held lock, the print and the stamp are "(stage 2 only)" and neither runs, so no concurrent hook can observe the intermediate state and the post-arm state is byte-identical under either order. An unpinned outcome is the defect class the round-10 split existed to close, reopened one row down. (The alternative mechanism — force the set-stage rename to fail and assert a canonical card exists beside the pre-helper set — is §4's own forced-set-rename-failure state and remains available; the source-order pin is chosen because it needs no injected failure.) **Leg 2, producible as written:** the arm publishes the helper's staged `steered:false` **byte-for-byte** — run the helper standalone into a temp stage with the same argv and compare; any hook-side rewrite of that member reds. That no `jq` ever rewrites the canonical set is already carried by the inverted source scan below, which reds on an unlocked or canonical-targeted `_hook_write_atomic`. These are the only legs a Plan A fixture can produce, because Plan A runs neither the print nor the stamp |
...
| **(round 7, recognizer INVERTED round 9) absent-canonical has exactly one meaning** | The round-8 recognizer grepped for a canonical-set literal *adjacent to* a primitive, and measured against the shipped tree it matches ZERO canonical-set mutation sites: in `ccd/session-hook.sh` the literal `compactset` occurred only at `8e457995:ccd/session-hook.sh:795`, `:796`, `:1049`, `:1118`, `:1184` and the comment at `:1094` — all six read on the tree this row was written against and all six superseded at this tip, where `compactset` occurs on fourteen other lines and on none of these; there none of them carries the `rm`, `mv -f` or `link` COMMAND and `:1094`'s `unlinks` is prose, and it is the one line in either file where a canonical literal sits beside a primitive-shaped word, which is why it is named again below; every mutating line names only a variable — e.g. `:782`, `:802`, `:810`, `:817`, `:825`, `:836`, `:954`, `:964`, `:970`, **a SAMPLE and not the complete set (round 13, B-M3):** measured, further mutating lines exist at `:781`, `:797`, `:798`, `:997`, `:803`, `:811`, `:818`, `:824`, `:826`, `:831`, `:865`, `:923`, `:953`, `:955`, `:965`, `:971`, `:974`, `:1633` and `:1634`, and the claim holds of every one of them, so the argument survives an incomplete list. Two of those, `:865` (`rm -f "$cardf"`) and `:923` (`rm -f "$f"`), appear again in the allow-list below as entries (1) and (6) — there for a different reason, as canonical MUTATIONS the filter produces, not as evidence about literals; and `ccd/ccd` contains ZERO occurrences of `compactset`/`compactcard`/`compactions` anywhere (measured `grep -c` = 0), so its own named inventory entry is unreachable by the stated pattern. **Round 9 added "and the whole scan's single hit across both files is a comment (`ccd/ccd:3074`)"; that clause is dropped as unmeasured (round 11, M2).** Measured, `ccd/ccd:3074` reads `# this \`rm -f "$REG/$id".*\` directly were an unanchored glob: \`$id.*\`` — a primitive beside a registry GLOB, carrying no canonical-set literal at all, so it cannot be a hit of a recognizer defined as "a canonical-set literal *adjacent to* a primitive". The one line in either file that actually pairs a canonical literal with a primitive-shaped word is `ccd/session-hook.sh:2443` ("unlinks them with the row: `.compactset`"), also a comment. What the row needs to say is only what is measured: the round-8 recognizer's match set across both files is empty of real mutation sites, which is the whole argument. **The recognizer is therefore inverted.** Enumerate every rename/unlink/link/write primitive in the named files — `mv`, `rm`, `link`, `>` redirection and `_hook_write_atomic` in `ccd/session-hook.sh`; the BODY of `ccd/ccd:3037-3089`'s purge sequence, not only its glob line; and `renameSync`/`unlinkSync`/`linkSync` in `ccd/compact-card.mjs`, which imports all three at `:23` and after Task 9 retains exactly the two inside `writeAtomic` (`:422-431`) — its `renameSync` at `:426` and the failure-path `unlinkSync` at `:428` — permitted only because no canonical pathname reaches the helper's argv; every other current occurrence (`:502`, `:506`, `:507`, `:508`, `:517`, `:519`, `:520`, `:528`, `:529`, `:537`, `:538`, `:546`, `:552`) sits inside `:433-560`, which Task 9 deletes. **THE ORDER MATTERS, and round 9 stated it backwards (round 10, A-M3): the canonical-pathname resolution is the FILTER that PRODUCES the found set, and only then must the found set EQUAL the allow-list.** Read the other way round — "require EACH primitive to appear on a named allow-list" — the row demands that every `mv`/`rm`/`>` in a 1,400-line hook and the whole purge-loop body sit on a four-entry list, which no correct tree satisfies. **"CANONICAL PATHNAME" IS FIXED FOR THIS SCAN as the four canonical row artifacts — `$REG/<id>.compactset`, `$REG/<id>.compactcard`, `$REG/<id>.compactions` and `$REG/<id>.generation`** (Plan A's own Global Constraints), not the set and card alone. The narrower reading and this allow-list cannot both stand: entry 13 is a `.generation` unlink and entries 14–16 below are `.compactions` and `.generation` mutations, so a scan scoped to set+card produces none of them and the equality breaks. **The filter, and it covers the canonical CARD as well as the canonical SET (round 11, B-I2 — round 10 enumerated only the SET bindings while allow-listing a CARD rename, so the allow-list had a member the filter could never produce):** resolve the canonical pathname through the variables and positionals that carry it — **SET** `set="$REG/$id.compactset"` at `session-hook.sh:851` and `:1118` (the third binding at `:1184` belongs to `_hook_compact_served`, which this task DELETES, so it is absent from the post-Task-9 tree the scan runs over); **CARD** `cardf="$REG/$id.compactcard"` at `:1049` and `f="$REG/$id.compactcard"` at `:1118` (measured: `:1049` binds `set` and `cardf` on one line, `:1118` binds `f` and `set` on one line); `_hook_write_atomic`'s positional `$1` (`ccd/session-hook.sh:993`, mutating at `mv -f "$tmp" "$1"`, `:997`) traced from every call site that passes one; **a LITERAL canonical pathname written directly as a primitive's argument resolves to canonical** — the clause round 11's list needed and did not state, and the one that produces entry 13's `rm -f "$REG/$id.generation"`, which names canonical with no variable to trace and no glob to match (measured: `ccd/ccd` carries ZERO `compactset`/`compactcard`/`compactions` literals today, so every literal-canonical site in it is one this design ADDS); and — **new clause, round 10** — **a variable bound by a GLOB whose pattern can match a canonical basename resolves to canonical.** That last clause is what puts `ccd/ccd:3178` (`rm -f "$f"`, with `f` bound by `for f in "$REG/$id".*` at `:3175`) into the found set honestly: measured, `ccd/ccd` contains ZERO `compactset`/`compactcard`/`compactions` literals, so its allow-list entry is unreachable by variable-binding or text adjacency alone, and a second glob-based canonical unlink added elsewhere in `ccd/ccd` outside the lock would otherwise evade the scan entirely. **"Adjacent" means same statement**, not same line and not same file. The helper's two surviving `writeAtomic` primitives (`renameSync` `:426`, `unlinkSync` `:428`) are excluded **BY THE FILTER** — no canonical pathname reaches the helper's argv at all, which is option A's load-bearing property — **not** by an allow-list entry; they appear on no entry and must not. **THE ALLOW-LIST, ENUMERATED BY MEASUREMENT (round 11, B-I2).** Round 10's four entries were unsatisfiable: at least five further canonical mutations exist on a CORRECT post-Task-9 tree and appeared on no entry, so the equality was RED on the correct tree and the only green path was to weaken the filter — losing the mutation-effectiveness the row exists for. The complete list is every rename/unlink/link of the canonical SET or CARD in the shipped hook plus every one the target protocol adds, each named here with its arm, its protocol step and its lock state; **the filtered found set must EQUAL exactly the SIXTEEN SURVIVING entries — (1) and (3)–(17), entry (2) being withdrawn.** (1) PreCompact step 6's ambiguous-card removal, `rm -f "$cardf"` (`ccd/session-hook.sh:1439`), FIRST held lock. (2) **WITHDRAWN (D-2756, fix round 1):** §3.1 item 5 dropped the redundant-canonical-alias unlink as measurably redundant on this implementation, so there is no site for this entry to name and none may appear. The ID IS RETAINED RATHER THAN RENUMBERED, so every citation to entries (3)–(16) elsewhere in this document stays true; the equality below is over the SIXTEEN SURVIVING entries. (3) PreCompact step 7's initial publication, `_hook_write_atomic "$set" "$nonce" "$doc"` (call site `ccd/session-hook.sh:1488`, mutating at `mv -f "$tmp" "$1"`, `:997`), FIRST held lock — round 8's inventory omitted this site entirely. (4) PreCompact step 13's card-stage rename into `$REG/<id>.compactcard`, SECOND (reacquired) held lock. (5) PreCompact step 13's set-stage rename into `$REG/<id>.compactset`, same lock. (6) SessionStart(compact) step 2's aged-card deletion, `rm -f "$f"` (`ccd/session-hook.sh:1905`), the one retained lock. (7) SessionStart(compact) step 3's claim by rename, `mv -f "$f" "$claim"` (`:1989`), same lock. (8) SessionStart(compact) step 3's card restore on a losing, crossed or body-less claim, `link "$claim" "$f"` (`:2009`, `:2021`), same lock. (9) PostCompact settlement's claim link off canonical, `link "$set" "$claim"` — it names canonical, which is what puts it in the found set — settlement lock. (10) PostCompact settlement's canonical unlink, strictly before the claim `touch`, same lock. (11) PostCompact settlement's no-clobber restore after a failed `touch`, `link "$claim" "$set"`, same lock. (12) `_reg_purge`'s purge-loop body, `rm -f "$f"` with `f` bound by `for f in "$REG/$id".*` (`ccd/ccd:3158` → `:3070`), stable lock held for the whole function body. (13) `_reg_purge`'s generation-last unlink, `rm -f "$REG/$id.generation"` (`ccd/ccd:2547`), placed AFTER the archived/reaping tail (`:2433-2439`) and therefore OUTSIDE `:3037-3089`, same lock — it is on the list because `.generation` is a canonical row artifact by Plan A's own Global Constraints, beside `.compactcard`, `.compactset` and `.compactions`, and it is reached by the LITERAL-canonical clause above. (14) PostCompact's final journal transaction renaming its stage over `$REG/<id>.compactions` (§3.4, Journal contract), under the reacquired and validated FINAL lock — a `.compactions` mutation, invisible to any set+card scoping. (15) Row creation's no-clobber `link` mint of `$REG/<id>.generation` from its stable-lock-protected private source (§3.4, Generation), under the ROW-CREATION lock — a `link` whose DESTINATION names canonical, the same shape as entries 8 and 11. (16) PreCompact step 5's generation-read hard-link alias off `$REG/<id>.generation` and its alias unlink (§3.1's ADDED-BY-TASK-9 list), under the FIRST held lock — a `link` whose SOURCE names canonical, the same shape as entry 9. (17) `_reg_generation_read`'s generation-read hard-link alias off `$REG/<id>.generation` (`ccd/ccd:2793`, `link "$p" "$al"`) — a `link` whose SOURCE names canonical, the ccd-side twin of entry (16)'s hook-side one, under the caller's held lock. **Added in fix round 1 (D-2757):** this list enumerated the hook's generation-read alias and not ccd's, and the shipped scan produces this site on no entry — a gap in the LIST, not in the tree. Entries 14–16 are all on a CORRECT post-Task-9 tree and appeared on no earlier list, which is exactly why the thirteen-entry equality was red on that tree. **Named for completeness and EXCLUDED BY THE FILTER — each must appear on no entry, and a scan that produces any of them is over-broad:** PreCompact step 6's exact-family age sweep (`find … -name ".$id.*compact*.tmp" … -delete`, `:869`, and its Task 9 exact-family successor), whose dot-LEADING `.tmp` grammar cannot match a canonical basename, so the glob clause does not reach it; PreCompact step 13's stage `rm`s on the non-publishing outcomes, which remove the unrenamed private stages and never name canonical; SessionStart's `( set -C; : > "$claim" )` placeholder (`ccd/session-hook.sh:2197`) and its `rm -f "$claim"` at `:2220`/`:2010`/`:2220`/`:2255`; PostCompact's `touch "$claim"`, which is private and is not in the primitive list at all; the marker source's `mktemp`/`link`/`rm`; **the permanent lock's own init `mktemp`/`link`/`rm` and every acquisition's lock-open-alias `link`/`rm` (§3.4, Stable lock protocol steps 1–2) — the largest new family Task 9 adds, and absent from this list until round 13 (B-M4)**, excluded because `$REG/.<id>.compactions.lock` and `.<id>.compactions.lock-open.…` are dot-LEADING (so `for f in "$REG/$id".*` cannot match them) and are not among the four canonical row artifacts, which is also why the lock-open `link`'s SOURCE naming the lock does not make it entry-9-shaped; and the helper's two `writeAtomic` primitives named above. **NON-VACUITY control, mandatory (strengthened round 10):** assert the found set is NON-EMPTY and contains BOTH `_hook_write_atomic`'s call site at `ccd/session-hook.sh:1488` AND `ccd/ccd:3178` — a scan that finds nothing cannot red on anything, the round-8 spelling found nothing, and requiring only `ccd/session-hook.sh:1488` would leave the glob clause itself unproven. Added mutation: a second unlocked `rm -f "$REG/$id".*` loop elsewhere in `ccd/ccd` must red; under the round-9 resolution rule, with no glob clause, it stayed green. Mutations: restore `_hook_compact_rollback_set` — the family Task 9 DELETED, superseded at this tip and re-anchorable at no line, so both numbers below name the era they were true in — (`8e457995:ccd/session-hook.sh:792-819`) verbatim ⇒ reds, reached through `$set` → `$1` → the `mv -f "$set" "$claim"` at `8e457995:ccd/session-hook.sh:802`; add an unlocked `_hook_write_atomic "$set" "$doc"` outside any held lock ⇒ reds; **add an unlocked `mv "$stage" "$REG/$id.compactions"` outside the final held lock ⇒ reds** (green under every list before this one, because no entry and no filter clause reached the journal at all); delete any ONE of the surviving entries ⇒ reds. Both stay GREEN under the round-8 literal-adjacency spelling, and both leave every behaviour test green |
...
| **(round 9, STRENGTHENED round 10 — B-3) widen `_ws_slug_free` without widening `_ws_slug_residue` AND `cmd_ws_add`'s message template, so `ws-add`'s refusal names nothing — or names a path that does not exist** | Plant `.<project>-<slug>.compactions.lock-open.1.2.3` in a fixture `$REG` beside a second live id `<project>-<slug>.mesa`, run `ccd ws-add <project> <slug>` — **the POSITIONAL form, which is the only one that exists (round 14, B-I1)**; `cmd_ws_add` has no `--slug` flag, its usage being `ccd ws-add [--no-rc] [--surface <word>] [--actor <text>] <project> [slug]` (`ccd/ccd:5980`), and its arg loop's catch-all `*) lc_args+=("$1"); shift ;;` (`:4950`) would bind `project` to the literal string `--slug`, which `_ws_project_valid` (`:4530-4532`) ACCEPTS (`-` is in its character class), so the verb dies at the `[[ -d "$main/.git" ]]` gate (`:4978`), whose message reads `not a git repo:`, having never reached the flock gate (`:5059`), `_ws_slug_free` (`:5078`) or the die template (`:5079`) — leg (a) passing for the wrong reason and legs (b) and (c) failing on a CORRECT tree. `server/test/ccd-workspaces.test.ts:249-251` already spells the operator path the same way ("The operator path: `ccd ws-add <project> <slug>`, i.e. the slug given positionally"); the `CCD_WS_SLUG` env route (`ccd/ccd:5773-5776`, used at `server/test/ccd-workspaces.test.ts:272`) is an equally valid spelling. Assert **(a)** it refuses, **(b)** the die message contains the literal basename `.<project>-<slug>.compactions.lock-open.1.2.3`, and **(c) every filesystem path the message names EXISTS** — parse the message, `stat` each path. **(c) is the mutation-effective part:** it goes RED under the round-9 text as written, because the `$REG/<id>.{…}` template cannot express a dot-leading name, and GREEN only once the template is replaced by a plain list rooted once. Reverting `_ws_slug_residue` alone must still red (a)+(b), while `_ws_slug_free`'s own not-free pin stays green, so the two functions cannot drift apart again. Measured on the shipped tree, `_ws_slug_free` (`ccd/ccd:4642-4653`) and `_ws_slug_residue` (`:5353-5362`) carry identical dot-skip logic and `cmd_ws_add`'s refusal at `:6083-6085` interpolates the latter, so widening only the former yields `slug in use: <slug> — in $REG:` with nothing after the colon — naming no file, falsifying the shipped contract five lines above it (`:5819-5823`, "The refusal NAMES WHAT IT FOUND … The list is the reclaim instruction") |
| **(round 14, A-2) drop or add a key in `JOURNAL_RECORD_PRED`'s sorted-key list** — the four subjects §8 records as D-2605's headline rejections had no row at all, so a writer could drop a key, reintroduce an ordinal, or cache the measurement and nothing named would red | a fixture record carrying exactly the sixteen keys is REFUSED after a key is dropped from the predicate's list, and a fixture record carrying a seventeenth is ACCEPTED after one is added — in both mutants no journal line commits for that compaction, which is the observable the arm must show. The control on the correct tree is the same two fixtures passing and refusing respectively |
| **(round 14, A-2) replace the build-and-rename journal transaction with `printf >>`** | the partial-failure fixture (the write interrupted between the first and last byte of the appended record) LOSES the old bytes — the journal file after the mutant is shorter than, or not a prefix of, the file before it. On the correct tree the same fixture leaves the pre-existing bytes exact, because the new content is built beside the file and renamed over it |
| **(round 14, A-2) add an `n` member to the committed record** | the sixteen-key equality reds — the committed line's sorted key set is compared for EQUALITY against the sixteen, so a seventeenth key fails it regardless of value. This is the row for "no persisted `n`": position in the file supplies the ordinal, and the record must not carry one |
| **(round 14, A-2) write any `compaction` member into hookstate** | a hookstate source-and-shape scan reds: no arm of `ccd/session-hook.sh` may write a `compaction` key into `$REG/<id>.hookstate.json`, and the shipped hookstate fixture's key set must not gain one. This is the row for "no hookstate cache" — Plan A keeps no cached chip value to carry or clear |
...
- **Waits and lock division.** Ordinary hook paths are lock-free. PreCompact, compact SessionStart, and
  ...
  (`ccd/ccd:6110`, `:8185`, `:11742`), which this design does not touch. `_reg_purge` reached through
  `ws-rm`, `forget` or `ws-gc --prune` keeps working as it did before D-2605 **only on a row with no
  `$REG/<id>.generation`** — a row that carries one refuses, recoverably, per §3.4's generation gate
  (§3.4, Platform outcome, and §4's row). **That gate is `_reg_purge`'s alone.** Row creation and
  `_spawn_start` are **NOT gated on generation presence** at any value: they skip generation
  initialization/export and continue, for row writes that do not pass through `cmd_ws_add` — `ws-add`
  itself dies at `ccd/ccd:6110`. Gating them would refuse spawn on every row that has ever compacted,
  which is the permanent wedge §10's scoping residual declines to pay. Mechanism-absence is established by
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  call it — only the dead-reg pre-action caller may decline. **Declining was a code change there, not the
  absence of one:** the arm BEFORE Task 9 (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by
  `ccd/ccd:12594-12659`) never read `_reg_purge`'s status, so left as it
  was it followed a refusal with an unconditional `_lc_done destroy` (`:11669`) and `_gc_reclaimed` (`:11670`)
  — a journaled completed destroy and an operator-facing `reclaimed` line for a row that still stands. Task 9
  branches on the status and emits `_gc_declined` (`ccd/ccd:13996`, `:14736`, the same shape as `:14287`,
  `:14293`, `:14747`) with neither of those, plus
  exactly one `_lc_refuse_return destroy <id> "$lctx"` — the **non-fatal, tx-bearing refusal emitter Task 9 adds**
  beside `_lc_fail`/`_lc_refuse` (`ccd/ccd:4206-4235`), which emits the `refused` fact and RETURNS so the
...
- **Card text reaches the model through one envelope, and never through a shell.** The card and standing
  texts are repo-controlled (graph labels, repo-relative paths, symbol names), and they reach stdout ONLY
  through `_hook_emit_context`'s `jq -cn --arg c "$text"` envelope (`ccd/session-hook.sh:103`) — the single
  site every subject passes through. They are clipped twice before it: the standing text by `CARD_MAX_CHARS`
  (`ccd/session-hook.sh:90`), the compact card by `COMPACT_CARD_MAX_CHARS` (`ccd/session-hook.sh:97`), and
  the concatenation by `CARD_TOTAL_MAX_CHARS` (`ccd/session-hook.sh:98`), which is DERIVED from the other two
  (`ccd/session-hook.sh:2472`), never a third budget. **No
  lifecycle pathname, nonce, graph label, path or symbol is ever interpolated into a `jq` program's text, and
  none is ever left as an UNQUOTED expansion in a command word, `eval`, or `bash -c`** — every such value
  arrives at `jq` as `--arg`/`--argjson`/`--rawfile`, the way `at` already does at `ccd/session-hook.sh:1475`,
  and reaches every other command as a QUOTED expansion, which is the compliant form (round 13, A-M3: the
  earlier "never … a shell word" spelling forbade the shipped tree's own `mv -f "$f" "$claim"` at
  `ccd/session-hook.sh:2219` and `rm -f "$cardf"` at `ccd/session-hook.sh:1439`, neither of which can take a
  pathname as `--arg`, so a scan built from it was
  red on a correct tree; §5's row and Plan A's Task 9 item 2b already carried this satisfiable spelling). This binds the writers D-2605 adds as much as the shipped ones, **and, when stage 2
  ships, the `steered` stage rewrite of §3.1 item 10 / §3.5, which must pass its value by `--argjson`**
  rather than building its program by string interpolation — **Plan A builds no such statement**, the stamp
  being one of the two stage-2-only acts §3.1 item 10 and §3.5 say do not run here. This is the only injection invariant the design
  has for the one artifact that reaches the model's context, and it governs every new `mktemp`/`link`/`rm`/`jq`
  statement Task 9 adds around nonces, claim names and marker names.
...
1. **Plan A — agent lane**: §3.0–§3.4 (the scope rule, the three arms, the helper's two subcommands),
   §5's deferred Task 1–8 rows — §5 names where they live, and this item does not re-describe the artifact —
   plus the Task 9 rows §5 adds. The payload measurement this item once named as its first task
...
4. Deviation numbers are minted at plan time through `ccrc-api ledger allocate`; this spec points to Plan A's already allocated D-2605 and defines no number itself.
   Candidates the plans will number: the R1 sentence and hook-header correction, which live in Plan A's
   Global Constraints ("Plan C alone amends R1 for steering stdout") and in Task 9's comment scope — NOT in
   §6, which carries no R1 content; the `(not in
...
- **(round 9, SCOPED round 10) The scope of that inertness, and the part of it this design declines to pay.**
  ...
  wrongly put it there (round 10, B-2):** `cmd_ws_reap` already refuses at `ccd/ccd:12745-12747` on such a
  ...
  worth recording: on a box where `ccd ws-add` also refuses (`ccd/ccd:6317-6318`), **no workspace row and hence no slug is
...
- **A compaction that outlives `COMPACT_CARD_MAX_AGE` can have its journal line attributed to a SUCCESSOR's
  ...
  `ccd/session-hook.sh:1433`; §3.1 item 5
  ...
  PostCompact then settles by PATHNAME, finds B's set, claims it, unlinks canonical and measures it — and
  because `overlap` is false, §3.0's claimed-overlap normalisation, the design's only defence against
  attributing a set a different compaction wrote, does not fire. A's journal line therefore commits B's
  `scope` and all six provenance fields, and B's own reacquire finds canonical gone and publishes nothing.
  §2's `COMPACT_CARD_MAX_AGE` row asserts the premise that makes this rare ("a card or canonical set older
  than this belongs to no compaction that can still arrive", argued from the longest measured compaction,
  826 s × 1.45), but the premise is a bound, not a mechanism, and nothing measures its violation: PostCompact
  has no same-nonce check to make, because the payload carries no nonce. Stated here as the cost of the
  young-only overlap test rather than left implicit.
  **The SERVE half of the same premise, restated (round 13, B-I6 — the pre-D-2605 §10 carried a wrong-context
  serve residual; round 7 deleted it with a supersession that covers only the helper's old slot re-read-to-
  rename window, and the young-only overlap test reintroduces the class by a different mechanism, which this
  bullet stated only for the journal until now).** On the same premise, B publishes card_B beside set_B and
  does NOT remove A's card-era pair, so if A's own SessionStart(compact) has still not fired it finds card_B's
  first line equal to set_B's nonce — a MATCH by §3.3 steps 2–3, whose ONLY validity check is that
  nonce-equality (§4's crossed-pair row) — and serves a card mined from B's transcript into A's compacting
  context. `served` then reads TRUE off B's marker and the record lands under B's `transcript`. That is the
  thing §3.0 calls "wrong context, and wrong context is worse than none", reached without any wrong-context
  resolution having happened. **Which closure would shut it — the two candidate closures, neither adopted in
  Plan A, each defined here rather than pointed at (round 14, A-7):** (1) give PreCompact a rule degrading
  its own set to `overlap:true` whenever it overwrites ANY pre-existing canonical set, aged or not; (2) carry
  the nonce to PostCompact, so settlement can refuse a set it did not write. Of those two, **only the nonce
  carried to PostCompact is even about settlement, and it does NOT close the serve half** — SessionStart runs
  before PostCompact and would still serve the crossed pair. The `overlap:true`-on-overwriting-any-canonical-
  set rule is the one that bears on it, and it closes the serve half only in the aged-pair shape this bullet
  describes: PreCompact's step 6 removes the canonical card whenever its own verdict degrades to `ambiguous`
  (`ccd/session-hook.sh:1439`), so B's overwrite would take A's card with it and leave A's SessionStart nothing
  to serve. It does nothing for a wrong-context serve arising any other way. Neither closure is adopted in
  Plan A, and the serve half is disclosed here rather than left to be rediscovered.
