...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  call it — only the dead-reg pre-action caller may decline. **Declining is a code change there, not the
  absence of one:** the shipped arm (`ccd/ccd:11665-11670`) never reads `_reg_purge`'s status, so left as it
  is it follows a refusal with an unconditional `_lc_done destroy` (`:11669`) and `_gc_reclaimed` (`:11670`)
  — a journaled completed destroy and an operator-facing `reclaimed` line for a row that still stands. Task 9
  branches on the status and emits `_gc_declined` (`:11653`, `:11659`, `:11677`) with neither of those, plus
