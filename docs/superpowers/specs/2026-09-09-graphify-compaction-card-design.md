...
UNCONDITIONAL subset every run of the section produces is: two `find` (`:862`, `:869`), the
`at=$(_hook_epoch_ms)` substitution (`:871`), the `jq -cn` (`:875`), and `_hook_write_atomic`'s publishing
`mv -f` (`:782`) — plus, once Task 9 builds them, the generation-read `link` and its alias-unlink. The
...
BEFORE any irreversible action on that row, so it alone may decline on a lock miss, in the
`_lc_refuse`-shaped sense round 5 described for all four — by emitting `_gc_declined` instead of its
unconditional `_lc_done destroy`/`_gc_reclaimed` pair. No rollback fiction. **The one-terminal-fact guard is
keyed on the CALLER's own act, not on `_reg_purge`'s emit: exactly one terminal fact per (act, id, tx) among
a caller's own `_lc_done <act>`, `_lc_fail <act>` and `_lc_refuse <act>`.** `_lc_done purge` is explicitly
OUTSIDE it: it occurs once in the whole file (`ccd/ccd:1828`, measured — `grep -c '_lc_done purge'` is 1) and
always passes a LITERAL empty tx, its own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not
half of a pair", `:1822-1823`), while `_lc_tx` (`:2144-2156`) mints `<ns>.<BASHPID>.<seq>` and can never be
empty — so no path can pair the purge fact with anything under one tx, and a guard keyed there would bucket
every purge in the file under one empty-string key together with `cmd_ws_restore`'s two `""` failures
(`:6838`, `:6844`), which never call `_reg_purge` at all. The caller-keyed tuple, by contrast, has real
subjects: `_ws_gc_prune_row`'s orphan arm emits `_lc_done destroy … "$lctx"` (`:11813`) and
`_lc_fail destroy … "$lctx"` (`:11816`) for the SAME (act, id, tx), and `cmd_ws_rm` likewise at `:4913` and
`:4858`. Measured, neither pair can fire twice today — the first is an `if`/`else`, and the second is kept
apart only by `die`'s `exit 1` (`:1022`) — so the guard is a MUTATION pin, not a live-bug pin: it reds when a
fixture flattens that `if`/`else` or drops the `die`, which is precisely the regression it exists to catch.
...
| card text reaches the model | repo-controlled labels and paths only (graph labels, repo-relative paths, symbol names), clipped twice — `CARD_MAX_CHARS` (`ccd/session-hook.sh:81`) or `COMPACT_CARD_MAX_CHARS` (`:88`), then the derived `CARD_TOTAL_MAX_CHARS` (`:89`, `:1114`) — and passed to stdout only through `_hook_emit_context`'s `jq -cn --arg c` envelope (`:94`); never interpolated into a `jq` program's text, a shell word, `eval` or `bash -c`, the `steered` stage rewrite (§3.1 item 10, §3.5) included |
...
suite — "the heading regex loses a spelling" is `server/test/compact-card.test.ts:833-840`, which exercises
...
a set" is `:949-951` plus `:959-968`; "`files: []` is a known empty set, not a missing one" is `:969`;
"PostCompact measures
raw text" is `:827-831`, where an `<analysis>` block is excluded from `chars`; and the citation boundary and
within-set suffix-uniqueness rules are `:897-931`. **The `cited` row's suffix rule — a suffix's left boundary
...
| ignore `_reg_purge` result in any of four callers, or report a post-action lock-miss as decline/unchanged instead of `_lc_fail` | held-lock real fixture emits a named `_lc_fail` (not a suppressed success or a bare decline) for `cmd_ws_rm`, `_ws_reap_locked`, and `cmd_forget`, naming completed action and retained registry/generation. The dead-reg pre-action caller may DECLINE, but it may not ignore the result either: restoring `ccd/ccd:11665-11670`'s shipped status-blind arm makes a `_lc_done destroy` and a `reclaimed` row appear for a row still standing, and reds |
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse <act>` on the same (act, id, tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:11813` / `:11816`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:4858`, `die` at `:1022`) so its own `_lc_done destroy` (`:4913`) is reached too; the guard reds, and removing the guard makes both mutants green. **Not keyed on `_reg_purge`'s `_lc_done purge`**, which is outside this guard by design: one call site (`:1828`), a literal empty tx (`:1822-1823`), so it can pair with nothing — a tx-keyed guard would have no producible fixture at all |
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  branches on the status and emits `_gc_declined` (`:11653`, `:11659`, `:11677`) with neither of those.
...
- **Card text reaches the model through one envelope, and never through a shell.** The card and standing
  ...
  lifecycle pathname, nonce, graph label, path or symbol is ever interpolated into a `jq` program's text, a
  shell word, `eval`, or `bash -c`** — every such value arrives as `--arg`/`--argjson`/`--rawfile`, the way
  `at` already does at `:877`. This binds the writers D-2605 adds as much as the shipped ones, **explicitly
  including the `steered` stage rewrite of §3.1 item 10 / §3.5, which must pass its value by `--argjson`**
  rather than building its program by string interpolation. This is the only injection invariant the design
...
1. **Plan A — agent lane**: §3.0–§3.4 (the scope rule, the three arms, the helper's two subcommands),
   §5's deferred Task 1–8 rows (the plan's own per-task mutation tables, which §5 defers to by name) plus the
   Task 9 rows §5 adds. The payload measurement this item once named as its first task
...
- **A compaction that outlives `COMPACT_CARD_MAX_AGE` can have its journal line attributed to a SUCCESSOR's
  ...
  young-only overlap test rather than left implicit. The two closures available, neither adopted in Plan A:
  give PreCompact a rule degrading its own set to `overlap:true` whenever it overwrites ANY pre-existing
  canonical set, aged or not; or carry the nonce to PostCompact so settlement can refuse a set it did not
  write.
