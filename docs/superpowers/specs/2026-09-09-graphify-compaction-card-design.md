...
refusals (`ccd/ccd:5797`, `:7568`, `:11025`); round 9 asserted a blanket over "ccd's own registry operations"
...
that status rather than on a boolean — `_rm_prc` (`ccd/ccd:6478`), `_rt_prc` (`:13020`), `_pr_prc` (`:13950`)
...
(`ccd/ccrc:9726`), `_upd_backup_copy` in `ccrc update` (`ccd/ccrc:11053`), and the matching removal line
`"$reg/compact-card.mjs"` in `ccrc uninstall`'s own list (`ccd/ccrc:11698`). **Task 11** is this documentation
...
no-clobber `link`. `_plat_uuid` itself (`ccd/ccd:380-382`) emits a trailing LF on both its branches
...
five, read at this tip (D-2605's own acquire gate, `ccd/ccd:2545`, is the sixth `command -v flock` in the
...
`cmd_ws_rm` (`ccd/ccd:6465`) and `cmd_forget` (`:19488`) report `_lc_fail` naming the cause (mechanism absent
...
- **`ws-rm` recovers by re-running from a flock-capable PATH**, full stop. Measured: its re-run skips the
  dirty/foreign gates when `[[ -d "$workdir" ]]` is false (`ccd/ccd:6278`, the gate inside `cmd_ws_rm` — its base
...
- **`forget` recovers by re-running ONLY while a tmux server is answering.** Its liveness precondition is
  `case "$(_session_verdict "$id")" in` (`ccd/ccd:19109`), whose `unknown` arm is
  ...
  verdict `case` maps only `*"can't find session"*` to `gone` (`ccd/ccd:2081`); both absence spellings fall
  through to that `case`'s `*)` arm and become `unknown`, which the shipped comment at
  `ccd/ccd:2062-2063` names cmd_forget in by name — "Every other caller — `_alive`, `_ws_status`,
...
- **The remedy, traced rather than assumed: bring a tmux server back up, then re-run.** Any server on the
  ...
  measured: its dead-reg arm recomposes an id from `$project-$slug` (`ccd/ccd:13950`) and its scan walks only
  `for f in "$REG"/*.workspace` (`:13720`), while `forget` refuses any row with a `workspace` field at all
  (`:19440-19442`) — `ccd/ccd:19393-19396` states the same disjointness in shipped prose ("archive/reap are
...
(`ccd/ccd:3060`) — so the loop leaves it standing, and added one further explicit
`rm -f "$REG/$id.generation"` (`ccd/ccd:3199`) after the existing archived/reaping tail (which itself must run first, per the
...
`_compact_lock_why_remedy` (`ccd/ccd:2646`) rather than falling straight through to an unconditional
...
`ccd/ccd:4112-4113`), and the withheld `purge` fact is itself what lets a caller or a reader distinguish a
lock-miss
...
next row. `_lc_refuse` cannot serve here: it "EMITS, THEN DIES. Never returns." (`ccd/ccd:4121`, `die` at
`:4139`), and `ws-gc --prune` must keep sweeping. No rollback fiction.
...
positionals against `_lc_refuse`'s four — `act id token detail [k v]...` (`ccd/ccd:4121`), bound by
...
new `_lc_refuse_return <act>`.** `_lc_tx` (`ccd/ccd:3482-3494`) mints `<ns>.<BASHPID>.<seq>` with
`printf '%s.%s.%s'` (`:3493`) and can never return empty, so the non-empty key is exactly the set of minted
...
tx positional at all (`ccd/ccd:4121`) and emitting a literal `""` tx (`:4125`) — and separately it never
returns, so it can never be followed by a terminal fact; (b) `_lc_done purge`, which occurs once in the whole
file (`ccd/ccd:3130`, measured — `grep -c '_lc_done purge'` is 1) and always passes a LITERAL empty tx, its
own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not half of a pair", `:3064-3065`); and
...
(`ccd/ccd:5585-5596` and `:5608-5617`), and `cmd_ws_add`'s refusal interpolates the second into the message
...
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse_return <act>` on the same (act, id, MINTED tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:14157` / `:14160`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:6700`, `die` at `:1223`) so its own `_lc_done destroy` (`:6822`) is reached too; the guard reds, and removing the guard makes both mutants green. **The tx key is `_lc_tx`-MINTED and NON-EMPTY** (`ccd/ccd:3482-3494`, `printf '%s.%s.%s'` at `:3493`, never empty), which is what keeps the three literal-empty-tx emitters outside it — `_lc_refuse` (`:4121`, emitting `""` at `:4138`, and never returning), `_lc_done purge` (one call site, `:3130`, empty by design, `:3064-3065`: "UNCONDITIONAL, and with NO `tx`: this is not half of a pair") and `cmd_ws_restore`'s `:8948`/`:8993`/`:9012` triple, which a `""`-admitting guard would red on the CORRECT shipped tree. **GREEN control:** drive `ws-restore` to a landed undo plus a failed spawn and assert the guard passes; widening the guard to admit empty-tx facts must red that same control |
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:13921-13923` with zero terminal facts and reds; restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:13601-13666`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
...
- **Waits and lock division.** Ordinary hook paths are lock-free. PreCompact, compact SessionStart, and
  ...
  (`ccd/ccd:5797`, `:7568`, `:11025`), which this design does not touch. `_reg_purge` reached through
  ...
  itself dies at `ccd/ccd:5797`. Gating them would refuse spawn on every row that has ever compacted,
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  branches on the status and emits `_gc_declined` (`ccd/ccd:13618`, `:14010`, the same shape as `:13561`,
  `:13567`, `:14021`) with neither of those, plus
  exactly one `_lc_refuse_return destroy <id> "$lctx"` — the **non-fatal, tx-bearing refusal emitter Task 9 adds**
  beside `_lc_fail`/`_lc_refuse` (`ccd/ccd:4006-4035`), which emits the `refused` fact and RETURNS so the
...
- **(round 9, SCOPED round 10) The scope of that inertness, and the part of it this design declines to pay.**
  ...
  wrongly put it there (round 10, B-2):** `cmd_ws_reap` already refuses at `ccd/ccd:12367-12369` on such a
  ...
  worth recording: on a box where `ccd ws-add` also refuses (`ccd/ccd:5810-5811`), **no workspace row and hence no slug is
