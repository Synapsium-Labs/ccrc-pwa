...
`_ws_gc_prune_row` (`:12608`) and `_fg_prc` in `cmd_forget` (`:19131`) — and each renders the value through
...
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:13573-13575` with zero terminal facts and reds; restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:12607-12672`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
