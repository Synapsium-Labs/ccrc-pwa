...
members (round 13, B-M6) — it validates the environment generation,
...
forks repeatedly: `$(find "$f" -mmin …)` and the `rm -f "$f"` beside it at `ccd/session-hook.sh:1903`, the
...
`cmd_ws_rm` (`ccd/ccd:5667`) and `cmd_forget` (`:16642`) report `_lc_fail` naming the cause (mechanism absent
...
- **The remedy, traced rather than assumed: bring a tmux server back up, then re-run.** Any server on the
  ...
  measured: its dead-reg arm recomposes an id from `$project-$slug` (`ccd/ccd:12595`) and its scan walks only
  `for f in "$REG"/*.workspace` (`:12365`), while `forget` refuses any row with a `workspace` field at all
  (`:16603-16605`) — `ccd/ccd:16556-16559` states the same disjointness in shipped prose ("archive/reap are
...
3. **Sub-rule A.** A citation that names a function while pointing at a STATEMENT must have its range inside
   that function's body. `citedCount` is declared at `ccd/compact-card.mjs:708` and its body reaches `:717`,
   which is the line that calls `hasUniqueSuffixOccurrence` — so "called from `citedCount` at `:717`"
   stands under this sub-rule while a citation naming a function and landing outside its body does not.
...
`ccd/ccd:3346-3347`), and the withheld `purge` fact is itself what lets a caller or a reader distinguish a
lock-miss
...
absence of a lifecycle fact. Measured on the shipped arm, `ccd/ccd:11665-11667` has already emitted
`local lctx; lctx=$(_lc_tx)` and `_lc_intent destroy "$project-$slug" "$lctx" verb ws-gc …` before
`_reg_purge` runs at `:11668`, and `_gc_declined` (`ccd/ccd:12199`) is
...
next row. `_lc_refuse` cannot serve here: it "EMITS, THEN DIES. Never returns." (`ccd/ccd:3355`, `die` at
`:3373`), and `ws-gc --prune` must keep sweeping. No rollback fiction.
...
positionals against `_lc_refuse`'s four — `act id token detail [k v]...` (`ccd/ccd:3355`), bound by
...
new `_lc_refuse_return <act>`.** `_lc_tx` (`ccd/ccd:2733-2745`) mints `<ns>.<BASHPID>.<seq>` with
`printf '%s.%s.%s'` (`:2744`) and can never return empty, so the non-empty key is exactly the set of minted
...
tx positional at all (`ccd/ccd:3355`) and emitting a literal `""` tx (`:3372`) — and separately it never
returns, so it can never be followed by a terminal fact; (b) `_lc_done purge`, which occurs once in the whole
file (`ccd/ccd:2387`, measured — `grep -c '_lc_done purge'` is 1) and always passes a LITERAL empty tx, its
own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not half of a pair", `:2321-2322`); and
...
that never mentioned them. **The `cited` row's suffix rule — a suffix's left boundary
may be `/`, while a full path's may not — is implemented on this tree by `hasUniqueSuffixOccurrence`
(`ccd/compact-card.mjs:692`, called from `citedCount` at `:717`, distinct from `hasFullPathOccurrence` at
`:682`), so the prose and the code agree here rather than the prose describing an unported mechanism.**
...
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse_return <act>` on the same (act, id, MINTED tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:12802` / `:12805`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:5614`, `die` at `:1022`) so its own `_lc_done destroy` (`:5736`) is reached too; the guard reds, and removing the guard makes both mutants green. **The tx key is `_lc_tx`-MINTED and NON-EMPTY** (`ccd/ccd:2733-2745`, `printf '%s.%s.%s'` at `:2744`, never empty), which is what keeps the three literal-empty-tx emitters outside it — `_lc_refuse` (`:3355`, emitting `""` at `:3372`, and never returning), `_lc_done purge` (one call site, `:2387`, empty by design, `:2321-2322`: "UNCONDITIONAL, and with NO `tx`: this is not half of a pair") and `cmd_ws_restore`'s `:7603`/`:7661`/`:7667` triple, which a `""`-admitting guard would red on the CORRECT shipped tree. **GREEN control:** drive `ws-restore` to a landed undo plus a failed spawn and assert the guard passes; widening the guard to admit empty-tx facts must red that same control |
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:12566-12568` with zero terminal facts and reds; restoring the shipped status-blind arm (`:11665-11670`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  branches on the status and emits `_gc_declined` (`ccd/ccd:12611`, `:12655`, the same shape as `:12554`,
  `:12560`, `:12666`) with neither of those, plus
...
- **Card text reaches the model through one envelope, and never through a shell.** The card and standing
  ...
  through `_hook_emit_context`'s `jq -cn --arg c "$text"` envelope (`ccd/session-hook.sh:94`) — the single
  site every subject passes through. They are clipped twice before it: the standing text by `CARD_MAX_CHARS`
  (`:81`), the compact card by `COMPACT_CARD_MAX_CHARS` (`:88`), and the concatenation by
  `CARD_TOTAL_MAX_CHARS` (`:89`), which is DERIVED from the other two (`:1114`), never a third budget. **No
  ...
  arrives at `jq` as `--arg`/`--argjson`/`--rawfile`, the way `at` already does at `:877`, and reaches every
  other command as a QUOTED expansion, which is the compliant form (round 13, A-M3: the earlier "never … a
  shell word" spelling forbade the shipped tree's own `mv -f "$f" "$claim"` at `ccd/session-hook.sh:1989` and
  `rm -f "$cardf"` at `:865`, neither of which can take a pathname as `--arg`, so a scan built from it was
