...
3. Via `/proc/self/fd/$fd` or `/dev/fd/$fd`, require a regular FD target; require current canonical regular
   ...
   **THE REFUSAL IS A `WHY`, NOT A THIRD STATUS.** `canonical-vanished` is carried in `HOOK_LOCK_WHY` /
   ...
   boolean and two functions across three of its seven sites (`cmd_start`, and `_spawn_start`, which holds
...
five, read at this tip (D-2605's own acquire gate, `ccd/ccd:2453`, is the sixth `command -v flock` in the
...
reachable on the second declared userland: `ccd/ccd:621-642` records that "a LaunchAgent gets launchd's own
minimal PATH" and that this "already bit this port once", and `_svc_job_path` (`:648-654`) appends
...
`cmd_ws_rm` (`ccd/ccd:6465`) and `cmd_forget` (`:19131`) report `_lc_fail` naming the cause (mechanism absent
...
- **`forget` recovers by re-running ONLY while a tmux server is answering.** Its liveness precondition is
  ...
  `_lc_refuse forget "$id" session-verdict-unknown` (`:19098`) — and that EMITS, THEN DIES. Measured on tmux
  ...
  verdict `case` maps only `*"can't find session"*` to `gone` (`ccd/ccd:1989`); both absence spellings fall
  through to that `case`'s `*)` arm and become `unknown`, which the shipped comment at
  `ccd/ccd:1970-1971` names cmd_forget in by name — "Every other caller — `_alive`, `_ws_status`,
...
- **The remedy, traced rather than assumed: bring a tmux server back up, then re-run.** Any server on the
  ...
  measured: its dead-reg arm recomposes an id from `$project-$slug` (`ccd/ccd:13602`) and its scan walks only
  `for f in "$REG"/*.workspace` (`:13372`), while `forget` refuses any row with a `workspace` field at all
  (`:19092-19094`) — `ccd/ccd:19045-19048` states the same disjointness in shipped prose ("archive/reap are
...
`rm -f "$REG/$id.generation"` (`ccd/ccd:3107`) after the existing archived/reaping tail (which itself must run first, per the
...
`ccd/ccd:4020-4021`), and the withheld `purge` fact is itself what lets a caller or a reader distinguish a
lock-miss
...
next row. `_lc_refuse` cannot serve here: it "EMITS, THEN DIES. Never returns." (`ccd/ccd:4029`, `die` at
`:4047`), and `ws-gc --prune` must keep sweeping. No rollback fiction.
...
positionals against `_lc_refuse`'s four — `act id token detail [k v]...` (`ccd/ccd:4029`), bound by
...
new `_lc_refuse_return <act>`.** `_lc_tx` (`ccd/ccd:3390-3402`) mints `<ns>.<BASHPID>.<seq>` with
`printf '%s.%s.%s'` (`:3401`) and can never return empty, so the non-empty key is exactly the set of minted
...
tx positional at all (`ccd/ccd:4029`) and emitting a literal `""` tx (`:4033`) — and separately it never
returns, so it can never be followed by a terminal fact; (b) `_lc_done purge`, which occurs once in the whole
file (`ccd/ccd:3038`, measured — `grep -c '_lc_done purge'` is 1) and always passes a LITERAL empty tx, its
own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not half of a pair", `:2972-2973`); and
...
(`ccd/ccd:5330-5341` and `:5353-5362`), and `cmd_ws_add`'s refusal interpolates the second into the message
...
| ccd purge LOCK miss (or unsafe lock) | no purge-done fact, NOTHING deleted, generation and registry remain; the three post-action callers report `_lc_fail`; the dead-reg pre-action caller alone may DECLINE — but declining is a code change, not the shipped arm: it must branch on `_reg_purge`'s status and emit `_gc_declined` **beside one `_lc_refuse_return destroy <id> "$lctx"`** (the non-fatal, tx-bearing refusal emitter Task 9 adds, §3.4), with no `_lc_done destroy` and no `_gc_reclaimed`. The refusal is not optional garnish: `$lctx` was minted and an `intent destroy` already emitted at `ccd/ccd:13573-13575` before `_reg_purge` ran, so a decline with no terminal fact orphans that intent on every sweep |
| ccd purge MUTATION failure (an `rm -f` inside the unlink loop) | **the purge-done fact is ALREADY journaled** — the emit (`_lc_done purge`, `ccd/ccd:3038`) precedes the loop unconditionally (before `:3037-3089`) — so registry and/or generation may remain WITH the fact on disk; callers report `_lc_fail`, never success. (Round 9's single combined row said "no purge-done fact" for this case too, which is false; corrected round 10, B-4) |
| (round 9, SCOPED round 10) lock MECHANISM absent — no `flock(1)` resolvable by the calling process, established by `command -v flock` before any acquire is attempted, never by a failed acquire | the three hook arms fail CLOSED: no card, set, stage, claim, marker or journal is ever published. `ccd ws-add` (`ccd/ccd:5797`), `ccd ws-restore` (`:7568`) and `ccd ws-reap` (`:11025`) keep their SHIPPED fail-CLOSED refusals and are outside this ruling. Of the `_reg_purge` callers, only `ws-rm` (`:5667`), `forget` (`:19131`) and `ws-gc --prune` (`:12595`) are governed, and they are gated on `$REG/<id>.generation`: **absent** ⇒ fail OPEN (`_reg_purge` runs its unlink loop and emits its purge-done fact, exactly as before D-2605 — no hook on that row ever held a generation, so nothing can race); **present** ⇒ fail CLOSED with the distinct mechanism-absent refusal, the **two** post-action callers that can be reached on such a box — `cmd_ws_rm` (`:5667`) and `cmd_forget` (`:19131`) — reporting `_lc_fail`, while `ws-gc --prune`'s dead-reg arm (`:12595`), which reaches `_reg_purge` before any irreversible act, DECLINES — and `_ws_reap_locked` is unreachable. **Declining is not "unchanged".** The arm BEFORE Task 9 (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:13588-13653`) never read `_reg_purge`'s status — `ccd/ccd:9` is `set -uo pipefail` with no `-e`, then and now — and followed it UNCONDITIONALLY with `_lc_done destroy … "$lctx"` (`:11669`) and `_gc_reclaimed "dead registry entry …"` (`:11670`), so left unchanged it would have journaled a completed `destroy` and printed `reclaimed` for a row whose registry entry and `.generation` still stand, and repeated that on every sweep — the success claim §3.4's "never claims success" forbids. Task 9 must branch on `_reg_purge`'s status and, on a non-zero result, emit `_gc_declined` (this function's own decline reporter, the shape at `ccd/ccd:13561`, `:13567`, `:13673`) **plus exactly one `_lc_refuse_return destroy <id> "$lctx"`**, with **no `_lc_done destroy` and no `_gc_reclaimed`** — the refusal closing the `intent destroy` already emitted at `:13560-13562`. Recovery is per-verb, not one clause (round 14, B-I5): `ws-rm` recovers by re-running from a flock-capable PATH; `forget` recovers by re-running from a flock-capable PATH **only while a tmux server is answering**, because its own liveness precondition — `case "$(_session_verdict "$id")" in` (`ccd/ccd:19109`), whose `unknown` arm emits `_lc_refuse forget "$id" session-verdict-unknown` (`:19098`) — refuses on `unknown`, which is what both no-server spellings yield, and since `forget` runs `tmux kill-session` (`:19120`) one line before `_reg_purge` (`:19131`), forgetting the box's LAST session takes the server down with it. The remedy is to bring any tmux server up and re-run; `ws-gc --prune` cannot substitute, its scan walking `for f in "$REG"/*.workspace` (`:12365`) while `forget` refuses any row carrying a `workspace` field (`:16603-16605`). Row creation and `_spawn_start` skip generation initialization/export and continue **for row writes that do not pass through `cmd_ws_add`** — `ws-add` itself dies first. `_ws_slug_free` answers consistently, since on a box where no arm ever published, no compaction private residue exists to see (§3.4 Platform outcome, the five-site table, the generation gate, and facts i–iv) |
...
| ignore `_reg_purge` result in any of four callers, or report a post-action lock-miss as decline/unchanged instead of `_lc_fail` | held-lock real fixture emits a named `_lc_fail` (not a suppressed success or a bare decline) for `cmd_ws_rm`, `_ws_reap_locked`, and `cmd_forget`, naming completed action and retained registry/generation. The dead-reg pre-action caller may DECLINE, but it may not ignore the result either, and its decline must carry one `_lc_refuse_return destroy` for the minted `$lctx`: restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:12594-12659`) makes a `_lc_done destroy` and a `reclaimed` row appear for a row still standing, and reds |
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse_return <act>` on the same (act, id, MINTED tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:13809` / `:13812`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:6425`, `die` at `:1223`) so its own `_lc_done destroy` (`:6547`) is reached too; the guard reds, and removing the guard makes both mutants green. **The tx key is `_lc_tx`-MINTED and NON-EMPTY** (`ccd/ccd:3390-3402`, `printf '%s.%s.%s'` at `:3401`, never empty), which is what keeps the three literal-empty-tx emitters outside it — `_lc_refuse` (`:4029`, emitting `""` at `:4046`, and never returning), `_lc_done purge` (one call site, `:3038`, empty by design, `:2972-2973`: "UNCONDITIONAL, and with NO `tx`: this is not half of a pair") and `cmd_ws_restore`'s `:8609`/`:8654`/`:8673` triple, which a `""`-admitting guard would red on the CORRECT shipped tree. **GREEN control:** drive `ws-restore` to a landed undo plus a failed spawn and assert the guard passes; widening the guard to admit empty-tx facts must red that same control |
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:13573-13575` with zero terminal facts and reds; restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:13601-13666`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  branches on the status and emits `_gc_declined` (`ccd/ccd:13618`, `:13662`, the same shape as `:13561`,
  `:13567`, `:13673`) with neither of those, plus
...
- **(round 9, SCOPED round 10) The scope of that inertness, and the part of it this design declines to pay.**
  ...
  wrongly put it there (round 10, B-2):** `cmd_ws_reap` already refuses at `ccd/ccd:12019-12021` on such a
