...
refusals (`ccd/ccd:5059`, `:7568`, `:11025`); round 9 asserted a blanket over "ccd's own registry operations"
...
predicate is `JOURNAL_RECORD_PRED` with sixteen keys and no persisted ordinal (`:2398`), and the journal
commits by `mv -f "$stage" "$journal"` under the reacquired lock (`:1785`). `ccd` carries the twinned
`COMPACT_LOCK_WAIT` and the three-status `_reg_purge` (`ccd/ccd:2251`), and all four of its callers branch on
that status rather than on a boolean — `_rm_prc` (`ccd/ccd:5680`), `_rt_prc` (`:12026`), `_pr_prc` (`:12608`)
and `_fg_prc` (`:16655`). §4's mechanism-absent row and §3.4's decline paragraph describe those arms as they
...
(`ccd/session-hook.sh:1235`), step 6's **conditional ambiguous-card `rm -f "$cardf"` (`:1241`)** — `rm` is an
...
therefore no excuse for leaving it out — the exact-family sweep `find` (`:1251`), the
`at=$(_hook_epoch_ms)` **command substitution (`:1262`)**, which forks a subshell even though
...
ever implemented it: `_hook_compact_card` (`ccd/session-hook.sh:1884-1888`) acquires, then calls
...
a `.<id>.generation-read.*` alias (`ccd/session-hook.sh:1112`, `:1117`, `:1120`), a §3.4 family member, so
...
forks repeatedly: `$(find "$f" -mmin …)` and the `rm -f "$f"` beside it at `ccd/session-hook.sh:1931-1933`, the
...
own `case`/`${#x}` idiom (`ccd/session-hook.sh:777-778`) is the equivalent already shipped here. §6 states the
...
(`ccd/ccd:5680` in `cmd_ws_rm`, `:12026` in `_ws_reap_tail` under `_ws_reap_locked`, `:12608` in
`_ws_gc_prune_row` under `ws-gc --prune`, `:16655` in `cmd_forget`) — three of them only AFTER irreversible
...
| `ccd/ccd:5072-5073` | `cmd_ws_add` | fail **CLOSED** — `die "flock (util-linux) is unavailable — refusing to create a workspace unserialised"` |
...
`cmd_ws_rm` (`ccd/ccd:5667`) and `cmd_forget` (`:16655`) report `_lc_fail` naming the cause (mechanism absent
...
- **`forget` recovers by re-running ONLY while a tmux server is answering.** Its liveness precondition is
  `case "$(_session_verdict "$id")" in` (`ccd/ccd:16633`), whose `unknown` arm is
  `_lc_refuse forget "$id" session-verdict-unknown` (`:16622`) — and that EMITS, THEN DIES. Measured on tmux
  ...
  verdict `case` maps only `*"can't find session"*` to `gone` (`ccd/ccd:1530`); both absence spellings fall
...
- **The remedy, traced rather than assumed: bring a tmux server back up, then re-run.** Any server on the
  ...
  measured: its dead-reg arm recomposes an id from `$project-$slug` (`ccd/ccd:12608`) and its scan walks only
  `for f in "$REG"/*.workspace` (`:12378`), while `forget` refuses any row with a `workspace` field at all
  (`:16616-16618`) — `ccd/ccd:16569-16572` states the same disjointness in shipped prose ("archive/reap are
...
`holdersOf` (`:1144-1145`) filters exactly that corpus on non-comment lines — the mechanism seven existing rules
...
walks `$REG/$id.*` (`for f in "$REG/$id".*; do`, `ccd/ccd:2412`) and skipped only
...
(`ccd/ccd:2422`) — so the loop leaves it standing, and added one further explicit
`rm -f "$REG/$id.generation"` (`ccd/ccd:2468`) after the existing archived/reaping tail (which itself must run first, per the
...
`_rm_prc` in `cmd_ws_rm` (`ccd/ccd:5680`), `_rt_prc` in `_ws_reap_tail` (`:12026`), `_pr_prc` in
`_ws_gc_prune_row` (`:12608`) and `_fg_prc` in `cmd_forget` (`:16655`) — and each renders the value through
...
`ccd/ccd:3359-3360`), and the withheld `purge` fact is itself what lets a caller or a reader distinguish a
lock-miss
...
next row. `_lc_refuse` cannot serve here: it "EMITS, THEN DIES. Never returns." (`ccd/ccd:3368`, `die` at
`:3386`), and `ws-gc --prune` must keep sweeping. No rollback fiction.
...
positionals against `_lc_refuse`'s four — `act id token detail [k v]...` (`ccd/ccd:3368`), bound by
...
new `_lc_refuse_return <act>`.** `_lc_tx` (`ccd/ccd:2746-2758`) mints `<ns>.<BASHPID>.<seq>` with
`printf '%s.%s.%s'` (`:2757`) and can never return empty, so the non-empty key is exactly the set of minted
...
tx positional at all (`ccd/ccd:3368`) and emitting a literal `""` tx (`:3372`) — and separately it never
returns, so it can never be followed by a terminal fact; (b) `_lc_done purge`, which occurs once in the whole
file (`ccd/ccd:2400`, measured — `grep -c '_lc_done purge'` is 1) and always passes a LITERAL empty tx, its
own comment saying why ("UNCONDITIONAL, and with NO `tx`: this is not half of a pair", `:2334-2335`); and
...
(`ccd/ccd:4642-4653` and `:4665-4674`), and `cmd_ws_add`'s refusal interpolates the second into the message
...
| ccd purge LOCK miss (or unsafe lock) | no purge-done fact, NOTHING deleted, generation and registry remain; the three post-action callers report `_lc_fail`; the dead-reg pre-action caller alone may DECLINE — but declining is a code change, not the shipped arm: it must branch on `_reg_purge`'s status and emit `_gc_declined` **beside one `_lc_refuse_return destroy <id> "$lctx"`** (the non-fatal, tx-bearing refusal emitter Task 9 adds, §3.4), with no `_lc_done destroy` and no `_gc_reclaimed`. The refusal is not optional garnish: `$lctx` was minted and an `intent destroy` already emitted at `ccd/ccd:12579-12581` before `_reg_purge` ran, so a decline with no terminal fact orphans that intent on every sweep |
| ccd purge MUTATION failure (an `rm -f` inside the unlink loop) | **the purge-done fact is ALREADY journaled** — the emit (`_lc_done purge`, `ccd/ccd:2400`) precedes the loop unconditionally (before `:2399-2450`) — so registry and/or generation may remain WITH the fact on disk; callers report `_lc_fail`, never success. (Round 9's single combined row said "no purge-done fact" for this case too, which is false; corrected round 10, B-4) |
| (round 9, SCOPED round 10) lock MECHANISM absent — no `flock(1)` resolvable by the calling process, established by `command -v flock` before any acquire is attempted, never by a failed acquire | the three hook arms fail CLOSED: no card, set, stage, claim, marker or journal is ever published. `ccd ws-add` (`ccd/ccd:5059`), `ccd ws-restore` (`:7568`) and `ccd ws-reap` (`:11025`) keep their SHIPPED fail-CLOSED refusals and are outside this ruling. Of the `_reg_purge` callers, only `ws-rm` (`:5667`), `forget` (`:16655`) and `ws-gc --prune` (`:12595`) are governed, and they are gated on `$REG/<id>.generation`: **absent** ⇒ fail OPEN (`_reg_purge` runs its unlink loop and emits its purge-done fact, exactly as before D-2605 — no hook on that row ever held a generation, so nothing can race); **present** ⇒ fail CLOSED with the distinct mechanism-absent refusal, the **two** post-action callers that can be reached on such a box — `cmd_ws_rm` (`:5667`) and `cmd_forget` (`:16655`) — reporting `_lc_fail`, while `ws-gc --prune`'s dead-reg arm (`:12595`), which reaches `_reg_purge` before any irreversible act, DECLINES — and `_ws_reap_locked` is unreachable. **Declining is not "unchanged".** The arm BEFORE Task 9 (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:12594-12659`) never read `_reg_purge`'s status — `ccd/ccd:9` is `set -uo pipefail` with no `-e`, then and now — and followed it UNCONDITIONALLY with `_lc_done destroy … "$lctx"` (`:11669`) and `_gc_reclaimed "dead registry entry …"` (`:11670`), so left unchanged it would have journaled a completed `destroy` and printed `reclaimed` for a row whose registry entry and `.generation` still stand, and repeated that on every sweep — the success claim §3.4's "never claims success" forbids. Task 9 must branch on `_reg_purge`'s status and, on a non-zero result, emit `_gc_declined` (this function's own decline reporter, the shape at `ccd/ccd:12567`, `:12573`, `:12679`) **plus exactly one `_lc_refuse_return destroy <id> "$lctx"`**, with **no `_lc_done destroy` and no `_gc_reclaimed`** — the refusal closing the `intent destroy` already emitted at `:12566-12568`. Recovery is per-verb, not one clause (round 14, B-I5): `ws-rm` recovers by re-running from a flock-capable PATH; `forget` recovers by re-running from a flock-capable PATH **only while a tmux server is answering**, because its own liveness precondition — `case "$(_session_verdict "$id")" in` (`ccd/ccd:16633`), whose `unknown` arm emits `_lc_refuse forget "$id" session-verdict-unknown` (`:16622`) — refuses on `unknown`, which is what both no-server spellings yield, and since `forget` runs `tmux kill-session` (`:16644`) one line before `_reg_purge` (`:16655`), forgetting the box's LAST session takes the server down with it. The remedy is to bring any tmux server up and re-run; `ws-gc --prune` cannot substitute, its scan walking `for f in "$REG"/*.workspace` (`:12365`) while `forget` refuses any row carrying a `workspace` field (`:16603-16605`). Row creation and `_spawn_start` skip generation initialization/export and continue **for row writes that do not pass through `cmd_ws_add`** — `ws-add` itself dies first. `_ws_slug_free` answers consistently, since on a box where no arm ever published, no compaction private residue exists to see (§3.4 Platform outcome, the five-site table, the generation gate, and facts i–iv) |
...
| emit two terminal facts for one CALLER act — `_lc_done <act>` beside `_lc_fail <act>`/`_lc_refuse_return <act>` on the same (act, id, MINTED tx) | flatten `_ws_gc_prune_row`'s orphan `if`/`else` (`ccd/ccd:12815` / `:12818`) so both fire under one `$lctx`, or drop the `die` that follows `cmd_ws_rm`'s `_lc_fail` (`:5627`, `die` at `:1022`) so its own `_lc_done destroy` (`:5749`) is reached too; the guard reds, and removing the guard makes both mutants green. **The tx key is `_lc_tx`-MINTED and NON-EMPTY** (`ccd/ccd:2746-2758`, `printf '%s.%s.%s'` at `:2757`, never empty), which is what keeps the three literal-empty-tx emitters outside it — `_lc_refuse` (`:3368`, emitting `""` at `:3385`, and never returning), `_lc_done purge` (one call site, `:2400`, empty by design, `:2334-2335`: "UNCONDITIONAL, and with NO `tx`: this is not half of a pair") and `cmd_ws_restore`'s `:7616`/`:7661`/`:7680` triple, which a `""`-admitting guard would red on the CORRECT shipped tree. **GREEN control:** drive `ws-restore` to a landed undo plus a failed spawn and assert the guard passes; widening the guard to admit empty-tx facts must red that same control |
| the dead-reg arm declines without closing its own minted transaction | fixture HOME, generation-present dead-reg row, `flock` made unresolvable by the `ccd-ws-reap.test.ts:355` `command` shim; run `ccd ws-gc --prune`. The forced refusal in `_ws_gc_prune_row`'s dead-reg arm journals **exactly one `refused` fact for the minted `$lctx`** — no `done`, no `fail` — beside its `declined` row, with the registry row and `.generation` still standing, and the sweep continues to the next row. Dropping the `_lc_refuse_return` emit leaves the `intent destroy` of `ccd/ccd:12579-12581` with zero terminal facts and reds; restoring the status-blind arm Task 9 replaced (`8e457995:ccd/ccd:11665-11670`, superseded at this tip by `ccd/ccd:12607-12672`) so a `_lc_done destroy` and a `reclaimed` row appear also reds |
...
- **Waits and lock division.** Ordinary hook paths are lock-free. PreCompact, compact SessionStart, and
  ...
  (`ccd/ccd:5059`, `:7568`, `:11025`), which this design does not touch. `_reg_purge` reached through
  ...
  itself dies at `ccd/ccd:5059`. Gating them would refuse spawn on every row that has ever compacted,
...
- **Purge honesty.** The stable lock precedes lifecycle journaling/rotation; the full ordering is reap lock
  ...
  branches on the status and emits `_gc_declined` (`ccd/ccd:12624`, `:12668`, the same shape as `:12567`,
  `:12573`, `:12679`) with neither of those, plus
...
- **Card text reaches the model through one envelope, and never through a shell.** The card and standing
  ...
  (`ccd/session-hook.sh:2274`), never a third budget. **No
  ...
  arrives at `jq` as `--arg`/`--argjson`/`--rawfile`, the way `at` already does at `ccd/session-hook.sh:1277`,
  ...
  `ccd/session-hook.sh:2021` and `rm -f "$cardf"` at `ccd/session-hook.sh:1241`, neither of which can take a
...
- **(round 9, SCOPED round 10) The scope of that inertness, and the part of it this design declines to pay.**
  ...
  worth recording: on a box where `ccd ws-add` also refuses (`ccd/ccd:5072-5073`), **no workspace row and hence no slug is
...
- **A compaction that outlives `COMPACT_CARD_MAX_AGE` can have its journal line attributed to a SUCCESSOR's
  ...
  **The SERVE half of the same premise, restated (round 13, B-I6 — the pre-D-2605 §10 carried a wrong-context
  ...
  (`ccd/session-hook.sh:1241`), so B's overwrite would take A's card with it and leave A's SessionStart nothing
