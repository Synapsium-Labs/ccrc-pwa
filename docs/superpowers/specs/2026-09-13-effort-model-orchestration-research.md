...
branch table. D-numbers are `D-TBD-<slug>` here because the measuring session could not reach the
allocator; the controller mints and defines them.

1. **The effort lever — `D-TBD-effort-lever-a-plus-argv`. Branch taken: Lever A, with the argv flag
...
2. **The `--settings` keys — `D-TBD-settings-both-keys`. Branch taken: "arm 3 honours `enableWorkflows`
...
3. **The pinned lanes — `D-TBD-pinned-lane-no-edit`. No operator-approved edit is proposed.** The
   ...
   **The prior question this ruling had to answer first — who WRITES those pins — and its measurement
   (`D-TBD-spawn-settle-not-a-pin-writer`).** A plain `/effort <level>` persists into
...
4. **The `/model` lever, recorded for slice 4 — `D-TBD-model-lever`.** The persisting form is
