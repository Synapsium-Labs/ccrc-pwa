...
- `merged→archived`: **NO LONGER AUTOMATIC (operator ruling, 2026-09-10).** `archiveMerged` ran on a
  timer and killed sessions their operator was still using — five of the box's ~20 live sessions had
  been archived on merge and revived by hand — so the act was removed. `sweepMerged` announces the
  merge and archives nothing; the edge is now a human one (`POST /api/sessions/:id/archive`, or
  `ccd ws-archive`). What follows describes the sweep as it was, and is kept because §(c) below
  builds on it. `archiveMerged` (watch.ts:1797), `PR_SWEEP_MS = 120_000`. Requires: measured identity (`measuredIdentity(r) !== null`, checked *before anything else* — see its own comment on why a degraded row is unsafe), phase `merged` (unknown never archives), no hold, and `archiveSafety` verdict `ok` **read fresh at the decision point, not from the sweep's opening snapshot**. Metadata-only: `ws-archive` destroys nothing (ccd:1840).
...
The exemplar was `archiveMerged` (watch.ts:1797). **DO NOT COPY IT VERBATIM (operator ruling,
2026-09-10):** the act it performed was removed for causing the harm its own ladder could not see —
an instantaneous `idle` reading cannot tell a finished workspace from one whose operator is reading
the last answer, and no number of rungs fixes that. What survives, and what a new collector should
take, is the *discipline* of the rungs, not the confidence that they add up to a licence to act
unasked:
...
| 4 | archived-but-unreaped workspaces | as #1 | `cmd_ws_archive` ccd:1728 sets the state, destroys nothing (ccd:1840: *"worktree kept at $workdir, nothing deleted"*) | **W**, state `archived` | none — inflow WAS automatic (`archiveMerged`; removed 2026-09-10, so both ends are manual now), outflow is manual | **the queue with an inflow and no outflow.** This is the policy's single largest structural defect | 10 workspaces / **9.77G** (ws-gc's own read-only scan); warm-meadow alone 6.8G, idle 4 units |
