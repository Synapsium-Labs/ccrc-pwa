...
- **D-2586** — two comments in `pwa/test/mail-screen.test.tsx` (added by this task's own prior commit) cited `` `MailScreen.tsx:108` `` as the authority for the newest-first display convention. This commit's own D-2584/D-2585 additions moved that line to `:141` — the citation did not merely go stale, `:108` now sits inside the feed read's own `.catch()`, so the number points somewhere actively misleading rather than merely wrong. The identical citation appeared twice more in this plan document (the D-2583 narrative and its own bullet). All four fixed: replaced the line-number form with a citation by the comment's own text ("newest first on screen"), which a future line move cannot falsify the same way. In the same pass, trimmed `fleet.css`'s `.mail-chip` block comment, which claimed "the header above each list says in words what the chip selected" — untrue of the **All** chip, under which every header shows at once, not the one the chip names — to say so only of every chip but All.
