...
- The verb-gate scanner counts `CCD_ARGV.projectPoolClear('')[0]` — a NAME READ that is never run —
  as an ungated call site. That is wave-3 fallout (the site is `server/src/pools.ts`, which does not
  exist on `main`), so the scanner fix travels with wave 3 and NOT with this PR, which stays the
  smallest `ccd` change that closes the regression.
