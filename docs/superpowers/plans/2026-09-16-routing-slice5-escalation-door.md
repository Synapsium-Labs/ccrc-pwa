...
Issued by the allocator on 2026-09-17 after the whole-slice review and defined in the same act: D-2934 through D-2960 (27 departures; the block's unspent tail is never spelled). Per-task commits: Task 1 4a2b1632, 5d03e7b0, 98351aa7; Task 2 b0f5fb92, 759a68c2, 41ea92bf, 67b328cc; Task 3 b213000e, 2c21453d, d4922417; Task 4 e4e3cd8d, c3d1e8ac; Task 5 36f16226, 2680922c, 69736f87; Task 6 81fe39e0, ac59f9f5; the merges e54efb54, dc8404d5; the final fix waves b5357de2 and the round after it. Rulings S5-R1..S5-R19 are in the SDD ledger (archived in the session scratchpad as sdd-archive/slice5-ledger.md).

- **D-2934** (mode-demote) — the brief's RungTarget `mode` union had two literals ('escalate' | 'reverse-demotion'); demote() answered 'escalate' for a rung DOWN. Gained 'demote' (S5-R1); later derived as Exclude<RouteMode,'manual'> (T3).
- **D-2935** (reversal-clamp) — the brief's reversal was unconditional; it is scope-clamped: a subagent demoted off effort 'max' or off a class above SUBAGENT_CLASS_CEILING answers `ceiling` on the next failure (S5-R2).
- **D-2936** (ultracode-demote-refuses) — demote(effort) off 'ultracode' answers `no-effort-rungs`; off 'auto' counts auto as high (S5-R3); the brief's docstrings did not say.
- **D-2937** (unreadable-io-double) — the unreadable-registry test uses ioDoubles' unreadableField instead of the brief's chmod 000 (root-safe, same observable).
- **D-2938** (unrouteable-record) — a new 409 refusal: a present, readable `.class`/`.effort`/`.degraded` whose content is not a rung (ccd's `default` class; a torn field) is refused instead of resolving to the ladder's bottom rung (the review's Critical).
- **D-2939** (routable-states-derived) — the brief's three-state list ('dispatched','working','awaiting-review') became RUN_STATES minus TERMINAL_RUN_STATES at module level; run-closed = terminal, planned → no-session (S5-R4).
- **D-2940** (manual-clears-demotion) — a `route:manual:<field>` event on the demoted field clears lastDemotion (S5-R5); the brief's walk knew only demote/reverse-demotion.
- **D-2941** (census-files) — five files outside the brief's list changed because a new gated route trips repo-wide censuses: auth/gate.ts's EXEMPT table + count words, README's lane counts, mail-routes' refusal union, routes.test's :id census, crossrepo-prose's ledger claim (the 2026-09-14 census restored dated, with the 2026-09-16 correction beneath).
- **D-2942** (door-section-by-task-2) — the reference's `## The routing door` section was written by Task 2's fix round (the skill test's linkage census needed the route documented); Task 4 extended it instead of writing it.
- **D-2943** (route-mode-runtime-list) — RouteMode is derived from a runtime ROUTE_MODES list in shared/api.ts (peers-claims-l0 pins api.ts to one import line, so the ladder's type could not be imported; the reverse import derives the ladder's mode from it instead).
- **D-2944** (arm-gates-on-binding) — the wave-1 `arm:` event is recorded after coord.setSession, not on ws-add's res.ok: §1.5's adoption path returns a successful dispatch with res.ok false, so gating on it lost the adopted spawn's arm.
- **D-2945** (arm-unparsed) — RunSignals gained `armUnparsed` beside the brief's three fields: `arm: null` with armUnparsed > 0 is an arm that could not be read, not an absent one (S5-R9).
- **D-2946** (formatters-beside-parsers) — routeEventDetail/armEventDetail in shared/api.ts; the writers call them and the round-trips pin the grammar (S5-R8).
- **D-2947** (spec-5.3-amended) — §5.3 "Coordinator decisions" first sentence replaced: the coordinator calls POST /api/runs/:id/route through ccrc-api; `ccd route` is the server's call (clause 1).
- **D-2948** (every-call-qualified) — the brief's mandated sentence "Every call is one run event and one journal row" became "Every call that CHANGES a record …; a refusal … records neither" (S5-R10).
- **D-2949** (unsearchable-registry-unmeasured) — a present-but-unsearchable ~/.cc-sessions (mode 000 answers -d true) is unmeasured, never a measured zero (the D-1848 class _check_pools guards).
- **D-2950** (fail-needs-a-population) — the brief's `missing == 0 → FAIL` gained `live ≥ 1`; zero live sessions is a vacuous zero with its own WARN wording; the FAIL's remedy names that `ccrc update` stops at it (S5-R12).
- **D-2951** (stub-reuse) — the test reused the file's existing stubSystemctl/fixture-unit convention instead of the brief's new helper.
- **D-2952** (launchd-unmeasured) — on CCD_OS=darwin the census is unmeasured without calling systemctl (the brief said 'systemctl unreadable → unmeasured' only).
- **D-2953** (classes-in-capability-order) — the chooser lists the four classes CLASSES-reversed (fable first) to match the new-session sheet's capability order; the brief said 'CLASSES order'.
- **D-2954** (default-forecasts) — 'Default' fetches `?class=default` (the server accepts it and skips the shares read) and seeds `{ class: 'default' }`; the brief left the choice open.
- **D-2955** (second-merge-as-remedy) — the merge reviewer found the merged tree contradicting itself on the cap rule (main fixed three shipped copies one commit after the pinned sha); the remedy was a SECOND merge of origin/main (03ecda65, #132 + #114) rather than a cherry-pick (S5-R15).
- **D-2956** (run-route-route-fixture) — run-route-route.test.ts's copied OPEN_BODY gained `homeProject: PROJECT` at the merge (main's #133 refuses an open without it); the brief asked only to verify the suite (S5-R13).
- **D-2957** (run-scoped-trail) — spec §3's 'any failed check first reverses the last demotion' is kept per RUN (one run row per wave), not per session: a demotion taken at a wave boundary is not reversed by the next wave's failure; the reference says so and the coordinator re-states it by hand (S5-R16). The session-scoped trail needs the `route:` event grammar to carry the session (a run routes its worker and its coordinator) — carried to slice 6.
- **D-2958** (class-rung-one-argv) — a class rung writes `routeSet(sid, { class, effort })` in ONE argv carrying the effort reset (high; `auto` onto haiku, ccd's pair rule), not the brief's single-pair `route(id, field, value)`; the final review reproduced ccd refusing `--set class=haiku` alone.
...
- **D-2988** (census-cases-linux-only) — Task 5's six systemd-census cases (stopped-unrecorded, mixed-live, zero-live, truncated, fail-zero and the systemctl exit-1 stub) are skipped on darwin (`it.skipIf(NO_SYSTEMD_CENSUS)`): `ccrc` recomputes CCD_OS from `$OSTYPE`/`uname` at source time, so on a Mac `_check_routing` answers the D-2952 launchd arm before the fixture `systemctl` is asked anything, and the census wording five of them pin is never produced there — measured on PR #116's `test-macos` leg at 162bd427 (run 35210902959: four red on a census-wording `toContain`, the fail-zero case red on the FAIL the launchd WARN displaces, and the exit-1 stub green for the wrong reason, its `unmeasured` coming from the launchd arm rather than the systemctl answer it stubs). Direct-sourcing the six with `CCD_OS=linux` was weighed and not taken: it would run the loop on a Mac at the cost of the end-to-end `ccrc doctor` path the six exist to measure. The darwin arm stays pinned on every platform by the direct-source "on a launchd box" case, the end-to-end launchd line by a new `itDarwin` case beside it (the outcome the five reds had been showing incidentally), and the unsearchable-registry case now names its own arm's reason, since on darwin the launchd arm's `unmeasured` would otherwise stand in for it (reviewer finding). The Linux legs run the six unchanged. The brief said nothing about platforms.
