...
- **D-3367** (final fix `cd546eeb`: three unledgered departures, bundled — review 170 F8) — the final fix round's commit `cd546eeb` made three departures no number covered, each ruled sound. (1) **Branch under drift.** The plan's `branch="${wthead:-$regbranch}"` ("$wthead is the branch reclaimed", plan:1209-1212, :1323-1326) became `branch="$regbranch"` on both arms (`_ws_reclaim_eval` at ccd/ccd:24107, `_ws_reclaim_resume_eval` at :24224): a worktree HEAD that drifted onto another branch keeps that branch instead of failing `pin-failed`, and the pin records the drift in `RECLAIM_KEPT`/`keptBranches`. `_ws_tombstone`'s comment (ccd/ccd:13333) still calls `branch` git's own worktree record — true under the new reading, since `REAP_BRANCH` is set to `$regbranch` at :24108. (2) **Index lock moved.** Rung 6 (ccd/ccd:24092-24100) now refuses `tree-busy` on a held `index.lock`, inside `if (( ! defer ))`, so `--defer-expired` passes it; `_ws_reclaim_index_src` (ccd/ccd:24338-24352) no longer checks the lock at all — it fails only on a missing or unlocatable index, because the pin only copies the index and git replaces one by atomic rename. (3) **Main-line refusal on the child's own branch.** `_ws_reclaim_main_line_refuse` (ccd/ccd:23880-23887) is called on the ladder's fresh arm (:24119) and resumed arm (:24236); the tail's branch step (5) asks the same predicate inline through `_ws_reclaim_main_branch` (:25240-25244) before its CAS. D-3355(3) covers only a NESTED line's main-line rung; this is the child's OWN branch. All three are the safer direction — nothing is deleted that is not first proven the child's own, and a branch or index that cannot be proven safe to touch is kept or deferred rather than destroyed or wedged. Numbered by the coordinator (review 170 rulings).
