...
- **D-3368** (review 170 F11 + F13: `actor-less-wording-restored`, `review-child-close-order-true`, `readme-1676-exception-added`) — Task 10's fix round `07e8d552` altered plan-prescribed, word-for-word-pinned text in `ccd/coordinator-skill/references/wave-lifecycle.md` (~:833, :838-839, :843-844) and `ccd/reviewer-skill/SKILL.md` (~:146-148), unledgered: "picked up later" → "picked up later by wave 4's sweep", "the server reclaims it" → "wave 4's sweep reclaims it" (twice), and an added "No `childReclaimWhy` at all …" sentence — none covered by a D-number (D-3362 covers only the word-pin, above). "Wave 4" names nothing outside this programme, so a skill shipped to every rostered home reads it as a real wave of ITS OWN programme. Separately, README's two additions in `fcd84cd0` went beyond Task 10 Step 6's two prescribed replacements: README:1676's abandon-child sentence never gained the review-child exception README:2129-2130 states, and wave-lifecycle.md's §6 review-child sentence claimed "closing the review run releases the reviewer and answers `review-report-live`" unconditionally — false when the reviewed run is ALREADY terminal at the review close, where `closeReviewRun` passes `final:true` and the close itself answers `childReclaim:'queued'` (`close.ts:534`; `childReclaim.ts:266-272`, `:279-300`; asserted at `child-reclaim-close.test.ts:294-322`). What ships: every "wave 4" mention in both skill files is reverted to the plan's actor-less form ("picked up later"; "the reviewer is reclaimed only after the run it reviewed has closed") or, in `reviewer-skill/SKILL.md`, to the pre-fix-round actor "the server reclaims it" (the true, non-programme-specific wording the base plan used); the added "No `childReclaimWhy` at all…" sentence is KEPT — measured true against `close.ts`'s `handOffChildReclaim` (`:693-703`): a decision of `reclaim:true` with `deps.childReclaim` undefined, `gate.request` null, or a thrown port, all answer `not-queued` with NO `childReclaimWhy` — with its own actor named removed ("wave 4's sweep reaches it regardless" → "it is reached regardless"); `wave-lifecycle.md`'s review-child sentence now states both close orders truthfully (the reviewed run open at the review close → `review-report-live`; already terminal → the review close finishes the reviewer directly, with no deferral); and README:1676 gains the same review-child exception README:2129-2130 already carries. Numbered by the coordinator (review 170 rulings).
