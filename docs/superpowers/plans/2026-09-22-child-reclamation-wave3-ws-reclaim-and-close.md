...
- **D-3513** (review 170 F1: `session-probe-anchor-two-lines`, `tail-pane-remeasure`, `unit-still-active-names-the-pane`, `unit-still-active-word-in-lc-map`, `child-stubs-tmux-model`) — the plan prescribed rung 5 as a bare `if tmux has-session -t "=$(_tmux "$id")" 2>/dev/null; then …; fi` with no else arm (plan:1157), and the tail's pane kill as `tmux kill-session … 2>/dev/null || true` (plan:2692), after which the tail re-measured the unit alone. Both read "tmux could not be asked" as "no session". MEASURED red first on this branch, before the fix: with the test's tmux answering `error connecting to … (Permission denied)` at exit 1, `ws-audit --reclaim` minted a token and `cmd_ws_reclaim` removed the worktree; with the same fault arising between the ladder and the tail, the tail removed it too. What ships: (1) `_session_probe` takes an optional second argument that anchors its target, `-t "${2:+=}$(_tmux "$1")"`, edited in place on BOTH of its target lines (the function-stub branch and the `_plat_timeout` branch) — two lines, not the ruling's one, because the target is spelt twice; line-neutral, no line or comment added above the frozen boundary, and every existing caller passes one argument and is unchanged. The frozen corpus quotes neither line; the five S6-R11 cases are green at the base and at the tree. (2) Rung 5 in `_ws_reclaim_eval` asks `_session_probe "$id" anchored`: `gone` (`can't find session`) passes, `live` runs the existing attached-client check, and `unknown` answers `_ws_reclaim_unmeasured` with tmux's own words — `probe-unmeasured` at exit 1 from the verb, the verdict `unmeasured` at exit 1 from the audit, which the server already reads as `failed`, `resumable:false` (no server change). (3) `_ws_reclaim_tail` keeps its anchored kill's own exit status and, after the unit's re-measure and before the first deletion, re-measures the pane through the same anchored probe: `gone` passes, and `live` or `unknown` fails `unit-still-active` with the breadcrumb kept. The one exception is tmux's exit-empty: an `unknown` whose detail says `no server running`, after THIS attempt's kill exited 0, passes. Real tmux does exactly that (measured by the coordinator on tmux 3.4 with an isolated `-S` socket: `kill-session` of the last session exited 0, `has-session` then printed `no server running on <sock>` at rc 1, and the socket file remained). A resumed attempt with no rc-0 kill of its own still stops on it, the rulings' accepted residual. Neither site reads PROBE_SUBSTRATE, so `no server running` and a missing socket stay unknown. (4) No fifteenth token: `unit-still-active` is reused, and its sentence — in `shared/api.ts`'s `LC_REFUSAL_WORD`, since that word has no `SENTENCES` entry (the two maps are disjoint), where the brief placed it (`unit-still-active-word-in-lc-map`) — now says the service and its terminal pane could not be proven stopped, edited in place and line-neutral. (5) `CHILD_STUBS`' `tmux` (`server/test/childReclaimFixture.ts`) answered exit 1 with no message to every call, which the probe reads as unknown; it is now a model that answers in tmux's words, resolves a `=` target exactly and a bare one exactly first and otherwise by prefix (so an unanchored `cc-<id>`, with no `cc-<id>` up, finds a sibling `cc-<id>x`), exits empty when its last session is killed, and can turn unaskable right after a kill that exited 0. Sixteen mutation rows, each red on a named case — three of them from the fix's review round: the exception reduced to its rc-0 condition, its detail test swapped for `PROBE_SUBSTRATE == absent` (each red on an rc-0 kill followed by a refused connection or a missing socket), and the pane re-measure dropped on the vanished arm and on resumes at `worktree` and `branch`. Numbered by the coordinator (review 170 rulings).
- **D-3514** (review 170 F2: `a10-nested-arm`, `nested-rows-own-global`, `nested-literal-arm-not-implied`, `nested-detail-names-rows`, `spelled-through-child-refuses`) — A10 ("no second registry row may name this workdir", Task 2) refused only when another row's `.workdir` EQUALLED the child's, literally or resolved, so a session rooted INSIDE the child was not seen. MEASURED red first on this branch, before the fix: a nested same-repository worktree `inner` on `ws/other-live` with its own registry row, and a row rooted at `<child>/server`, each answered `reclaimable` from `_ws_reclaim_eval`, and each was reclaimed by `cmd_ws_reclaim` on the fresh arm (with a token minted before the other row appeared) and by a resumed tail — the child's tree, the other session's inside it, removed. What ships: (1) `_ws_reclaim_workdir_shared` also answers `_WS_NESTED_ROWS`, the OTHER rows whose `.workdir` lies inside the child's at a path-component boundary — `"${mine%/}/"*` on the `_ws_realpath`-resolved path, or `"${wd%/}/"*` on the literal one, each only when that spelling is one plain path (`_ws_reclaim_plain_path`) — so `<child>x` is a sibling, not inside. The resolved spelling must be plain too because `_ws_realpath` resolves only the existing prefix and re-attaches the rest as written: once the child's tree is gone (the vanished arm, or a resumed tail past `worktree`), `<child>/..` comes back as `<child>/..`, below the child as a string. The equality arm (`_WS_SHARED_ROWS`) and the fail-closed answers for an unlistable registry or an unreadable row are unchanged, and a row ABOVE the child, spelled plainly, is not refused. A narrow fail-closed departure from the ruling's "do NOT refuse on an ANCESTOR row": a row whose `.workdir` is spelled THROUGH the child's path but not as one plain path (`<child>/..`, `<child>/../../../elsewhere`), or whose resolved path is below the child but not plain, and that no plain spelling proves inside, is answered in a third global, `_WS_THROUGH_ROWS`, and refuses too — even when it names an ancestor. `ccd start` stores a workdir as given, checked only with `-d` (reachable from `POST /api/sessions`' `body.workdir`), so such a spelling is writable, and it stops resolving once the child is removed; its detail says the row "spell[s] their workdir through <child>, not as one plain path, so ccd cannot prove it lies outside", never that it is rooted inside — on every arm, the child's tree standing or gone. (2) Both callers refuse on it exactly where A10 already runs: `_ws_reclaim_eval` answers the terminal `containment-unproven` with a detail naming the nested row id(s) and the equality arm's closing clause ("ccd cannot prove the tree at that path is <id>'s own"); `_ws_reclaim_owned`, the tail's ownership re-proof on every arm, fails `worktree-remove-failed` naming them; both sites refuse a spelled-through row the same way, with its own detail. (3) Unlike the equality arm, the nested arm's literal comparison is NOT implied by the resolved one: a row spelled through a link inside the child resolves outside it while the path it names goes with the child; a case pins it. (4) No fifteenth token, and `containment-unproven`'s sentence in `server/src/wsaudit.ts` is untouched (F9 rewrites it). Thirteen mutation rows, each red on a named case — drop the nested and spelled-through arms, a bare string prefix (the `<child>x` control reds), drop the resolved comparison, drop the plain literal comparison, drop the tail's nested arm only (the two resumed-tail cases red), drop the ladder's nested arm only (the ladder cases red, and the fresh-arm verb cases red on `nothing was pinned`: the tail alone refuses after the pin), move the ladder's nested refusal below the vanished fork (the vanished-arm case reds), drop the spelled-through classification, fold it into the nested one (its detail cases red), drop its ladder or tail refusal alone, accept a non-plain RESOLVED path as nested (the vanished-arm and gone-tree resumed-tail `<child>/..` cases red), and drop the through class's resolved comparison (the vanished-arm spelling through a symlinked ancestor reds). The `<child>x` control and the plain-ancestor control are separate cases. Every new `ccd/ccd` line sits in the RECLAIM region, below `ccd/ccd:19131`; no `_reg_get` call is added; the five S6-R11 cases are green at the base and at the tree. Numbered by the coordinator (review 170 rulings).
- **D-3515** (review 170 F3: `child-reflogs-kept-completely`, `reclaim-stops-calling-attic-pin`, `keep-at-pin-and-before-each-deletion`, `commit-writer-shared`, `keep-date-is-gits-clock`, `pin-absent-takes-workdir`, `reflog-file-guards`) — the pin phase's reflog step was ws-reap's `_ws_attic_pin`, whose header says "the worktree's HEAD reflog" but which reads `reflog show --all` — every reflog of the SHARED repository — and keeps the 200 lowest ids (`sort -u | head -200`); the vanished-worktree arm pinned no reflog at all; and every reclaim wrote attic refs for other sessions' commits. MEASURED red first on this branch, in an isolated copy of the commit before the fix: a commit only the child's HEAD and branch reflogs named, ranked at or above 200 among all reflog ids after 650 unrelated entries on another branch, was in no ref after `cmd_ws_reclaim`; so was a commit only a nested branch's reflog named (same noise), and, on the vanished arm, a commit only the child branch's reflog, or only the vanished tree's HEAD reflog, named; and the attic kept three commits only another worktree's HEAD reflog, another branch's reflog and `main`'s reflog named. What ships, all in the RECLAIM region (`_ws_attic_pin` itself is unchanged; its copy of the defect is wave 4's): (1) `_ws_reclaim_keep_reflogs` reads each entry's OLD and NEW ids from the reflog FILE (never a `%H` listing), for exactly four kinds — the child worktree's HEAD reflog, the child branch's, each nested same-repository checkout's HEAD reflog and each nested branch's — located through git by `_ws_reclaim_log_of`: `rev-parse --git-path logs/HEAD` inside a checkout that stands, the common dir's `worktrees/*/gitdir` entry naming `<path>/.git` for one that is gone, and the common dir's `logs/refs/heads/<branch>`. A file proven absent holds nothing; one that stands and cannot be read, is not a regular file (gawk skips a directory operand and exits 0 — measured), or holds a line that is not an entry fails; an entry naming an object git no longer has, and the all-zero id, is skipped; there is no cap; an `extensions.refStorage` other than `files` fails with that reason (git 2.43 cannot open a reftable repository, so its case stubs the one config read). Every distinct commit is kept by ONE ref, `refs/ccrc/attic/<id>/reflogs`, at a synthetic commit of the empty tree whose parents are those commits, a hundred to a commit, in a chain whose first parent is the ref's previous value (so a later keep adds to an earlier one), moved by compare-and-swap; the tombstone's existing `attic` list names that ref, so no field is added. Measured: `commit-tree` took 3000 parents on this box, but the chain keeps every argv small on any `ARG_MAX`. (2) `_ws_reclaim_pin_contained` no longer calls `_ws_attic_pin`. What that call pinned that a reclaim still needs is pinned on its own: the branch tip (a required pin) and the stashes attributed to the branch, by `_ws_reclaim_attic_extra` (the existing cases `commits onto a DETACHED HEAD…` and `pins the WIP commit, the tip, every stash of the branch…` hold both), and these four reflogs; `refs/stash` and every other session's reflog are never deleted by a reclaim. (3) Where: each kind is kept at the pin phase, before the breadcrumb — so an unreadable reflog fails `pin-failed` while nothing is destroyed, on the fresh arm and, through `_ws_reclaim_pin_absent` (which gains the workdir as a fifth argument), on the vanished arm — again by the settle, which re-runs the pin, and again in `_ws_reclaim_tail` just before each act that deletes it (`_ws_reclaim_keep_of`, contained): before each nested `worktree remove`, before each nested branch's CAS, before step 4's `worktree remove`, before step 5's removal of a vanished tree's record, and before the branch CAS. The tail's keep is the one that binds on every arm: a resumed tail past the settle, and the vanished arm, have no settle in front of them, and each act follows an arbitrary gap; a failure there is `pin-failed` with the breadcrumb kept. A record git holds of a gone checkout whose `worktrees/*/gitdir` entry cannot be found — a git that writes relative worktree paths (`worktree.useRelativePaths`, 2.48+); git 2.43 prints the entry verbatim, so there both readers miss together — is unmeasured too: its HEAD reflog was never located, so the tail fails `pin-failed` and the record stands (`_ws_reclaim_log_of`'s `recorded` argument, passed by the two tail sites that hold the record and by the vanished pin phase when the ladder read one; fix round 1). (4) `_ws_reclaim_commit_tree` is the region's one commit writer and one identity: `_ws_wip_commit` and the keep both write through it, and the pin suite's `the only commit ccd writes` case now pins its two callers. The brief's "fixed identity and date": the WIP commit never fixed a date, so the keep takes git's clock, as the WIP does. Tests: the brief's five (the review's shape and a nested branch, each with 650 unrelated entries and a rank-≥200 control; a mode-000 child branch reflog on the fresh arm, on a resume at `branch`, and on the vanished arm; the attic holding nothing another session's reflogs name; the vanished arm's branch and HEAD reflogs), each read back after `git gc --prune=now` with a control that gc pruned an unreferenced commit, plus a HEAD-only and a branch-only case, an ORDER case (an entry written after the pin, kept before a resumed CAS), and pin-level cases for 250 reflog commits, an entry's OLD side, a missing commit, a malformed line, a non-regular file and `refStorage`. Seventeen mutation rows, fifteen red on a named case — skip each of the four kinds at every site, fold unreadable into absent, move the branch keep after its CAS, a `head -200` cap, the NEW side only, drop the regular-file, `refStorage` or malformed-line guard, fail on a missing commit, a later keep that replaces an earlier one, and the fresh or the vanished pin phase keeping nothing — and two disclosed survivors: the zero-id filter (`cat-file --batch-check` answers `missing` for it, so the skip holds without it) and the keep's compare-and-swap (no fixture has a concurrent writer). Fix round 1 adds a case per tail keep — an entry written into the one reflog that act deletes, after every earlier keep (from inside the settle where one runs in the same invocation) — each red when that keep ALONE is `true` (five rows), and the unlocated-record case, red when its guard is dropped or its flag not passed at step 5 (two rows); on the pre-fix code that case ends `worktree-remove-failed`, because git 2.43 itself cannot find the respelled record. A follow-up pins the flag at the other tail site: the same case for a gone NESTED checkout's record (resume at `children`), red when that keep does not pass `recorded` (one row); the vanished pin phase's `${rhead:+1}` stays a disclosed survivor, since step 5's keep, which is pinned, answers the same `pin-failed` later with the breadcrumb kept. Every new `ccd/ccd` line sits in the RECLAIM region, below `ccd/ccd:19131`; no `_reg_get` call is added; the five S6-R11 cases are green at the base and at the tree. Numbered by the coordinator (review 170 rulings).
- **D-3516** (review 170 F4: `hidden-edits-found-by-content`, `hidden-flags-cleared-one-per-call`, `hidden-edit-staged-as-raw-bytes`, `raw-bytes-match-is-no-edit`, `landing-check-by-object-id`, `foreign-hidden-edit-refuses`, `audit-sensitive-lists-hidden-secrets`, `hidden-step-after-status`, `hidden-set-in-the-token`, `settle-records-a-lone-secret`, `verb-answer-counts-secrets-dropped`) — `_ws_wip_commit`'s header said an entry git is told not to look at (skip-worktree, assume-unchanged) "is not reported, so it keeps what the copy holds", and `server/test/ccd-child-reclaim-pin.test.ts` pinned "a hidden local edit is never committed": such an edit was neither staged nor recorded, the ladder's `statusDigest` could not see it, and the tail's forced worktree removal deleted it. MEASURED red first on this branch, in an isolated copy of the commit before the fix: twenty-one ccd cases red (nineteen new, and the two verb cases extended to read the answer's count), the two new controls — an mtime alone, a sparse path absent from disk — green; among them a non-secret skip-worktree and an assume-unchanged edit not in the WIP, a secret-shaped one (`.env`, `secrets/db.yml`) not in `secretsDropped`, a nested checkout's hidden edit in no attic WIP, the token unmoved when a hidden path started differing (so `cmd_ws_reclaim` spent a stale token and reclaimed), an unreadable hidden path passed over, a same-size edit of a TRACKED file in the second the index was written left out of the WIP, and the verb's answer and record silent on a dropped hidden secret — and the server's feed row said "Nothing uncommitted was left." with a secret dropped. What ships, all in the RECLAIM region: (1) `_ws_reclaim_hidden` (body `_ws_reclaim_hidden_contained`, run under `_ws_reclaim_contained`) reads `git ls-files -v -s -z` — `S` is skip-worktree, a lowercase tag assume-unchanged, never `-t` — and, for each flagged stage-0 file or link PRESENT on disk, decides "differs" by CONTENT: `git hash-object --path=<p> -- <p>` against the index blob AND the raw bytes, `git hash-object --no-filters -- <p>`, against it too (fix round 1, I2a) — a file whose bytes ARE its blob is never an edit, which a CRLF blob under `text=auto` otherwise read as one (measured by the review: an untouched flagged `.env` recorded as dropped, and a spurious WIP for an untouched non-secret file) — or for a symbolic link the hash of its `readlink` target (hash-object follows a link — measured), a link-for-file swap differing whatever it holds. A path absent from disk (sparse) is not an edit; a gitlink or sparse directory entry is not a file; a failed read, a directory at a file's path, and a tag it does not know are UNMEASURED — the ladder answers `unmeasured`, the pin fails `pin-failed`. (2) `_ws_wip_commit`'s new step 3 (`hidden-step-after-status`), taken AFTER step 2's `status` and `add` (which never see these paths, their flags still standing in the copy, so nothing there restages them): a secret-shaped hidden edit goes to `RECLAIM_SECRETS` (the tombstone's `secretsDropped`) and is never staged; any other is KEPT — its flags cleared in the SCRATCH copy only, by two `update-index` calls, because one call given `--no-skip-worktree --no-assume-unchanged` cleared only the second (measured) — and staged as its RAW disk bytes (fix round 1, I2b, replacing the first round's `add --renormalize`, `hidden-edit-staged-by-renormalize`): the blob `git hash-object -w --no-filters -- <p>` writes (a link: its `readlink` target), entered in the scratch copy with `update-index -z --index-info` at the DISK's mode for a regular file (`-x`: 100755, else 100644, as `git add` takes it; the index's mode when `core.fileMode` is false — fix round 2, N1: the first fix round kept the index mode, so an edited and `chmod 755`d flagged `run.sh` was kept at 100644), so the WIP holds exactly what is on disk — `--renormalize` stored a CRLF edit under `text=auto` with its CRs stripped (measured by the review), and a bare `add` exited 0 and staged nothing whenever the user's stat record still matched (measured on this box, on the non-secret and the link cases). A `sparse.expectFilesOutsideOfPatterns=true` out-of-cone edit, on which `--renormalize` failed every attempt, is now kept (measured, flags unchanged). `_ws_reclaim_landed` proves each kept edit by OBJECT ID — `rev-parse <wip>:<path>` equal to the blob written at staging, or HEAD's when nothing was staged — since `git show <wip>:<path>` exits 0 on HEAD's content too. The user's own index is unchanged — its flags (`ls-files -v`) and its BYTES, in the child and in a nested checkout (fix round 1, m3). (3) The scratch copy of the index now keeps its timestamp (`cp -p`); that departure, which closes a loss of ordinary tracked edits rather than hidden ones, is its own entry, D-3520. (4) The token: `_ws_reclaim_eval` — the one ladder the audit and the verb both run — reads the child's tree and every nested same-repository checkout, and adds `hiddenDigest`, the sorted digest of the hidden-edit PATH set (a nested one as `<checkout>/<path>`), so a hidden path that starts or stops differing is `state-changed`; an mtime alone moves nothing, and no content hash enters the tombstone. (5) Nested same-repository checkouts go through the same `_ws_wip_commit`, their secrets recorded with their prefix. A nested checkout of ANOTHER repository cannot be pinned here, so a hidden edit in it REFUSES (fix round 1, I1, `foreign-hidden-edit-refuses`): `_ws_reclaim_foreign_clean` — rung 9's one predicate, re-run by the pin on every call — asks `_ws_reclaim_hidden` of it and answers `containment-unproven` naming the checkout and "a hidden-flag … edit … which ccd will not delete unkept", or unmeasured when that read fails; measured by the review: the ladder answered `reclaimable` and `ws-reclaim` deleted the only copy. And the audit's `sensitive` list names every secret-shaped hidden edit the pin will drop, the child's and each nested same-repository checkout's (fix round 1, m1, `audit-sensitive-lists-hidden-secrets`), a nested one named relative to the RESOLVED workdir as the pin records it (fix round 2, N2: under a workdir spelt through a symlinked parent the first fix round listed it as an absolute path). (6) `settle-records-a-lone-secret`: the settle's tombstone patch also fires when `RECLAIM_SECRETS` is non-empty — a hidden secret edit made after the pin, with nothing else moved, was otherwise dropped unrecorded. (7) `verb-answer-counts-secrets-dropped`: `ws-reclaim`'s success document gains `"secretsDropped":<n>|null`, the count the tombstone names (`_ws_reclaim_dropped_count`) — an additive key beyond the contract's §3 success document, which is not edited; `parseChildReclaimResult` reads anything but a non-negative integer, or no key, as `'unreadable'`, `ChildReclaimOutcome`'s `reclaimed` arm carries it, and the feed says "Nothing uncommitted was left." only with no WIP AND a count of 0 — otherwise "No work was committed." and the count (fix round 1, m5). Mutation rows, each red on a named case — the first round's twenty: `-t` for `-v`; stat-based "differs"; the flags cleared in the user's index; the landing check dropped, and dropped on the no-commit and the commit arm alone (a case whose staging exits 0 without staging, on a stat-matching edit); the hidden set dropped from the token (ladder and verb); no secret classification; an absent path read as an edit; no `readlink` arm; a failed read folded to not-an-edit; the nested ladder read dropped; an unknown tag read as unflagged; the copy without `-p`; a bare `add` for `--renormalize`; the settle's lone-secret patch dropped; the answer counting 0; the feed ignoring the count; absence read as 0 (its bare-`add`-for-`--renormalize` row is superseded); and fix round 1's: the foreign hidden check dropped (ladder, pin and verb cases), the raw-bytes match not consulted, the kept blob written through the clean filter, a bare `add` for the raw-bytes staging, the child's and the nested hidden secrets left out of `sensitive`, the staging written to the user's index, and the landing and flag rows re-run against the new staging; and fix round 2's: the index mode kept whatever the disk says, `core.fileMode` false not honoured, and the literal workdir stripped from a nested `sensitive` path. Every new `ccd/ccd` line sits in the RECLAIM region, below `ccd/ccd:19131`; no `_reg_get` call is added; the five S6-R11 cases are green at the base and at the tree. Numbered by the coordinator (review 170 rulings).
- **D-3517** (review 170 F5, F6: `ladder-reads-contained-at-the-fork`, `reclaim-audit-contained-whole`, `audit-flag-assigned-at-read`, `containment-comment-names-three`, `containment-comment-cites-plan`, `filter-drivers-still-run`) — the ladder's git reads ran OUTSIDE `_ws_reclaim_contained`: `_ws_reclaim_classify` and `_ws_collect_ignored` ran `git status` on the LIVE child with the repository's hooks, its `core.fsmonitor` and optional locks all in force, reached by `ws-audit --reclaim` and by `ws-reclaim` through `_ws_reclaim_fork`; that falsified `_ws_reclaim_eval`'s "A refusal writes NOTHING — with one named exception" and the audit prose's "THE ONE EXCEPTION TO READ-ONLY … a mode changes", and took `index.lock` against the live session. And `_ws_reclaim_contained`'s header said "NO REPOSITORY-CONFIGURED PROGRAM RUNS INSIDE THE PIN PHASE", while a gitattributes clean filter still runs there. MEASURED red first on this branch, before the ccd edit, with `post-index-change`, `reference-transaction`, `post-checkout` and two commit hooks in the main repository's hooks directory, a `core.fsmonitor`, and the child's index made stale (a tracked file's mtime moved, its bytes not): `ws-audit --reclaim` (verdict `reclaimable`) and a `ws-reclaim` refused `state-changed` each ran `fsmonitor` and `post-index-change`, and each rewrote the child's index. What ships, in the RECLAIM region save one in-place line: (1) `_ws_reclaim_fork` runs its body, now `_ws_reclaim_fork_contained`, under `_ws_reclaim_contained` — a function call, not a subshell, so every REAP_*/RECLAIM_* global the evals set survives; `_ws_collect_ignored` and `_ws_attic_pin` are not edited. (2) A DEPARTURE from the brief's "wrap at the fork" alone: `cmd_ws_audit` reads the child again AFTER the fork, for its own document — the `dirty` list's `git status` and `_ws_child_manifest`'s `status` of each nested checkout, lines it shares with the plain audit — and with only the fork wrapped the audit case stayed red (`fsmonitor`, `fsmonitor`, `post-index-change`, measured). So `--reclaim` re-enters `cmd_ws_audit` once, whole, under the containment: `_ws_reclaim_audit_contained` (RECLAIM region) sets a LOCAL `_WS_AUDIT_INNER=1` and calls `_ws_reclaim_contained cmd_ws_audit "$@"`, and the argv parse's `mode=reclaim` line gains, in place and line-neutral, `[[ $_WS_AUDIT_INNER == 1 ]] || { _ws_reclaim_audit_contained "$@"; return; };` plus a pointer comment. The global `_WS_AUDIT_INNER=0` is assigned whenever ccd is read, so an environment exporting `_WS_AUDIT_INNER=1` cannot skip the containment (a case of its own). The plain `ws-audit` is not contained. (3) The two falsified comments are corrected in place: the ladder's says its git reads run contained from `_ws_reclaim_fork`, its one caller, and a filter driver still can run; the audit prose gains "THE AUDIT READS THE LIVE CHILD CONTAINED". (4) F6, narrowed as ruled, no filter neutralisation: `_ws_reclaim_contained`'s header now says it disables EXACTLY three things — hooks (`core.hooksPath=/dev/null`), fsmonitor (`core.fsmonitor=false`), optional locks (`GIT_OPTIONAL_LOCKS=0`) — and that a gitattributes filter driver (`filter.<x>.clean`/`.process`) still runs in `status`, `add` and `hash-object --path`, as the user's own configured program under the single-user trust model; `_ws_reclaim_pin`'s "no hook, no fsmonitor" adds optional locks and the filter caveat, and `_ws_reclaim_pin_absent`'s "a program the repository names, and no such program runs inside a pin" now says a hook. A further DEPARTURE, `containment-comment-cites-plan`: `_ws_reclaim_contained`'s header cited spec §5.5 for "no repository-configured program"; the spec has no hook sentence (grep), so the fix re-cites the plan's Task 3 ("no hook of the repository runs") instead, the source the review names; the ccd-child-reclaim-audit.test.ts describe block naming the same claim was re-cited to match, but in the FOLLOWING fix task's commit (fix task H, review 170 F9's round), not this one. Tests: the audit case (two rows: plain, and the exported flag) and the refused-verb case, each asserting no planted program ran and the child's index bytes and mtime unchanged, each with a CONTROL after the subject (an uncontained `status` of the same tree runs `post-index-change` and `fsmonitor` and rewrites the index); and the optional pin case that a clean filter still runs inside the contained pin. Mutation rows, in an isolated copy with a sha-checked restore: the fork wrap removed reds the verb case only (the audit's re-entry contains the fork too); the audit gate removed reds both audit rows; `GIT_OPTIONAL_LOCKS=0` dropped reds all three on the index assertion (hooks stay off, the index is still rewritten); the flag's global made to honour the environment reds the exported-flag row; a filter neutralised in the containment reds the filter case. No `_reg_get` call is added; the five S6-R11 cases are green at base AND at the tree. Numbered by the coordinator (review 170 rulings).
- **D-3518** (review 170 F7: `darwin-stamped-failed-asks-launchd`, `darwin-rows-forced-on-any-host`) — the plan let the tail's unit re-measure pass on `inactive|failed`, and on Darwin `failed` is not launchd's answer: `_svc_is_active` prints it whenever `$REG/<id>.svcfailed` exists, BEFORE it asks `launchctl print`, and neither `_ws_supervise`'s re-enable nor the swap success path's `_svc_start` clears that stamp. So a job running under a stale stamp, whose bootout in `_ws_unsupervise` also failed (that function only warns), read as stopped, and the tail removed the tree while the job stayed loaded and able to restart the pane. The existing cases stub `_svc_is_active`, so nothing measured it. MEASURED red first on this branch, before the ccd edit, with the Darwin arm forced on the Linux box: with the stamp planted and `_svc_launchctl print` answering `state = running` at exit 0, and again answering exit 1, `cmd_ws_reclaim` removed the worktree. What ships, in the RECLAIM region only (`_ws_reclaim_tail`; `_svc_is_active` and its other callers are not edited): directly after the existing re-measure, when `CCD_OS` is `darwin` and the answer is `failed`, the tail asks `_svc_launchctl print` by `$(_svc_domain)/$(_svc_label "claude-session@$id")` — the helpers `_svc_is_active` and `_svc_is_loaded` spell the label with, no second spelling — and passes ONLY on its exit 113 ("Could not find service", not loaded). Every other status fails `unit-still-active` with the status in the detail and the breadcrumb kept, exit 1 included (a missing binary and the sandbox guard both answer 1); `! _svc_is_loaded` is not used, because that boolean folds "could not be asked" into "not loaded". systemd's `failed` is its own answer, so the Linux arm is unchanged and every existing `unit-still-active` and tail case stays green. The token is reused; no fifteenth. Tests (`server/test/ccd-child-reclaim-verb.test.ts`, run on any host): the Darwin arm is forced the way `macos-platform.test.ts` forces it — `CCD_OS=darwin` assigned after the source, in the same payload — the REAL `_svc_is_active` is put back over `CHILD_STUBS`' `inactive` from a subshell source of ccd, so the stamp is what answers, and `_svc_launchctl` records and answers: (i) a stamp with `state = running` and (ii) a stamp with exit 1 each fail `unit-still-active` with the worktree, its files, the branch, the row and the breadcrumb standing and the print asked by `gui/<uid>/app.ccrc.session.<id>`; (iii) the control, a stamp with exit 113, reclaims. Mutation rows, in an isolated copy with a sha-checked restore: any non-zero print status accepted reds (ii); `! _svc_is_loaded` in place of the print reds (ii); the check dropped entirely reds (i), (ii) and, on its call-log assertion, (iii). No `_reg_get` call is added; every added line is below the frozen boundary, and the five S6-R11 cases are green at the base and at the tree. Numbered by the coordinator (review 170 rulings).
- **D-3519** (review 170 F20 + F21: `verb-non-json-failed-always-resumable-true`, `wip-collapses-to-string-or-null`) — [F20] `parseChildReclaimResult`'s fallback for any non-JSON (or unrecognised-JSON) `ws-reclaim` answer read `failed{resumable:true}` unconditionally off stderr alone, so `cmd_ws_reclaim`'s PRE-LOCK dies — a usage error, a malformed token/run id/session id, a missing `python3`, a missing `flock` binary, an unopenable lock file (ccd/ccd's RECLAIM region, everything above `_ws_reclaim_locked`) — read exactly like a genuine post-start abort or a call cut short: the feed said "the box resumes where it stopped" for a die that never reached the reap lock and recurs identically on any retry. [F21] `wip: string | null | 'unreadable'` on `ChildReclaimVerbRead`'s and `ChildReclaimOutcome`'s `reclaimed` arms collapses in TypeScript to `string | null` — `'unreadable'` is just another string, so nothing forced a consumer to branch on it (the seam rule). What ships, in `server/src/coord/childReclaim.ts` only (no ccd edit: recognition is by ccd's own stderr TEXT — route (b) — never the exit status, which `childReclaimAct` already discards before this fix): (1) `CHILD_RECLAIM_PRE_LOCK_DIE_PATTERNS` and `isChildReclaimPreLockDie` match `cmd_ws_reclaim`'s pre-lock die messages (usage, bad token, bad run id, bad session id, python3-unavailable, flock-unavailable, and the reap-lock-unopenable message's fixed prefix) after stripping `die`'s `ccd: ` wrapper; a match makes `parseChildReclaimResult` answer `{kind:'failed', resumable:false, preLockDie:true, detail:<ccd's own message>}`; an unmatched non-JSON answer stays `resumable:true, preLockDie:false` exactly as before. `preLockDie` is threaded through every `ChildReclaimVerbRead`/`ChildReclaimOutcome` `failed` construction (`false` everywhere else), and `childReclaimFeedBody`'s `case 'failed'` renders a THIRD sentence for it, distinct from both existing `resumable` branches, that does not promise a retry succeeds and names the box, not the workspace, as where to look. (2) `wip` becomes `ChildReclaimWip = {kind:'none'} | {kind:'commit', sha} | {kind:'unreadable'}` on both types; `parseChildReclaimResult` and `childReclaimWipText` are its only producer and consumer, both updated; `WIP_SHAPE` stays the one parse check. MEASURED: `command grep -rln '\.resumable\b\|\.wip\b' server/src pwa/src agent/src shared` names only `server/src/coord/childReclaim.ts`, so neither field crosses the wire and nothing outside that file needed a change. Why an ADDITIVE `preLockDie` rather than a third `resumable` value or a repurposed string: D-3360(5) already ruled `resumable: boolean` in place of a `stage` word, and a pre-lock die and a substance-refusal both already answered `resumable:false` before this fix — only the die recurs identically — so the feed needs a distinct word to pick its sentence without overloading `resumable` itself, the same seam rule F21 is about. `bad session id` is recognised alongside `bad token`/`bad run id` (immediately adjacent in `cmd_ws_reclaim`'s own argv validation, same class, same call shape) though F20's text names only six of the function's pre-lock dies; the `--actor`/`--reason` validation dies (shared `decFlags` plumbing used by many verbs, not reclaim-specific, and unreachable through `sweepDec`'s composed actor string) are left unrecognised, matching F20's stated scope. Tests (`server/test/child-reclaim.test.ts`): every `wip` literal across `parseChildReclaimResult`/`reclaimChild` assertions takes the discriminated shape; a `describe('a pre-lock die of cmd_ws_reclaim', …)` extracts each recognised message from ccd/ccd's own RECLAIM region text at test time (never copied by hand) and asserts `resumable:false, preLockDie:true` for six die kinds, the reap-lock prefix match against a path this build never composed, an unrecognised non-JSON stderr staying `resumable:true, preLockDie:false` (a post-lock abort also has empty stdout), and one end-to-end `reclaimChild` case pinning the feed sentence. No `_reg_get` call added; no `ccd/ccd` edit, so no re-stamp and no S6-R11 tax. **Corrected in place (review fr-I, fix round 1):** the claim that the reap-lock-unopenable message is "recognised by its fixed prefix" was FALSE, MEASURED (I1) — `exec {lfd}>>"$lock"` failing makes bash print its OWN redirection diagnostic FIRST (`<script>: line <N>: <lock path>: <errno text>`), so the real stderr is never the single `ccd: <prefix><path>` line the shipped pattern and test assumed; the pattern was dead code and the shipped test pinned a fiction. What ships now: the seventh die has its OWN recogniser, `matchChildReclaimLockUnopenableDie` (the six single-line dies keep `CHILD_RECLAIM_PRE_LOCK_DIE_PATTERNS`, unchanged) — it requires the FINAL line to be exactly `ccd: cannot open the reap lock at <path>` and every earlier line (zero or more) to be bash's own diagnostic for THAT SAME path; both recognisers are called from one `matchChildReclaimPreLockDie`, and the positive case is pinned against a REAL capture (a fixture HOME, the lock path turned into a directory, `childReclaimVerb` run for real — never a hand-typed stderr string). The two booleans `resumable`/`preLockDie` are replaced by one three-way `ChildReclaimResume = 'resumable' | 'not-resumable' | 'pre-lock-die'` (fr-I m1): the pair admitted `{resumable:true, preLockDie:true}`, a state no producer built, the same seam this D-number's own F21 half is about. The pre-lock-die feed sentence no longer says "the problem is on the box, not the workspace" (fr-I m2): that misdirects an operator looking at an argv/version die (usage, a malformed token/run id/session id), which is the SERVER's composition against the box's ccd, not the box alone — the sentence now says the call was refused before anything started, nothing was touched, and the same call is refused again until what caused it — its own arguments, or the box's ccd, python3 or flock — changes, true of both families this recognises. The test's `dieMessage` helper is re-anchored (fr-I m3) to `die "…"`/`_lc_refuse …"…"` calls themselves, never the first quoted string containing the needle (a comment could have collided). A five-row mutation table for the recogniser was added (fr-I I2, isolated copy, sha-checked restore): the matcher forced to always answer no match (red, 9 cases); the `ccd: ` strip dropped on the six single-line dies (red, 7 cases — the lock-unopenable path has its own prefix logic, unaffected); the six single-line patterns' end anchors dropped (red, exactly the two NEW "an EXTENDED die is NOT recognised" cases fr-I I2 asked for — an unanchored `^bad token` would keep matching `bad token (want 64 hex)`); the empty-stderr guard dropped (green, equivalent — no pattern matches `''`, matching the review's own prior finding); the lock-unopenable recogniser's same-path requirement across preceding lines forced to always pass (red, the "a bash diagnostic naming a DIFFERENT path" case). **Corrected in place (review fr-I, fix round 2, rereview fr-I-r1):** two residuals from fix round 1. (1) **m2 was not actually fixed** — the reworded sentence ("…until what caused it — the call's own arguments, or the box's ccd, python3 or flock — changes") is FALSE for the lock-unopenable die itself, whose cause (EISDIR/EACCES/a missing `$REG`) is none of the four things named; the round 1 claim that this wording is "true of both families this recognises" was wrong on a THIRD family (the recogniser covers three: argv/version, missing binary, lock/registry state) that no single named list covers. The sentence is now CAUSE-NEUTRAL — "ccd refused the call before anything started; nothing was touched, and it refuses the same way every time until that changes." — and leaves the cause to `detail` (ccd's own message), rendered immediately before it, for every family alike. A feed-body assertion for the lock-unopenable die itself was added, fed from the SAME real capture I1 measures (never a second synthetic string), asserting the identical sentence and that it names none of "ccd, python3 or flock". (2) **I2's end-anchor pin covered only 2 of the 6 single-line dies** (bad token, python3-unavailable); dropping the `$` of usage, bad run id, bad session id or flock individually stayed green. The EXTENDED-die `it.each` now shares the SAME six-needle list the positive case uses, so a single dropped anchor on ANY of the six reds exactly that die's case (measured: dropping `bad run id`'s `$` alone reds only "an EXTENDED bad run id die…", isolated copy, sha-checked restore). Also added, as a genuinely NEW case (not a residual of I2): the lock recogniser's last-line pattern's `^` start anchor, correct in the shipped code but unpinned by any case — a final line with content BEFORE `ccd: ` (e.g. `x ccd: cannot open the reap lock at /p`) is now asserted `resumable`; measured that dropping the `^` (isolated copy, sha-checked restore) reds exactly that case and returns a garbled, mid-string-cut `detail`, the exact failure mode the anchor exists to prevent. Numbered by the coordinator (review 170 rulings).
- **D-3368** (review 170 F11 + F13: `actor-less-wording-restored`, `review-child-close-order-true`, `readme-1676-exception-added`; corrected in place, review fr-J I2 + M3) — Task 10's fix round `07e8d552` altered plan-prescribed, word-for-word-pinned text in `ccd/coordinator-skill/references/wave-lifecycle.md` (~:833, :838-839, :843-844) and `ccd/reviewer-skill/SKILL.md` (~:146-148), unledgered, inserting "wave 4" FOUR times: (1) "picked up later" → "picked up later by wave 4's sweep"; (2) an added "No `childReclaimWhy` at all … wave 4's sweep reaches it regardless" sentence; (3) "the reviewer is reclaimed only after the run it reviewed has closed" → "the reviewer is reclaimed by wave 4's sweep only after …" — a different edit shape from (4), not a second copy of it; and (4) reviewer-skill/SKILL.md's "the server reclaims it" → "wave 4's sweep reclaims it", made ONCE, not twice (`git show 07e8d552 -- ccd/`) — none covered by a D-number (D-3362 covers only the word-pin, above). "Wave 4" names nothing outside this programme, so a skill shipped to every rostered home reads it as a real wave of ITS OWN programme. Separately, `fcd84cd0` made THREE README additions (`git show fcd84cd0 -- README.md`: three hunks), only TWO of which this number covers — the child-close pair, both beyond Task 10 Step 6's two prescribed replacements: (a) README:1676's abandon-child sentence ("A CHILD goes further: an abandon finishes it, so once no other open run names it the server reclaims it after the release — pinned, then removed (**A child is not a reap**, below; `wave-lifecycle.md` §6)"), which never gained the review-child exception (b) already states two paragraphs later — README step 6's own paragraph opening "**Except a CHILD** (**A child is not a reap**, above)" (a snapshot at e592d617: README:2138-2143 — locate it by its opening words, never by counting, since README keeps moving), fcd84cd0's SECOND addition, which reads "the server then reclaims it (pinned, then removed; a review child once the run it reviewed is terminal)". Addition (b) is not itself edited here; it is the source (a) was missing, and this entry now records it as fcd84cd0's own departure rather than leaving it unledgered. fcd84cd0's THIRD README hunk — the scratch-slug `*--cc-tmp-*` infix sentence (near the "Scratch slugs are skipped" paragraph), the commit's own third bullet verbatim ("README's scratch-slug sentence and single-definition's describe/it titles name the `*--cc-tmp-*` infix beside the four prefixes; the mirror row now also refuses a second declaration of SCRATCH_INFIXES. single-definition is edited in place, line count unchanged.") — is untouched by anything F11/F13 or this number covers; it is orthogonal to child-close prose and carries no departure of its own. The wave-lifecycle.md §6 review-child sentence this task ALSO fixed is separate: it is plan-prescribed Task 10 text (plan:6833-6839, word for word), not a README addition and not from fcd84cd0 — it claimed "closing the review run releases the reviewer and answers `review-report-live`" unconditionally, false when the reviewed run is ALREADY terminal at the review close, where `closeReviewRun` passes `final:true` (`close.ts:534`, at eac64860) and the close itself answers `childReclaim:'queued'` (`childReclaimDecision`'s review-child gate and `finished` computation, `c5962a94:server/src/coord/childReclaim.ts:266-272` (both, verified with `git show c5962a94:server/src/coord/childReclaim.ts | sed -n '258,273p'`) — at eac64860 the same logic sits at `:294-296` and `:298-300`; asserted at `child-reclaim-close.test.ts:294-322`). What ships: every "wave 4" mention in both skill files is reverted to the plan's actor-less form ("picked up later"; "the reviewer is reclaimed only after the run it reviewed has closed") or, in `reviewer-skill/SKILL.md`, to the pre-fix-round actor "the server reclaims it" (the true, non-programme-specific wording the base plan used); the added "No `childReclaimWhy` at all…" sentence is KEPT — measured true against `close.ts`'s `handOffChildReclaim` (`:693-703`): a decision of `reclaim:true` with `deps.childReclaim` undefined, `gate.request` null, or a thrown port, all answer `not-queued` with NO `childReclaimWhy` — with its own actor named removed ("wave 4's sweep reaches it regardless" → "it is reached regardless"); `wave-lifecycle.md`'s review-child sentence (Task 10 text, not a README addition) now states both close orders truthfully (the reviewed run open at the review close → `review-report-live`; already terminal → the review close finishes the reviewer directly, with no deferral); and README:1676 gains the review-child exception fcd84cd0's OWN second addition, README step 6's "Except a CHILD" paragraph, already states. Numbered by the coordinator (review 170 rulings).
- **D-3520** (review 170 F4 fix: `index-copy-keeps-its-mtime`) — `_ws_wip_commit` builds the WIP in a scratch copy of the child's index (`GIT_INDEX_FILE`), and until f7649728 that copy was a plain `cp -- "$_WS_INDEX_SRC" "$idx"` (so at c5962a94), which stamps the copy with the time of copying. git treats an entry whose file changed in the same second the index was written as RACY and compares its content, but it decides that from the index file's own timestamp: in a copy stamped later the entry looks settled, so step 2's `status`/`add` in the copy read a same-size edit made in that second as unchanged and left it out of the WIP, and the tail's forced worktree removal then deleted it. That was a pre-existing loss of ORDINARY tracked edits, older than F4; F4's new cases exposed it. What ships (f7649728, in `_ws_wip_commit`, below `ccd/ccd:19131`): the copy is `cp -p -- "$_WS_INDEX_SRC" "$idx"`, keeping the index's timestamp, with a comment above it. The pin is `server/test/ccd-child-reclaim-pin.test.ts`'s `it.each` "keeps a same-size edit made in the second the index was last written", run for a tracked file and for an assume-unchanged one, with `core.trustctime false`, every timestamp set by `touch -t` (never raced), and the edit made after the ladder, in the settle window. MEASURED by mutation in an isolated `git archive` copy of f2b32a86, with a sha-checked restore: the control ran 2/2 green; `cp -p --` changed to `cp --` reds the tracked-file case and leaves the assume-unchanged case green, the same result as fix task E's M14 row. The hidden variant stays green because step 3 finds a hidden-flag edit by its content, not by stat, so it is a control there rather than a second pin. The comment above the copy says a hidden edit would be lost too; at this tree that is not so, and it is left unedited here (no `ccd/ccd` change in this commit) for wave 4. Numbered by the coordinator (review 170 rulings).
