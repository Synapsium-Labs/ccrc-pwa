...
- **D-3514** (review 170 F2: `a10-nested-arm`, `nested-rows-own-global`, `nested-literal-arm-not-implied`, `nested-detail-names-rows`) — A10 ("no second registry row may name this workdir", Task 2) refused only when another row's `.workdir` EQUALLED the child's, literally or resolved, so a session rooted INSIDE the child was not seen. MEASURED red first on this branch, before the fix: a nested same-repository worktree `inner` on `ws/other-live` with its own registry row, and a row rooted at `<child>/server`, each answered `reclaimable` from `_ws_reclaim_eval`, and each was reclaimed by `cmd_ws_reclaim` on the fresh arm (with a token minted before the other row appeared) and by a resumed tail — the child's tree, the other session's inside it, removed. What ships: (1) `_ws_reclaim_workdir_shared` also answers `_WS_NESTED_ROWS`, the OTHER rows whose `.workdir` lies inside the child's at a path-component boundary — `"${wd%/}/"*` on the literal path, `"${mine%/}/"*` on the `_ws_realpath`-resolved one — so `<child>x` is a sibling, not inside. The equality arm (`_WS_SHARED_ROWS`) and the fail-closed answers for an unlistable registry or an unreadable row are unchanged, and a row ABOVE the child is not refused. (2) Both callers refuse on it exactly where A10 already runs: `_ws_reclaim_eval` answers the terminal `containment-unproven` with a detail naming the nested row id(s) and the equality arm's closing clause ("ccd cannot prove the tree at that path is <id>'s own"); `_ws_reclaim_owned`, the tail's ownership re-proof on every arm, fails `worktree-remove-failed` naming them. (3) Unlike the equality arm, the nested arm's literal comparison is NOT implied by the resolved one: a row spelled through a link inside the child resolves outside it while the path it names goes with the child; a case pins it. (4) No fifteenth token, and `containment-unproven`'s sentence in `server/src/wsaudit.ts` is untouched (F9 rewrites it). Six mutation rows, each red on a named case — drop the nested arm, a bare string prefix (the `<child>x` control reds), drop the resolved comparison, drop the literal comparison, drop the tail's arm only (the two resumed-tail cases red), drop the ladder's arm only (the fresh-arm verb cases red on `nothing was pinned`: the tail alone refuses after the pin). Every new `ccd/ccd` line sits in the RECLAIM region, below `ccd/ccd:19131`; no `_reg_get` call is added; the five S6-R11 cases are green at the base and at the tree. Numbered by the coordinator (review 170 rulings).
