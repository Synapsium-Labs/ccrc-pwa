...
**⚠ TASK 1 ALONE MUST NOT SHIP TO THE REFERENCE FLEET.** Nothing writes the flag file yet — `ccrc install`'s seed and deploy's fleet-lane seed are Task 2, and `grep -n remote-control ccd/ccrc deploy/deploy.sh` is zero hits until they land. So on every existing box `_rc_enabled` answers off, and installing this ccd on the fleet host strips `--remote-control` from all ~11 live sessions at their next respawn. **The standing AGENT-FIRST rule does not save this — it is the procedure that would cause it:** `ccd/` changes normally go to the fleet host *before* the server, and here the ccd install must land *after* Task 2 Step 2's seed. That ordering is Task 2's to pin; until it exists, this branch is test-green and deploy-blocked, and `ccd/ccd`'s own comment block above `_rc_enabled` says so in the file an operator would be reading.
...
- **Doctor owes a THIRD state, not two** (Task 2 Step 3, `_dr_rc_state`). As specced it prints `on` / `off (default)` / `off`, which reports a newline-less `printf 'on'` file as a deliberate `off` — the operator's edit did not take and the surface says they chose it. `off (unparseable — the file must hold one line reading 'on' or 'off')` closes the whole operator-surprise concern at the reporting layer, which is the right place to pay for it, and leaves the reader strict.
