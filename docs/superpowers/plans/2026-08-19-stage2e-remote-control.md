...
- [x] **Fix round 2 (whole-task review, 2 Important).** `answerDialog`'s widening was unpinned — the whole 3618-test suite stayed green with a busy veto re-added inside it; fixed with an ~8-line pin in `answer.test.ts`. The D-102 ledger's redundancy claim was wrong as stated (tested reverting to the OLD predicate, not deleting the gate); corrected to what deletion actually proved. `paneState`/`PaneState`'s now-zero production consumers ruled: keep as the test-facing classifier contract. Commit: `fix(server): answerDialog's busy-proof walk is pinned; D-102 says what is actually true`
...
Every caller of `parseDialog` was enumerated first (`git grep -n parseDialog server/src server/test`) before touching anything: the two now-fixed call sites; `inject/send.ts`'s `answerDialog` (walks the cursor and confirms an answer, at :915 and :926) — does not rely on the busy-veto to suppress anything, only on staleness (`dialog.id !== dialogId`) and walk verification, so the fix is a strict improvement there (a busy flicker mid-walk no longer breaks an otherwise-successful confirm); `inject/send.ts`'s `sendPrompt`/`clearBox` guards (:320, :495, :865) call `hasMenu` directly and never route through `parseDialog` at all, so they were never at risk; and every test caller (`answer.test.ts`, `routes.test.ts`, `dialog.test.ts`) uses pure-menu fixtures, unaffected.

**Fix round 2 (whole-task review, two Important findings).**

1. **The `answerDialog` improvement was real but unpinned.** Round 1's report noted "no existing test constructs a busy+menu combo pane for it" and stopped there, reading regression-clean as proof-of-safety. The reviewer measured otherwise: re-adding a busy veto *inside* `answerDialog` (`if (paneState(pane) === 'busy') return { ok: false, error: 'stale-dialog' };`, immediately before the parse) left **all 3618 tests green** — the widening in the one path that types keys into a live session was invisible to the entire suite. Fixed: `test/answer.test.ts` gained an ~8-line pin (`D-102: a busy spinner painted alongside the menu no longer refuses the answer`) built from the file's own `fakeTmux`/`menuPane`/`deps`/`sendKeysCalls`/`DIALOG_ID` helpers, prefixing a real busy-spinner row (verified: `Dialog.id` is content-derived from labels+title, so the prefix does not change it). Measured: passes at HEAD; reds when `parseDialog`'s gate is reverted; reds when the busy veto is re-added inside `answerDialog` directly.

2. **The redundancy claim below was wrong as originally stated.** The two mutation rows it cited (reverting each call site's gate back to the OLD `paneState(pane) === 'menu'` predicate) prove the gates must not be the *old* predicate — they do not test redundancy. The redundancy test is *deleting* the gate: `sessionws.ts:253`'s `let dialog = pane !== null ? parseDialog(pane) : null;` (gate removed entirely) left **all 59 `sessionws.test.ts` tests green**, because the outer gate is now the identical predicate to `parseDialog`'s own first line (`hasMenu(pane.replace(SGR,'')) ? parseDialog(pane) : null` ≡ `parseDialog(pane)` for a non-null pane, since `parseDialog` opens with the same `hasMenu` check). **Corrected claim:** the outer call-site gates are redundant-by-predicate with `parseDialog`'s own gate today — deleting either changes nothing (measured) — and are retained deliberately as defense in depth: a cheap short-circuit before parse work, and a local statement of intent matching the `send.ts:320` idiom the rest of the file uses, not as an independent guard. What the two mutation rows below DO establish, correctly: reverting either call-site gate to the OLD `paneState` predicate still reds its behavioral test (mutations A/B), and reverting `parseDialog`'s own gate reds BOTH (mutation C) — together these three single-point mutations are what replaced the round-1 structural pins, and each pins a distinct point in the pipeline even though two of those points currently compute the same boolean.

Re-measured, corrected: reverting `parseDialog`'s own gate alone → both behavioral tests (`sessionws.test.ts`, `push-copy.test.ts`) red. Reverting EITHER call site's outer gate to the OLD `paneState(pane) === 'menu'` predicate, with `parseDialog`'s fix left standing → the SAME behavioral test still reds (confirmed, both sites) — informative, but not a redundancy proof, since deleting the same gate outright does not red anything (see above). The two `mutation-sensitive` source-text pins fix round 1 added (to compensate for the then-unobservable call-site mutation) are now genuinely redundant with the behavioral tests, which are mutation-sensitive to the same "wrong predicate" revert, and were removed rather than kept as decoration.
