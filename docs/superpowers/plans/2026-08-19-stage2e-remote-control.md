...
**Deploying a fleet host still means running the agent lane, not hand-copying `ccd`.** The ordering guarantee lives in `deploy.sh`; an `install_atomic`/`scp` of `ccd/ccd` done by hand onto an unseeded box reopens exactly the window this closed.
...
- **A fleet host's doctor never reports its RC state** (found in Task 2, left as specced). `_dr_rc_state` hangs off `_check_config`'s PASS arms, and a fleet-role box has no `ccrc.env` at all — D-86's topology branch SKIPs it. So the one box in the topology that actually runs `on` is the one box whose doctor stays silent about it. Appending to the SKIP is not the fix (a skip that also reports is a half-verdict); the fix is a check of its own, or a fleet-role arm, and it is not Task 2's to design. `server/test/ccrc-doctor.test.ts` ("leaves the fleet-role SKIP alone") pins today's behaviour so the gap is deliberate rather than forgotten.
