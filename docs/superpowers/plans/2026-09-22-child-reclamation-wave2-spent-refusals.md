...
| 3 | superseded by b0ab69c7: the line no longer exists — shipped `childMarkOf` answers `listed ? unreadable : none` for BOTH failure reasons, the opposite of this row's prediction | row 1 above already mutates the shipped line to `return { kind: 'none' };` and covers this case: *LISTED, and its bytes did not come back → unreadable, never none* |
