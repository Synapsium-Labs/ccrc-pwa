...
| 2 | In the `line === undefined` branch, replace the whole `return { kind: 'unmeasured', … };` with `return { kind: 'unspent' };` | superseded by D-3347: `merge-unproven` moved out of this table into its own assertion, leaving four, not five — measured: reds all FOUR remaining *… → unmeasured, never unspent* cases, 58/62 elsewhere green |
...
| 7 | In `LIVE_PHASE`, change `unknown: 'unmeasured'` to `unknown: 'unspent'` | superseded by D-3347: the same-branch scan now answers before `phaseFor` runs for any row it could bind, so this arm is unreachable from this call site — measured: all-green, 62/62 across child-reclaim-spent/-bind/-refusals |
