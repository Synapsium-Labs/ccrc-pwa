...
- **D-3340** (`task4-first`) — Task 4 (`feat(coord): childSpent — the three-valued spent verdict, measured when asked`, b718964d) was committed before Tasks 1-3, because Tasks 1-3's files were under other workers' claims at dispatch. Coordinator ruling 2026-09-23: proceed; the departure is in commit order only, and no task's own content depends on another landing first.
- **D-3341** (`anchors-measured-not-plan`) — the plan's README anchor snapshot into `shared/api.ts` (`:7465-7467` / `:7505` `:7513` `:7526`, at 62d9c485) predated `main`; the branch base measured `:7474-7476` / `:7514` `:7522` `:7535` instead, and each paying task re-anchored by content rather than by the plan's own numbers: Task 1 -> `:7492-7494` / `:7532` `:7540` `:7553`; Task 2 -> `:7539-7541` / `:7579` `:7587` `:7600`; Task 5 -> `:7555-7557` / `:7595` `:7603` `:7616` — SAME-BYTES each time, and the `shared/api.ts` citation-debt entry (`session-hook.test.ts`) measured 1 -> 1 in all three, sum 195. Coordinator ruling 2026-09-23: matches the plan's own "anchors are snapshots" clause; recorded as a deviation because the plan's drafted numbers never held against this branch's base.
- **D-3342** (`mutation-row1-6-of-8`) — Task 2 Step 7's mutation row 1 predicted the mutated final throw would red all eight `rejects the WHOLE session` cases (`child-reclaim-mark.test.ts`); measured 6/8, because `asObj` (`shared/api.ts:2428`) already throws on a bare string and on an array before that final throw is ever reached. Coordinator ruling 2026-09-23: accepted as the true count; the row's prediction was wrong about `asObj`'s own earlier guards, not about the mutation's own effect.
- **D-3343** (`run-routes-stale-sentences`) — Task 6 made true the older sentences `run-routes.test.ts`'s second counter test and the first counter test's own title carried (a SECOND-directory-read claim, where Task 5's counter move had already made the resumed session's own listing the FOURTH), which Task 5's move had falsified without a matching test-text update. Coordinator ruling 2026-09-23 (a controller ruling, on the same lines Task 6 edits): rename in place; the test bodies themselves were already exercising the right read.
- **D-3344** (`fresh-child-default-branch-prose`) — from the final whole-branch review: §2's `workspace-spent` row, §5's one-PR paragraph and SKILL.md step 6 now state that a fresh child branches from the project's default branch (`ccd ws-add`'s `base=` from `origin/HEAD`) and carries none of a spent producer's unmerged commits, so a dependent wave N+1 is dispatched only after wave N's PR is proven merged — the cross-project arm's condition for a different-project producer, and the producer run's own verified `handoffCommit` for a same-project one (held item 2, this round). Coordinator ruling 2026-09-23: accepted, landed at 08e40675 and refined this round.
- **D-3345** (`leftovers-60s-lane-remote-only`) — the wave-1 six-prose-leftovers commit's §2 `child-omitted` rewrite (46b46ecd, 4b6bdaa7) departed from the ledger's three-remedy wording: the 60 s caps lane is remote-only and never resolves a local box's boot window; the local boot window itself clears within seconds, on the one bounded local boot probe (`server/src/localcaps.ts`, 10 s); and (extension per F10, review run 144) a failing `ccd caps` is NOT evidence of an old ccd — look at the ccd, and restart the server afterward on a local box to re-probe. Coordinator ruling 2026-09-23: accepted; D-3338's wave-2 addendum (wave-1 plan) is corrected to match this text.
- **D-3346** (review F9) — commit 08e40675 extended §2's dispatch `spent-unmeasured` row beyond Task 6 Step 5(a)'s prescribed text: dispatch lists the registry for its own pause check FIRST, so a registry that is already unlistable there answers `paused` instead, and this code's own unlistable case is only a listing that fails at the resume arm's re-read. Coordinator ruling 2026-09-23: accepted; the clause is accurate against `dispatch.ts` and no existing slug named it.
...
- **D-3348** (review F4) — the plan's `childMarkOf` mapped `absent` -> `none` whatever the listing said, so a `.child` the listing names that reads back absent (a dangling symlink, or a file removed between list and read) permitted the bind. Shipped (b0ab69c7): that case now answers `unreadable`, and the bind refuses, retryably — the safe direction where the server sees more than ccd's own `_reg_get` does.
- **D-3349** (review F3 + F6) — every `unbound:false` the dispatch `workspace-spent` arm returned carried no `detail`, including `cleared:false` after a successful fleet act, contradicting the `DispatchOutcome` docstring and §2's own row ("`detail` whenever `unbound` is `false`"). Shipped (3515cb16, a65257da): a `detail` is always carried; §2's row and the docstring now say a detail naming a permanent cause — an unsupported verb, an invalid survivor hold, an unreadable sibling list — means stop and report, and any other detail means retry once then stop and report; "that release did not happen" is corrected for the hand-over arm (`ws-hold`).
