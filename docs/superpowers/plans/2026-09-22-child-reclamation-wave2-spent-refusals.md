...
| 3 | superseded by b0ab69c7: the line no longer exists — shipped `childMarkOf` answers `listed ? unreadable : none` for BOTH failure reasons, the opposite of this row's prediction | row 1 above already mutates the shipped line to `return { kind: 'none' };` and covers this case: *a PROVEN ENOENT on a listed marker … → unreadable, never none (F4, D-3348)* (`child-reclaim-mark.test.ts:109`) — measured: 4 red of 116 in child-reclaim-mark + registry |
...
| 2 | In the `line === undefined` branch, replace the whole `return { kind: 'unmeasured', … };` with `return { kind: 'unspent' };` | superseded by D-3347: `merge-unproven` moved out of this table into its own assertion, leaving four, not five — measured: reds all FOUR remaining *… → unmeasured, never unspent* cases, 58/62 at 90c51032; 61/65 at 95703aa7 elsewhere green |
...
| 7 | In `LIVE_PHASE`, change `unknown: 'unmeasured'` to `unknown: 'unspent'` | superseded by D-3347: the same-branch scan now answers before `phaseFor` runs for any row it could bind, so this arm is unreachable from this call site — measured: all-green, 62/62 at 90c51032; 65/65 at 95703aa7, across child-reclaim-spent/-bind/-refusals |
...
The coordinator issued D-3340 through D-3349 for this wave in its fix-round mail (2271, 2026-09-23, run 138's fix round) after review run 144; each is defined below. The six contract reconciliations above are settled: items 1, 2, 4 and 6 were ruled ACCEPTED by the contract's owner on 2026-09-23 (R3, R18) and amend contract §2 in place; items 3 and 5 needed no ruling. Nothing gates dispatch.

- **D-3340** (`task4-first`) — Task 4 (`feat(coord): childSpent — the three-valued spent verdict, measured when asked`, b718964d) was committed before Tasks 1-3, because Tasks 1-3's files were under other workers' claims at dispatch. Coordinator ruling (mail 2234, 2026-09-23): "doing Task 4 first is fine. Name it as a departure slug." Numbered by the coordinator (mail 2271).
- **D-3341** (`anchors-measured-not-plan`) — the plan's README anchor snapshot into `shared/api.ts` (`:7465-7467` / `:7505` `:7513` `:7526`, at 62d9c485) predated `main`; the branch base measured `:7474-7476` / `:7514` `:7522` `:7535` instead, and each paying task re-anchored by content rather than by the plan's own numbers: Task 1 -> `:7492-7494` / `:7532` `:7540` `:7553`; Task 2 -> `:7539-7541` / `:7579` `:7587` `:7600`; Task 5 -> `:7555-7557` / `:7595` `:7603` `:7616` — SAME-BYTES each time, and the `shared/api.ts` citation-debt entry (`session-hook.test.ts`) measured 1 -> 1 in all three, sum 195. Measured by this worker, matching the plan's own "anchors are snapshots" clause; recorded as a deviation because the plan's drafted numbers never held against this branch's base. Numbered by the coordinator (mail 2271).
- **D-3342** (`mutation-row1-6-of-8`) — Task 2 Step 7's mutation row 1 predicted the mutated final throw would red all eight `rejects the WHOLE session` cases (`child-reclaim-mark.test.ts`); measured 6/8, because `asObj` (`shared/api.ts:2428`) already throws on a bare string and on an array before that final throw is ever reached. Measured by this worker; the row's prediction was wrong about `asObj`'s own earlier guards, not about the mutation's own effect. Numbered by the coordinator (mail 2271).
- **D-3343** (`run-routes-stale-sentences`) — `run-routes.test.ts` carried two sentences Task 5's own counter move (90a3d713, `n === 2` -> `n === 3`, its comment "is the THIRD") had already left stale: the second counter test's "A blanket `unlistableIO` fails the PAUSE check … answers `paused` … so this scopes the failure to the SECOND read" comment, and the first counter test's own "SECOND directory read" title. Task 6's move (0bc06e70, `n === 3` -> `n === 4`) is what made both true for the fourth-read world its own commit ships, rewriting both comments and the first test's own title to say FOURTH. The SDD controller ruled it fine to fix in place, on the same lines Task 6 edits; the test bodies themselves were already exercising the right read. Numbered by the coordinator (mail 2271).
- **D-3344** (`fresh-child-default-branch-prose`) — from the final whole-branch review: §2's `workspace-spent` row, §5's one-PR paragraph and SKILL.md step 6 now state that a fresh child branches from the project's default branch (`ccd ws-add`'s `base=` from `origin/HEAD`) and carries none of a spent producer's unmerged commits, so a dependent wave N+1 is dispatched only after wave N's PR is proven merged; SKILL.md step 6's Different-project arm and §5 both spell out the `producerSha` proof for a different-project producer, but the same-project proof — the producer run's own verified `handoffCommit` — is §5-only (held item 2, this round). Landed 08e40675 and refined this round. Numbered by the coordinator (mail 2271).
- **D-3345** (`leftovers-60s-lane-remote-only`) — the wave-1 six-prose-leftovers fix round's §2 `child-omitted` rewrite (46b46ecd, then 4b6bdaa7) departed from the ledger's three-remedy wording: the 60 s caps lane is remote-only and never resolves a local box's boot window; the local boot window itself clears within seconds, on the one bounded local boot probe (`server/src/localcaps.ts`, 10 s); and (shipped in 4b6bdaa7, immediately after 46b46ecd's review-136 fix — review run 144's F10 found only that it carried no departure slug) a failing `ccd caps` is NOT evidence of an old ccd — look at the ccd, and restart the server afterward on a local box to re-probe. This worker's fix round C corrected D-3338's wave-2 addendum (wave-1 plan) to match this text. Numbered by the coordinator (mail 2271).
- **D-3346** (review F9) — commit 08e40675 extended §2's dispatch `spent-unmeasured` row beyond Task 6 Step 5(a)'s prescribed text: dispatch lists the registry for its own pause check FIRST, so a registry that is already unlistable there answers `paused` instead, and this code's own unlistable case is only a listing that fails at the resume arm's re-read. Coordinator ruling (F9, 2026-09-23): "ACCEPTED; your §2 spent-unmeasured clause is accurate" — no existing slug named it. Numbered by the coordinator (mail 2271).
- **D-3347** (review F1) — the plan's `childSpent` live rung trusted `phaseFor`'s `none` -> `unspent` (contract §2's `none|no-commits -> unspent`); a same-repo PR opened from the child's own branch that did not BIND — a stacked base, or a head not reachable from the local tip — read "no PR" and left a spent child unrefused. Operator rule: "a PR OPENED from its branch spends a child", binding not part of it. Shipped (32367a7c, 5c22303e): any live row with `isCrossRepository === false` and `headRefName` equal to the line's own branch answers spent, at the HIGHEST such number; an unestablishable repository or a numberless match answers unmeasured; `phaseFor` decides only when no such row exists; a MERGED row whose merge ccd cannot prove now answers spent rather than unmeasured. Measured on real `ccd pr-state` output (fixture HOME, stub `gh`, through `parsePrLines`): ccd queries `--head` only and prints every annotated row. Consequence ruled 2026-09-23: a recycled slug inherits its head name's PR history and reads spent on a RE-bind only — a fresh child is never gated; wave 3 separates incarnations with a `createdAt` check. The ruling amends contract §2; the coordinator writes that amendment (not part of this branch). Numbered by the coordinator (mail 2271).
- **D-3348** (review F4) — the plan's `childMarkOf` mapped `absent` -> `none` whatever the listing said, so a `.child` the listing names that reads back absent (a dangling symlink, or a file removed between list and read) permitted the bind. Shipped (b0ab69c7): that case now answers `unreadable`, and the bind refuses, retryably — the safe direction: the server now REFUSES more than ccd does, since for a dangling link ccd's own `_reg_get` also answers `""` (not-a-child). Numbered by the coordinator (mail 2271).
- **D-3349** (review F3 + F6) — at 08e40675, `refuseSpentChild`'s four `keep(...)` returns (an unreadable sibling list, an invalid survivor hold, an unsupported ccd verb, a failed fleet act) already carried a `detail`; only its final return after a successful fleet act — `unbound: cleared.cleared`, including on `cleared:false` — carried none (F6). Separately, §2's row and the `DispatchOutcome` docstring told a coordinator to retry the same dispatch on any `unbound:false` with no stop condition, though several of those already-detailed causes are permanent (F3). Shipped (3515cb16, a65257da): the successful-act return now carries a `detail` whenever `cleared` is false — i.e. every `unbound:false` carries one; §2's row and the docstring now say a detail naming a permanent cause — an unsupported verb, an invalid survivor hold, an unreadable sibling list — means stop and report, and any other detail means retry once then stop and report; "that release did not happen" is corrected for the hand-over arm (`ws-hold`). Numbered by the coordinator (mail 2271).
- **D-3350** (review 145 F2 + F3) — the review-144 minors the coordinator ruled "all fixed" changed text this plan itself PRESCRIBES, and none of the changes carried a deviation number: F5, the symlink exception added to `registry.ts`'s prescribed text (plan:341, shipped b0ab69c7); F7, the "under the same actor" comments at plan:2670 and plan:2740, now naming two actors (shipped 3515cb16); F8, the `RunRefuseCode` sentence at plan:1966, reworded line-neutral (shipped 741a48cd); F13, `survivorOf` hoisted into `server/src/coord/rundefs.ts` and imported by `server/src/coord/close.ts`, neither file named in the plan's File Structure table (shipped b34c921e); F14, the `i` flag added to the prescribed UNBIND regex at plan:1195 (shipped 741a48cd); F16, the MARKED/UNMARKED qualification added to the coordinator prose the plan prescribes at plan:2163, plan:2174 and plan:2205 (shipped 187147a8, bc6f2d81), plus this round's own F4 fix to §1's "And it is PR-FREE" paragraph (this round's docs commit). Plus F3: the three stale mutation rows are now re-spelt or marked superseded, each with its measured result — Task 4 Step 5 row 7 (plan:1560, superseded by D-3347, measured all-green/unreachable, 62/62 at 90c51032; 65/65 at 95703aa7); row 2 (plan:1555, superseded by D-3347, measured: reds the four remaining cases, 58/62 at 90c51032; 61/65 at 95703aa7 elsewhere green); and Task 1 Step 7 row 3 (plan:739, superseded by b0ab69c7, whose shipped verdict — listed-and-absent answers `unreadable` — row 1's own mutation already covers). Numbered by the coordinator (mail 2279).
- **D-3351** (review 145 F1 + F7) — "childSpent answers unmeasured when the branch or a row's head cannot be read (review 145)." What the plan's live rung said: it had no same-branch scan at all — once `line` was found it called `phaseFor(line)` directly and mapped the phase through `LIVE_PHASE`, so it fell through to `phaseFor` on literally ANY line the pr-state call returned, whatever its rows, `branch` or `tip`. (D-3347 is what added the same-repo scan this description used to (wrongly) attribute to the plan's own text.) The rename shape (F1): a MARKED child registered `ws/a`; the branch is hand-renamed inside the worktree; ccd queries `gh pr list --head ws/a`, which answers nothing, and `rev-parse refs/heads/ws/a` fails, so ccd emits `tip:null, ahead:null`; `phaseFor` answered `none` -> `unspent`, and the gate permitted a bind on a workspace whose branch might carry a live PR under its new name. The malformed shapes (F7): a line with no string `branch`, and a non-fork row (`isCrossRepository !== true`) whose `headRefName` is not a string — both fall through the same-branch filters (`r.headRefName === line.branch` answers `false` for either, for every row with a string head) straight to `phaseFor` too. Shipped (commit 1 of this round, 7c643a71): `childSpent` now refuses `unmeasured`, in order — no string `branch`; the existing same-repo/`unestablished` rungs, widened so a non-fork row with an unreadable head refuses too (a fork's unreadable head is still ignored, since forks never count); then a `tip` that is not a string — before `phaseFor` is ever called. Measured mutations: M1 (delete the tip check) reds the rename case in both child-reclaim-spent and child-reclaim-bind; M2 (delete the branch-not-string check) reds the no-branch case; M3 (delete the unreadable-head widening) reds both malformed-head cases. Numbered by the coordinator (mail 2279).
