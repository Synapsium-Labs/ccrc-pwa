...
| 2 | In the `line === undefined` branch, replace the whole `return { kind: 'unmeasured', … };` with `return { kind: 'unspent' };` | superseded by D-3347: `merge-unproven` moved out of this table into its own assertion, leaving four, not five — measured: reds all FOUR remaining *… → unmeasured, never unspent* cases, 58/62 elsewhere green at 90c51032; 61/65 at 95703aa7 |
