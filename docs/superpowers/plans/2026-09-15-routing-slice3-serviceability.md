...
Issued by `POST /api/ledger/deviations` on 2026-09-15 (one block of eight, D-2850..D-2857) and defined here in the same act. Every one records a defect in THIS PLAN's text or a ruling the controller made on a plan-mandated finding; the shipped tree is as the entries describe. The plan's task text above is a snapshot and is not repaired (it still spells `_svc_gate`, the inline placement guard, the raw `time.time()`, the ceiling-only reason).
