...
Issued by `POST /api/ledger/deviations` on 2026-09-15 (one block of eight, D-2850..D-2857, and one more, D-2858, issued after the whole-branch review) and defined here in the same act. Every one records a defect in THIS PLAN's text or a ruling the controller made on a plan-mandated finding; the shipped tree is as the entries describe. The plan's task text above is a snapshot and is not repaired (it still spells `_svc_gate`, the inline placement guard, the raw `time.time()`, the ceiling-only reason).

- **D-2850 — Task 1: the plan's `classBelow` docstring respelled the four class names.** Step 3's docstring read "fable → opus → sonnet → haiku → nothing"; `single-definition.test.ts`'s four-names scan reads comments too, so the file became an eighth holder and the guard reddened. Shipped: the docstring describes the walk over `CLASSES` (predecessor-of) without spelling the sequence; behaviour unchanged. Commit 91a34472.
- **D-2851 — Task 2: the plan's fixture snippet omitted the roster's fourth home-able account and a required field.** Step 1's five class cases named three accounts; `DEFAULT_TEST_ROSTER` has four home-able Anthropic accounts, so in `fable-at-ceiling-on-every-lane-places-nothing` the fourth, unmeasured, stayed eligible and won the fallback instead of `null`; and `LeastLoadedCase` requires a `why` string the snippet left out. Shipped: `claude-d` at the ceiling in that case, a `why` on each case. Commit c683692f.
- **D-2852 — Task 3: the plan's freshness arithmetic made its own boundary row unsatisfiable, and its rounding mutation could not red.** Step 3's `_share_pct` aged the pass with raw `time.time()`, so the fixture's "at exactly SHARE_FRESH_S is still fresh" row (a pass exactly at the bound) failed on subprocess overhead alone — measured 5/5 red; shipped: `int(time.time())`, and the row pinned deterministically in fix round 1 (the reviewer measured the raw form as a clock-phase flake as well). Step 5 predicted that replacing `int(est*100+0.5)` with `round()` reds the 0.395 row; Python's banker's rounding of 39.5 is 40 too, so nothing red — fix round 1 added a row that discriminates the two. Also: the parity runner's stdout/rc delimiter is `|rc=` (the harness trims output and the empty-stdout row collapsed on the brief's leading-space form). Commits d74bbdaf, 738f9a96.
- **D-2853 — Task 3, ruling S3-R1: `_svc_gate` renamed `_class_gate`.** The plan's name collided with `server/test/macos-platform.test.ts`'s pinned `_svc_` platform-layer vocabulary (launchd/systemd helpers inside PLATFORM LAYER sentinels); Task 3 shipped a named exemption with a control (738f9a96). Ruled: the first consumer (Task 5) renames the helper, removes the exemption and re-measures the guard's control; Task 6 uses the new name. The plan's text keeps `_svc_gate` as a snapshot.
- **D-2854 — Task 4, ruling S3-R3: the placement walk's empty answer gains a third meaning, and the refusal-reason arm is slice 4's obligation.** With a class, `_ws_least_loaded` answers `""` when every in-pool lane is measured unservable for the class — beside "at ceiling or disabled" and "tag undecidable" — and `cmd_ws_add`'s reason builder walks only `-x wrapper`/`_lane_enabled`/`_pool_ok`, so a class-emptied placement would refuse with an empty reason list. No caller passes a class in this slice. Shipped: the header paragraph names the third meaning; slice 4's `ws-add --route` task MUST land the `_serviceable` arm in the reason builder in the same commit that passes the class. Also in Task 4: the brief's Step 4 suite glob `test/ccd-ws-add*.test.ts` matches no file (the zero-argument pins live in ccd-pool-ok, ccd-authdead, ccd-account-ok and ccd-default-pool); and the brief's runner snippet named a nonexistent `project` variable with an unquoted positional — shipped with `JSON.stringify` on the runner's own project expression. The same obligation holds on the TypeScript side (D-2858's fix wave): `projectHome` now answers `null` for a class-emptied pool with a docstring naming it, and slice 4's placement route must carry `class` on `ProjectPlacement`'s `none` arm so the wire tells the third meaning apart. Commits c63779a4, 25dd0975.
- **D-2855 — Task 4 minor #3 (plan-mandated form): the placement filter's inline guard duplicated the gate helper.** The brief spelled the filter as an inline `[[ -n "$cls" && "$cls" != default ]]` block around `_serviceable`; the helper that folds `default` already existed. Shipped in Task 5 with the rename: `_class_gate "$w" "$cls"; [[ $? -eq 1 ]] && continue`.
- **D-2856 — Task 5, ruling S3-R4: the plan's home-recovered stay gate answered rc 5 under `force`.** Step 3's gate returned 5 on an unmeasured class window for the recovered home whether or not the call was forced; `_swap_target`'s own header says a forced call (a hard block) must always reach the candidate loop. Shipped: the unmeasured answer on that arm returns 5 only when not forced; forced falls through to the loop. Also in Task 5 (ruling S3-R5, three plan-mandated minors): a `_route_degrade` write failure is loud and stands still; `_route_restore` journals the measured standing value; the loop asks `_class_gate` like the gates. Commits fde7a29b, 90531cb4.
- **D-2857 — Task 6, rulings S3-R6 and S3-R7: the plan's settle block wrote a reason it had not measured and trusted a stamp it had not checked.** Step 3's degrade reason read "at or above the ceiling" whatever `_serviceable` had said (its `backend` refusal writes no figure), and the block honoured any standing `degraded` on the unmeasured arm even after the operator moved `class` elsewhere (a stale `opus` stamp composing ABOVE a newly intended `haiku`). Shipped in fix round 1: the why-word is captured and interpolated (settle and tick), a stamp that is not the rung below the current intended class is cleared as stale with a journaled reason before the switch, a failed stamp write composes the intended class and logs `degrade-unwritable`, and the brief's three under-asserting test titles gained their assertions. Commits ff6d4e95, cae2ab44.
- **D-2858 — final review: two departures from the plan's own text, both defects in it.** (1) Task 3's `_serviceable` mapped `_share_pct`'s rc 1 and 2 and let any other exit (a dead or missing interpreter, a traceback) fall through to the FIGURE test with an empty figure — `[[ "" -ge 40 ]]` is false, so the ceiling silently switched OFF toward the credit edge; shipped: every rc other than 0 is `unmeasured` (`no-figure`), pinned by a row that drives a non-{0,1,2} exit. (2) Task 5's tick wrote the `degraded` stamp in the rc-6 arm, BEFORE the hold/mid-turn/idle gates that can decline the move — a stamp without a move, and the ledger's premise that every rc-6 path had measured the session's own lane unable to serve the class was false whenever `_avail "$home"` was what sent it to the loop; shipped: the stamp is written at the two dispatch sites, so it follows the move. Also: `projectHome`'s docstring (server/src/limits.ts) now names the class filter and the third meaning of `null`, which the TS side of D-2854's obligation lacked — see that entry's amendment. Commits f73778f6, 23c42178, d1316499.
