...
- [ ] **Step 1: RED — tree + dist preflight.** Tests: running `ccrc install` from a fixture CHECKOUT tree (not `<home>/ccrc`) whose `server/dist/server/src/index.js` or `server/dist-pwa/index.html` is missing → dies naming the missing artifact with remedy "build first: bash install.sh (or npm run build in server/ and pwa/)". With both present → `<home>/ccrc/{server,agent,shared,deploy,ccd}` exists afterward, node_modules and `.git` and `*.env` and `ccrc-mail.token` excluded, dist and dist-pwa INCLUDED. Running from `<home>/ccrc/ccd` itself → no rsync invoked (self-copy guard; assert via the recording rsync stub) and no error.
- [ ] **Step 2: GREEN — `_inst_tree`.**
