...
- [ ] **Step 5: Idempotence, whole-verb.** Test: run the COMPLETE `ccrc install` twice on one fixture; second run: `accounts.json`/`ccrc.env` bytes identical, `accounts.sh` + every `_inst_atomic` target mtime-stable — EXCLUDING `build.json` by name (its `builtAt` measures THIS run; it legitimately rewrites), settings.json byte-identical, systemctl recording shows the same idempotent-safe calls (enable --now twice is fine), zero `.tmp.` strays anywhere under the fixture HOME.
...
- [x] **Step 3: single-definition sweep.** `install.sh` has a bash shebang → it joined the scanned corpus. Run `./node_modules/.bin/vitest run test/single-definition.test.ts`; it must be green (install.sh spells no pinned strings — the floor is read, not typed; the stamp path never appears).
- [x] **Step 4: End-to-end convergence proof (CI-runnable).** In `ccrc-install.test.ts`: the fresh-box→converged→re-run scenario of Task 8 Step 5, but entered through `bash install.sh` with stubbed npm — proving the bootstrap→verb handoff. Plus the README section (5-8 lines: clone, `bash install.sh`, what doctor green means, where the PWA answers — and that `rsync` is a hard by-name dependency of `ccrc install` with no doctor check yet) — and RETIRE the now-false README:607 bullet saying "`ccrc install` does not exist yet" (grep the whole README for other sentences the new verb makes false). The real-VM proof (spec: "Fresh VM → install.sh → doctor green → real session runs (RC off) → PWA on localhost, under 15 minutes") is the OPERATOR's stage gate and requires Stage 2e's RC-off work — record it as pending in the ledger, do not claim it.
- [x] **Step 5: Full suites + commit.** All three suites foreground green (`server`, `agent`, `pwa`). Commit: `feat(install): install.sh bootstraps a single box from a clone and hands off to ccrc install`
...
- **D-92** — Task 8 adds `BOX_UNIT_DIR="$HOME/.config/systemd/user"` to `ccd/ccrc`, beside `BOX_ENV_FILE`/`BOX_STAMP_FILE`/`BOX_TREE_DIR`, and the brief did not ask for it: as of `_inst_units` this file both WRITES that directory and (through `_inst_enable`) asks systemd to read it, which is the exact writer/reader pair `ccrc:91-101` exists to keep on one spelling. `ccrc-doctor-checks` keeps its own `CCRC_UNIT_DIR` with the same value and that is DELIBERATE, not an oversight left behind: the check table is sourced under `set -u` by things that are not `ccrc` (`ccrc-doctor.test.ts`'s `tableNames()` sources it standalone to read `CCRC_DOCTOR_CHECKS`), so a top-level `CCRC_UNIT_DIR="$BOX_UNIT_DIR"` would make sourcing it fail outright, and a `${BOX_UNIT_DIR:-…}` fallback is the second spelling with an extra branch in front of it. `_check_path` already records the same trade for `WRAPPER_BIN_DIR` and the same remedy — keep the mechanism instead of the promise — so the agreement is held by a test that compares the two literals (`ccrc-install.test.ts`, "names the same unit directory the doctor check table does") and goes red on a drift. Unifying them properly means moving both to a third file both can source, which is a change to `ccrc-doctor-checks`' load contract and not Task 8's to make.

- **D-93** — **FIXED in fix round 1** (`deploy/gen-wrappers.mjs`), after shipping as "measured, not fixed" with a diagnosis that was wrong twice. The symptom: every `ccrc install` printed `ORPHAN ccd: $HOME/.local/bin/ccd carries ccrc's marker and no account in the roster claims it`, whose remedy is `add an account "ccd" to ~/.ccrc/accounts.json … or remove $HOME/.local/bin/ccd by hand` — four lines above the same transcript's own closing advice, `next: add your first session with: ccd menu`. Not merely wrong advice about ccrc's own executable: **self-destructive advice about the binary the same transcript tells the operator to run next**, on the first verb a fresh user types. It touched no exit code and removed nothing, which kept it out of Critical.

  **The two corrections to this entry's first version.** (1) The marker on `ccd/ccd:2` is **not spurious**: `41bdf60` added it deliberately as provenance with a computed digest, and `server/test/ownership.test.ts:139-153` pins line 2's shape and its `ccrc-unmodified` verdict precisely so that ccrc's own shipped `ccd` can be replaced by the installer on a box. Dropping the marker is therefore not "the other fix" — it is **not a fix at all**, and would go red in ownership. (2) The claim that CLAUDE.md records the marker as predating its meaning is **false**; CLAUDE.md says no such thing. That wording comes from `docs/superpowers/plans/2026-08-15-stage2b-ccrc-cli-and-doctor.md:124`, which is itself the wrong claim propagated forward. Both errors are corrected here rather than left as history, because the first one names a fix that would break a test and the second one cites a file that does not say it.

  **What shipped:** `TOOLCHAIN_EXECUTABLES = new Set(['ccd', 'ccrc', 'ccd-cap-scopes'])` beside `ID_RE` in `deploy/gen-wrappers.mjs`, skipped in the orphan walk **after** `rosterIds` (so a roster that legitimately declares an account named `ccd` is settled by the roster, never by this list) and **before** the marker test (the marker is exactly what made these three reachable). It is the single source for these three names AS A SET — `_inst_bins` and `deploy.sh` each enumerate what they *install*, a different question with a different answer the day one of them installs a fourth thing. Guarded on both sides: `gen-wrappers.test.ts` asserts no `orphan\tccd|ccrc|ccd-cap-scopes` row for marked files at those three names **and** that a real `leftover` beside them is still reported (the exclusion must not widen into "nothing is an orphan"); `ccrc-install.test.ts` asserts a converged install transcript carries no `ORPHAN ` line and that the converger's own summary counts `0 orphaned` — the field the pre-existing assertion stopped one short of. Deleting the exclusion reddens both. The deeper alternative — orphan = marked **and** wrapper-shaped — is more principled and much heavier (shape parsing inside the manifest generator); not now.

- **D-94** — `_inst_enable` shipped without `systemctl --user restart ccrc.service`, which `deploy.sh:719-721` has between the enable and the verify; added in fix round 1. The brief spelled the chain without it, so this is a plan-level omission rather than an implementer error, and its consequence is on the SECOND run — the path `usage()`'s own promise ("re-running converges") points every operator at. `enable --now` on an already-active unit is a no-op, and `ccrc.service` is `ExecStart=… node %h/ccrc/server/dist/server/src/index.js`, so the running process is pinned to the `dist` it started with: `_inst_tree` rsyncs a new one, `_inst_stamp` writes the new sha, the transcript says every step converged and `ccrc version` reports the new build, **while the box goes on serving the old code**. Nothing catches it on a single box — `_check_fleet` SKIPs in local mode ("no second box to disagree with") and no check compares the running server's build to the stamp — and `verify-service.sh`, whose own header calls it *post-restart* verification, certifies the process the PREVIOUS install started, because with no restart MainPID is trivially stable. Restarting only `ccrc.service`: the timer holds no code in memory, and every `claude-session@` unit beside it is a long-lived `ccd supervise` this verb must never touch (CLAUDE.md); deploy's supervisor sweep is a fleet-host operation with its own KillMode preflight that a single-box installer does not inherit. Pinned by the recorded-argv order test (`daemon-reload` → both enables → `restart` → verify) and by a `restart` that refuses, whose die names the true state ("this box is still running the server the previous install started"). Restart-on-every-run stays idempotence-safe: the whole-verb test compares run 2's recorded argv to run 1's.

- **D-95** — `cmd_install` gained a second by-name preflight, `command -v systemctl`, beside the `node` one and before the first write (fix round 1). Without it a box with no systemd — macOS, WSL1, a bare container — reached step 10 of 14 having already written `~/.ccrc`, `~/.local/bin`, `~/ccrc` and the operator's `~/.tmux.conf`, and refused with "systemctl --user daemon-reload failed … read what it says: `systemctl --user status ccrc.service`" — a remedy naming the command the box does not have, and two conditions an operator acts on completely differently ("this box has no systemd" / "systemd refused these units") collapsed into one sentence. `ccrc-doctor-checks` already keeps that distinction as its own verdict, so this is the install-time half of a line doctor already draws. `loginctl` is deliberately NOT probed beside it: `_inst_linger` is built to survive its own failure, so an absent loginctl degrades to a printed remedy and a `FAIL linger` from doctor, while a box with no systemd cannot run a fleet at all.

- **D-96** — the closing line of `cmd_install` was `install: done — every step above converged`, unconditionally, which is literally false in the one case this task created an exception for: `_inst_linger` neither converges nor dies. The first round's test pinned the false sentence on the linger-refused fixture. Fixed in round 1 by making the claim a measurement: `_inst_linger` appends its name to a file-scope `INST_DEGRADED=()` (declared with the other arrays for `set -u`'s reason, cleared by `cmd_install` before the first step), and the tail prints `install: done — converged with 1 degraded step (linger)` when it is non-empty. The clean line survives unchanged on a clean run, and the two variants are now asserted on two fixtures. Linger is the only degrade-and-continue step today; the array is what makes a second one cost one line instead of a rewrite.

- **D-97** — Task 9's own Step 3 text (and the task brief it was handed as) claims "`install.sh` has a bash shebang → it joined the scanned [single-definition] corpus" — measured false rather than fixed. `server/test/single-definition.test.ts`'s `bashRoots` (`:683`) is `[ccd/, deploy/]` only; the recursive walker it feeds never visits the repository root, so `install.sh` is unreachable by any scan in that file regardless of its shebang — `grep -n install.sh server/test/single-definition.test.ts` names it nowhere. Step 3's outcome (55/55 green) is identical with or without `install.sh` existing at all — confirmed by running the suite both before and after Step 2 created it — which is the direct evidence the claimed auto-join never happened. **Correction (fix round 1, review Minor 1):** the first version of this entry went on to claim "this does not leave anything unguarded" on the strength of `install.sh` reading the floor via `require(...).engines.node` rather than retyping it — true of the code as first shipped, but at that point NOTHING measured that it stayed true: replacing the read with a hardcoded `floor=">=22.13.0"` left `install-sh.test.ts`, the `ccrc-install.test.ts` handoff test and `single-definition` all green, so the claim was aspirational, not guarded. Fixed in the same round by `install-sh.test.ts`'s "refuses against an absurd floor from the FIXTURE package.json, with a REAL node" — `fixtureRoot(home, '>=99.0.0')` plus an unstubbed real `node`, asserting the refusal names `99.0.0`, which can only pass if the floor came from *that fixture's* `server/package.json` and not from any copy, hardcoded or otherwise. Measured: hardcoding the floor reddens exactly this test. So the property IS guarded today — by `install-sh.test.ts`, not by `single-definition`, which remains the D-76 caveat this entry records: widening `bashRoots` to the repository root is still out of this task's scope, and nothing in this plan needs it to be.

- **D-98** — **FIXED in fix round 1** (review Important 1), a defect in the task brief's own pinned `install.sh` snippet rather than an implementer deviation. The snippet's argument handling was a single `-h`/`--help` check with no `else`: `bash install.sh --dry-run` (or any typo) fell straight through it, ran the full build, and `exec`ed `ccrc install` for real — exit 0, complete build, a genuine install performed. Measured directly. This is the exact defect `cmd_install`'s own comment names one layer down (`ccd/ccrc:1613-1618`, "an install that half-ran because argument 2 was a typo is worse than one that did not start") — and `install.sh` is now the OUTERMOST entry point a new operator types, one Stage 2d itself made more likely to be tried: the plan's own deferred list names `ccrc install --dry-run` as a real future flag, so a `--dry-run` typed against `install.sh` today is not a hypothetical. **What shipped:** the `-h`/`--help` `if` became the `while`/`case` loop `cmd_install` already uses (`ccd/ccrc:1619-1624`) — `-h|--help` prints usage and exits 0, anything else prints `install.sh: unknown argument: <it>` plus usage on stderr and exits 2, before `command -v node` or anything else runs. Pinned by `install-sh.test.ts`'s "refuses an argument it does not recognise, before anything runs" (exit 2, both stderr lines, `npm-argv` never created); mutation-measured — reverting to the brief's original `if`-only shape reddens exactly that test and none other.
...
- **The real-VM install proof** (spec: "Fresh VM → install.sh → doctor green → real session runs (RC off) → PWA on localhost, under 15 minutes") is the OPERATOR's stage gate, not a claim this plan's suites make. It is PENDING, not done: a real session run needs `CCRC_REMOTE_CONTROL` default-off, which is Stage 2e's — listed above — and does not exist yet, so "RC off" is not even possible today. Task 9's `install-sh.test.ts` and `ccrc-install.test.ts`'s "install.sh: the bootstrap that hands off to ccrc install" describe prove the MECHANICAL half hermetically (the floor refusal naming both versions, the build order, the handoff landing on the fixture's own `ccd/ccrc`) — CI-runnable, on no real VM. The human half of the gate — an actual fresh VM, a stopwatch, a real Claude Code session, RC verifiably off — has not been run and is not claimed here; it is blocked on Stage 2e.
