...
- **D-93** — MEASURED, NOT FIXED: every `ccrc install` now prints `ORPHAN ccd: $HOME/.local/bin/ccd carries ccrc's marker and no account in the roster claims it`, with the remedy `add an account "ccd" to ~/.ccrc/accounts.json … or remove $HOME/.local/bin/ccd by hand`. Both halves of that remedy are wrong advice about ccrc's own executable. The cause is pre-existing and is written down in CLAUDE.md already: `ccd/ccd:2` carries a `# ccrc:generated` marker "spuriously (it predates the marker meaning something)", so `deploy/gen-wrappers.mjs`'s orphan walk classifies the `ccd` that `_inst_bins` just installed as an account wrapper nobody claims. It has always been reachable through `ccrc wrappers`; Task 8 makes it appear on every install transcript, because `_inst_wrappers` runs that converger on every run. It is REPORTED, never removed, and never touches an exit code, so nothing is at risk — but it is noise with bad advice attached, printed by the one verb a new operator runs first. Not fixed here because both fixes are out of Task 8's scope and out of its allowed files: dropping the marker means editing `ccd/ccd` (banned for this task), and teaching the orphan walk that ccrc's own three executables (`ccd`, `ccd-cap-scopes`, `ccrc`) are never account wrappers is a change to `deploy/gen-wrappers.mjs`'s manifest contract. The second is the right fix and belongs in a stage-2c follow-up.
