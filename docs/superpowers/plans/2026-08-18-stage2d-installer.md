...
- **D-97** — Task 9's own Step 3 text (and the task brief it was handed as) claims "`install.sh` has a bash shebang → it joined the scanned [single-definition] corpus" — measured false rather than fixed. `server/test/single-definition.test.ts`'s `bashRoots` (`:683`) is `[ccd/, deploy/]` only; the recursive walker it feeds never visits the repository root, so `install.sh` is unreachable by any scan in that file regardless of its shebang — `grep -n install.sh server/test/single-definition.test.ts` names it nowhere. Step 3's outcome (55/55 green) is identical with or without `install.sh` existing at all — confirmed by running the suite both before and after Step 2 created it — which is the direct evidence the claimed auto-join never happened. This does not leave anything unguarded: `install.sh` reads the node floor via `require(...).engines.node` (never retypes it) and never spells the build-stamp path, so it has no second-copy risk for `single-definition` to police in the first place. It is the general caveat D-76 already carries ("single-definition sees only `.tsx?` + bash-corpus caveats — own blast radius, unchanged"), not a new gap, so it is recorded here as a correction to Task 9's own text rather than fixed: widening `bashRoots` to the repository root is out of this task's scope, and no test in this plan needs it to be.
