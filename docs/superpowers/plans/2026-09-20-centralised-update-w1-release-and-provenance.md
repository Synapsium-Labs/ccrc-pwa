...
- **D-3141** — Task 12 review (Important): the plan's mutation row 12 ("move the verifier call below `tar -xzf` → the failing-bundle case reds") measured GREEN — `_upd_resolve` arms `trap '… rm -rf "$UPD_STAGE"' EXIT` before any fetch, so after a refusal nothing on disk says whether extraction happened first, and a payload that would also fail a later check does not discriminate either (the mutation's minimal spelling lands the call between `tar -xzf` and the MANIFEST check, so the provenance die still wins). The property the task exists for — nothing extracted before provenance is settled — is pinned instead by the fixture's `node` shim, which runs at the moment of verification: it records whether the staging tree beside `--blob` exists at that moment (`staging-at-verify` = `no` under the correct order, `yes` under any placement after `mkdir "$UPD_TREE"`), and the OK case asserts `no`. Cost if wrong: a pin that reads a fixture-side file rather than the box — but the shim IS the verifier the tests run, so what it observes is what the verifier would have seen.
- **D-3142** — Task 12 review (Important, plan-mandated): the brief's `_upd_fetch` folded EVERY non-zero verifier exit into "provenance verification FAILED … --allow-unsigned does not apply to a bundle that FAILS", while `deploy/verify-provenance.mjs` speaks 1 (refused) and 2 (usage) and node itself adds module-not-found (1 with a stack, before the verifier's own code runs) and node-missing (127) — a box whose `server/node_modules` lacks the sigstore majors (a partial `npm ci`) was told the RELEASE failed, with the sentence closing the only escape hatch; an adapter narrowing a distinction it received, under a comment claiming "three answers, never folded". Exit 1 keeps the refusal sentence; any other exit gets its own — `could not RUN the installed verifier (<path> exited <rc>) — this is not a verdict on the bundle; nothing on this box was changed` — pinned by `fixture-verify-exit` = 2 and = 127. Cost if wrong: one more sentence for the operator to read; the refusal is unchanged.
- **D-3143** — Task 12 review (minor, plan-mandated): the brief's admitted-absence WARN said `sigstore.json is absent (404)`, but `curl -f`'s exit 22 means ANY HTTP status ≥ 400 — a 403 or a 500 from the release host was reported as a 404 and admitted by `--allow-unsigned` as absence; the sentence now says `absent (the release host answered an HTTP error — curl exit 22)`, which is what was measured, and the test's pin follows the sentence. Absence remains the one shape `--allow-unsigned` admits; a non-22 curl failure is still refused. Cost if wrong: none — the message no longer asserts a status code nothing read.
- **D-3144** — whole-branch review (Important I1): D-3134's top-level catch in `deploy/verify-provenance.mjs` wraps the `createRequire` block, so a missing `@sigstore/verify` — the exact "partial `npm ci` under server/node_modules" case D-3142's own text names — funnels into `refuse()` and exits **1**, which `_upd_fetch` reads as a real verdict and reports as "provenance verification FAILED … `--allow-unsigned` does not apply to a bundle that FAILS". A false statement about the release, with the one escape hatch named and denied, for a local dependency problem; measured by the reviewer against an empty `server/node_modules`. Reachable on this fleet rather than theoretical: `_inst_tree`'s own comment calls `npm ci` the verb's most likely failure, and `npm ci` DELETES `node_modules` before repopulating it, so a box whose install died there has the new tree, no deps, and blames the next release. The three `req(...)` calls now sit in their own try/catch exiting **3**, outside {0,1}, so `_upd_fetch`'s existing `elif [ "$vrc" -ne 0 ]` arm produces the "could not RUN the installed verifier" sentence D-3142 wrote for precisely this condition; the `fixture-verify-exit` mechanism already pins that arm. Cost if wrong: one more exit code in a vocabulary that already has three.
- **D-3145** — whole-branch review (Important I2): `deploy/verify-provenance.mjs` passed `tsaThreshold: 0` to `@sigstore/verify`'s `Verifier`, and in v3.1.1 the constructor reads `timestampThreshold: options.timestampThreshold ?? options.tsaThreshold ?? 1` — so `0` propagates and `verifyTimestamps` accepts an empty list. `verifyCertificate` then runs `verifyCertificateChain` inside `timestamps.forEach(...)`, i.e. ZERO times, leaving the chain-to-a-trusted-CA check and the cert-validity-window check unrun; the only thing refusing such an entity is an incidental `TypeError` on `path[0]` inside `verifySCTs`. NOT currently exploitable — `tlogThreshold: 1` independently requires a Rekor inclusion proof the attacker cannot forge, and the TypeError fires first — but an accidental guard standing in for two real ones is exactly what this tree bans. The option is dropped; the library's own default is the intent ("do not require a TSA timestamp"). Cost if wrong: none measured — the fixtures verify identically either way, which is why nothing caught it.
- **D-3146** — whole-branch review (Important I3): Task 11's `_upd_floor_check` separates absent, unreadable and malformed with an explicit `-r` test and three sentences (D-3136), but Task 10's `_inst_installed` guards its floor read with `-f` alone, so a present-but-UNREADABLE floor yields `cur=""`, falls through every branch, and is silently REPLACED by the `mv -f` — which needs write permission on `~/.ccrc`, not read permission on the file — possibly with a LOWER version. D-3136's own sentence ("absent and malformed are two words with two remedies") names the principle; unreadable was the third word, collapsing into the first. Not attacker-reachable (anyone who can chmod the file can delete it) and the update path is shielded by `_upd_floor_check`, but a direct `ccrc install` or `install.sh --release` reaches it. `_inst_installed` now carries the same `-r` arm and leaves an unreadable floor untouched. Cost if wrong: an install prints one more line and declines to raise a floor it cannot read.
- **D-3147** — whole-branch review (Important I4): `~/.ccrc/ccrc-caps` is written only by `_inst_caps` and removed only by uninstall, while `cmd_update` clears `~/.ccrc/installed` before running the staged spine but NOT the caps file. So the documented rollback — `ccrc update --to <pre-W1 tag> --downgrade --allow-unsigned` — runs a pre-W1 spine that writes a ONE-LINE marker and never touches caps, leaving a one-line marker BESIDE a `verify` cap, which spec §6 reads as `provenance: verified`: the precise false claim D-3117 exists to prevent, surviving in the rollback direction. Nothing consumes it until W2 ships the reader, which is exactly why it is cheap now and awkward later. `cmd_update` now clears the caps file beside the marker; an old spine leaves caps absent, which §6 already reads as `unknown` — honest. Cost if wrong: a box that rolled back reports `unknown` capabilities until its next W1-or-later install rewrites them.
- **D-3148** — whole-branch review (Important I5, PRE-EXISTING — introduced in stage 4, not by this branch): `_upd_resolve`'s guard on the tarball name `SHA256SUMS` carries is a glob, `case "$UPD_TARNAME" in ccrc-*.tar.gz)`, which `ccrc-x/../../../../some/dir/thing.tar.gz` satisfies; `curl -fsSL -o "$UPD_STAGE/$UPD_TARNAME"` then creates or clobbers that path as the box user BEFORE the transport checksum, before provenance, and outside the `EXIT` trap's `rm -rf "$UPD_STAGE"`. The `.tar.gz` suffix bounds the damage to `*.tar.gz` (and `*.tar.gz.sigstore.json`) files and no install proceeds — the provenance gate still refuses — but an attacker controlling the release assets can destroy arbitrary tarballs with no cleanup. A separator arm now refuses any name containing `/` before the shape check. Fixed here rather than deferred because it lives in the function this wave exists to harden and the fix is one arm; recorded as a scope expansion so it can be reverted alone. Cost if wrong: a release whose SHA256SUMS legitimately names a path is refused — no such release exists, `build-release.sh` writes a bare basename.
- **D-3149** — whole-branch review (Minor M5): README (from this plan's own Task 14 text) states the update spine's order as "extract, bind the extracted `build.json` to the resolved version, check the per-file `MANIFEST`", following spec §5's sentence, while `_upd_fetch` checks the MANIFEST first and binds after. Both precede any install write, so nothing is weaker either way, but the document and the code disagreed and the departure was undeclared. README is corrected to the code's order rather than the code to the spec's: the MANIFEST check is the cheaper refusal and belongs first, and re-ordering shipped, reviewed code to match a sentence would be the tail wagging the dog. Spec §5's sentence is the one that is now stale; W2 re-states it. Cost if wrong: one sentence rewritten again when the spec is next touched.
