...
Plan-level departures from the spec's literal text, found while planning against the measured tree. The ten numbers below were issued by the allocator on 2026-09-20 (`ccrc-api ledger allocate`, one block, floor 3115 before, 3125 after) and are defined here in the same act; `D-TBD-no-tag-fixture` is a named contingency a worker mints only if Task 7 Step 5 is declined.
...
| `gh attestation verify --bundle` (no custom root) | same command, `--custom-trusted-root` omitted | 0 | 9 (DNS to the local resolver, then real TLS connects to a public sigstore-TUF-mirror endpoint and to GitHub's Pages CDN over both IPv4 and IPv6 — full addresses in the untracked scratch strace logs and Task 1's report, deliberately not repeated here per `topology-clean`'s public-IPv4 class — fetches the TUF root live) | verifies fine, but NOT offline |
...
**Deviation — bundle carries two attestation entries, not one (D-TBD-bundle-dual-attestation):** `gh attestation download` for `cli/cli`'s release tarball returned a bundle with two signed entries (a centralized GitHub-release-signer entry at index 0, and the repo's own reusable-workflow entry at index 1), where the brief's SAN-extraction script reads only index 0. Index 0's SAN does not match the brief's stated "Expected" shape (`https://github.com/cli/cli/.github/workflows/<file>.yml@refs/heads/<branch>`); index 1's does. The spike used index 1 throughout (SAN extraction, both backends' `--cert-identity`). Not yet allocated through `POST /api/ledger/deviations` (session had no box-token path in scope for this task) — reported as a concern in the Task 1 report for the controller to mint/reconcile.
