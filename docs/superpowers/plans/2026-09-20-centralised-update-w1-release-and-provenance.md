...
Plan-level departures from the spec's literal text, found while planning against the measured tree. Eleven numbers are defined below, issued in two allocator acts on 2026-09-20: `D-3115` through `D-3124` at planning time (`ccrc-api ledger allocate`, one block, floor 3115 before, 3125 after), and `D-3125` afterward — once Task 1's spike surfaced its own deviation — allocated by the controller through the ledger allocator; each is defined here in the same act as its allocation. `D-TBD-no-tag-fixture` is a named contingency a worker mints only if Task 7 Step 5 is declined.
