...
|---|---|---|---|---|
| `sigstore` (`@sigstore/verify` + vendored root) | *filled by Task 1* | | | |
| `gh attestation verify --bundle --custom-trusted-root` | *filled by Task 1* | | | |
| `gh attestation verify --bundle` (no custom root) | *filled by Task 1* | | | |

**Default backend chosen:** *filled by Task 1* — `sigstore` if its row shows exit 0 and zero non-loopback connects; otherwise `gh` (and D-3120's cost is live).
**Artifact used:** *filled by Task 1* (owner/repo, asset, sha256, the SAN the bundle's certificate carries).
**Import names measured:** *filled by Task 1* (`@sigstore/verify` exports used by Task 8's code; a name that differs from Task 8's draft is corrected there and recorded as a deviation).
