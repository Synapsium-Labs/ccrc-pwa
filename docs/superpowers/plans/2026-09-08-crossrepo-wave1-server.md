...
1. **`GET /api/feed` is EXEMPT-BUT-AUTHENTICATED after acceptance correction D-2507.** The PWA reads it with a live session, while `ccrc-api feed list` is cookieless and already sends the box token. The global session hook therefore exempts the matched route, and the handler requires one of those two credentials before revealing even `501 not-configured`. No route was added or removed, but the auth census and its derived counts moved with this reclassification.
...
Twenty-three. **Eight were found while PLANNING** against `origin/main` `d0064e6e` — every one is a place where the spec's or the cross-wave contract's own words could not be followed exactly against the tree as it stands. **No number is minted here for those eight.** Each carries a `D-TBD-<kebab-slug>`; the orchestrator mints a contiguous block and defines the numbers into these entries in one act. `server/test/dtbd.test.ts` is RED until it does, which is the mechanism, not a problem to work around. **The ninth, D-2338, was found while EXECUTING** — Task 6, measured against the tree rather than read off the plan — so it was minted on its own, by that task's own session, in the same act that defined it, and it sits last rather than being folded into a planning block it does not belong to. **The tenth, D-2342, was found while EXECUTING** — Task 2, ruled by the worker session on the review's measurement and minted by the coordinator on a deviation-request, the protocol the brief prescribes. **The eleventh to fifteenth, D-2349–D-2353, were found in the coordinator's REVIEW ROUND 1 on PR #75** — five ruled fixes whose shipped form departs from this plan's own text, minted by the coordinator in one contiguous allocation and defined here in the same act. **The sixteenth, D-2410, was forced by the final rebase** — current main had already landed the ask pre-emption lane as migration 9, so the coordinator minted one number for moving this wave's unchanged additive entry to migration 10. **The seventeenth to twentieth, D-2505–D-2508, were allocated by the coordinator for the acceptance review of merged PR #75** — four corrections measured against `origin/main` `ec11030d`, defined below without minting or renumbering anything in this worker session. **The twenty-first, D-2518, was allocated by the coordinator after that correction round exposed a second acceptance gap** — grammar-valid slugs remained unbounded, so the PWA could create a coordinator before deterministic kickoff or hold overflow made it unusable. **The twenty-second, D-2545, was allocated by the coordinator during the acceptance review of PR #86** — a persisted-integer read that crashes instead of refusing, ruled out of this wave because no shipped build can write the row and the same coercion is store-wide. **The twenty-third, D-2546, was allocated in that same review** — the operator hold route is the fourth `.hold` producer and the only one still unbounded, ruled to wave 2 because its reason is free-form operator text whose budget is the registry field's question, not the programme grammar's.
...
- **D-2338** — Task 6's brief (Step 4) put the worker-arm predecessor park inline in `requeueAbandonedMail`'s own body, as `this.db.prepare('UPDATE mail_deliveries …')`. Shipped instead as a separate private method, `parkSupersededDeliveries(displaced, mailIds)`, called from that same `if (role.role === 'worker' && rows.length > 0)` branch. Measured necessary rather than stylistic: `requeueAbandonedMail`'s signature carries a DECLARED multi-line exemption from `server/test/mail-hardening.test.ts`'s delivery-row-writer census — the census walks back from each prepared `UPDATE`/`INSERT` to the nearest single-line method signature, and a wrapped one mis-attributes — granted on the stated premise that this method reaches delivery rows only through `queueDelivery` and `setDeliveryEnvelope`. An `UPDATE mail_deliveries` inlined there breaks that premise, and the exemption comment's own trip-wire is what says so. MEASURED on this branch at `9791b644`, by writing the brief's text literally and running the suite: three cases of "every delivery-row writer names a shared terminality guard (wave 8)" go red with `AssertionError: the walk-back from store.ts:1170 crossed a method close — its signature is not single-line: expected true to be false`, the walk having skipped past the multi-line signature and attributed the write to a method several definitions earlier in the file. Task 6's own file list does not touch that describe block, yet its Step 8 requires that suite green — so the brief's two instructions are jointly unsatisfiable against this tree. Resolved on the codebase's own convention (the single-line-signature rule for delivery-row writers) rather than by weakening the census: extract the `UPDATE` into its own private, single-line-signature method. The SQL is byte-identical to the brief's inlined version; only its container changed. Consequences that ship with it, each already demanded by its own pin: `CLAUDE.md`'s void-writer sentence gains `parkSupersededDeliveries` (`mail-hardening.test.ts`), and `requeueAbandonedMail`'s reader-walk docstring gains a holder bullet for it (`single-definition.test.ts`). Distinct from D-2059, which argues the park's role-generalised SQL and says nothing about where the statement lives. Ledger note, recorded rather than hidden: this number was minted by a call whose title field reads "probe - do not use" — the mint was real and permanent, only the title is wrong, and THIS entry is the number's definition; it is spent, not free, and must never be re-issued. Spelled as a `- **D-N** —` BULLET, unlike the eight above, and deliberately: `server/test/deviation-refs.test.ts`'s floor-seed scan recognises only the heading and bullet forms, so a tracked `D-2338` ref with no entry it can SEE reds that pin — which is the mechanism that caught this entry's absence (measured: without this line, `expected 2388 to be 2120`, evidence `CLAUDE.md names D-2338`). Re-spelling it to match its neighbours re-opens that red; the eight above are invisible to the same scan and stay that way only because their numbers sit below a high-water two sibling plans already carry.

- **D-2342** — spec §3 F1 and Task 2 say the dispatch rung compares `record.project !== run.project` and carries `by: record.project`. Measured: `SessionRecord.project` is `project ?? id` (`server/src/registry.ts:682`) over `field()` (`:333`), which reads through `io.readFile` and collapses absent and unreadable to one null — so a row whose `.project` is missing (rows from before 2026-07-28 carry none, `ccd/ccd`'s `_project_pool_state`) or transiently unreadable reaches the guard as the SESSION ID, compares unequal to every run's project, and is refused `project-mismatch` with `by: '<session id>'` — a refusal wave-lifecycle.md §2 marks "do not retry" and answers by opening a second workspace. That is the "refusing on a fact not measured" the same spec paragraph forbids, and `measuredIdentity` cannot catch it (`unmeasured` carries uuid/wrapper/workdir only). Done instead: the rung reads `<id>.project` through `fieldMeasured` (exported from `registry.ts` for this one caller) at the decision point — unreadable → `registry-unmeasurable`, whose own skill row says stop and report rather than retry blind because the same wire shape can also follow a killed `ws-add`; absent or empty → permits, because a proven ENOENT is not a crossing and the row's repository is unknown to the registry, so the dispatch proceeds exactly as it did before this wave; measured → compared, and `by` names what was READ. `record.project` keeps its `?? id` default for every display consumer. Pinned by three run-routes cases (unreadable → 502 with no hold placed; absent → 200 dispatched; whitespace-only → 200) measured red when the unreadable arm is deleted, when the compare is reverted to `record.project`, and when the empty-content clause is removed. Cost: one extra measured read per resume dispatch (`.project` is now read twice — buildRecord and the rung), and a guard whose mechanism differs from the spec's literal sentence while keeping its principle. The fuller remedy — `project` on the measured ladder inside `readRegistryMeasured`, `unmeasured` carrying it — changes every consumer of `SessionRecord.project` (fleet.ts, watch.ts, lifecycle.ts, divergence.ts, routes.ts, store.ts) and is left for its own task under its own ruling.

- **D-2349** — Task 4's open route guards `homeProject` as `typeof === 'string' && homeProject.trim() !== ''` and then passes the RAW value to `homeProjectVerdict`, `openRun` and the response (the route comment this plan prescribes: "Folding them would write a whitespace home onto a programme row forever"), and its `required` verdict answers the same bare `400 {error:'bad-request'}` the body-shape guard sends. Measured in review round 1 (F2, F4, F3): `'demo\n'` passes the trim CHECK and is stored raw — permanently, since `setProgramHome` is `WHERE homeProject IS NULL` and `openRun`'s `ON CONFLICT` arm rewrites `title` only — so every later open sending the clean spelling is refused `home-mismatch` against a `by` that renders the same; `'../x'` yields a `ledgerAbsPath` outside `projectsRoot`, the very path the skill tells a wave in another repository to read; and the `required` arm, dormant while `HOME_PROJECT_LEGACY_ACCEPTED` is true, would go live at wave 3's flip as an unattributable 400. Done instead: ONE shaping at the door — `shapeHomeProject(raw)` (pure, exported beside `homeProjectVerdict`, driven by `home-project.test.ts`) trims and refuses `''`, `.`, `..` and any `/` with `400 {error:'bad-request', detail}`; the shaped value feeds the verdict, `openRun`, `setProgramHome` (through the verdict's own `home`) and the response; the `required` arm answers `detail:'homeProject is required'`, pinned by a source scan because the constant cannot be flipped from a test (D-2056). Pinned by run-routes cases red when the trim or segment check is removed. Cost: a `/`-bearing home this plan would have accepted is now a 400; the `program` slug reaches the same `path.join` unshaped and is left alone — an existing field's contract, its own task; a home a pre-fix build already stored untrimmed cannot be repaired by any route (none is live: this PR is unmerged).

- **D-2350** — Task 4's quoted open route (inherited from main) calls `coord.setSession(opened.id, sessionId)` BEFORE `CCD_ARGV.wsHold`, "persist the id onto the row" first, the hold second. Measured in review round 1 (store-1, by probe through the real route): a retried open naming a DIFFERENT `sessionId` reaches `openRun`'s idempotent dup arm — keyed on `(program, wave, waveOf, claimedBy, state='planned')`, not on `sessionId` — and `setSession` → `bindSession` re-binds the run, re-issues the predecessor's outstanding `to:'worker'` mail to the heir and parks the predecessor's rows, all in autocommit; a then-failing `ws-hold` answers 502 while every one of those writes stands, so the coordinator is told the open FAILED with the occupant already moved and a queued delivery waiting for a session nobody held. Done instead: the hold first, and the bind only once it holds; a refused hold (501/502) leaves `runs.sessionId`, the predecessor's delivery and the heir's queue exactly as they were. Pinned by a run-routes case whose runner fails the SECOND `ws-hold` — red when the order is reverted. Cost: `dispatch.ts`'s fresh-spawn arm keeps the OPPOSITE order (bind before hold, "persist the spawn right away") for its own reason — a workspace `ws-add` just minted must be findable on the row even if its hold then fails — so the two sites now argue their orders separately rather than sharing one.

- **D-2351** — Task 6 (`setSession`, the plan's own body comment) keeps `setSession(runId, sessionId): void` and says "its two callers act on nothing — the open route and the fresh-spawn arm both bind a run that names no session yet, where `bindSession`'s answer is always `{rebound:false, reissued:0}`". Measured in review round 1 (store-2, MUT-3): false of the open route by the same dup-arm path D-2350 describes — a live re-bind that the wave's own final review had already measured into `bindSession`'s docstring, so the file carried two contradictory claims about one call site — and the `void` dropped the distinction `bindSession` had just measured: an adapter narrowing what it received, and an occupant change no `run_events` row recorded. Done instead: `setSession` RETURNS `bindSession`'s `{rebound, reissued}` (the shape itself unchanged — a cross-wave name); the open route reads the predecessor (`resolveWorker(opened.id)`, which IS `runs.sessionId`) before binding and, when `rebound`, records `run_events.detail` `session-rebound: <predecessor> -> <heir>, <n> re-issued` — a kebab literal `mail-routes.test.ts`'s scan sees and its `NOT_CODES` declares on the `home-project-backfilled` model. Pinned by a run-routes case that opens the same planned wave twice with different sessions and asserts the heir's brief, the predecessor's park and the event — red when the event is deleted. Cost: one more run-event kind that only the run's own trail carries; the fresh-spawn arm ignores an answer that is always `{rebound:false,reissued:0}` there.

- **D-2352** — Task 4 states "`homeProject` is reconstructible and therefore does NOT join `UNRECOVERABLE`: `reconstruct` returns `this.run(id)!`, whose join reads the column the same INSERT left NULL, and NULL is the honest answer for a programme rebuilt from a ledger that never carried a home". Measured in review round 1 (store-3, by probe): after `reconstruct`, `programHome` is NULL for every rebuilt programme and the NEXT open BACKFILLS whatever it is told (`setProgramHome` is `WHERE homeProject IS NULL`; `homeProjectVerdict({known:true, stored:null})` is `backfill`, never `mismatch`) — so the fact spec §3 F2 says a programme "carries forever" survives exactly as long as the database does, `home-mismatch` cannot fire after a DB loss until a coordinator re-homes the programme by accident or design, and the drill whose title is "names exactly what CANNOT be reconstructed" was silent about it even though its `RUN_SUMMARY_KEYS` trip-wire fired on the same task. NULL is honest as a VALUE; it is not "reconstructible" as the census means it. Done instead: `'homeProject'` joins `reconstruction-drill.test.ts`'s `UNRECOVERABLE` (count 17 after the ask lane added its own unrecoverable row) with the reason, and `reconstruct`'s docstring says the ledger, the registry and `.prhistory` name no home. Teaching `reconstruct` to carry the home needs an artifact that proves it — wave 2/3, by the coordinator's ruling, not here.

- **D-2353** — Task 2 Step 6 prescribes `wave-lifecycle.md` §2's `project-mismatch` row verbatim, remedy included: "Nothing was spawned, no hold was placed … Open the wave again WITHOUT `sessionId` so it spawns fresh in the target repo". Measured in review round 1 (F1, by probe; corpus-1): the OPEN that named the crossing session had already placed a hold on that wrong-repo workspace and it stands after the dispatch refusal; re-opening the same wave without `sessionId` hits `openRun`'s dup arm and returns the SAME planned run, still bound, so the next dispatch refuses identically — a loop only an operator abandon (which releases the hold) leaves. The row as shipped by Task 2 had also grown a parenthetical the plan never had — "answers `registry-unmeasurable` instead (retry once, as for any unmeasurable registry)" — contradicting the same file's own rule for that code (`wave-lifecycle.md`: never a blind retry, because the identical wire shape can land after `ccd ws-add` already spawned a workspace). Done instead (prose only, by ruling): the row names the standing hold, says stop and report so the operator abandons the wedged run from the console, and only after the operator reports it abandoned says to open without `sessionId`; it names no operator route because the armed auth sweep and the skill corpus prohibit inviting a coordinator through that door. It also sends `registry-unmeasurable` to its own row — stop and report — because the response cannot tell the safe rung from the post-`ws-add` one. The mechanism that would let a session-less retry clear the binding and release the hold is the coordinator's wave-2 item. Cost: the operator spends one abandon and the programme spends a second workspace on the escape this row offers; D-2061's two-slots sentence stays in the row.

- **D-2410** — Task 3 and Task 9's rollback paragraph prescribe this wave's one additive entry as migration 9 (`user_version 8 -> 9`) and say `COORD_SCHEMA_VERSION` derives to 9. Measured at final rebase: current main had already landed the ask pre-emption lane as migration 9, and an applied entry is frozen — placing these columns into it would never migrate a database on which the ask lane had already run. Done instead: main's ask migration stays byte-for-byte at 9; `programs.homeProject` and `feed_events.runId` move together, unchanged, into migration 10; `COORD_SCHEMA_VERSION` derives to 10; and `coord-db.test.ts` constructs a real version-9 file with the ask table before proving both new columns arrive nullable and defaultless. The plan's earlier step-by-step migration-9 text remains its anchored snapshot; shipped source, comments and test names say 10. Rollback keeps the same contract at the new number: an older build meeting `user_version 10` refuses to migrate, never to read. Cost: one higher schema ordinal and one extra migration application after the ask lane; no wire or downstream wave contract changes.

- **D-2505** — The accepted D-2350/D-2351 form placed `ws-hold` before `setSession`, but left the successful post-hold bind, worker-mail reissue, predecessor park and `session-rebound` event as separate autocommit writes. Measured with a SQLite trigger that aborts the late event insert: the request answered 500 after the run had moved to the heir and its mail had moved with it, so a failed open still committed most of an occupant transfer. Done instead: keep the external `ws-hold` outside SQLite, then put predecessor lookup, `setSession`/transaction-free `bindSession`, reissue/park and event insert in one route-owned `tx(coord.db, ...)`. `bindSession` deliberately gains no internal transaction because dispatch already calls it inside `dispatchRun`'s transaction and `DatabaseSync` transactions do not nest. The fault test proves all database writes roll back; after the trigger is dropped, retry proves exactly one transfer and exactly one rebound event. Cost: the external hold can stand if a later SQLite write fails, because fleet state and SQLite cannot share a transaction; the database remains internally atomic and a retry completes the idempotent row.

- **D-2506** — The plan treated programme-home backfill and `home-project-backfilled` as a recorded logical fact, but the accepted route wrote the home first and the audit event second in autocommit. Measured with a trigger that aborts that event insert: the request answered 500 while `programs.homeProject` stayed permanently populated and no event dated the backfill. Done instead: wrap `setProgramHome` and `recordRunEvent` in one synchronous `tx(coord.db, ...)`. The trigger fault proves the home remains NULL, then a retry reuses the existing planned run and writes one home plus one event. Cost: none on the wire; the transaction is route-owned because both store methods also serve callers with different transaction scopes.

- **D-2507** — The plan explicitly left `GET /api/feed` outside box-token auth while adding `ccrc-api feed list`, but that client is a cookieless fleet-host caller and already sends `x-ccrc-mail-token`; on an armed server the global session hook therefore refused the machine request before the handler ran. Done instead: add the matched feed route to `EXEMPT` as EXEMPT-BUT-AUTHENTICATED and make its handler accept either a live session or a valid box token, authenticating before it reveals `501 not-configured`. A real client/armed-Fastify fixture now proves the shipped client credential reaches the handler, and then substitutes a wrong fixture token to prove the real 401 body remains protocol output with exit status zero. The mechanically derived census moves from twenty-one to twenty-two coordination token lanes, twenty-two to twenty-three with notify, six to seven dual-credential GETs, twenty-eight to twenty-nine `EXEMPT` entries, and twenty-seven to twenty-eight scanned exemptions; feed joining `EXEMPT` lowered the gated non-exempt HTTP routes by one at the time. **Re-measured on the merged tree (2026-09-11) that count is forty-four, not forty-three** — account-pools' `POST /api/projects/:project/pool` is not exempt and restored the one feed removed — from seventy-five scanned registrations less three websockets less twenty-eight scanned exemptions (twenty-nine `EXEMPT` entries, of which `GET /*` names no scanned registration). `auth-gate.test.ts` derives that number from the sets rather than asserting a literal, so it is measured there and only restated here. Cost: feed's handler now owns the same dual-credential decision as runs, lifecycle, peers, claims and asks rather than relying solely on the global session hook.

- **D-2508** — `program` at `POST /api/runs` and `slug` at `POST /api/sessions/:id/kickoff` both flowed into `ledgerPath(slug)` as raw HTTP text, so separators and dot components could name a path outside `docs/superpowers/programs`, while whitespace produced different stored, looked-up and returned spellings. Done instead: one shared L0 boundary shape trims and accepts only `^[A-Za-z0-9_-]+$`; each HTTP ingress shapes once and passes only the shaped slug into lookup, `openRun`, programme storage, home lookup, `ledgerPath`, absolute ledger construction, kickoff composition and the response. The PWA start sheet uses that same shape before `createSession`, so a value kickoff would later refuse cannot first leave an unbriefed coordinator running. Traversal tables cover slash and backslash separators, dot components, dotted names, spaces and query punctuation; positive tests prove normalization and, for run-open, use `path.relative` to prove the absolute ledger stays beneath `<projectsRoot>/<home>`. Cost: programme display labels containing punctuation or spaces are now refused at these ingress routes; existing name-like slugs, including uppercase letters, underscores and hyphens, remain accepted. `ledgerPath` itself stays a pure interpolator for non-HTTP callers, so every future HTTP ingress must use the shared shape before calling it.

- **D-2518** — D-2508 shaped the programme slug's grammar but left every accepted string unbounded. Measured: a 113-character safe slug plus a tiny title passed the PWA kickoff check, created a coordinator, then made `POST /api/runs` deterministically answer `hold-oversize`; a successfully held legacy programme can likewise disappear when its complete `program:<slug> wave:<n>/<of> run:<id>` hold exceeds the session hook's 127-character display window. Done instead: every numeric hold field is validated at every JavaScript seam against one explicit domain — positive safe integers, maximum `9007199254740991`, 16 decimal characters — while the BUDGET reserves each slot at its own widest decimal rather than at that one width. Wave and denominator are JavaScript numbers end to end and reserve 16 digits each; the run id is an int64 SQLite column, so its slot reserves SQLite's full `9223372036854775807`, 19 digits, carried as decimal TEXT because that value is not representable as a JavaScript number (`Number(...)` silently yields 9223372036854775808, so coercing it would derive the budget from a width no row can hold). Rendering the reserved shape through the real serializer gives `program: wave:9007199254740991/9007199254740991 run:9223372036854775807`, 71 characters, so `PROGRAM_SLUG_MAX_CHARS` is `127 - 71 = 56`; shared shaping enforces that conservative ASCII cap before the PWA can call `createSession`. Budgeting the id slot against the COLUMN rather than against the JavaScript domain is what makes the guarantee unconditional: a slug the cap admits cannot compose an over-long hold for any id `runs.id` can assign, not merely for the ids this process agrees to serialize. One browser/server-compatible `programKickoffVerdict` shapes the slug, trims the title, validates resume numbers, composes the actual wave-1 or resume body, and measures UTF-8 bytes. The PWA applies it before `createSession`, the kickoff route applies it once to raw input before registry measurement, and `queueProgramKickoff` independently enforces it at the mail-writing seam. Exact tests hold the slug at the shared budget and derive titles from the real composers to pin 8,192 bytes accepted and the next character refused BY BYTES — the over-cap fixture appends one two-byte character, so the composed body is 8,194 bytes while its length is 8,193, and the reported `bytes` is where a cap measuring `String.length` would be caught — across both the wave-1 and resume templates, together with resume-template divergence and multibyte overflow. Holds use one full-string grammar-and-length `holdReasonVerdict` at every boundary that can write one: run-open, dispatch, abandon/survivor handoff, and ordinary close/next-wave or survivor handoff. `hold-invalid` truthfully distinguishes hook grammar or numeric-domain failure from `hold-oversize`, and the verdict checks grammar and domain BEFORE length so that a row which is both malformed and over-cap reports the code whose remedy can actually repair it — `hold-oversize` therefore only ever describes a hold that is otherwise entirely valid. Dotted reconstructed programme names remain compatible with the hook even though new HTTP ingress is narrower; the hook stays deliberately WIDER than this server's output (it renders `wave:0` and unsafe widths that every boundary here refuses) because it is a display gate over strings already on disk, and the pinned property is the subset direction — everything the server writes renders — never equality. `CoordStore.openRun` checks a duplicate row's exact id read as decimal text and keeps a fresh SQLite `lastInsertRowid` as bigint until safe-range validation; a private typed sentinel rolls both fresh inserts back on refusal. Boundary tests prove the budget-sized slug plus the reserved widths serializes to exactly 127 characters while one further slug character overflows it; that the widest hold the runtime can actually serialize is 124, inside the cap by exactly the three digits the wider reservation bought; and that a close whose successor wave leaves the safe-integer domain refuses with the overflow as its sole cause. Zero, negative, fractional, unsafe and exponent-scale values are pinned independently for wave, denominator, run id and both resume fields. Cost: new programme slugs above 56 characters are refused even when a particular small-number hold or kickoff body would fit, and the reservation is three characters more conservative than the JavaScript domain alone would require; this reserves enough room for every future supported wave, denominator and exact run id without creating an intrinsically unusable coordinator. Existing in-cap names and wire protocol remain unchanged.

- **D-2545** — `CoordStore.openRunsForSession` (`server/src/coord/store.ts:1851`) selects `id`, `wave` and `waveOf` straight into JavaScript numbers, and `node:sqlite` throws `RangeError` while converting an INTEGER wider than `Number.MAX_SAFE_INTEGER` — so a persisted out-of-domain row crashes the READ before `close.ts`'s `holdReasonVerdict` can answer `hold-invalid`, and the same untyped throw reaches `POST /api/sessions/:id/archive`, `sweepMerged`, the rename sweep's twelfth condition and the coordinated-ask hold. `openRun`'s duplicate-row arm already reads `CAST(id AS TEXT)` for exactly this reason, so the store protects one persisted-read path and not its sibling. MEASURED, and the reason it is NOT fixed in this wave: no shipped build can write such a row — `openRun` is the only reachable `INSERT INTO runs` (the route validates `wave`/`waveOf` as positive safe integers and the insert rolls back a generated id that is not one), and `reconstruct` has no production caller — so reaching this state needs a NEWER build's database read back after a rollback, the same latent class `schema.ts:41-45` argues for the delivery-state vocabulary. The identical coercion is store-wide (`run`, `runs`, `hydrateRun`, ask hydration), so repairing this one seam would correct the instance and leave the claim. RULED by the coordinator: wave 2 hardens the persisted-integer read surface in ONE pass — an all-or-failure typed result per read (`setReadBigInts(true)`, prove `1n <= v <= BigInt(Number.MAX_SAFE_INTEGER)` before `Number`, never return partial rows and never carry the offending value out), fail-shut at every consumer (close refuses `hold-invalid` before any fleet act; the archive route answers `409 run-open` with no row detail; the rename sweep treats it as CLAIMED; the merged notice falls back to a generic open-run reason; the ask hold aborts into its existing immediate-operator-push fallback, minting no ask row and no parent mail) and no bigint or rounded number on any wire, plus a defensive positive-safe guard on `insertAsk`'s `runId`. Cost, named: until wave 2 lands, an unrepresentable row answers an untyped 500 instead of refusing in words.

- **D-2546** — `POST /api/sessions/:id/hold` (`server/src/server.ts`) is the FOURTH producer of a `.hold` reason and the only one D-2518 left unbounded: it refuses an empty or whitespace reason and then passes the string straight to `CCD_ARGV.wsHold` with no length and no grammar check, so an operator hold of arbitrary width lands in the registry and in every surface that displays `held`. Deliberately NOT fixed here, and not by copying `holdReasonVerdict`: this route's reason is FREE-FORM operator text, which the session hook is already expected not to render as a programme card, so bounding it is a decision about the registry field's own budget rather than about the programme grammar — and that decision is wave 2's. Recorded because it is the same class D-2518 closed at three seams, and the fourth should not have to be found again from scratch. RULED by the coordinator during the acceptance review of PR #86.
