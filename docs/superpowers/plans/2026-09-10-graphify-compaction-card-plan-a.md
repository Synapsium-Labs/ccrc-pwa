...
- Modify: `ccd/ccd` — row-generation initialization, `_spawn_start` primary/retry environments and FD
  ...
  the new non-fatal `_lc_refused <act> <id> <tx> <token> <detail> [k v]…` emitter** that emits the `refused`
  ...
  and `_ws_slug_free` (`:3940-3951`; round 8, I4: widened to an
...
- **Comment scope in `ccd/ccd` and `ccd/session-hook.sh`, each a shipped sentence this task falsifies
  ...
  **And WIDEN the audit itself (round 13, A-I1/B-I4): it covers EVERY tracked-file reference in either
  ...
  alike — assert the token quoted beside it occurs within exactly those lines of the shipped file. The
  `session-hook.sh`-only scoping was measurably insufficient: the merge that brought Task 8 fix round 4's
  `hasFullPathOccurrence`/`hasUniqueSuffixOccurrence` split shifted `ccd/compact-card.mjs` by +8 below
  `~:681`, and eight anchors across the two documents — `REQUIRED_CARD` (`:779`→`:787`), the `k in o` loop
  (`:793`→`:801`), the `const maxChars = Number(...)` idiom (`:795`→`:803`) and the nonempty-nonce check
  (`:799`→`:807`) — survived a round that re-measured the TEST file's anchors across that same merge and
  stopped there. Mutating any one corrected anchor by ±1 line must red; re-scoping the audit to
  `session-hook.sh` alone makes all eight mutants green, which is the pin the widening buys. Also assert that
  every double-quoted string presented as a ROW of a named mutation table occurs verbatim in that table
  (round 13, A-M1) — reintroducing an invented row name reds, and the genuine row names stay green. **`ccd/session-hook.sh:1115-1119` (round 13, A-M8)** — the shipped `COMPACT_CARD_MAX_AGE` comment
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  **The nonce-required assertion must MOVE to an argv that already satisfies every other `REQUIRED_CARD`
  ...
  A source scan over `server/test/**` asserting every `_ws_slug_residue` invocation or assertion is named in
  this disposition list reds when an unlisted one appears.
