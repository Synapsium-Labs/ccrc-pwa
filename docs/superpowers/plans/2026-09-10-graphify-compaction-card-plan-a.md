...
- Modify: `ccd/ccd` — row-generation initialization, `_spawn_start` primary/retry environments and FD
  ...
  (`_rm_prc`, `ccd/ccd:5680`), `_ws_reap_tail` (`_rt_prc`, `:12026`) and `cmd_forget` (`_fg_prc`,
  `:16655`)** — round 12 named only the dead-reg arm's status branch as a change to BUILD, while these three
  ...
  **The NAME is `_lc_refuse_return`, not `_lc_refused` (round 14, B-M5):** the earlier spelling sat one
  ...
  `local a="${1-}" i="${2-}" tok="${3-}" msg="${4-}"; shift 4` at `ccd/ccd:3384` — and in fatality
  (`ccd/ccd:3368`, "EMITS, THEN DIES. Never returns.", `die` at `:3386`), so a mistyped call would shift
...
- **`cmd_ws_add`'s refusal MESSAGE at `ccd/ccd:5090-5092` is in this task's modify scope too (round 10,
...
- **Comment scope in `ccd/ccd` and `ccd/session-hook.sh`, each a shipped sentence this task falsifies
  ...
  **Two citations fail under every reading and are corrected in this round rather than left for the audit
  ...
  flock-unavailable` string that belongs to `ccd/ccd:11039` and occurs nowhere in that test — that same
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  **The brace-template assertion needed its own disposition, and the round-13 scan could not see it
  ...
  (`ccd/ccd:5092`); after the change the message reads `slug in use: quiet-mesa — in
  ...
  (`server/test/ccd-workspaces.test.ts:233-258`): the brace template occurs nowhere in the file, and the two
  halves stand as `expect(out, 'the root is named once')` (`server/test/ccd-workspaces.test.ts:251`) and
  `.toContain('demo-quiet-mesa.archived')` (`server/test/ccd-workspaces.test.ts:252`), under a comment that
  ...
  **18** `rm` calls, and the tail adds three more — `rm -f "$REG/$id.hookstate.json"` (`ccd/ccd:1861`),
  `rm -f "$REG/$id.reaping"` (`:2446`) and the conditional `rm -f "$REG/$id.archived"` (`:2452`) — for **21**
