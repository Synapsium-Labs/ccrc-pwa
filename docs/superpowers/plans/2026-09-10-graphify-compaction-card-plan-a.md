...
# the only way two independently-argued bounds can share one acquire path.
...
- Modify: `ccd/session-hook.sh` — one lock/generation/family implementation; locked PreCompact rewritten to the
  ...
  so any surviving instruction to call it must instead name this head-parse idiom, already used at `:958`;
  ...
  (unchanged, forks nothing); settlement (including the genuinely-absent-canonical-set branch, whose premise is
  ...
  `ccd/session-hook.sh:792-847` and their call site at `:899`.
- Modify: `ccd/compact-card.mjs` — retarget `writeAtomic`/`cardCommand` (`:420-431`, `:569-598`) and `main`'s
  ...
  `--set`/`--out`; no slot check, no rollback, no canonical read of any kind. **Delete outright**:
...
- Modify: `ccd/compact-card.d.mts` — **delete** the `slotIsMine` (`:42`) and `rollbackHook` (`:50`) type exports.
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  `_hook_write_atomic` verbatim pin at `:2467` onto the new construction in the same commit), `server/test/compact-card.test.ts` (**delete** the ten helper-rollback race tests — their SUBJECT ceases to exist, replaced by the stronger CAS/argv/stage-completeness controls below), `server/test/ccd-lifecycle-purge.test.ts` (move the `_reg_purge always journals, and journals BEFORE it unlinks` source-order pin, `:37`/`:111`, onto whatever Task 9's edit shifts it to, in the same commit), `server/test/ccd-workspaces.test.ts`, `server/test/ccd-spawn-split.test.ts`, `server/test/ccd-session-lifecycle.test.ts`, and `server/test/ownership.test.ts`.
...
3. **PreCompact lock scope — round 7, option A (staging-only helper), superseding round 6's Important 2.**
   ...
   only `--set-stage`/`--card-stage` (two private paths it names itself) — never `--set`/`--out`. A held
   ...
   **reacquires** the lock immediately after the helper returns or times out and, still under that lock,
   ...
   hook already uses at `:958` — bash has no `slotIsMine` counterpart, so this head-parse **is** the ownership
   ...
   stage 2 only, after the steer switch is absent) prints `STEER_TEXT`, then renames the set-stage file to
   canonical carrying `steered := (that printf's own exit code == 0)` — card before set, one uninterrupted
   section, so the steering bit is decided and published without a second lock acquisition. rc 3 renames only
   ...
   stage through its own `<stage>.<pid>.part` temp plus rename, so a stage exists iff it is complete, and
...
6. **(round 7, option A) Add the eight staging-only-helper controls, replacing the twenty-one deleted rollback/slot-check race tests:**
   ...
   - No canonical path in the helper's argv: extend the landed argv-assertion fixture to require `--set-stage`/`--card-stage` and to forbid `--set`/`--out` outright; passing `--set` reds.
   ...
   - Publication order and the steering bit: assert card-rename precedes the print, which precedes set-rename; a failed `printf` leaves `steered:false` on the set that same rename publishes; stamping `steered` before the print, or under a second lock acquisition, reds.
   - Parameterized wait (already covered by item 3's control above — cross-reference it here rather than duplicating).
   - Absent-canonical has exactly one meaning: a source scan asserting no code path renames, unlinks, or replaces the canonical set outside a held stable-lock region; reintroducing the deleted unconditional rollback rename reds this scan even where behavior tests still pass.
