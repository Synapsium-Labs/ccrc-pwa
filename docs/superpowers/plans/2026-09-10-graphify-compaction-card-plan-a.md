...
- Modify: `ccd/ccd` — row-generation initialization, `_spawn_start` primary/retry environments and FD
  ...
  **The NAME is `_lc_refuse_return`, not `_lc_refused` (round 14, B-M5):** the earlier spelling sat one
  ...
  (`ccd/ccd:4121`, "EMITS, THEN DIES. Never returns.", `die` at `:4047`), so a mistyped call would shift
  ...
  `_ws_slug_free` (`ccd/ccd:4032-4043` — spelled in full because the clause above names a `.test.ts`, and a
...
- **`cmd_ws_add`'s refusal MESSAGE at `ccd/ccd:6083-6085` is in this task's modify scope too (round 10,
...
- **Comment scope in `ccd/ccd` and `ccd/session-hook.sh`, each a shipped sentence this task falsifies
  ...
  **Two citations fail under every reading and are corrected in this round rather than left for the audit
  ...
  flock-unavailable` string that belongs to `ccd/ccd:12381` and occurs nowhere in that test — that same
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  **The brace-template assertion needed its own disposition, and the round-13 scan could not see it
  ...
  (`ccd/ccd:6085`); after the change the message reads `slug in use: quiet-mesa — in
  ...
  **18** `rm` calls, and the tail adds three more — `rm -f "$REG/$id.hookstate.json"` (`ccd/ccd:3175`),
  `rm -f "$REG/$id.reaping"` (`:3177`) and the conditional `rm -f "$REG/$id.archived"` (`:3183`) — for **21**
