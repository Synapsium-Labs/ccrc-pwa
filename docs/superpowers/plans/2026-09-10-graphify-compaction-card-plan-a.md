...
- Modify: `ccd/compact-card.mjs` — retarget `writeAtomic`/`cardCommand` (`:420-431`, `:569-598`) and `main`'s
  ...
  never `trigger`; no slot check, no rollback, no canonical read of any kind. **Delete outright**:
...
- Modify: `ccd/compact-card.d.mts` — **delete** the `slotIsMine` (`:42`) and `rollbackHook` (`:50`) type
  ...
  dropping `set`/`out` entirely.
- Modify: `ccd/ccd` — row-generation initialization, `_spawn_start` primary/retry environments and FD
  boundaries, `_reg_purge`, exact cleanup, and `_ws_slug_free` (round 8, I4: widened to an exact-family
  dot-leading scan — strip the literal `.<id>.` prefix, then match the remainder against §3.4's
  private-family suffix grammar or the bare `.generation` field; report not-free on a match; the
  permanent lock's own suffix is the sole named exclusion); re-stamp in this task.
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  retarget** (round 8, I6) every test asserting `readSet().served` — `:2938` ("the fact of serving is
  stamped into the set"), `:2979`/`:2987`, and `:3011`/`:3016` all assert the canonical `set.served`
  rewrite `_hook_compact_served` performs, which this task deletes outright; retarget each to check the
  nonce-marker file's existence instead, or delete it where a marker-based sibling already covers the
  same case; **delete or retarget** (round 8, M6) the broad-sweep test at `:2390` ("sweeps this id's
  ...
  `served` — a field this task's helper no longer writes; and retarget the four remaining
  `cardCommand`/CLI calls that still pass `out`/`set` (`:425`, `:493`, `:781`, `:803`) to the new
  `--set-stage`/`--card-stage`/`--parent-live`/`--live-agents` argv (round 8, I9/M11 — the original "ten"
  count and the `:15`/`:441`/`:500`/`:425`/`:493`/`:781`/`:803` omissions both undercounted what this
  task's deletion enumeration must name)), `server/test/ccd-lifecycle-purge.test.ts` (move the
...
3. **PreCompact lock scope — round 7, option A (staging-only helper), superseding round 6's Important 2.**
   ...
   **reacquires** the lock immediately after the helper returns or times out and, still under that lock,
   ...
   stage 2 only, after the steer switch is absent) prints `STEER_TEXT`, then (round 8, I1) stamps `steered`
   onto the still-private set-stage file — a `jq` rewrite of that one stage, never of canonical, since the
   stage is not yet canonical — to `(that printf's own exit code == 0)`, then renames that UPDATED set-stage
...
6. **(round 7/8, option A) Add the NINE staging-only-helper controls, replacing the twenty-three deleted rollback/slot-check race tests (twelve in `compact-card.test.ts`, eleven in `session-hook.test.ts` — round 8, I9 corrects the earlier "ten"/"twenty-one" counts):**
   ...
   - **(round 8, Critical) The provenance channel itself:** run the full PreCompact arm at `auto`/`main` and `auto`/`subagent`; assert the canonical set after publication carries `liveAgents:0,parentLive:null` and `liveAgents:1,parentLive:false` respectively, and that `measure --set <it>` yields a record `JOURNAL_RECORD_PRED` accepts with non-null `scope` and `cited`; dropping either flag, or having the helper derive them instead of copying the hook's values, reds.
