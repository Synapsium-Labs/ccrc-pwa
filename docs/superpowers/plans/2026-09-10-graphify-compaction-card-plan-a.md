...
- Modify: `ccd/ccd` — row-generation initialization, `_spawn_start` primary/retry environments and FD
  ...
  (`_rm_prc`, `ccd/ccd:6753`), `_ws_reap_tail` (`_rt_prc`, `:13368`) and `cmd_forget` (`_fg_prc`,
  `:19488`)** — round 12 named only the dead-reg arm's status branch as a change to BUILD, while these three
  ...
  **The NAME is `_lc_refuse_return`, not `_lc_refused` (round 14, B-M5):** the earlier spelling sat one
  ...
  `local a="${1-}" i="${2-}" tok="${3-}" msg="${4-}"; shift 4` at `ccd/ccd:4245` — and in fatality
  (`ccd/ccd:4229`, "EMITS, THEN DIES. Never returns.", `die` at `:4247`), so a mistyped call would shift
  ...
  `_ws_slug_free` (`ccd/ccd:4140-4151` — spelled in full because the clause above names a `.test.ts`, and a
...
- **`cmd_ws_add`'s refusal MESSAGE at `ccd/ccd:6335-6337` is in this task's modify scope too (round 10,
...
- **Comment scope in `ccd/ccd` and `ccd/session-hook.sh`, each a shipped sentence this task falsifies
  (round 9):** `ccd/ccd:1422-1425` ("`_reg_purge`, `_ws_slug_free` and `_ws_slug_residue` all glob
  ...
  **Two citations fail under every reading and are corrected in this round rather than left for the audit
  ...
  flock-unavailable` string that belongs to `ccd/ccd:12759` and occurs nowhere in that test — that same
  ...
  **Do NOT add a "verbatim row name" leg.** Round 13's A-M1 clause required that every double-quoted string
  ...
  items and quotes the plan's actual wording, so the audit needs no such leg (round 14, A-1 = B-I4). **`ccd/session-hook.sh:2473-2481` (round 13, A-M8)** — the shipped `COMPACT_CARD_MAX_AGE` comment
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  **Disposition, per assertion, all seven:**
  ...
  - **`server/test/session-hook.test.ts:3132-3219` (round 13, B-I7) — the compact-SessionStart cost-ratio
  ...
  **The brace-template assertion needed its own disposition, and the round-13 scan could not see it
  ...
  (`ccd/ccd:6337`); after the change the message reads `slug in use: quiet-mesa — in
  ...
  **18** `rm` calls, and the tail adds three more — `rm -f "$REG/$id.hookstate.json"` (`ccd/ccd:3283`),
  `rm -f "$REG/$id.reaping"` (`:3377`) and the conditional `rm -f "$REG/$id.archived"` (`:3383`) — for **21**
...
- **D-2986 — the stable lock's "a later canonical disappearance refuses, never recreates" rule is NOT enforced on a box without `/proc`, and that is recorded Darwin platform residue rather than repaired.** D-2793 built the refusal with two arms, and only arm (b) can see the scenario §4 names: a holder past its own acquire leaves NO exact-family name in `$REG` — the acquire unlinks its alias the instant the FD is held — and keeps only a DESCRIPTOR, which Linux names through `/proc/<pid>/fd/<n>`. Both twins gate that arm `[ -d /proc ] || return 1`. Darwin ships no `/proc`, so there the two states §4 separates are indistinguishable: `_hook_lock_vanished` / `_compact_lock_vanished` answer "first-ever mint", the acquire mints a SECOND inode at the one pathname under the live holder, and two processes can each be told by `flock` that they hold "the" lock — the exact hazard `ccd/session-hook.sh`'s header says the link-based design removes, standing on that platform. MEASURED as five failures on the `test-macos` leg at `ab08bd92` (Darwin 25.6.0 arm64, bash 5.3.15, coreutils g-prefixed): `ccd-lifecycle-purge`'s `REFUSES rather than recreating canonical…` (`RC=0 WHY=` where §4 wants `RC=1`) and its `(d1c)` four-caller leg, `ccd-ws-reap`'s canonical-vanished tail (`a reap whose purge was refused is not a success: expected +0 not to be +0`), and the hook suite's `a holder past its acquire leaves NO name in $REG…` (`the live holder is nameable through /proc: expected '' not to be ''`) and `PreCompact REFUSES and recreates no canonical when it vanished under a live holder` (`canonical was NOT recreated: expected true to be false`). RECORDED, NOT REPAIRED, and the two candidate repairs are why: `lsof` is the only Darwin detector of a descriptor on an unlinked path and its deleted-path reporting there is UNMEASURED; and failing CLOSED on the ambiguity would refuse every FIRST-ever mint, because canonical is minted lazily while the generation is minted at row creation — the ordinary state of a row's first compaction. WHAT SHIPPED INSTEAD (PR #135, round M4), with no weakened assertion: both `[ -d /proc ]` gates say it in place, line-count neutral (`ccd/ccd` 19334, `ccd/session-hook.sh` 3174, `ccd/ccd` re-stamped); the five cases become `itLinux` one at a time, each with its own reason (linux counts unchanged 330/330, 58/58, 88/88; macOS skips 244 → 249); the spec's §4 row and §5 mutation row and the plan's Task 9 lock-protocol ruling each gain one sentence of qualification inside the existing line. The same PR carries the two fixes that made the macOS leg measurable at all: `_compact_lock_same`/`_hook_lock_same` decide by inode through `/dev/fd` (stat there carries the fdesc DEVICE with the file's inode, MEASURED `1245178674:2962058` against `16777230:2962058`), and the hook fixture's minimal PATH gains `ls`, which only that fallback forks; the compact-arm cost-ratio row gets a Darwin bound of 8 beside its untouched linux 4 (three runner observations 3.31–4.64). WHAT WOULD CLOSE IT: a measurement of whether `lsof` on macOS names an unlinked hard-link alias by its former pathname while a holder lives. Allocation: D-2986 allocated 2026-09-17 08:26 UTC through `ccrc-api ledger allocate` (project ccrc-pwa, byId ccrc-pwa-amber-prairie), count 1, floor then 2987, title "Compaction card on macOS — the later-disappearance rule is unenforced without /proc (Darwin platform residue)".
