...
- Journal raw validator: first valid record succeeds because `jq -Rse` receives one raw string and `jq -e` emits true; non-newline tail plus every terminal-LF malformed old physical line (blank, garbage, scalar, array, concatenated objects, wrong-shape/unknown-key, `n`, negative/fractional counts, `filesChars > chars`, or inconsistent `cited`/`setSize`) preserves old journal bytes exactly and leaks no stderr. Pin raw-string binding, terminal-LF/split logic, `all(...; ((fromjson? // false) | JOURNAL_RECORD_PRED))` so parse failure is not vacuously accepted, integer and relationship predicate clauses, and no `empty` success path. Stub the seam below the transaction, not its builder.
- Journal FD/CAS safety: unreadable, regular-file symlink, other non-regular journal, stage-target replacement, and live-journal replacement before final identity/absence CAS each refuse without overwriting a directory and preserve both old and staged bytes. Pin opening/reading from the retained regular FD, descriptor/path identity before read and immediately before rename, and absence staying absent.
...
Generate two decimal random components and validate the full internal claim grammar before use. While that
shared mutex is held, require a readable non-symlink regular canonical set in the same `$REG` filesystem and
measure its original age into `claim_was_aged` **before** settlement. Choose an exact candidate
`$REG/.$id.compactpost.$$.$r1.$r2.claim`; do **not** noclobber-create, touch, link, or otherwise reserve it in
advance. Settlement is POSIX `link "$REG/$id.compactset" "$claim"`: atomic no-clobber link success alone
publishes the claim, while an existing file/symlink/directory target refuses (and never becomes a directory
child). Immediately after successful link and while locked, `touch "$claim"`, then unlink only the canonical
alias. The saved `claim_was_aged`, not active claim mtime, controls provenance/`--set`; touch makes a live claim
young throughout helper plus journal work. Link failure leaves canonical untouched and records nothing. A kill
immediately after link leaves the authoritative claim, while canonical is safely replaceable by a next locked
PreCompact. If canonical alias unlink fails, retain claim, treat settlement complete, and never delete it; a
later locked publisher may refuse safely. A missing/non-regular canonical set releases the lock and proceeds
no-set. The same bounded same-UID threat statement applies to this pathname CAS.

After settlement, parse only that settled pathname. Validate its nonce even when `claim_was_aged`: a safe nonce
controls exact marker lookup/cleanup, while an unsafe nonce yields `served:false` and forms no path. An
originally aged set runs `measure` without `--set` and supplies null scope/provenance regardless of the touched
claim mtime; no other claim field is trusted. Only an originally young claim is additionally validated against
the provenance grammar. Treat absent `overlap` as legacy-compatible ordinary/false because the already-implemented
Task 1–8 helper rewrites ordinary sets without carrying that member; explicit false is equivalent.
...
3. If journal exists, require readable non-symlink regular pathname, open it on a read FD, and verify descriptor regularity plus descriptor/path inode identity. Read exact old bytes **from that retained FD**. If it was absent, record expected absence; never overwrite a directory fixture.
4. Validate raw old bytes using `jq -Rse` as **one string**: bind `$raw`, require `$raw == ""` or terminal LF, remove the one final split element, reject remaining empty physical lines, and require `all($lines[]; ((fromjson? // false) | JOURNAL_RECORD_PRED))` so garbage cannot disappear into a vacuous success. Run `jq -e` so valid input emits `true`, never `empty`. A first valid record must pass; terminal-LF blank, garbage, scalar, array, concatenated-object, wrong-shape/unknown-key, `n`, negative/fractional, and impossible relationship lines must refuse.
...
6. Immediately before final `mv -f`, recheck the stage target; recheck an initially present journal pathname is non-symlink regular and still same-inode as the retained FD, or an initially absent journal is still absent. On any mismatch refuse and preserve both byte sequences. Only then atomically replace journal while locked. This detects cooperating-external replacement; arbitrary same-UID mutation after the last check remains outside the stated portable-Bash boundary.

One cleanup path removes only the private claim and, for a separately safe nonce parsed from that settled claim, its exact marker. It runs after success and every post-settlement failure, including an aged claim after its nonce-only parse. If canonical-alias unlink failed immediately after link, retain that authoritative claim for stale sweep rather than cleanup. Cleanup never names canonical set/card or another marker.
...
In an isolated copy, mutate one seam at a time: move each early guard after claim; pre-read nonce from canonical or let an originally aged claim reach `--set`/provenance; pre-create the final claim placeholder; replace `link` with copy or weaken its exact-target failure handling; skip saved-age, post-link touch, canonical-alias unlink, or claim-retention-on-unlink-failure behavior; skip claim cleanup; marker before emit or broad delete; ignore/rewrite young claim; relax a per-family exact owner matcher against a shared-prefix/dotted legal id; trust overlap provenance or relax any provenance type/cross-field row; mutate each claim/marker-temp/stage filename component or full-name grammar; replace raw-string truth-valued validator with a slurp/array/`empty` form; add `n`; weaken integer or relationship predicate clauses; bypass each dependency; remove safe noclobber/RW-open/descriptor-regular/path-regular/pre-or-post-flock inode-identity checks, broaden lock wait, or unlink/replace lock; delay locking until settlement; append live with `>>`; omit FD read/identity recheck/absence CAS/stage target check/old-byte copy/newline/prefix/all-physical-line validation/atomic rename; omit each provenance key; remove future-set served removal or restore stale helper comments; remove compact early exit; emit either stream. Every named test must redden, with the concurrency mutation repeated to prove deterministic power.
...
Re-run any documented load flake in isolation before calling it a break. `typecheck-tests` must prove the helper declaration import. Then run stale-claim searches over the spec/plan/Task 9/Task 11 briefs and require no operative `hookstate.compaction`, persisted `n`, direct live-journal append, set-served rewrite, final-claim placeholder/reservation, canonical-set rename/move settlement, lock-after-settlement wording, shallow terminal-LF-only or array-shaped raw validation, nonce-before-claim, or five-second timeout instruction. Historical ledger entries may retain a rejected pre-amendment description only when the same entry explicitly says D-2605 supersedes it and states the final hard-link/saved-age/shared-lock protocol.
