...
- **D-2993 — `cmd_ensure` mints a missing row generation, and the upgrade transition it repairs.** §3.4 and `_reg_generation_init`'s own header state the standing rule as "called by row creation and by nobody else", and both minting sites — `cmd_ws_add` and `cmd_start` — are creation. The systemd unit reaches NEITHER: `claude-session@.service` runs `ccd supervise`, which calls `cmd_ensure`, which spawned without ever minting. So a row created before the generation mechanism shipped could not gain one by any automatic path, and `_spawn_start`'s own warning — "inert until its next respawn" — promised a repair no respawn performed. MEASURED on the fleet host 2026-09-17, hours after the card was first deployed: 31 of 34 live rows carried no generation, zero compaction journals existed anywhere, and the warning string appeared in no log on the box because the absent-generation case took the silent arm. THE DEPARTURE: `cmd_ensure` gains a third minting site, mirroring `cmd_start`'s block but BEST EFFORT and never fatal — a contended lock, an absent `flock` mechanism and a present-but-invalid generation all warn and continue, because this is the supervisor's ExecStart path and a `die` there crash-loops the unit to FAILED (adversarially measured at ~40 s). SAFETY: the mint cannot authorize a pane that should not publish, because `_hook_generation_ok` requires a NON-EMPTY `CCRC_SESSION_GENERATION` in the pane's environment AND byte equality with the row's file; a pane spawned before the mint carries no value and is refused whatever the file then says, so the mint is strictly forward-looking. `_reg_generation_init` still mints only on genuine absence, still never repairs a present-invalid generation, and its `link`-based publish still leaves an incumbent untouched. ALSO IN SCOPE, because a permanent inertness that says nothing is half the defect: `_spawn_start` now warns on all THREE causes rather than the contended acquire alone — the README recorded the missing two as a deferred `ccd/ccd` change, and this closes it.

- **D-2994 — `cmd_ws_restore` mints a missing row generation too, the third respawn path D-2993 left inert.** D-2993 gave `cmd_ensure` a mint so the supervised path repairs a pre-D-2605 row at its next respawn, and that is what healed the fleet on 2026-09-17. It does not reach an ARCHIVED row: `cmd_ws_restore` is the only caller of `_spawn_start` that passes through neither `cmd_ensure` nor `cmd_start` (theseven call sites are `cmd_ws_add`, `cmd_ws_restore`, `_spawn`, `_supervised_start` twice, `cmd_start` and `cmd_ensure`; `_supervised_start` is reached only from the last two, and both mint first), so a row archived without a generation came back with a pane that could never publish a compaction artifact, and no respawn sweep could reach it because it was not running. The mint is BEST-EFFORT AND NEVER FATAL — a different argument from `cmd_ensure`'s crash-loop one: by that line the restore has already cleared `archived` and put the worktree back, so a `die` would strand a half-restored row, and the insertion sits inside the reap flock region whose contract is that success and failure leave through the same close (`ccd-restore-reap-lock.test.ts` pins that no `return` appears between the lock and its release). rc 2 — no flock mechanism — skips and continues, `cmd_start`'s own precedent. Pinned by two cases in `ccd-archive.test.ts`: a planted generationless row GAINS one on restore (red before this change: "expected null not to be null"), and a row that already carries one keeps it BYTE-IDENTICAL, the control that makes the first mean "mints on absence" rather than "writes unconditionally" — re-minting would be the worse defect, since the hook compares the row's bytes to the value its pane was spawned with.
