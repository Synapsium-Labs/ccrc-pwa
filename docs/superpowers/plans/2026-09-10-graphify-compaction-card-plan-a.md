...
# private residual family ONLY once Task 9 widens it (round 8, I4, spec §3.4):
# strip the literal `.<id>.` prefix, match the remainder against the family
# grammar or the bare `.generation` field; the permanent lock alone is the
# named exclusion. Locked exact-ID cleanup ignores only that permanent lock.
...
- **Comment scope in `ccd/ccd` and `ccd/session-hook.sh`, each a shipped sentence this task falsifies
  ...
  sentence at `:1100-1101` already in this task's range. `ccd/ccd:1822-1827`, whose last line reads
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  retarget** (round 8, I6; **enumeration corrected round 9, B-I7/M-6 — there are FIVE `readSet().served`
  assertions, not three**) every test asserting `readSet().served`. Measured, `grep -n '\.served'
  server/test/session-hook.test.ts` returns exactly `:2938`, `:2967`, `:2987`, `:3016`, `:3038`; round 8
  named only three of them, and the two it missed go red for the strongest possible reasons once
  `_hook_compact_served` is deleted. **Disposition, per assertion, all five:**
  ...
  - `:3038` (inside "a nonce with no text after it is not served" at `:3027`) — **retarget to "no marker
    ...
    its own. Note `:3032` plants `served: false` inside a fixture set literal; that is an INPUT, not an
    assertion, and the `\.served` scan below does not match it (measured: the grep returns five hits and
    `:3032` is not among them).

  **Make the enumeration self-checking:** add a source scan asserting **zero `\.served` occurrences remain**
  in `server/test/session-hook.test.ts`, so missing any one of the five reds rather than being discovered as
  an unexplained failure in Step 4. **delete or retarget** (round 8, M6) the broad-sweep test at `:2390` ("sweeps this id's
...
6. **(round 7/8, option A) Add the NINE staging-only-helper controls, replacing the twenty-three deleted rollback/slot-check race tests (twelve in `compact-card.test.ts`, eleven in `session-hook.test.ts` — round 8, I9 corrects the earlier "ten"/"twenty-one" counts):**
   ...
   - Publication order and the steering bit: assert card-rename precedes the print, which precedes the private stage-rewrite of `steered`, which precedes set-rename; a failed `printf` leaves `steered:false` stamped into the stage that same rename then publishes; stamping `steered` before the print, rewriting canonical directly instead of the stage, or stamping under a second lock acquisition, reds (round 8, I1).
