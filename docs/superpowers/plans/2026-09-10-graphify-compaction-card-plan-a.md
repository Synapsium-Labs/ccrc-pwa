...
- **Minor 1 — `readSetForMeasure`'s docstring claimed a guarantee ("a bad set must never cost the whole measurement") that only held for the top-level parse, not for an individual malformed `files[]` entry.** Measured through the real CLI (own reproduction): `files:[null]` and `files:[{"path":5}]` each threw (TypeError on `.path` of null, then `p.split is not a function`), losing the whole measurement (exit 1); `files:[{}]` against a summary containing the literal word "undefined" did not throw but silently scored a phantom citation (`cited:1, setSize:1`) because `String.prototype.includes` coerces `undefined` to the string `"undefined"`. Unreachable through today's writers (the hook never emits such a `files[]` entry), but a false docstring is still a defect this repo treats as one. Fix: `measureCommand`'s path extraction now filters `set.files` to entries with `f && typeof f.path === 'string'` before mapping, so a malformed entry costs only itself, never the measurement; `readSetForMeasure`'s docstring is corrected to describe only what it actually guarantees (top-level shape), with the downstream hardening now named explicitly. Pinned by new unit-level and CLI-level tests reproducing all three shapes plus a mixed valid/malformed list.
- **Minor 3 — `citedCount`'s FULL-PATH branch (`text.includes(p)`) had no left-boundary check, so a shorter set member's full path could be credited merely for being a raw substring of a LONGER member's own cited full path** (`['a/b/c.ts','x/a/b/c.ts']`, summary naming only `x/a/b/c.ts`, scored 2). §3.4 requires citation to be path-segment-aligned; this half violated it. Fix: a new `hasAlignedOccurrence` helper requires the character immediately before any match of the full path to be either absent (start of text) or not a path-continuation character (the same `[A-Za-z0-9_./-]` alphabet `tokenRegex` mines with). The suffix-loop half is DELIBERATELY untouched: the reviewer's other example (a summary naming `pwa/src/watch.ts` crediting the set's `server/src/watch.ts`) is not a defect — that suffix is genuinely segment-aligned in the text and unique within the set, so the rule as specified counts it correctly; that behaviour is a spec limitation to record, not a bug to fix, and this round leaves it alone.
