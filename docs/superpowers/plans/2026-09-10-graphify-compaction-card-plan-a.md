...
permanent lock does not. `_reg_purge` gains an honest lock-guarded result, and Task 9 BUILDS the status branch in all four callers (measured, none reads it today).
...
- Modify: `ccd/ccd` — row-generation initialization, `_spawn_start` primary/retry environments and FD
  ...
  `_lc_refuse`'s own neighbourhood (`ccd/ccd:2756-2785`, `_lc_fail` through `_lc_refuse`), where Task 9 adds
  ...
  **The NAME is `_lc_refuse_return`, not `_lc_refused` (round 14, B-M5):** the earlier spelling sat one
  ...
  `local a="${1-}" i="${2-}" tok="${3-}" msg="${4-}"; shift 4` at `ccd/ccd:2782` — and in fatality
  (`ccd/ccd:2766`, "EMITS, THEN DIES. Never returns.", `die` at `:2784`), so a mistyped call would shift
  ...
  `_ws_slug_free` (`:3940-3951`; round 8, I4: widened to an
...
- **Comment scope in `ccd/ccd` and `ccd/session-hook.sh`, each a shipped sentence this task falsifies
  ...
  **And WIDEN the audit itself (round 13, A-I1/B-I4): it covers EVERY tracked-file reference in either
  ...
  1. **The premise.** The rule governs the relationship between a QUOTATION and a RANGE. Where a reference
     ...
     line that has no token to quote. Measured: `ccd/session-hook.sh:843` is a BLANK line, cited as such
     ("`:843` is blank, `:844` starts the next section banner"); a universal form refuses it by construction.
  ...
  **Two citations fail under every reading and are corrected in this round rather than left for the audit
  ...
  §3.4's five-site cell cited `server/test/ccd-ws-reap.test.ts:342` beside a quoted `_lc_refuse reap …
  flock-unavailable` string that belongs to `ccd/ccd:10203` and occurs nowhere in that test — `:342` being
  the `it('refuses to run the destructive verb unserialised when flock is missing')` title, which sub-rule B
  is exactly the rule for.
...
- Test: `server/test/session-hook.test.ts` (**delete** the eleven timeout-rollback race tests; move the
  ...
  **The nonce-required assertion must MOVE to an argv that already satisfies every other `REQUIRED_CARD`
  ...
  order (`ccd/compact-card.mjs:801`), and `nonce` is last in that array (`:787`), so once the list gains
  ...
  `--nonce ''` must still return USAGE (the separate nonempty check at `ccd/compact-card.mjs:807`, whose message is `--nonce must be a nonempty string` — a different message from the loop's, so the two assertions do test two different guards).
  ...
  `server/test/ccd-workspaces.test.ts:151` feeds `:154-158`, which branches only on `residue === ''` versus
  non-empty, and `:204` asserts `toBe('')` — measured, both key on emptiness alone, never on field spelling.
  **`server/test/ccd-workspaces.test.ts:182` needs its own disposition, and the round-13 scan could not see it
  (round 14, B-I2).** That line is `expect(out).toContain('demo-quiet-mesa.{archived}');` inside
  `it('refuses ws-add on the residue the purge is documented to leave')` (`:169-188`), and it asserts the
  `$REG/<id>.{…}` BRACE TEMPLATE that round 10's B-3 ruling requires this task to delete from
  `cmd_ws_add`'s die (`ccd/ccd:4357`). After the change the message reads `slug in use: quiet-mesa — in
  <REG>: demo-quiet-mesa.archived`, which does not contain `demo-quiet-mesa.{archived}`, so `:182` goes red on
  a correct tree. It is neither an invocation of `_ws_slug_residue` nor an assertion on one, so the scan above
  is blind to it. **Disposition: retarget `:182` to the rooted-list form** — `toContain('in ')` plus
  `toContain('demo-quiet-mesa.archived')` — keeping both halves of what the line was buying (the root is named
  once; the basename is named in full). **And WIDEN the self-checking scan**, which until now covered
  ...
  **18** `rm` calls, and the tail adds three more — `rm -f "$REG/$id.hookstate.json"` (`ccd/ccd:1861`),
  `rm -f "$REG/$id.reaping"` (`:1862`) and the conditional `rm -f "$REG/$id.archived"` (`:1868`) — for **21**
