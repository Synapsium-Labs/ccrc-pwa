...
- **D-3157 — `runPy` cannot be used with a same-process mock server, and this plan's own Task 10 example
  ...
  `spawnPy` in that task's fix round, sharing `runPy`'s declared outcome type rather than re-declaring
  the shape inline — a second declaration of one outcome type is exactly what this repo's
  single-definition rule exists to stop.
...
- **D-3158 — `lane.json`'s probe-model field name was chosen by an implementer, with no authority to
  ...
  that reads a fixture generated BY the materialiser rather than hand-written. Until then this entry is
  the only thing recording that the key is a choice rather than a contract.
