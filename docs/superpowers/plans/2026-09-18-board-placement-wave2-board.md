...
- Produces: `runCard(run: { sessionId: string | null; project: string }, cardOf: ReadonlyMap<string, string>): string` in `runWords.ts`; `FleetGroup.elsewhere: readonly { project: string; count: number }[]`.
...
Numbers here are ISSUED at the moment of the departure (`ccrc-api ledger allocate`), never taken from a floor and never pre-minted. Task 10 owes one entry (the `resume-reclaim-l0.test.ts` exact-equality pins, reds by design). Add each as `- **D-<n> — <title>.** <what departed, why, what shipped instead>`.
