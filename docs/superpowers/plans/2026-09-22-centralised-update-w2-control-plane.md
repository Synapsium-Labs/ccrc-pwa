...
| reads are validated (units) | in `secondsToMs` delete `&& v <= REPORT_TIME_MAX_S` | `reportFrom: SECONDS in, ms out; a ms value is refused` |
...
| the projection is seconds (renderer guard) | in `renderProjection`, delete `|| issuedAtS > MAX_UNIX_S` | `a ms issuedAt is refused, never rendered` (`update-projection`) |
