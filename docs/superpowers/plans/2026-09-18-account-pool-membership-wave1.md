...
3. **`poolRule()` has NO production call site.** Spec §5.6 implies the server forecasts through it. Measured: `shared/poolrule.ts:66` is called only from `server/test/pool-rule-core.test.ts`. The server's live refusal path is `refusePool` (`server/src/server.ts:2058-2075`) over `poolFor`. Task 6 must wire the account arm into that path or the rule stays decorative.
