...
**Producers:** W2 — `git show 1258a57a:docs/superpowers/plans/2026-09-22-centralised-update-w2-control-plane.md` (the plan as executed, its Deviations and Interfaces corrected through the fix rounds; merged on `main`; the copy there at `4b2ff904` is the one to read); wave 3 — `docs/superpowers/plans/2026-09-23-centralised-update-w3-settings-and-notification.md` on `main` (merged as `4b2ff904`, its Deviations as shipped); wave 4 — `git show 4b361c00:docs/superpowers/plans/2026-09-22-centralised-update-w4a-node-side.md`. ALL THREE MERGE BEFORE this wave is dispatched; every task assumes their code is on `main` and spells every shared name exactly as those plans do. Anchors into files none of them touches are at `4b2ff904`; anchors into files they change are by QUOTED producer text, never by line.
...
- **Cited files move no cited line** (ruling R13): `shared/api.ts` above `:7672`, `single-definition.test.ts` (`:32-37`, `:1274`, `:1303-1304`, `:1319-1320`, census 8) and README's anchors — every edit is appended after them or line-neutral, each task says which, and that task's green run includes `server/test/session-hook.test.ts`.
...
- **D-3494** — Found re-pointing this plan onto W2's executed tree (2b5d4ce1). Spec §12 says of `apply`'s `tag`: "the route checks `isReleaseTag` (`400`)". §4 says "`isReleaseTag` is the ONE tag-shape guard: the agent's op handler, the routes, the resolver and `cmd_update`'s bash twin … all agree on it". W2 D-3216 leaves `RELEASE_TAG`/`isReleaseTag` untouched. On top of them it adds an ingress-only gate, `isIngestibleReleaseTag` (`server/src/update/resolve.ts`), which calls `isReleaseTag` first and then refuses a tag over 64 bytes, one with a leading zero in any component (`v0.0.010`, which `compareReleaseTags` would read as a second spelling of `v0.0.10`), or one with any component over 18 digits (`RELEASE_TAG_COMPONENT_MAX_DIGITS`, W2 D-3216 as amended). W2 applies it at the catalogue's element parse and at `parseIntentBody`'s `pinnedTag`, answered `400 bad-tag`. The two move routes do exactly what the intent route does. Task 6's `tagField`, shared by `parseApplyBody`'s `tag` and `parseRollbackBody`'s `to`, calls `isIngestibleReleaseTag` and answers `400 bad-tag` naming the field, so one tag answers the same on every update route. The dispatcher, the agent's `validateReq` and `updateSpawnArgv` keep `isReleaseTag`. Pinned by Task 6's parser case, which lists `v0.0.010` among the `bad-tag` shapes. Cost if wrong: none a caller can reach. Such a tag can never be a catalogue row (the catalogue's ingress skips it), so the spec's literal rule would have answered `409 unknown-tag` for it; this plan answers `400 bad-tag` instead.
...
- `pwa/src/screens/FleetScreen.tsx` — `<UpdateBanner updates={updates.view} health={fleetHealth} onMoved={updates.reload} />`, one line in place (Task 8).
...
- Modify: `server/src/index.ts` — two import lines directly after `import { readLocalCcdCaps } from './localcaps.js';` (`:21` at `4b2ff904`): `import { localUpdateSpawnFor } from './update/converge.js';` and `import { UPDATE_OP_TIMEOUT_MS } from '../../shared/agent-protocol.js';`; in BOTH `deps` literals, on the line after W2 Task 13's `    updateIntentLog,`, `    updateRunner: localUpdateSpawnFor(realRunner, cfg.home),`; in the REMOTE literal only, the line after that, `    sendUpdateOp: (tag, kind) => fleet.client.request({ t: 'req', op: 'update', tag, kind }, UPDATE_OP_TIMEOUT_MS),`. CORRECTED: the skeleton's `fleetClient.request({ op: 'update', tag, kind }, …)` does not compile — `FleetClient.request`'s payload is `Omit<AgentReq-member, 'id'>` and keeps `t: 'req'` (every existing caller passes it: `client.ts:238` at `4b2ff904` (Task 1's inserts above `FleetClient` move it again; re-locate by the quoted text), `this.request({ t: 'req', op: 'caps' })`), and W2's hoisted `fleetClient` is `FleetClient | null` while `fleet.client` is the non-null value in scope in that arm
...
**Interfaces:**
- Consumes: Task 3's `requestNode`, `dispatchNode`, `noteDispatchRefusal` and their result types; W2's `releaseLease`, `settleNode`, `refuseRelease`, `markUnreachable`, `upsertNodeMeasurement`, `applyReleaseListing`, `nodes()`, `node()`, `releases()`, `refusalsFor()`, `intentFor()`, `updateEpoch()` (Tasks 4–6), `NodeRow`, `ReleaseRow`, `RefusalRow`, `UpdateIntentRow`, `NodeMeasurement`, `ReleaseListingRow`; `resolveInputFor` (W2 Task 12, `project.ts` — its store parameter is `Omit<ProjectStore, 'resolveNode' | 'nodes'>`, which INCLUDES `updateEpoch()`), `resolveNodeIntent` (`resolve.ts`); `SERVER_LABEL`, `FLEET_LABEL`, `sweepPlanFor`, `SweepPlan` (W2 Task 11, tests); `FleetWatcher.inventoryNow()`/`triggerInventory()` (Task 11), `sweepThenProject` as wave 3 leaves it (its tail `if (this.sweptEnoughToDecide(inv, outcomes)) this.inventorySwept = true;` / `if (this.inventorySwept) this.pushRelease(now);` / `return outcomes;`), wave 3's `private inventorySwept = false;`; `cfg.role`, `cfg.home`, `cfg.ccrcDir`, `cfg.updateDeadlineMs` (W2 Task 9); `FleetState.connected`, `FleetState.agentOps?: readonly string[]` (W2 Task 8 — `undefined` before any `ready`); Task 4's `planDispatch`, `deadlineExpired`, `dispatchRefusalDetail`, `DEADLINE_DETAIL`, `DispatchMove`, `DispatchNodeView`, `DispatchPlan`; Task 1's `UPDATE_OP`, `UPDATE_OP_ERRORS`, `UpdateOpError`, `isUpdateOpError`, `UPDATE_OP_TIMEOUT_MS`, `UPDATE_SPAWN_TIMEOUT_MS`, `updateLauncherPath`, `updateSpawnArgv`, `firstStderrLine`, `inFlightReport`, `InFlightReport`, `AgentOpError`; `NODE_FILES.report` (W2 Task 8); `Runner`, `ExecResult`, `realRunner` (`exec.ts:39-72` at `4b2ff904`); `FleetIO` (`lstatMeasured`/`statMeasured`/`readFileMeasured`, `io.ts:113-135` at `4b2ff904` — reached only through `readNodeFile`); `readNodeFile(io, filePath, deadline)`, `NodeFileRead` and `INVENTORY_BUDGET_MS` (W2 Task 11, `inventory.ts`: lstat to `regular` only, size ≤ `NODE_FILE_CAP_BYTES`, every step raced against the deadline); `openPoolReadDeadline` (W2 Task 10, `pools.ts`); W2's `ackNode` (tests only); `FleetClient.request(payload, timeoutMs?, signal?)` (`client.ts:197-230` at `4b2ff904`; a transport failure rejects a plain `Error('disconnected' | 'timeout' | 'aborted')`, `:199`, `:201`, `:212`, `:218`, `:254`, `:445`; a late `res` whose entry timed out is dropped at `:310-311`, `const entry = this.pending.get(id); if (!entry) return;` — Task 1's inserts above `FleetClient` move every one of these, so re-locate by the quoted text); `testDeps`, `mkTmp`.
...
```bash
grep -n "^    if (this.inventorySwept) this.pushRelease(now);$" server/src/watch.ts
grep -n -A1 "^    if (this.inventorySwept) this.pushRelease(now);$" server/src/watch.ts | tail -n1
...
export function parseApplyBody(body: unknown): ParsedApplyBody;         // exactly one of nodeId (non-empty string) / all (=== true) — both → bad-request 'all', neither → 'nodeId'; an unknown key → bad-request; tag: absent → null, a string `isIngestibleReleaseTag` refuses (off RELEASE_TAG, over 64 bytes, a leading-zero component, or a component over 18 digits — D-3216) → bad-tag, anything else (null included) → bad-request 'tag'
...
grep -n "^import { UPDATE_GATE_CAP } from '../src/update/resolve.js';$\|^  } = {},$\|^    ...(o.push ? { push: o.push as never } : {}),$\|^describe('POST /api/updates/refresh announces what its poll listed" server/test/update-routes.test.ts
```
...
- in `open()`'s options type (W2's multi-line `o: { … }` block, as wave 3 leaves it), insert `    updateRunner?: LocalUpdateSpawn;` as a new line directly before its closing `  } = {},` (unique in the file);
...
- `./node_modules/.bin/vitest run test/session-hook.test.ts -t 'every line citation is anchored'` — the citation audit over `shared/api.ts` (one line added below `:7672`, the rest appended), whose census (`'shared/api.ts': 1`, `'server/test/single-definition.test.ts': 8`) and README's empty entry stay unchanged
...
- Modify: `pwa/src/screens/FleetScreen.tsx` — the quoted `      <UpdateBanner updates={updates.view} health={fleetHealth} />` (wave 3 Task 11; `health` added by its fix round 2) becomes `      <UpdateBanner updates={updates.view} health={fleetHealth} onMoved={updates.reload} />` *(CORRECTION: the skeleton listed no FleetScreen edit, but spec §13's "a `202` closes it and re-polls" needs the banner to reach the screen's one poll — FleetScreen injects the view, so `useUpdatesView(0)`'s own `reload` is a no-op there; one line replaced in place, line-neutral)*
...
**Interfaces:**
- Consumes: Task 4's `compareDispatchOrder` (`shared/api.ts`, end-of-file block); Task 6's `ApplyUpdateBody`, `RollbackUpdateBody`, `MoveRequestAnswer` (its `skipped: MoveSkip[]`), `MoveSkipWhy` and the widened `UpdateRouteError`, whose new words `UPDATE_ERROR_TEXT` carries (its `Record<Exclude<UpdateRouteError, 'unauthenticated'>, string>` makes that a compile error without them, D-3302); the two routes' answers — `202 MoveRequestAnswer` · `400`/`404`/`409`/`501` refusals (`UpdateRouteRefusal`); wave 3's `UpdatesView`/`NodeWire`, `pendingTag`, `nodeVersion` (`pwa/src/fleet/useUpdatesView.ts`), `UpdatesPoll.reload`, `releaseDirection`, `bannerRelease`, `updateErrorText`, `ApiError`, `postJsonOr` (`pwa/src/lib/api.ts:464` at `4b2ff904`), `toast` (`pwa/src/components/Toast.tsx:33`), `Sheet` (`pwa/src/components/Sheet.tsx:27`), `.qc-consequence`/`.qc-actions`/`.btn-primary`/`.btn-ghost` (`pwa/src/components/primitives.css`), the `AbandonSheet.tsx` busy/error/generation idiom (`gen` `:164`, its cleanup `:169`, `confirm` `:177` at `d759c914`); `isReleaseTag` (W2 Task 1), `isNewerTag` (W2 Task 2, `shared/semver.ts` — throws `RangeError` on a non-tag, so every call sits behind `isReleaseTag`).
...
Expected: an `ALL <p> PASS` line and a `# <u> rules set a colour with no ground this auditor can recover …` line. Write both numbers down: after Step 13, `<u>` is unchanged and `<p>` is exactly `<p> + 2` — the two measurements of `.settings-move-note` (dark, light) leave with its rule, and four arrive (`.update-move-sheet` and `.update-move-sheet .update-move-error`, two themes each). Measured at `4b2ff904`, wave 3's `.settings-move-note` rule and entry real: `ALL 606 PASS` / `255` before (the tree), `ALL 608 PASS` / `255` after (a scratch copy of `pwa/` with Step 13 applied); on the branch the numbers are whatever wave 3 left, which is why this step measures rather than quotes.
...
In `pwa/test/fleet-screen.test.tsx`, append after the file's last line (the closing `});` of wave 3's fix-round-1 describe `a malformed /api/updates element does not blank the fleet screen (F11)`, which follows Task 12's):
...
In `pwa/src/screens/FleetScreen.tsx`, replace the quoted line `      <UpdateBanner updates={updates.view} health={fleetHealth} />` with:
...
Expected: `ALL <p+2> PASS` (Step 1's `<p>` plus 2), the uncovered line still reading Step 1's `<u>`, and exactly these four rows (measured on a scratch copy of `pwa/` at `4b2ff904` with this CSS: `ALL 608 PASS`, uncovered `255`):
...
10c. The unmeasured floor (D-3492) — in `takesForward`, delete the line `  if (n.highestVersion === null && n.floorRead === 'unmeasured') return false;   // floor-unread: resolveOnChannel's own test (resolve.ts); undefined = an older server, previews nothing`. Run `./node_modules/.bin/vitest run test/move-plan.test.ts` → RED: "a fleet update never names a node the tag leaves at or below its FLOOR …" (`unfloored` listed; unmeasured — confirm it reds this case alone). Restore; `grep -c "n.floorRead === 'unmeasured'" src/fleet/movePlan.ts` → `1`.
11. The screen's re-poll reaches the banner — in `src/screens/FleetScreen.tsx`, change `<UpdateBanner updates={updates.view} health={fleetHealth} onMoved={updates.reload} />` to `<UpdateBanner updates={updates.view} health={fleetHealth} />`. Run `./node_modules/.bin/vitest run test/fleet-screen.test.tsx` → RED: "opens the move sheet naming the fleet node first … and re-polls the screen" (`expected "spy" to be called 2 times, but got 1 times`). Restore; `grep -c 'onMoved={updates.reload}' src/screens/FleetScreen.tsx` → `1`.
...
- Modify (W3 review 161): `pwa/src/screens/SettingsScreen.tsx` (`UpdatesBody`'s `writeIntent`/`gateRefusal`, and its comments), `pwa/src/fleet/useUpdatesView.ts` (`isNodeElement`, `pendingTag`), `pwa/src/fleet/BuildLine.tsx` (comment), `shared/update-summary.ts` (`statedOf`'s docstring, `versionsSummary`), `server/src/watch.ts` (`sweptEnoughToDecide`, the `inventorySwept`/`pushReleaseAfterPoll` docstrings); pins in `pwa/test/settings-screen.test.tsx`, `pwa/test/use-updates-view.test.tsx`, `server/test/update-summary.test.ts`, `server/test/single-definition.test.ts` (the `statedOf is declared once` case), `server/test/push-copy.test.ts`; F-I's test comments in `pwa/test/update-banner.test.tsx` and `server/test/update-projection.test.ts`.
- Modify: `docs/superpowers/plans/2026-09-23-centralised-update-w3-settings-and-notification.md` — D-3303, D-3307, D-3313, D-3315, F-C's rows and F-I's two lines, in place.
- Modify (W4 C33): `server/src/update/inventory.ts` (`leaseActionFor`'s precedence), `server/src/coord/store.ts` (`releaseLease`/`settleNode`'s `stale-report` arms); pins in `server/test/update-inventory.test.ts`, `server/test/update-store-nodes.test.ts`.
...
- **From W3's review run 161 (W3 merged with these carried, by the bar its coordinator announced before that review).** W3's files are on `main`; this wave already edits `SettingsScreen.tsx`, `UpdateBanner.tsx`, and `watch.ts` (Tasks 8 and 5; `useUpdatesView.ts`, BuildLine and `shared/update-summary.ts` are this task's own), so the items land here. Anchor each by symbol.
  ...
  - **Pins with no red** (measure each red after adding it):
    ...
    - F-L: `update-summary.test.ts`'s "an omitted `stated` defaults to stated" case can never fail. Delete it or make it true, and remove the dead `versionsSummary` if nothing ships a call to it. If it goes, also correct the comments that name it (`server/src/update/notify.ts`'s, which this task then modifies, and `UpdateBanner.tsx`'s header) and `update-summary.test.ts`'s `versionsSummary` describe.
  - **Prose** (correct each against `main`):
    ...
    - F-I: "measured this poll"/"this run"/"this sweep" (statedOf's docstring and `update-summary.ts`'s header, BuildLine's comment, the tests' comments, W3's plan lines) where `statedOf` checks "ever measured".
...
- [ ] **Step 3: Prose.** Correct every open prose item (P2, P3, P5, P6, P7, P8, P11, the nits, review 149's F5, review 150's F5–F10, review 151's F3–F5, W3 review 161's F-A prose and F-C, F-E, F-F, F-H, F-I, F-M) in place. Where W2's or W3's plan is corrected, change only the named sentence. Their historical Interfaces snapshots stay as they are.
...
- [ ] **Step 5: Suites.** `cd server && ./node_modules/.bin/vitest run test/update-catalogue.test.ts test/update-resolve.test.ts test/update-routes.test.ts test/update-summary.test.ts test/single-definition.test.ts test/push-copy.test.ts test/update-inventory.test.ts test/update-store-nodes.test.ts`, then `cd ../pwa && ./node_modules/.bin/vitest run test/settings-screen.test.tsx test/use-updates-view.test.tsx test/build-line.test.tsx`. Expected: PASS. Task 9's full gate follows.
...
once("Update all — is shown disabled until the next\nrelease.",
     "Update all — opens one confirm sheet\n(below).",
     "W3 disabled sentence")

# (d) Wave 4's "Update and rollout" opener: an `auto` intent runs the verb unattended (Tasks 5, 7).
once("per box, explicit, never automatic.",       # the dash before it may end the line above (it does at 4b2ff904)
...
CI's server leg installs `agent/` and `pwa/` modules too, because `typecheck-tests.test.ts` spawns their compilers (the `server-deps` composite action every server job runs, `.github/actions/server-deps/action.yml` at `4b2ff904`); a workspace with only `server/node_modules` fails that one file rather than skipping it. So first:
...
gh run list -b main -w ci -e schedule --limit 5
```
Compare the server, agent and pwa totals against the newest of those whose test legs ran (a push to `main` is a map refresh with no test legs, `.github/workflows/ci.yml:12-13` at `4b2ff904`, and a daily run skips its legs when `main`'s head already carries a green `full-suite`) (`gh run view <id> --log | grep -E 'Test Files|Tests '`): the difference must be exactly this wave's new cases when that run's `headSha` (`gh run view <id> --json headSha`) is `git merge-base HEAD origin/main`; when it is older, the difference also carries every case merged between that head and the base, so name both shas in the wave-done — a green that ran fewer cases than `main` is not the same green. If a sharded run is killed on this box, CI's server shards are the arbiter (every server test while `CCRC_SELECTION` is `shadow`, `.github/workflows/ci.yml:20-21` at `4b2ff904`); say so in the wave-done.
...
```
Run it in the foreground and re-issue it until it returns: exit `0` is all green, `1` a failure (read the failing leg's log first — `gh run view --log-failed`), `8` still pending (an answer, not an error). Never end the turn to wait for CI; a session that does is not woken by the result. The macOS legs (`test-macos`, sharded, non-required, each cut at 55 minutes, `.github/workflows/ci.yml:466-487` at `4b2ff904`) can take most of an hour — re-issue the watch; do not merge around them. A red that also reds on the newest `main` `ci` run whose test legs ran (a daily `schedule` run — a push to `main` runs none, `ci.yml:12-13`) is `main`'s, not this PR's: name both runs in the wave-done.
...
The one tap needs a release to move TO. On the phone: `/settings` → channel `dev` (a prerelease is desired only on `dev`), then wait for the NEXT merge to `main` — any PR — to publish its prerelease (`gh release list --limit 1` names a tag newer than `W5_TAG`; call it `NEXT`), and tap **Check now** (the refresh door admits one catalogue poll per its derived interval, 212 s today (`REFRESH_MIN_INTERVAL_MS`, derived), counting the scheduled poll — a `429` `rate-limited` answer carries `retryAfterS`: wait that long and tap again); then open the fleet screen, where the banner lives (wave 3 renders `UpdateBanner` on `FleetScreen` only; `/settings` has no Update all). Expected: the banner reads `NEXT is out on dev — …`, and **Update all** is enabled. Tap it. Expected: the sheet is titled `Update NEXT`, lists `1. fleet (fleet) W5_TAG → NEXT` then `2. server (server) W5_TAG → NEXT`, and its confirm closes it.
