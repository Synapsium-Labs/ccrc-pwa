...
**Producers:** W2 — `git show 1288beec:docs/superpowers/plans/2026-09-22-centralised-update-w2-control-plane.md`; wave 3 — `git show d638c602:docs/superpowers/plans/2026-09-23-centralised-update-w3-settings-and-notification.md`; wave 4 — `git show 4b361c00:docs/superpowers/plans/2026-09-22-centralised-update-w4a-node-side.md`. ALL THREE MERGE BEFORE this wave is dispatched; every task assumes their code is on `main` and spells every shared name exactly as those plans do. Anchors into files none of them touches are at `d759c914`; anchors into files they change are by QUOTED producer text, never by line.
...
- **Cited files move no cited line** (ruling R13): `shared/api.ts` above `:7526`, `single-definition.test.ts` (`:32-37`, `:1274`, `:1303`, census 8) and README's anchors — every edit is appended after them or line-neutral, each task says which, and that task's green run includes `server/test/session-hook.test.ts`.
...
- `pwa/src/lib/api.ts` — `applyUpdate`, `rollbackUpdate` on `postJsonOr` (Task 8); the widened `UPDATE_ERROR_TEXT` (Task 6); `MOVE_DISABLED_TEXT` deleted (Task 8).
...
- `pwa/test/api.test.ts` — Task 6: `SENTENCES` gains the ten words, its length pins 12 → 22 and 10 → 20; Task 8: the route census reads six literals, the disabled-literal pin becomes a literal-absence scan, one describe for the two client methods.
...
- Modify: `shared/api.ts` — `InFlightReport`, a private `IN_FLIGHT_MAX_S` and `inFlightReport` APPENDED after the file's last line (wave 3's `export const UPDATE_GATE_CAP = 'update-gate';`), inside the end-of-file update block (D-3188, R13). No import line is added (`peers-claims-l0.test.ts:157-167` pins the three).
...
```
  ...
  The bound `99_999_999_999` is a PRIVATE `const IN_FLIGHT_MAX_S` with a comment naming its two twins. This is a correction the skeleton did not state: L0 imports nothing at runtime, and W2 declares the bound twice, as `inventory.ts`'s exported `REPORT_TIME_MAX_S` (L3) and as `resolve.ts`'s private `MAX_UNIX_S` (L1), neither of which this file may import. `resolve.ts` restates it for the same reason.
...
Expected:
- Each of the first five `git grep`s prints exactly one hit per pattern. The fourth prints the six Step 4 anchors of `shared/agent-protocol.ts` at their MERGED-tree lines (at `d759c914` they were `:13`, `:274`, `:278`, `:280`, `:288`, `:290`; W2 Task 8 moves every one but the first) — Step 4 edits at the lines printed here.
...
export interface InFlightReport { phase: UpdatePhase; target: string | null; startedAtS: number | null }
/** Unix SECONDS stay below this until the year 5138, and a 13-digit ms value
 *  never does. The same bound as `inventory.ts`'s `REPORT_TIME_MAX_S` and
 *  `resolve.ts`'s `MAX_UNIX_S`, restated because L0 imports nothing at runtime
 *  and neither of those files may be imported from here. */
const IN_FLIGHT_MAX_S = 99_999_999_999;
 ...
 *  integer ≤ `IN_FLIGHT_MAX_S`, else `null`, and the report is still kept. */
export function inFlightReport(text: string): InFlightReport | null {
  let parsed: unknown;
  ...
  return {
    ...
    startedAtS: typeof started === 'number' && Number.isSafeInteger(started) && started > 0 && started <= IN_FLIGHT_MAX_S
      ? started : null,
  };
}
```
...
**Files:**
- Modify: `agent/src/server.ts`. All anchors are at `d759c914`; none of W2, wave 3 or wave 4 touches this file, measured against their Files lists.
  ...
  - Value import line (`:30`): replaced by one import of `CCRC_DIR_NAME, NODE_FILES, parseCcdCaps, parseObservedEpochDoc, POOL_EPOCH_FILE_NAME, UPDATE_OP, UPDATE_SPAWN_TIMEOUT_MS, firstStderrLine, updateLauncherPath, updateSpawnArgv`, plus one new line after it: `import { inFlightReport, isReleaseTag, isRequestKind, type InFlightReport } from '../../shared/api.js';`.
  - `AgentOpts` (`:58-66`): gains `spawnUpdate?: UpdateSpawn` after `spawnPty?`.
  - `ReadyFrame` (`:145-171`): narrows `ops` to REQUIRED, the file's own `ccdVerbs`/`observedEpoch` idiom, with its docstring extended.
  - `fail` (`:183-185`): gains an optional `detail`, and `failUpdate` is added after it.
  - `UpdateSpawn`, `UPDATE_SPAWN_MAX_BUFFER`, `realUpdateSpawn` and `UpdateGate` go directly after `runExec` (`:274-293`), before `interface PtyEntry`.
  - `ConnCtx` (`:301-308`): gains `spawnUpdate: UpdateSpawn` and `updateGate: UpdateGate`.
  - `handleReq`: gains `case 'update'` directly before its `default:` (`:424`).
  - `ReqRefusal` goes directly above `validateReq`'s docstring (`:449`). `validateReq` gains `case 'update'` before its `default: return null` (`:520`), and its return type widens to `AgentReq | ReqRefusal | null`.
  - `UPDATE_REPORT_READ_MAX` and `readInFlightReport` go directly after `readObservedEpoch` (`:615-622`).
  - `handleConnection` (`:708-717`): takes `updateGate`, and its `ctx` carries both new fields.
  - The ready frame literal (`:757-759`, **CORRECTED from `:696-716`**, which is `refreshVerbs`): gains `ops: [UPDATE_OP]`.
  - The message handler (`:774-783`): sends `failUpdate(req.id, req.refuse)` for a `ReqRefusal`.
  - `startAgent` (`:827-852`, **CORRECTED from `:832-852`**; its `spawnPty:` default is `:838`): defaults `spawnUpdate: rawOpts.spawnUpdate ?? realUpdateSpawn`, makes ONE `updateGate` and passes it to every connection.
...
Expected:
- One hit per pattern in `agent/src/server.ts`, except `case 'ptyOpen': {`, which has two: `handleReq`'s (`:409`) and `validateReq`'s (`:512`). The `    default: {` hit is `handleReq`'s (`:424`); `validateReq`'s default is the bare `    default:` at `:520`.
...
```ts
import {
  CCRC_DIR_NAME, NODE_FILES, parseCcdCaps, parseObservedEpochDoc, POOL_EPOCH_FILE_NAME, UPDATE_OP,
...
Replace `ReadyFrame`'s docstring's last line and the type (`:167-171`, from ` *  compile while silently omitting it. */` through the closing `};`) with:
...
Directly after `runExec`'s closing `}` (`:293`), before `interface PtyEntry {`, insert:
...
In `handleReq`, directly before `    default: {` (`:424`), insert:
...
Directly after `readObservedEpoch`'s closing `}` (`:622`), insert:
...
Replace `handleConnection`'s signature and its `ctx` literal (`:708-717`) with:
...
Replace the ready frame literal (`:757-759`):
...
In the message handler, directly after the `if (!req) { … return; }` block (`:776-783`), insert:
...
const meas = (over: Partial<NodeMeasurement> = {}): NodeMeasurement => ({
  ...
  previousVersion: 'v0.0.8', os: 'linux', measuredAt: T0, report: null, ...over,
});
...
const superseded = (): CoordStore => {
  ...
  expect(s.rekeyNode('fleet', UUID_A)).toEqual({ ok: true, how: 'superseded', retired: 0 });
  return s;
};
...
```ts
  it('finds every W2, W3 and W5 writer writing — a renamed table or a rewritten call shape reds this, not disarms it', () => {
    const found = new Set(stmts.map((s) => s.method));
...
- `./node_modules/.bin/vitest run test/update-writer-groups.test.ts` — Expected: PASS, 14 cases (wave 3's count: no case added; the floor now finds all three W5 writers, and "every column a statement writes is owned by a group its method writes" reads `requestNode`'s statement as the request group's and `dispatchNode`'s and `noteDispatchRefusal`'s as the lease group's).
- `./node_modules/.bin/vitest run test/mail-routes.test.ts -t 'every quoted kebab token'` — Expected: FAIL, `no-request is not a declared MailRejectCode, RunRefuseCode, LifecycleGapReason, ClaimRefuseCode, SessionLifecycle, ReclaimRefuseCode, AskRefuseCode, RunRouteRefuseCode, SetAccountPoolsRefuseCode or UpdateStoreRefuseCode` (the message tail W2 Task 4 and wave 3 Task 1 quote): `DispatchNodeResult`'s `'no-request'` is the first undeclared single-quoted kebab token the scan reaches in `store.ts` — `'unknown-node'` and `'bad-tag'` before it are W2's, and `RequestNodeResult`'s `'bad-kind'` is already a `MailRejectCode` (`shared/api.ts:4965`), admitted by the chain's first clause. This red is the measurement that the scanner sees the new arms; Step 5 fixes it and touches no test file.
...
export interface DispatchRow {
  ...
  currentVersion: string | null; highestVersion: string | null;   // CORRECTED: + highestVersion, the floor (D-3403)
...
const fleet = (o: Partial<DispatchRow> = {}): DispatchRow => ({
  ...
  currentVersion: 'v0.0.9', highestVersion: 'v0.0.9', stampRead: 'ok', caps: W4_CAPS, agentOps: [UPDATE_OP],
...
describe('the L0 refusal vocabulary and the dispatch order (design 2026-09-20 §9/§10)', () => {
  it('DISPATCH_REFUSALS is the ten words, in order, with no duplicate', () => {
    expect([...DISPATCH_REFUSALS]).toEqual([
      'unknown-tag', 'not-newer', 'refused-by-node', 'stamp-unread', 'no-detach-cap',
...
describe('planDispatch — the direction and the met request (§18 "`apply` refuses an older tag…", Review Focus 1)', () => {
  ...
  it('an unversioned node (stamp read, no version, no floor) is never "not newer" and moves with the spec\'s detail', () => {
    ...
    const p = plan(view(fleet({ currentVersion: null, highestVersion: null, ...ask('v0.0.9') })));
  ...
  it('an unversioned node WITH a floor is not-newer at or below it; with none it is never not-newer (D-3403)', () => {
    ...
    expect(refusalOf(view(fleet({ currentVersion: null, highestVersion: null })), 'update', 'v0.0.9')).toBeNull();
...
// added either — the ten words are stated here as a LITERAL, on purpose (the
// W2 `VOCABS` stance): `update-dispatch.test.ts` holds L0's array equal to
...
import {
  ...
  type AutoMode, type DispatchRefusal, type NodeRole, type RequestKind, type StampRead, type UpdateChannel,
  type UpdateState,
} from '../../../shared/api.js';
import { UPDATE_OP } from '../../../shared/agent-protocol.js';
 ...
 *  unconstrained), read only through W2's `floorOf`. */
export interface DispatchRow {
  nodeId: string; role: NodeRole | null; label: string;
 ...
 *  halted; (update) stamp-unread, not-newer — the target not strictly newer than the node's floor, W2's
 *  floorOf (D-3403); unknown-tag — for an update, no listed (bundleListed), unyanked
...
triggerDispatch(): void;                     // void this.dispatchNow().catch(() => {})
...
const fleetMeas = (over: Partial<NodeMeasurement> = {}): NodeMeasurement => ({
  ...
  highestVersion: 'v0.0.9', previousVersion: 'v0.0.8', os: 'linux', measuredAt: T0, report: null, ...over,
});
 ...
 *  settled or released by the same plan and the same writers production uses. ONE difference, deliberate: the
...
```ts
  ...
  triggerDispatch(): void {
    void this.dispatchNow().catch(() => { /* one bad dispatch run must not kill the caller */ });
  }
...
- Modify: `pwa/test/api.test.ts` — *correction* (the skeleton said only "its census follows"): wave 3's `SENTENCES` table (typed by the same `Record`, so it too is a compile error until it moves) gains the same ten entries after its `'journal-unwritable'` entry; `expect(entries, 'guards the guard — an empty census passes everything').toHaveLength(12);` → `22`; `expect(updateOnly, 'guards the guard').toHaveLength(10);` → `20` and its comment's "The ten words" → "The twenty words"
...
export function parseApplyBody(body: unknown): ParsedApplyBody;         // exactly one of nodeId (non-empty string) / all (=== true) — both → bad-request 'all', neither → 'nodeId'; an unknown key → bad-request; tag: absent → null, a string off RELEASE_TAG → bad-tag, anything else (null included) → bad-request 'tag'
...
grep -n "^import { describe, it, expect, afterEach } from 'vitest';$\|^import { UPDATE_GATE_CAP } from '../src/update/resolve.js';$\|push?: { notify: (p: PushPayload) => Promise<void> } } = {},$\|^    ...(o.push ? { push: o.push as never } : {}),$\|^describe('POST /api/updates/refresh announces what its poll listed" server/test/update-routes.test.ts
```

Expected: `5`; five hits in `auth-gate.test.ts` (one per pattern); one hit in `gate.ts`; one `UPDATE_DOORS` line with the four W2 paths; `1`; one hit each for the Task 1–5 exports; and five hits in `update-routes.test.ts`, one per pattern, the `describe` the LAST of them (nothing is appended after wave 3's describe but its closing `});`). Any other answer means a producer landed differently from its plan: stop and re-read the landed code before writing a line — every edit below is anchored on these texts.
...
describe('POST /api/updates/apply — one named node (spec §12)', () => {
  ...
  it('409 not-newer for a tag older than current; an unversioned node is never "not newer" (§18)', async () => {
    ...
    plant(f.coord, serverNode({ currentVersion: null, highestVersion: null }));
...
describe('POST /api/updates/rollback (spec §12)', () => {
  ...
  it('409 no-previous when previousVersion is NULL — the route never goes looking for one', async () => {
    const f = await open();
    catalogue(f.coord);
    plant(f.coord, fleetNode({ previousVersion: null }));
    desire(f.coord, FLEET_ID, 'v0.0.10');
    const r = await post(f.app, '/api/updates/rollback', { nodeId: FLEET_ID });
    expect(r.statusCode).toBe(409);
...
In `server/test/update-routes.test.ts` (W2 Task 13's file, as wave 3 Task 3 leaves it) — four in-place edits, then one append:

- replace `import { describe, it, expect, afterEach } from 'vitest';` with `import { describe, it, expect, afterEach, vi } from 'vitest';`;
...
Expected: FAIL in `tsc`, twice — `src/lib/api.ts` (`UPDATE_ERROR_TEXT`) and `test/api.test.ts` (`SENTENCES`), each `error TS2740: Type '{ … }' is missing the following properties from type 'Record<Exclude<UpdateRouteError, "unauthenticated">, string>': …` naming the new words (`"unknown-tag"`, `"not-newer"`, `"refused-by-node"`, `"stamp-unread"`, and the rest). D-3302's keying is doing its job.

- [ ] **Step 6: The ten sentences, and the census that counts them**
...
`pwa/test/api.test.ts` — in wave 3's `SENTENCES` table, directly after `    'journal-unwritable': 'The server cannot write its intent journal — nothing was changed.',`, the same ten entries at the table's four-space indent:
...
and in the same file replace `    expect(entries, 'guards the guard — an empty census passes everything').toHaveLength(12);` with `    expect(entries, 'guards the guard — an empty census passes everything').toHaveLength(22);`, replace `    // The ten words no other table owns. `not-configured` and `bad-request`` with `    // The twenty words no other table owns. `not-configured` and `bad-request``, and replace `    expect(updateOnly, 'guards the guard').toHaveLength(10);` with `    expect(updateOnly, 'guards the guard').toHaveLength(20);`. (None of the ten words is owned by `apiErrorText`, `sendErrorText`, `submitErrorText`, `uploadErrorText` or `kickoffErrorText` — measured at `d759c914`: `grep -rn "'halted'\|'unknown-tag'\|'not-newer'\|'refused-by-node'\|'stamp-unread'\|'no-detach-cap'\|'no-rollback-cap'\|'agent-predates-update-op'\|'no-previous'\|'no-desired'" pwa/src` → no hits — so the pass-through case holds for all twenty.)
...
(a) Imports — merge into W2's existing lines in place: `CoordStore` and `RequestNodeResult` into the `import type { NodeRow, ReleaseRow, SetIntentResult, UpdateIntentPatch, UpdateIntentRow } from '../coord/store.js';` line, as bare names (the clause is already `import type` — an inline `type` there is TS2206); `SETTLED_UPDATE_STATES`, `compareDispatchOrder` and `type DispatchRefusal, type MoveRequestAnswer, type MoveSkip, type MoveSkipWhy, type RequestKind` into the `'../../../shared/api.js'` block; and two new lines after `import { SERVER_LABEL, buildInfoOfRow } from './inventory.js';`:
...
/** An OPTIONAL tag field through the one guard: absent → null; a string on the tag shape → itself; a string
 *  off it → `bad-tag` (§12's own answer); anything else — `null` included, because absence is spelled by
...
function tagField(o: Record<string, unknown>, field: string): TagField {
  ...
  if (!isReleaseTag(v)) return { ok: false, error: 'bad-tag', field };
...
function singleNodeMove(coord: CoordStore, nodeId: string, kind: RequestKind, named: string | null): SingleMove {
  ...
  if (target === null) {
    if (kind === 'rollback') return { ok: false, code: 409, body: { error: 'no-previous' } };
    return {
      ok: false, code: 409,
 ...
 * (D-3385), or a word the node cannot outgrow by waiting, `no-detach-cap` and the rest
...
const fleetNode = (over: Partial<NodeMeasurement> = {}): NodeMeasurement => ({
  ...
  currentBuiltAt: '2026-09-22T00:00:00Z', currentDirty: false,
  stampRead: 'ok', installState: 'complete', provenance: 'verified',
  caps: NO_GATE, agentOps: ['update'], highestVersion: 'v0.0.9', previousVersion: 'v0.0.8', os: 'linux',
...
// refused, a missing capability — MoveSkipWhy, shared/api.ts); the 202 names
...
stamp, halted, the node's own lease busy, a missing capability, an agent that predates the op, an unknown or refused
tag, no previous version, no desired tag); `{all: true}` always answers `202`, writing a request only for a node the
tag takes forward and the dispatcher could move, and naming every other live node as skipped, with its word.
