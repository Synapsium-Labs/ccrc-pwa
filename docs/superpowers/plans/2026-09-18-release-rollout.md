...
| 5 | `--to` derived per box (`_upd_resolve` moved inside the update loop) | pins the version first (single-curl assertion) | RED (2/12 failed) |
| 5 | role preflight removed | role case | RED (1/12 failed) |
| 5 | stop-on-failure DISABLED (`if [ "$rc" -ne 0 ]` → `if false`) | box one fails → stop | RED (1/19 failed) — RE-MEASURED in the final fix wave. The first measurement used `if :`, which is not a removal: `:` is TRUE, so the branch fired on EVERY box and the loop stopped after box one whether or not it failed. That mutation broke five unrelated happy-path cases (the 7/12) and left the stop-on-failure branch itself unexercised. `if false` is the removal — the branch never fires — and exactly one case reds: `box one fails → stop: box two is never touched, exit 1, the message says so`. |
| 5 | verify from update stdout instead of `ccrc version` | verification disagreeing | RED (6/12 failed) |
...
| 8 | `diff -r -q` → `[ -d ... ]` existence check | one edited byte | RED (1/1 failed — the stale byte no longer trips the comparison) |
| 8 | compared against `$HOME/.cc-sessions/$name` instead of `$tree/$name` | one edited byte | RED (1/1 failed — the fixture plants no `.cc-sessions` tree, so ALL THREE skills report "differs" instead of just the edited one: `FAIL skills: 3 installed skill(s) differ … claude: ccrc-coordinator differs; claude: ccrc-worker differs; claude: ccrc-reviewer differs`) |
| 8 | `homes -eq 0` skip removed | skips, never passes vacuously | RED (1/1 failed — reports `PASS skills: 0/0 homes …` instead of SKIP) |
| 8 | `skills` removed from the table | table-is-data `ORPHAN _check_<name>` pin | RED (1/1 failed — `ORPHAN _check_skills`) |
| 8 | (fix round 1, R10) remedy's second arm reverted to bare installer invocations (no `CCRC_SKILL_SRC`) | new pin: 'the remedy names the override seam, not the bare installer (R10)' | RED (1/1 failed) |

### Final fix wave (F1–F18) — every row measured on a scratch byte-copy, applied then restored

| Finding | Mutation | Case | Result |
|---|---|---|---|
| F1 (R11) | `_rollout_role`'s `rel="${BOX_ENV_FILE#"$HOME/"}"` → a second literal `rel=".ccrc/ccrc.env"` | `ccrc install: the env file is named once in the whole CLI > install writes the file status and doctor read — through BOX_ENV_FILE` | RED (1 failed \| 123 skipped, `-t "named once"`) |
| F2 (R12) | the `skills` FAIL remedy's three installer scripts pointed back at `$HOME/.cc-sessions/install-*-skill.sh` | `pool-name-parity.test.ts > ccd/ccrc-doctor-checks names .cc-sessions/<dir> exactly once…` | RED (1/19 failed) |
| F2 (R12) | the same mutation, doctor side | `ccrc doctor: skills … > fails on ONE edited byte…` and `> the remedy names the override seam…` | RED (2 failed \| 6 passed \| 430 skipped, `-t "skills"`) |
| F5 | the converged gate removed (`_upd_converged` opens with `return 1`) | `the "already there" gate > three matching shas → nothing to do: … NO SWEEP` | RED (1/33 failed). The assertion is no longer vacuous: `converged()` now plants the KillMode=process drop-in and two live units, and the new control case below measures the try-restart that arming produces. |
| F5 (control) | *(none — a control, not a mutation)* | `…and on that same fixture the sweep really does try-restart — so the skip case's "no sweep" is a real absence` | GREEN on the identical fixture with `--force`; the recording contains `--user try-restart claude-session@*`, which is exactly what the skip case asserts is absent |
| F6 | rollout step 6's `/health` block deleted | `both boxes report the target but the server /health still answers the OLD version → exit 1, naming /health` | RED (2/19 failed — the new case plus the happy path, which pins the `/health` ssh call) |
| F7 | a SECOND literal spelling of the record (`[ -f "$BOX_INSTALLED_FILE" ]` → `[ -f "$HOME/.ccrc/installed" ]`) | `one bash spelling of ~/.ccrc/installed > the ccrc CLI spells the path once…` | RED (1/158 failed) |
| F8 (R13) | rollout step 3 reverted to `2>&1` + "the check line is line ONE" | `ssh's own stderr chatter is relayed, not parsed: the rollout still reads the check line and proceeds` | RED (1/19 failed) |
| F9 (R13) | the published-release-name shape gate removed | `a published release name that is not vX.Y.Z-shaped refuses at exit 2 — it never reaches a remote shell` | RED (1/19 failed) |
| F10 | a write added to the `--check` arm (`touch "$HOME/.ccrc/checked"`) | `writes NOTHING under HOME (spec §11) — the whole home is byte-identical…` | RED (1/33 failed) |
| F13 | the `--check`/`--force` exclusivity refusal removed | `--check and --force are exclusive — one refusal, exit 2, nothing fetched` | RED (1/33 failed) |
| F14 | the dead clause put back as a REAL one (`/health` wrapped in `if [ "$single" -eq 0 ]`) | `a single-box fleet (same host in both coordinates) needs CCRC_ROLE=both and updates once` | RED (1/19 failed) — which is the finding's point from the other side: every shape reaches this block, so a guard here can only wrongly skip one |
| F15 | the `skills` FAIL headline reverted to "differ from the shipped tree" | `fails on ONE edited byte…` and `a missing skill directory is a FAIL in its own words` | RED (2 failed \| 6 passed \| 430 skipped, `-t "skills"`) |
| F16 | `grep -E "$SHAPE"` removed from `release-main.sh`'s HIGHEST derivation | `highest ["v0.0.1","vnext","v1.2","v1.2.3-rc1"] → next v0.0.2` | RED (1/14 failed). The OLD row's tags (`wip`, `backup/x`) are excluded by the `v*` glob alone, so that row stayed GREEN under this mutation and pinned nothing. |
| F18 | `_rollout_role` discards ssh's stderr again (`2>>"$ROLLOUT_ERRF"` → `2>/dev/null`) | `an unreachable fleet box is distinguished from a role-less one` | RED (1/19 failed) |

**F17 ships no row, deliberately.** `side()`'s two predicates are collapsed into one (`typeof b.version === 'string'`); for every value TypeScript admits on `BuildInfo.version` (`string \| undefined`) the old pair and the new single predicate agree, so no mutation of the shipped types can separate them and any row here would be a control dressed as a measurement. The existing Task 7 rows (`warn = false` → amber case RED) still bite.
...
- **D-3017** — Task 3: adding `_inst_installed` to `cmd_install`'s spine broke a pre-existing pinned test, `'ccrc install: the order is stated in one place' > 'cmd_install is the sequence, and the roster precedes the ccd it installs'` (`server/test/ccrc-install.test.ts`), which asserts the exact step sequence with `.toEqual([...])`. The brief did not mention this pin. Closed by appending `'_inst_installed'` to that array (with a one-line comment explaining why it is last), the smallest faithful fix that keeps the pin accurate rather than leaving it stale/red; the pin's own load-bearing ordering (deploy.sh's "roster before ccd") is otherwise untouched. Originally recorded as `D-TBD-<slug>` on the (false) premise that `~/.local/bin/ccrc-api` was unreachable from the implementer's sandbox — a fix-round re-check from this worktree found the allocator reachable (`ledger list` returned `{"ok":true,"floor":3017,...}`), so this number was minted for real rather than left as a placeholder.
- **D-3018** — Task 3: the brief's own test snippet for `server/test/ccrc-cli.test.ts` (the `install: complete` case) calls `rmSync(join(home, '.ccrc', 'build.json'))`, but that file did not import `rmSync` from `node:fs` before this task. Not a design departure — a missing import the brief's snippet silently assumed — closed by adding `rmSync` to the file's existing `node:fs` import line. Originally recorded as `D-TBD-<slug>` on the same false unreachable-allocator premise as D-3017; minted for real in this fix round once the allocator was confirmed reachable.
...
- **D-3020** — Task 5: the brief's own `server/test/ccrc-rollout.test.ts` fixture, `plantHost`, only ever WRITES a host's `role` file when `o.role` is given (`if (o.role !== undefined) writeFileSync(...)`) — it never removes one. The role-preflight test calls `plantHost(home, SERVER, { version: 'v1.0.0' })` with the comment `// role file absent`, but `twoBoxFleet` had already planted `role: 'server'` for that same host in the same test's setup, so the second call left the STALE `role='server'` in place: the case that means to test "no recorded role" was actually testing "role correctly recorded", the fleet's own boxes were both correctly roled, and the rollout ran to completion (exit 0) instead of refusing at exit 2 as the assertions expect. This is a fixture bug, not a `cmd_rollout` defect: the comment states the intent, the code doesn't deliver it. Closed by the smallest faithful fix — `plantHost` now removes an existing `role` file when `o.role` is omitted (`else if (existsSync(roleFile)) rmSync(roleFile)`), making a re-plant with no role argument actually leave the role absent, matching its own documented behavior. No other call site passes `role: undefined` to a previously-roled host, so this changes nothing else in the suite.
- **D-3021** — Task 6 (fix round 1): the brief's Context claimed `shared/api.ts` already imported `type { BuildInfo } from './buildinfo.js'`; it did not, so the original implementer added that import (and reworded the file's load-bearing header comment) to make `FleetHealth.builds` typecheck. This broke a pre-existing L0-purity pin, `server/test/peers-claims-l0.test.ts`'s `'shared/api.ts has exactly one import line, and it is a type'`, which asserts the exact one-element array over every `^import` line in the file — measured RED at 70063d59 in isolation (`Tests 1 failed | 18 passed (19)`). The report shipped calling this "only an import" with "[n]o behavior or interface changed", which is wrong: the import is exactly the interface this guard pins, and the guard was never updated. Not recorded as a deviation at the time. Closed in this fix round by updating the pin to the shipped two-import reality (`server/test/peers-claims-l0.test.ts`) — both lines asserted as `import type`, preserving the guard's purpose (L0 stays type-only, no runtime/`node:*` import) rather than loosening it — and by minting this number for the departure the original round should have recorded. Re-measured: `cd server && ./node_modules/.bin/vitest run test/peers-claims-l0.test.ts` → all green (see Mutation measurements below for the guard's continued bite).
- **D-3022** — Task 7 (fix round 1): `FleetHostBanner.tsx`'s skewed-arm `name()` helper renders `${b.version ?? 'unversioned'} (${b.sha.slice(0, 8)})` — the sha appended for EVERY side, versioned or not — where the brief's Step 3 code sketch (task-7-brief.md:194-195) reads `b.version ?? \`unversioned (${b.sha.slice(0, 8)})\`` (no sha once a version is present). The original report (task-7-report.md) argued, correctly, that the brief's own sketch cannot satisfy the brief's own banner test (`getByText(/fleet v0\.0\.7 \(bd2bf57a\)/)` requires the sha even when versioned) and shipped the working banner formatter with a matching `BuildLine` formatter that DOES drop the sha once versioned — but declined to record either shipped formatter as a spec departure, on the reasoning that a bug in the brief's illustrative code needs no deviation number. That reasoning is wrong for the banner: the shipped `name()` is user-visible banner text that departs from the brief's own written Step 3 code, and the brief text is binding regardless of whether its own sketch also fails its own test — the departure needed recording the moment the report chose different behavior, not a judgment call the implementer gets to skip. `BuildLine`'s `side()` is unaffected: the brief supplies no code sketch for it, only the test's expected strings, so there is no departure to record there. No code change accompanies this entry — the shipped behavior (sha on every banner side) is being ratified as the deliberate, tested departure it always was, closing the missing paper trail rather than the code.
- **D-3023** — Task 8: `_check_skills` compares each home's installed copy against `$BOX_TREE_DIR/ccd/<name>-skill` with `diff -r`, which requires `diff` on PATH — a binary this suite's contained fixture PATH (`ghContainedEnv`/`harnessBin`) does not otherwise carry. Neither the brief's `installCcrc` planting nor its `healthy()` planting mentions it, so without a third addition `skills` WARNs "diff is not on PATH" on every fixture in this file, including `healthy()` itself, which the suite's own summary-count pins require to PASS every check. Closed by adding one more line to `healthy()`, `linkReal(home, 'diff')`, the same pattern the fixture already uses for `jq`/`stat`/`date`/`realpath` — a fixture completeness gap, not a `_check_skills` design departure.
- **D-3025** — Task 8: `_check_skills`' role guard originally read `[ -r "$BOX_ENV_FILE" ] && role="$(_box_env_value "$BOX_ENV_FILE" CCRC_ROLE)"`, per the brief's own snippet. `-r` is true for a directory as well as a regular file, and `_box_env_value`'s `while IFS= read -r line … done < "$file"` on a directory fails immediately ("read error: 0: Is a directory") without ever assigning `line`, which then reads as an unbound variable under this file's `set -u` and kills the whole `ccrc doctor` subshell — not a `skills` verdict, the entire run. This is not hypothetical: `server/test/ccrc-doctor.test.ts`'s pre-existing `ccrc doctor: config > a ccrc.env that is not a regular file is its own answer too` case plants exactly that fixture (a directory at `~/.ccrc/ccrc.env`) and went red once `_check_skills` joined the table, reproducibly and in isolation. Every OTHER `BOX_ENV_FILE` reader in this file already guards `-f` before `-r` (`_check_config`, `_check_exposure`, `_check_build`, `_check_fleet`); `_check_skills`' guard was the one exception, introduced by this task rather than carried over from an existing pattern. Closed by matching the file's own established order: `[ -f "$BOX_ENV_FILE" ] && [ -r "$BOX_ENV_FILE" ] && role=...`.
- **D-3027** — Task 8, fix round 1 (RULING R10, controller): the `skills` FAIL remedy's second arm originally re-ran the bare `install-{coordinator,worker,reviewer}-skill.sh` scripts, which each default their source to `$HOME/.cc-sessions/<name>-skill` when `CCRC_SKILL_SRC` is unset — the SAME placed copy that was measured stale on 2026-09-17, not the `$BOX_TREE_DIR/ccd/<name>-skill` tree `_check_skills` itself just compared against. A box whose `.cc-sessions` copy lagged the tree would run this remedy, converge to the stale copy, and still FAIL on the very next `doctor` run. Closed by setting `CCRC_SKILL_SRC="$HOME/ccrc/ccd/<name>-skill"` (the installer's documented override seam, `ccd/install-*-skill.sh:22-27`) on each of the three invocations in the printed remedy string, so the remedy converges from the tree the check measured; `ccrc update` remains the first, preferred arm (re-places `.cc-sessions` from the tree and re-runs the installers, closing the gap at its source). Covered by a new case, `'the remedy names the override seam, not the bare installer (R10)'`, and by strengthening the existing 'installer runs it green again' case to invoke the reviewer installer with `CCRC_SKILL_SRC` pointed at the fixture's own tree path (`join(home, 'ccrc', 'ccd', 'reviewer-skill')` — what `$HOME/ccrc/ccd/reviewer-skill` resolves to on that box) rather than the repo checkout path; the installer SCRIPT itself is still invoked from the repo path, since the fixture plants no `.cc-sessions` copy of the installer scripts, only of the skill trees they read from. See the mutation table (Task 8, fix round 1 row) for the RED/GREEN measurement.

### Final fix wave (F1–F18)

One block of twelve, minted in one act by `~/.local/bin/ccrc-api ledger allocate` and defined here in the same act. Allocator response, verbatim: `{"ok":true,"numbers":[3042,3043,3044,3045,3046,3047,3048,3049,3050,3051,3052,3053],"floor":3054}`.

- **D-3042** — RULING R11 (F1). Spec §4 step 1 says this verb reads "the one `^CCRC_ROLE=` line of `~/.ccrc/ccrc.env`", and `_rollout_role` spelled that path literally inside its single-quoted remote command. That second spelling reddened a pre-existing pin, `server/test/ccrc-install.test.ts`'s `the env file is named once in the whole CLI` (D-88's rule: prose may name the path anywhere, a LINE OF SHELL may name it once), and the branch tip shipped RED on it. The shipped code now DERIVES the remote path — `rel="${BOX_ENV_FILE#"$HOME/"}"`, re-prefixed with the REMOTE `$HOME` inside the command so the remote still expands its own home. A departure from the spec's literal text (the spec names a path; the code names a variable), and the stronger reading of it: a rename of `BOX_ENV_FILE` now reaches this verb through the single definition rather than past it.
- **D-3043** — RULING R12 (F2). Spec §6's `skills` check and its remedy are written against the installers as `$HOME/.cc-sessions/install-*-skill.sh`. Three such spellings in one FAIL remedy reddened `server/test/pool-name-parity.test.ts`'s `ccd/ccrc-doctor-checks names .cc-sessions/<dir> exactly once` — the branch tip's second pre-existing RED. The remedy now names the SHIPPED TREE's own installers (`$HOME/ccrc/ccd/install-<role>-skill.sh`) beside the `CCRC_SKILL_SRC` sources R10 already moved there, so both halves come out of the tree the check measured; `ccrc update` remains the first, preferred arm. `installCcrc` (`server/test/ccrc-doctor.test.ts`) now symlinks the three installers into the fixture's tree, and the "remedy works" case runs EXACTLY the printed command shape — `join(home,'ccrc','ccd','install-reviewer-skill.sh')` under `CCRC_SKILL_SRC=join(home,'ccrc','ccd','reviewer-skill')`, through `ghContainedEnv` — rather than the repo's copy, which would have stayed green with a remedy naming a script no box has.
- **D-3044** — RULING R13 (F8), the stderr half. Spec §4 step 3 has `rollout` read `ccrc update --check` off each box with the check line as the FIRST line of the output; the shipped code captured `2>&1` to satisfy that. An ssh `Warning: Permanently added …` or a login banner — neither of them the remote ccrc's doing — therefore refused the rollout and BLAMED THE BOX for the transport. Step 3 now captures STDOUT alone for parsing and RELAYS the remote's stderr to ours with that box's own label (`sed "s/^/$label: /"`), because an operator needs to see a host-key warning rather than have it swallowed.
- **D-3045** — RULING R13 (F8), the parse half, recorded separately because it changes what the fixed-shape contract MEANS. Spec §4 step 3 and §6 both say `--check`'s **first line** is fixed-shape for `rollout` to parse. The shipped code now selects the first line matching `^check: ` ANYWHERE in stdout (`grep -m1 '^check: '`) and refuses only when no such line exists. `ccrc update --check` still prints it first; this relaxes the READER, not the writer, so a box that prefixes anything of its own stays readable.
- **D-3046** — RULING R13 (F9). Nothing in the spec asks for it: `ROLLOUT_VERSION` comes out of a DOWNLOADED `SHA256SUMS` (`_upd_resolve` takes it off the tarball name, which only has to match `ccrc-*.tar.gz`) and is interpolated unquoted into a command a REMOTE shell parses. `--to`'s own value is shape-checked at parse time; the published name was not. The shipped code now validates `^v[0-9]+\.[0-9]+\.[0-9]+$` the moment `_upd_resolve` returns and refuses at exit 2, before any box is touched.
- **D-3047** — RULING R13 (F14). Spec §4 step 6 re-measures `/health` "on the server box", and the shipped guard asked `[ "$single" -eq 1 ] || [ "${labels[*]}" != "${labels[*]/server/}" ]`. The second clause is TRUE for any `labels` containing `server`, i.e. for both two-box orders, and the first covers the only other shape — so no input could answer `no`. The guard is gone; `/health` is re-measured unconditionally on `$CCRC_BOX`. A condition no input can falsify is not a guard, it only hides which boxes reach the block.
- **D-3048** — F13, a refusal the spec does not describe. `ccrc update --check --force` silently dropped `--force`: the `--check` arm returns before `force` is ever read. Two different acts named in one argv, and which one the operator meant is not ours to guess, so the pair is now refused at exit 2 (`--check and --force are exclusive`), before anything is fetched. Spec §6 and §11 describe each flag alone and say nothing about the pair.
- **D-3049** — F18, beyond the spec's refusal text. Spec §4 step 1's refusal names the exit code; `_rollout_role` sent ssh's stderr to `/dev/null`, so an operator got `unreachable over ssh (exit 255)` and nothing else — no way to tell a wrong key from a wrong port from a host that is down. ssh's stderr now goes to `$ROLLOUT_ERRF` (a FILE, not a variable: every caller reads this function through `$( )`, and a subshell cannot hand an assignment back), and its last non-blank line is quoted into the refusal as ` — ssh said: <line>`. Silence stays visibly different from a message rather than printing an empty quote.
- **D-3050** — a departure from the FINDINGS' literal text, not the spec's, recorded because it changes what a pin proves. F9 asks the bad-release-name case to assert "no ssh at all". That is not assertable and must not be faked: the role preflight is spec §4 STEP 1 and precedes the version pin (step 2) by design, so two `CCRC_ROLE` reads have already happened when the refusal fires. The case asserts the property that actually matters instead — no ssh call carries the malformed name, and no `--check` ran — and says so in its own comment. Loosening the intent was the alternative; naming the reason was the choice.
- **D-3051** — RULING R15. Spec §4 step 0 said the ssh argv is `deploy.sh`'s, `ssh -p "$CCRC_SSH_PORT" -i "$CCRC_SSH_KEY"`, and the shipped `ROLLOUT_SSH` adds `-o BatchMode=yes`. The spec now RATIFIES that flag with its reason: the verb is unattended by construction, so an ssh that stopped to ask for a passphrase or a host-key confirmation would hang a rollout mid-fleet with no terminal to answer it; BatchMode turns that into a refusal the verb can report.
- **D-3052** — RULING R14. The claim "`deploy.sh` stamps no `version`" is FALSE and was carried in four places: spec §1's measurement row, spec §5's `(those stamps carry no version either)`, spec §6's "this is what makes a `deploy.sh` box *look* second-class", and the same sentence in CLAUDE.md's Deploy bullet and README §Deploy. `stamp_build` derives `BUILD_VERSION` from `git tag --points-at HEAD` filtered to `^v[0-9]+\.[0-9]+\.[0-9]+$` and writes the field whenever one matches — which auto-tagging makes the ordinary case on `main`. What a `deploy.sh` box never carries is §5's completed-install record, `~/.ccrc/installed`, which is why `ccrc version` reports `install: incomplete` there and `update --check` reports `incomplete` (or `unversioned`, when the stamp really has no `version`). All five sites now say that; README's §Deploy opening is line-neutral (five lines for five, `wc -l README.md` still 3195) and §Releases' preamble is scoped to the install/update verbs, which is what runbook step 12 actually proves.
- **D-3053** — F15, a wording departure with a caveat recorded here rather than buried. `_check_skills` entries read either `<id>: <skill> differs` or `<id>: <skill> missing`, while the FAIL headline said only "differ from the shipped tree" — so a run whose every entry was `missing` sent the operator hunting a diff that does not exist. The headline now reads "differ from, or are missing in, the shipped tree", the wording the ruling prescribes, applied verbatim. THE CAVEAT: read strictly, "missing IN the shipped tree" points at the wrong side — a `missing` entry means the skill is absent from the HOME, while the shipped tree has it (`_check_skills` FAILs separately, with its own sentence, when the TREE lacks one). The ruling's text is shipped as given rather than silently improved; a later wave that wants the strictly-correct phrasing should rule on it rather than treat this entry as licence.
- **D-3105** — Task 10 step 5, the spec §7 rehearsal against the REAL `v0.0.2` artifact (fixture HOMEs; the boxes' own `ccrc` binaries, `bd2bf57a` fleet and `53e6a438` server; real `npm`): every phase — the fleet box's first move, the server box's, and the contributor's `install.sh --release` — died at `npm ci in $HOME/ccrc/server`. `server/package.json`'s `postinstall` runs `node scripts/fix-node-pty-helper.mjs` (added `2b840d93`, 2026-08-25, the day AFTER `v0.0.1`), and `build-release.sh`'s pathspec named only each package's `package.json` and lock, so the tarball carried no `server/scripts/`. The staged install dies after `install: tree: placed at $HOME/ccrc`: a live box would have been left holding the new tree with no runtime deps — the fleet box first, where `rollout` stops. `v0.0.2` is unusable as published; the next patch supersedes it. Fix: `build-release.sh` packs every tracked `<pkg>/scripts/` and refuses, before any artifact is written, any install-time hook (`preinstall|install|postinstall|prepare|prepublish`) naming a path the release set does not hold. `build-release.test.ts` pins both in the fixture and, in a third case, pins the real packages' hooks to tracked files under `<pkg>/scripts/` so the refusal lands in a PR's CI rather than in `release-main.yml`. Measured: pathspec addition absent → 2 red (the layout case and the scripts case); hook loop absent → 1 red (exit 0 where 1 is expected); the real hook pointed at `tools/` → 1 red; all 22 green with the fix. Neither `install-sh.test.ts` nor `ccrc-update.test.ts` could have seen it: their FULL flavour copies `ccd/`, `shared/`, `deploy/` from the checkout rather than a built tarball, and `npm` is a recorder there.
- **D-3106** — spec §7's "The live one" opens with `ccrc rollout --check` on both boxes. Measured 2026-09-19 from the deploying machine: `rollout` exits 2 at step 1, because the server box's `~/.ccrc/ccrc.env` records no `CCRC_ROLE` — `deploy.sh` ships `deploy/ccrc.env` verbatim and never writes one; only `ccrc install` does (`ccd/ccrc`'s `CCRC_ROLE=$INST_ROLE` line) — and step 3 would have refused both boxes anyway: their `ccrc` (`bd2bf57a` fleet, `53e6a438` server, both `deploy.sh`-placed on 2026-09-17) predates `update --check`. Neither is a defect in `rollout`; both are the designed refusals, and the second guards a real hazard — a bare `ccrc update` on the role-less server box would install role `both`, an agent unit on the server box. So the FIRST move onto the channel is by hand, once per box, in this order: record `CCRC_ROLE=server` in the server box's `ccrc.env`; `ssh <fleet box> ccrc update --to vX.Y.Z` (the old verb knows `--to`, reads the recorded role, and execs the staged tree's own `ccrc install`, which writes the record and places the new `ccrc`); the same on the server box; then `ccrc rollout --check` names one version on both, and every later release is `rollout`. CLAUDE.md's Deploy bullet and spec §7 now say so.
- **D-3110** — RULING R17, made by the operator 2026-09-19 22:54 UTC: `ccrc rollout --check --force` is neither `update`'s refusal of the pair nor the silent drop it shipped with (spec §4 said nothing; `cmd_rollout`'s check arm returned before `--force` was read). It MEASURES, then says what `--force` would do — one line naming every box in the order it would run, each with the state the check just read: `rollout: with --force, N box(es) would run 'ccrc update --to vX.Y.Z --force': fleet [current], server [behind] — nothing was touched`. The exit code stays `--check`'s own (0 all current, 1 otherwise) and no update argv is ever sent. `ccrc update --check --force` keeps refusing (that pair names two acts on ONE box; this one is a forecast over two). Pinned by `ccrc-rollout.test.ts` (R17 case), measured red before the arm existed.
- **D-3111** — the installer and the doctor now say the same thing about a server-role box. Measured 2026-09-19 23:06 UTC on the live server box during the first move (D-3106): `cmd_install --role server` ran every per-account step — `_inst_files`, `_inst_dirs`, `_inst_hooks`, `_inst_skills`, `_inst_wrappers` carried no role gate while the graphify steps did — and converged 17 accounts on a box that hosts no sessions: 12 generated wrappers written, 2 refused (the operator's own hand-made launchers), 17 config dirs, the session hook registered in each account's `settings.json` INCLUDING the operator's own `~/.claude`, and their statusline moved aside. Doctor's `skills` check, meanwhile, SKIPs that role as "hosts no sessions". Ruled (the operator: "do the follow-up"): a server-role box converges nothing per account. The five steps print `install: <step>: skipped — a server-role box hosts no sessions` and return; the record is still written last. The doctor's per-account checks that still measured a server box — `wrappers`, `accounts`, `memory`, `routing`, `pools` — SKIP on role server with the same sentence shape as `skills`, so the trailing doctor of a server-box `ccrc update` no longer fails on launchers and homes ccrc does not manage there (the roster's own validity is measured when `accounts.sh` is generated, before any of them). Pinned by `ccrc-install.test.ts` (the `--role server skips every per-account step` case) and the doctor suites' server-role fixtures, whose skip counts moved by exactly the checks named here. What this does NOT undo: last night's writes on the live server box, which are the operator's to keep or revert (hooks: `install-session-hooks.sh --remove`; the statusline's `.pre-ccrc-…` copy; the 12 marker-bearing wrappers and dirs).
- **D-3112** — RULING R19, wording validated and approved by the operator 2026-09-19 22:54 UTC, superseding D-3053's caveat. `_check_skills`' entries come from exactly two producers: `<id>: <skill> missing` when the skill directory is absent from that account's HOME, and `<id>: <skill> differs` when `diff -r -q` between the shipped tree and the home's copy fails; a tree that lacks a skill is a separate FAIL with its own sentence. The count is of account-and-skill pairs, so "installed skill(s)" misdescribed a missing one. The headline now reads `N skill(s) in the rostered homes differ from the shipped tree or are missing from their home: …`. Two smaller departures ride with it, each measured red first: the tree-side probe names EVERY missing skill directory in one FAIL instead of the first it met; and the `homes == 0` SKIP no longer covers a roster whose every row the reader rejected — that condition gets `no usable account in $HOME/.ccrc/accounts.json — the 'wrappers' check above owns that`. Two test pins and D-3053's text were the only quoters of the old sentence.
- **D-3113** — RULING R19b, the operator 2026-09-19 22:54 UTC: "we need doctor to be able to also cure this, surely?" `ccrc doctor` had no repair arm at all (`cmd_doctor` took no arguments). It now takes `--fix`: a check that FAILS and declares a fixer (`_fix_<name>` in `ccrc-doctor-checks`) is cured and then measured AGAIN — the measured FAIL prints as measured, the fixer prints its `FIX <name>: …` line(s), and the tally counts only the second answer, so a PASS after `--fix` is a re-measurement, never the fixer's own word. The first and only fixer is `_fix_skills`: the shipped tree's own installers (`CCRC_SKILL_SRC="$BOX_TREE_DIR/ccd/<name>-skill" bash "$BOX_TREE_DIR/ccd/install-<name>-skill.sh" --homes <dir>`, the remedy the FAIL already printed) into every rostered home that exists — idempotent, and it reads the roster through the same `_skills_roster_rows` helper the check reads, so it cures exactly the homes the check measured. Without `--fix` doctor writes nothing, as before; an unknown flag is still a usage refusal (exit 2). The FAIL's remedy now names `ccrc doctor --fix` first. Pinned by `ccrc-doctor.test.ts` (the `--fix CURES` and `--fix on a healthy box` cases), measured red before the arm existed; `--fix` is a general arm so a later check can declare its own fixer without touching the driver.
- **D-3114** — measured 2026-09-20 12:56–13:00 UTC on the first `ccrc rollout` (v0.0.4 → v0.0.5): the server box wrote its record and was on v0.0.5, its trailing doctor FAILed on four pre-existing box facts (`git_email`, an old inactive `ccrc-agent.service` unit file, `gpt2` without an executable, two unreachable memory homes), `ccrc update` exited 1, and `rollout` printed "the server box's update exited 1 — stopped here … deploy.sh remains the fallback" over a box that had moved; its step-6 verification never ran, though `rollout --check` then said both boxes `[current]`. One exit code carried two facts. Now: `cmd_update` clears `~/.ccrc/installed` right before the staged install, so the record's presence afterwards means ONE thing — this run's spine completed (the spine writes it LAST). A non-zero staged install with the record present is "completed, but the trailing doctor failed": the update prints `update: the staged install completed (the record is written) but its trailing doctor exited N — this box IS on vX.Y.Z; the FAIL lines above are the box's health, not the update's`, still sweeps and reports, and exits **3**. Without the record it dies as before (exit 1, the backup named). `rollout` relays a 3 as `rollout: <box>: moved to vX.Y.Z, but its doctor failed — read the <box>: FAIL lines above`, continues to the next box, verifies both, and exits 3 itself; a 1 still stops it. `--check` is unchanged: a moved box with a failing doctor IS `current`, which is why a second rollout says nothing to do. Pinned by `ccrc-update.test.ts` (completed-but-unhealthy → 3; died-spine → 1 and no record) and `ccrc-rollout.test.ts` (a 3 continues and verifies; a 1 stops), each measured red first. The server box's four FAILs themselves are the operator's: two are real box facts (`git_email`, the stale agent unit file), two go away with D-3111's role skips.
