...
| _swap_target's FIRST rc-2 arm (`cur`) deleted | 3 failed | 54 passed | ``_swap_target` stands still on an unmeasurable marker — the ROOT-SAFE twin`; `a DANGLING SYMLINK marker stops `_swap_target` too — the pre-gate is shared, so it must agree`; `an UNREADABLE record stops `_swap_target` too — the marker survives AND the session stays` |
| _swap_target's SECOND rc-2 arm (`home`) deleted | 1 failed | 87 passed | `the SECOND crossing read is guarded too — a hiccup between the two is a moment, not a state` **CORRECTED 2026-09-08 (#67 review), and the PASSED count re-measured twice since.** This row read "_swap_target stops answering nothing on an undecidable marker | 3 failed" and covered BOTH `return` lines as one mutation. Measured: deleting the second line ALONE leaves the suite fully green (57 passed, 0 failed) — all three reds came from the first. The row claimed a census of two guards while measuring one, which is the unpinned-guard shape this wave exists to stop shipping. The second arm now has its own case, built on the `FLIP` `_reg_read` shadow already in this file. **The passed count read 57, then 64, then 65, and is 87 at the tip that ships** — the same row, the same one red, a suite that grew by thirty cases underneath it. **Which is the lesson for this whole column** (#69 review round 3): the FAILED count and the NAMED CASE are the falsifiable halves of every row here; the passed count is a property of the suite's size on the tree it was run against, and it goes stale on any commit that adds a case anywhere in the file. Re-run a row against the tree in front of you before concluding a guard moved — a mismatch here is far more likely to be thirty new cases than a lost guard. |
