...
| _swap_target's SECOND rc-2 arm (`home`) deleted | 1 failed | 65 passed | `the SECOND crossing read is guarded too — a hiccup between the two is a moment, not a state` **CORRECTED 2026-09-08 (#67 review).** This row read "_swap_target stops answering nothing on an undecidable marker | 3 failed" and covered BOTH `return` lines as one mutation. Measured: deleting the second line ALONE leaves the suite fully green (57 passed, 0 failed) — all three reds came from the first. The row claimed a census of two guards while measuring one, which is the unpinned-guard shape this wave exists to stop shipping. The second arm now has its own case, built on the `FLIP` `_reg_read` shadow already in this file. |
