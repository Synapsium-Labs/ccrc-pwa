...
- [ ] **R0. Remove the leaked registry before touching the fleet host.** `ls -la ~/.ccrc/models/` — measured 2026-09-08 on the fleet host, that directory ALREADY holds `gpt.classes.json`, `gpt.classes.tsv` and `gpt.effort.json` (mode 600, mtime 16:36:05Z), leaked by a Plan 1 implementer's experiment and never written by an operator. Measured against the pinned code (final wave, `deploy/models-op.mjs`'s `init` arm): `init` does NOT refuse over this. The refusal ("already has a class registry") fires only when the PROBE KIND DIFFERS from what is on disk; `init --probe codex` over an existing CODEX registry returns `{"ok":true,"created":false}` at exit 0 and calls neither `writeRegistry` nor `materialise` — it is a no-op, not a refusal (pinned: `models-op.test.ts`, "is idempotent — a second init changes nothing and says so"). There is no "skip R3" option: `rm ~/.ccrc/models/gpt.{classes.json,classes.tsv,effort.json}` before continuing to R1. Running R3 over the leaked files instead of after removing them leaves the fleet host exactly as leaked — no registry write, no materialise, and R5's block never appears.
