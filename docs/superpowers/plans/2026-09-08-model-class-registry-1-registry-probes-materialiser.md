...
- [ ] **R0. Check for a pre-existing registry before touching the fleet host.** `ls -la ~/.ccrc/models/` — measured 2026-09-08 on the fleet host, that directory ALREADY holds `gpt.classes.json`, `gpt.classes.tsv` and `gpt.effort.json` (mode 600, mtime 16:36:05Z), leaked by a Plan 1 implementer's experiment and never written by an operator. `init` (R3) refuses when a registry already exists (`deploy/models-op.mjs`: "already has a class registry"). `cat` all three files, then choose: either remove all three and continue at R1, or — only if their content is EXACTLY what R3 would write (`classes` = luna/terra/sol with `fable: null`, `subagent: sonnet`, `discovery: "catalogue"`) — skip R3 entirely and continue at R4. State both options to the operator; the operator chooses, this runbook does not guess.
