...
| `ccd/ccd` | Modify (constants ~line 999; `cmd_caps` 5927–6076; new `cmd_win_size`; `_pane_measurable`; nine reader functions 13351–15064; dispatcher 17525–17561) | The fleet box's own copy: the bash `READER_MIN_COLS`, the `win-size` verb, the `_pane_measurable` helper, and the stand-down at every typing site |
...
cmd_win_size() {   # ccd win-size --session <id> --mode smallest|canonical — pin or un-pin ONE session's window.
  ...
  #     for an operator on the box and for ccd's own use.
  #
  # SCOPED TO `-t cc-<id>`, NEVER GLOBAL. `set-option` with no `-t` writes the
  # SERVER-WIDE default and re-sizes every live session on this box at once.
  #
...
Numbers are ISSUED, never chosen: allocate with `POST /api/ledger/deviations` and DEFINE in the same act. A session that cannot reach the allocator writes `D-TBD-<slug>` and reports (worker clause 11). This wave's block — **D-2775..D-2781** — was assigned by the programme coordinator at run 62's dispatch; the entries below are drawn from it, not allocated by the worker.

Every entry here is a departure the pre-flight scan measured against `origin/main` at `7a91bcf1` BEFORE Task 1 ran. Three of the four exist because this plan was written against a tree that predates the routing slices (`c5dd6e41`), which landed a fifth gated verb, a sixth capability token and an eleventh bypass fixture between the plan's drafting and its execution.
...
Two things this entry exists to carry forward, because a later wave will size work from it. First, the site inventory GREW inside this wave: Task 4 added eight more unanchored targets, so a guard that reds on a bare `-t cc-` will red more sites than a reader of the original declaration would expect. Second, `server/src/clip.ts`'s `isSafeSessionId` is not the remedy it looks like — it bars `/`, `\`, NUL, `.` and `..` but ADMITS `*`, so adopting it at the pty route would not close this; only anchoring does.
