...
| `ccd/ccd` | Modify (constants ~line 999; `cmd_caps` 5927–6076; new `cmd_win_size`; `_pane_measurable`; eight reader functions; dispatcher 17525–17561) | The fleet box's own copy: the bash `READER_MIN_COLS`, the `win-size` verb, the `_pane_measurable` helper, and the stand-down at every typing site |
 ...
 * that true. `watch.ts` and `inject/send.ts` are un-width-aware, and teaching that
 * lane this floor is a deliberate later widening, not something to infer from this
...
cmd_win_size() {   # ccd win-size --session <id> --mode smallest|canonical — pin or un-pin ONE session's window.
  ...
  #     `tmux resize-window`, which works against an agent of any age. This arm is
  #     Its callers are an operator typing the verb on the box and the agent's
...
Numbers are ISSUED, never chosen: allocate with `POST /api/ledger/deviations` and DEFINE in the same act. A session that cannot reach the allocator writes `D-TBD-<slug>` and reports (worker clause 11). This wave's numbers come from THREE blocks, every one assigned by the programme coordinator and none allocated by the worker: **D-2775..D-2781** at run 62's dispatch; **D-2859..D-2866** when the first was exhausted mid-wave; and a third block, of which **D-2879** and **D-2880** are assigned here, when the held-out review panel surfaced two unledgered departures the second block could not cover. Each was requested by mail and issued in reply, because a worker that mints its own numbers mid-wave is the D-2338 shape. That third block's remaining numbers are unspent headroom, and are deliberately NOT written here in any form: spelling an unassigned number in a tracked file seals its band forever — it never enters the ledger and raises this project's floor anyway.

Only the first four entries (D-2775..D-2778) are pre-flight findings, measured against `origin/main` at `7a91bcf1` BEFORE Task 1 ran; three of those four exist because this plan was written against a tree that predates the routing slices (`c5dd6e41`), which landed a fifth gated verb, a sixth capability token and an eleventh bypass fixture between the plan's drafting and its execution. **Every other entry was found during execution or after it**, and each says so in its own text: D-2779 in Task 1's fix round 1, D-2780 while fixing Task 2's shipped verb, D-2860/D-2861/D-2863 in Task 4's fix rounds, and D-2864..D-2866, D-2879 and D-2880 in the held-out review panel's pass over the finished branch. This preamble previously claimed every entry was a pre-flight finding and counted "three of the four" against a section that had grown to twelve; both were false of the section they introduced.
