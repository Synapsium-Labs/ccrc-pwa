...
**The call-site count is deliberately stated in ONE place and bound to a commit, because it moved twice inside this wave.** Measured at `4dc29a8e` with `grep -n '_pane_measurable' ccd/ccd | grep -vE ':\s*#'`: ten lines — one definition and NINE calls. It was eight at `b6cb709c`, and the S3 fix in this wave's own final commit added the ninth (`_redrive_after_spawn`'s mid-wait re-measure). A reader MUST re-run that command rather than trust this sentence's number: the first draft of this ledger said nine when the tree had eight, the correction said eight, and the next commit made it nine again. The count is a function of the commit, not a property of the design.
