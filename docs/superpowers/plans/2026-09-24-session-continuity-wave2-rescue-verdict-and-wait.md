...
Re-measured 2026-09-24 on the UNEDITED base at `08701c22` (this branch with `main` `b501698a` merged — #176, #178, #179 touch no line of `ccd/ccd`, which is byte-identical to `a3a93b41`'s: `git diff --quiet a3a93b41 HEAD -- ccd/ccd`): both tools extract at 30 and 113 lines, byte-identical to their `a3a93b41` copies; `python3 "$SCRATCH/repoint-readme.py"` prints `ccd/ccd:21202` and `ccd/ccd:19989-19991` and leaves README unchanged; `python3 "$SCRATCH/cite-remeasure.py" "$SCRATCH" HEAD` prints `147 / 195 / 53 / 35` stated = base = tree, composition empty; the citation cases run `7 passed | 326 skipped`; the `_reg_get` census reads 156/132.
