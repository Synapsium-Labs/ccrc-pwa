...
- **Every path in the File Structure table was `ls`/`grep`-verified to exist** (or is marked Create:). No `infra/` path appears as a target anywhere in this plan (the only two mentions are this line and the "supersedes" note); the superseded plan's 24 `infra/ccrc/*` and `infra/<server-host>-portability/ccd` paths were each re-derived to `ccd/`, `server/`, `agent/`, `pwa/`.
