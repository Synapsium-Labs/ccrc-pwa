...
- **D-2824 (2026-09-14, found by the whole-branch review)** — **A send-back re-briefs the worker by MAIL, never by `runs dispatch`; the worker skill gains the matching `fix-round` bullet.** Spec §4 ("the same worker is re-dispatched to `working`") and §6 ("dispatch W with the report as brief"), and the coordinator skill's step 6 transcribed from them, told the coordinator to `POST /api/runs/:id/dispatch` a run it had just advanced to `working`. `dispatchRun` refuses any run whose state has no `dispatched` edge — only `planned` has one, in either table — so the branch's most common path half-succeeded: W re-entered `working`, counted against the cap again, and the dispatch was refused 409 `bad-transition` with no second instruction. Widening `RUN_TRANSITIONS` was rejected: a `dispatched` edge out of `working` would let dispatch re-spawn or `/clear` a live workspace mid-wave, which D-1 and the `/clear` discipline forbid. The mechanism that exists is mail: after `advance → working`, the coordinator sends a `status` mail with subject `fix-round` to the worker's session, the report's absolute path on the body's first line and in `artifacts`, and its rulings beneath; the idle-gated delivery lane wakes the worker. Spec §8's "Worker skill: unchanged" is therefore also departed from: the worker skill's "When something is wrong" list gains one bullet saying a `fix-round` mail's report is the brief for the fix round and ends in a FRESH wave-done. The spec's §4 and §6 sentences carry an inline correction naming this entry. `coordinator-skill.test.ts` pins that step 6's send-back arm names `mail send` and `fix-round` and never `runs dispatch` — the class of defect (an instruction the transition table refuses) had no pin in the tree.
...
- [ ] **Step 4: Deploy notes (for the PR body and the programme ledger).**
  - **SERVER FIRST — this branch inverts the standing AGENT-FIRST rule, and the reason is the rule's own.** AGENT-FIRST exists because the server reads what the hook writes and the agent caches `ccd caps`; nothing on this branch changes either. What this branch's AGENT lane carries is an INSTRUCTION (coordinator clause 12: open a `kind:'review'` run with no `project`/`wave`) whose SERVER half does not exist until the server lane lands — a coordinator re-kicked in the window would be answered a bare 400 `bad-request`, and the obvious improvisation (adding `project`/`wave`) makes the OLD server open a second WORK run for the wave. The server lane is inert until a coordinator opens a review run, so it goes first: `bash deploy/deploy.sh` (migration 11 runs at boot; `/health` reports the shipped sha; the cap change takes effect at once — see below), THEN `bash deploy/deploy.sh agent` (`ccd/`, the three skill trees and their installers; verify with `ls ~/.cc-sessions/reviewer-skill/SKILL.md` and, per rostered home, `ls <configDir>/skills/ccrc-reviewer/SKILL.md` — existence by `ls` only). If the two cannot be kept in that order, raise `$REG/coordinator-paused` (`POST /api/coord/pause`) across both — clause 4 makes a `paused` refusal a clean stop. The skill's step 5 names the window's signature so a coordinator caught in it waits rather than improvises.
