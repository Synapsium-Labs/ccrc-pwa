...
Expected: PASS — `coordinator-skill` 152/152, `reviewer-skill` 12/12, `install-coordinator-skill` 16/16, `box-token-census` 14/14, `routing-references` 11/11 (measured at `af64d9d2`, each file alone).
...
Expected: first run PASS (`session-hook-sync-advisory` 37/37; `macos-platform` `58 passed | 10 skipped (68)` — it scans the hook for GNU-only spellings, and the block uses bash `[[ =~ ]]`, `$'\n'` and `jq` only; `claims-advisory` 2/2; `routing-env-census` 3/3; `single-definition` 160/160); second run `333 passed (333)` — it is a known load flake, so a red there is re-run IN ISOLATION before it is called a break (prototype: 111.7 s, green first time; green again at `af64d9d2` with the revised block).
...
Expected: PASS (`measure-landing` 15, `ccrc-restamp` 8, `ccrc-cli` 35, `worker-skill` 45, `coordinator-skill` 152, `update-branch-absent` 2, `session-hook-sync-advisory` 37, `ownership` 14, `single-definition` 160 — each measured alone at `af64d9d2`); the second run `7 passed | 326 skipped (333)`.
