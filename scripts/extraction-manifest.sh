...
# Excluded everywhere: build output, dependencies, and tsc's incremental-build
# cache. tsconfig.tsbuildinfo is tracked (pwa/tsconfig.tsbuildinfo) but
# regenerated by any `tsc -b` run — hashing it would make the one instrument
# whose value depends on every difference being meaningful produce a spurious
# diff whenever someone typechecks between the before and after manifests.
is_excluded() {
  case "$1" in
