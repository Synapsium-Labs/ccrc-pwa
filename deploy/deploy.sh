...
install_ccrc_shim() {
  ...
  cat > "$shim" <<'CCRC_SHIM'
#!/usr/bin/env bash
# GENERATED by deploy/deploy.sh (install_ccrc_shim) — do not edit on the box.
# Edit ccd/ccrc in the repo and re-deploy; this file is only a launcher.
#
...
if [ ! -x "$CCRC_SHIPPED" ]; then
  echo "ccrc: $CCRC_SHIPPED is missing or not executable — this box's shipped ccrc tree is gone, so there is nothing for this launcher to run. Re-run deploy/deploy.sh against this box." >&2
  exit 1
fi
exec "$CCRC_SHIPPED" "$@"
CCRC_SHIM
