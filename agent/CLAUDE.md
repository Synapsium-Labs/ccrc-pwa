...
- **Gated verbs:** `ws-reap` requires `--expect` (confirmation token), `ws-rename` requires `--session` (its argv
  is built from model output with no human in the path), `ws-reclaim` requires `--expect` (the child-reclaim
  token; the server composes it for a child with no human in the path). **Ungrantable verbs:** `ws-rm`, `ws-gc`. An empty prefix
