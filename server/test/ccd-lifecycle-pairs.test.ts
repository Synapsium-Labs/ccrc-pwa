...
import { eventsOf, lcDir } from './lifecycleHelpers.js';
...
describe('ws-rm writes an intent/done pair sharing one tx', () => {
  ...
  it('records branchDeleted as a fact when the branch survives', () => {
    ...
    expect(half('destroy', 'done')[0]!['branchDeleted'], 'git refuses an unmerged branch')
      .toBe('false');
  });
...
describe('ws-gc --prune writes a pair per destroyed row', () => {
  ...
  // caught by running it: `_LC_ACTS` (ccd:842) and L0's `LifecycleAct`
