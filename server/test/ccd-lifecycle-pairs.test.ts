...
describe('ws-rm writes an intent/done pair sharing one tx', () => {
  ...
  // running it. `_lc_json`'s own `TOP` tuple (ccd:2215) is
...
describe('ws-gc --prune writes a pair per destroyed row', () => {
  ...
  // caught by running it: `_LC_ACTS` (ccd:919-921) and L0's `LifecycleAct`
  // (shared/api.ts:4210) are a CLOSED, cross-checked vocabulary with no
  ...
  // `act: unknown, badact: gc-prune` (`_lc_emit`, ccd:2302-2303) — invisible
  ...
  // ccd:10144-10151) is NOT a subshell, so a real run calls `_ws_gc_prune_row` —
  // and therefore `_lc_tx` — more than once in ONE process. `_lc_tx`'s own
  // uniqueness rests on a fresh `$BASHPID` per `$(...)` fork plus a
  // nanosecond clock read (ccd:945-957), which is structurally safe, but this
