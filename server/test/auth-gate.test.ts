...
describe('the scanner is looking at something', () => {
  ...
  it('found both files, and EXACTLY the route count the surface has', () => {
    ...
    // 26 since `POST /api/asks/:id/answer` (Task 9, the ask pre-emption lane) —
    // the parent presses a digit into its child's live menu, box-token gated
    // plus registry attribution, exactly the `POST /api/claims` shape.
    // 27 since `POST /api/asks/:id/release` (Task 10, the same lane's decline
    // route) — the parent declines to rule, box-token gated plus the same
    // registry attribution and `ask.parentId === fromId` ownership check.
    ...
    // `coord/routes.ts`, and DELIBERATELY not EXEMPT — its own docstring says
    ...
    // question from this one. 48 + 28 = 76.
...
describe('EXEMPT is complete in both directions', () => {
  ...
  it('the twenty-two box-token lanes in EXEMPT are those coord routes, and twenty-three with notify', () => {
    ...
    // pre-emption lane's decline route) joined the plain box-token lanes,
    // same shape as `/answer` directly above it.
...
describe('with the gate ARMED and no cookie', () => {
  ...
  it('sweeps EXACTLY the routes that are neither websockets nor exempt', () => {
    ...
    // The gated non-exempt routes this file reasons about by name are
    ...
    // 44 since account pools wave 3's `POST /api/projects/:project/pool` met
    // armed feed authentication in this merge — the pool route is NOT exempt,
    // while feed already existed and only joined EXEMPT, so the difference stayed
    // at 44. The caps pair before it are also NOT exempt (an operator dial is not
    // a machine lane), so both raised the scanned count without raising the exempt
    ...
    try {
      ...
      for (const r of httpRoutes) {
        ...
        try {
          ...
          //    is not itself flag-aware — the assertion that covers all 73 HTTP routes, not the 28 exempt.
