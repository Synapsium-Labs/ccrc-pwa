...
describe('the scanner is looking at something', () => {
  ...
  it('found both files, and EXACTLY the route count the surface has', () => {
    ...
    // The automations task's ten (spec §10's table) — `get`/`post` only, none
    // exempt, none websocket, so every count below moves by exactly ten. THE
    // THIRD SCANNED FILE: this scan read only `server.ts` and
    // `coord/routes.ts` before, so a route registered anywhere else was never
    // swept for `401 no-session` at all — green suite, false property.
    // `auto/routes.ts` was going to be that third file.
...
describe('with CCRC_AUTH off — the shipped default', () => {
  ...
  it('the gate changes the status of EXACTLY the gated routes, and of nothing else', async () => {
    // THE PROPERTY, in one loop over all 81 HTTP routes, with THREE probes each:
    ...
    try {
      ...
      for (const r of httpRoutes) {
        ...
        try {
          ...
          //    is not itself flag-aware — the assertion that covers all 81, not the 27 exempt.
