// The reviewer skill installer, tested exactly the way
...
const INSTALLER = path.resolve(__dirname, '../../ccd/install-reviewer-skill.sh');
const SRC = path.resolve(__dirname, '../../ccd/reviewer-skill');
const HOMES = ['.claude', '.claude-personal', '.claude-corp', '.claude-gpt'];

let home: string;
const skill = (d: string, ...rest: string[]): string =>
  path.join(home, d, 'skills', 'ccrc-reviewer', ...rest);

beforeEach(() => {
  home = mkTmp('ccrc-reviewerskillinstall-');
...
describe('install-reviewer-skill', () => {
  ...
  it('replaces a stale install and backs the old one up first', () => {
    ...
    run();
    expect(fs.readFileSync(skill('.claude', 'SKILL.md'), 'utf8')).toContain('name: ccrc-reviewer');
    ...
    expect(inside.some((n) => n.includes('ccrc-reviewer'))).toBe(true);
  });
  ...
  it('refuses the whole run when the source has no SKILL.md, touching nothing', () => {
    const empty = mkTmp('ccrc-reviewerskillsrc-');
  ...
  it('reports a failed home in the exit status but still processes the others', () => {
    ...
    // strand the account a swap could move a reviewer onto.
...
describe('install-reviewer-skill.sh default homes are the roster, behaviourally', () => {
  ...
  beforeEach(() => {
    rosterHome = mkTmp('ccrc-reviewerskillinstall-roster-');
  ...
  it("touches every rostered account's config dir when given no --homes argv", () => {
    ...
    for (const a of DEFAULT_TEST_ROSTER.accounts) {
      const got = fs.existsSync(path.join(rosterHome, a.configDirSuffix, 'skills', 'ccrc-reviewer', 'SKILL.md'));
      expect(got, a.id).toBe(true);
    }
  });

  it('refuses, naming the remedy, when the box has no roster at all', () => {
    const bare = mkTmp('ccrc-reviewerskillinstall-noroster-');
    try {
