...
// ccd:9383 and ccd:11279 redirect a CHILD'S stdout+stderr into it from inside a
 ...
 *  "nothing outside the block MENTIONS": ccd:2471's dot-artifact inventory
...
describe('nothing but the _lc_* block writes into .lifecycle/', () => {
  ...
  it('routes every journal write through exactly one printf, inside _lc_emit', () => {
    ...
    // (`^\s*printf`), but `_lc_emit`'s real append site (ccd:1573) wraps the
...
describe('the meas key vocabulary is ONE list', () => {
  ...
  // `cmd_ws_rm`'s attic pin now emits `meas.atticsrc` (`ccd:3106`) and
  // `cmd_ws_restore`'s R4-2 supersede now emits `meas.manifestBytes`
  // (`ccd:4719`) — re-measuring the same scan at this HEAD finds TWENTY-FOUR
  ...
  it('now invents no emit for anything OTHER than manifestBytes or atticsrc — the wire evidence for those two, not a blanket widening', () => {
    ...
    // evidence (`ccd:3106`, `ccd:4719`), so the two are now real members of
