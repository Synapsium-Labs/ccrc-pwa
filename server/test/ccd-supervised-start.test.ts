...
import { CCD, ghContainedEnv, harnessBin, makeCcdHarness, type CcdHarness, WIDE_PANE } from './ccdWsHelpers.js';
...
describeLinux('ccd ensure revives a live-but-unsupervised session', () => {
  ...
  it('says so on stderr when the unit will not start, instead of reporting a revival', () => {
    ...
    const NOUNIT = `sleep() { :; };
      ...
      tmux() { echo "tmux $*" >> "$HOME/ccd-calls"; ${WIDE_PANE} case "$1" in has-session) [[ -e "$HOME/pane-up" ]] ;; esac; };`;
...
describeLinux('the start waits on observables', () => {
  it('reports the unit\'s own verdict, read from the registry rather than guessed', () => {
    ...
    const r = shFail(`sleep() { :; };
      ...
      tmux() { echo "tmux $*" >> "$HOME/ccd-calls"; ${WIDE_PANE} case "$1" in has-session) return 1 ;; esac; };
      cmd_ensure myid`);
