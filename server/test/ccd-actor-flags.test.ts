...
//      Measured against wave 3 as shipped (`ccd:2395`) and against its own
...
// asked to ship: `_lc_dec_ok`'s own doc comment (`ccd:2411`) is length-only
...
// return 0 today. `cmd_ws_hold` (`ccd:4758`) has its OWN, separate
// `[[ -n "${reason//[[:space:]]/}" ]] || die` guard that never routes through
// `_lc_dec_ok` at all; `ws-rm`/`forget` (`ccd:3739`, `ccd:13461`) call ONLY
 ...
 *  (ccd:5118) before its own body when the directory is absent, which the
...
describe('_lc_surface_norm — ccd:2289 closed set, spelled once more and never a third time', () => {
 ...
 *  none of which reach past the reap-lock acquisition (`ccd:5661`) — every
...
describe('ws-restore takes the same three flags, and refuses through _lc_refuse', () => {
  ...
  it('refuses a blank --reason, naming the flag — the same non-blank check as --actor', () => {
    // `_lc_dec_ok` is length-only (D-210, ccd:2411) and returns 0 for '', so
    ...
    // `cmd_ws_hold` (ccd:4758). Not in the brief's own step-2 sample, added
...
describe('ws-hold keeps ONE reason — its own', () => {
  ...
  it('still refuses a whitespace-only hold reason — ccd:4450 is untouched', () => {
    // Line re-measured against THIS task's own edit, not copied from the
    // brief (its `ccd:3441` predates waves 2-3 and no longer points here —
    // RULE 2).
    const id = seedWorkspace();
...
// ccd:5556/ccd:4581 (`cmd_ws_restore`'s actor-length and reason-length
...
describe("ws-restore's length refusals omit ONLY the field that failed its own cap", () => {
  const STUBS = `tmux() { return 1; };`;
  // `_LC_DEC_MAX` is 512 BYTES (ccd:900); 513 plain-ASCII characters is 513
...
// (`ccd:5530` onward) — and they were leaking too: `lc_actor`/`lc_reason`
