...
describe('refusals are answers', () => {
  ...
  it('refuses rather than writing a tombstone it cannot quote', () => {
    ...
    // printf ARGUMENT LISTS. `cmd_ws_audit` (ccd:6695) and `_ws_manifest`
    // (ccd:4280) each probe once up front for exactly that reason, and this is
...
describe('destruction order', () => {
  ...
  it('removes the worktree, CAS-deletes the branch, and clears the registry LAST', () => {
    const { wt, main } = ready();
    const tok = tokenOf();
    // COUNTED, not `toContain`. `cmd_ws_archive` (ccd:4184) kills the same pane
...
describe('partial failure and resume', () => {
  it('resumes from the breadcrumb instead of refusing worktree-missing', () => {
    ...
    // (ccd:3140). Correct of it, and still a wedge: the resume path is what
    // finishes the job.
    const { wt, main } = ready();
    const tok = tokenOf();
  ...
  // resume `_ws_reap_locked` already documents (ccd:3039-3056): the resume
 ...
 * staged for deletion" (`_ws_reap_eval`, ccd:6012-6013), and it is the one
