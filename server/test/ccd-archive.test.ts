...
import { ACCOUNT_POOLS_CAP, ACTOR_FLAGS_CAP, CHILD_ARGV_CAP, POOLS_CAP, RECLAIM_CAP, ROUTE_APPLY_CAP, ROUTE_ARGV_CAP, ROUTE_CAP, WIN_SIZE_CAP } from '../src/ccdargv.js';
...
describe('ccd caps', () => {
  ...
  const KNOWN_CAPABILITY_TOKENS = ['account-pools', 'account-v1', 'actor-flags-v1', 'child-argv-v1', 'lifecycle-v1', 'pools-v1', 'reclaim-v1', 'route-apply-v1', 'route-argv-v1', 'route-v1', 'stop-surface', 'win-size-v1'];
  ...
  it('advertises exactly the verbs the dispatcher implements, plus the known capability tokens', () => {
    ...
    expect(KNOWN_CAPABILITY_TOKENS).toContain(ACCOUNT_POOLS_CAP);
    ...
    // (ccd:10575) comes first — so slicing from indexOf landed inside ws-gc, and
...
describe('_ws_stash_count', () => {
  ...
  it('counts an UNNAMED stash — git writes "WIP on <branch>:", with a lowercase "on"', () => {
    ...
    // cost: `_ws_reap_eval` gates `stashes-present` on this count (ccd:7839),
...
describe('ws-archive refuses rather than record a manifest that lies', () => {
  it('refuses when the worktree directory is gone', () => {
    ...
    // exit 0, marker set. ws-restore already refuses this shape (ccd:5674).
    const r = shFail(`${ARCH} cmd_ws_archive --session demo-quiet-basin`);
    expect(r.code).toBe(1);
  ...
  it('refuses when python3 cannot quote the JSON, instead of persisting garbage', () => {
    ...
    // (ccd:12170) warns and continues; that is right for a best-effort transcript
 ...
 *  and `cmd_ws_rm` (ccd:491-497) draws for this very divergence. */
describe('ws-archive records branch drift instead of refusing forever', () => {
  ...
  it('archives a branch renamed by hand, naming both records and no tip', () => {
    ...
    // user's (ccd:5273-5275). The registry's name is now the one that resolves to
...
describe('ws-restore', () => {
  ...
  it('leaves the started flag set even when the entry never had one', () => {
    ...
    // `cmd_start` read this flag to choose `new` over `resume` (ccd:12056,
    // ccd:11904), so a restored session missing it gets re-spawned as a FRESH
...
describe('ws-attic', () => {
  ...
  it('rejects any mode word other than --session or --drop', () => {
    ...
    // session` (ccd:5785) before the case is reached, so the assertion cannot
...
describe('the archive manifest is written atomically (registry-durability wave 2)', () => {
  it('rides _reg_set and KEEPS its trailing newline — the bytes do not move', () => {
    ...
    // consumer (`manifestBytes`, server/src/registry.ts:415) runs JSON.parse
