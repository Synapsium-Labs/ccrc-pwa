...
- **The lock and the generation.** Every compaction-lifecycle mutation runs under that one per-row mutex,
  ...
  THE FIRST IS NOW REPAIRED BY THAT RESPAWN RATHER THAN MERELY OUTLIVED BY IT. `cmd_ensure` mints a missing
  generation before it spawns (`_reg_generation_init "$id"`, `ccd/ccd:19094`) — best effort, never fatal,
  because this is the supervisor's path and a verb that dies here leaves the session down. It had to be that
  verb: the other two minting sites are row CREATION, and the systemd unit reaches neither, since it runs
  `ccd supervise`, which calls `cmd_ensure`. Measured before the fix, hours after the card first shipped to
  this fleet: 31 of 34 live rows carried no generation and no automatic path could give them one, so the
  sentence above promised a repair nothing performed. AND ALL THREE NOW SAY SO ON STDERR — the contended arm
  (`ccd/ccd:18038-18040`, `genrc == 1`) between an absent-or-invalid-generation arm and a mechanism-absent
  one. The silence this file used to record as a deferred `ccd/ccd` change is closed; what remains true is
  that the absence of the artifacts is still a signal, and now it is no longer the only one.
