...
- **The lock and the generation.** Every compaction-lifecycle mutation runs under that one per-row mutex,
  ...
  AND ALL THREE NOW SAY SO ON STDERR — the contended arm (`ccd/ccd:18073-18075`, `genrc == 1`) sits between an absent-or-invalid-generation arm and a mechanism-absent one. The silence this file recorded as a deferred `ccd/ccd` change is closed; the absence of the artifacts is still a signal, and no longer the only one.
